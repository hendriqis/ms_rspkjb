<%@ Page Language="C#" MasterPageFile="~/Libs/MasterPage/MPListEntry.master" AutoEventWireup="true"
    CodeBehind="PatientTransferEntry.aspx.cs" Inherits="QIS.Medinfras.Web.Inpatient.Program.PatientTransferEntry" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="plhCustomButtonToolbarLeft" runat="server">
...
<asp:Content ID="Content1" ContentPlaceHolderID="plhScript" runat="server">
    <script type="text/javascript">
        ...
        function UpdateToRoomAplicares() {
            var hsuID = $('#<%=hdnToServiceUnitID.ClientID %>').val();
            var classID = $('#<%=hdnToClassID.ClientID %>').val();
        ...
        function UpdateFromRoomAplicares() {
            var hsuID = $('#<%=hdnFromServiceUnitID.ClientID %>').val();
            var classID = $('#<%=hdnFromClassID.ClientID %>').val();
            var filterExpression = "HealthcareServiceUnitID = " + hsuID + " AND ClassID = " + classID;
            Methods.getObject('GetvServiceUnitAplicaresList', filterExpression, function (result) {
                if (result != null) {
                    var kodeKelas = result.AplicaresClassCode;
        ...
        function onAfterSaveRecord() {
            ...
            var filterExpression = "RegistrationID = " + registrationID + " AND GCPatientTransferStatus = 'X143^001'";
            Methods.getListObject('GetvPatientTransferList', filterExpression, function (result) {
                if (result.length > 0) {
            ...
            if (isBridgingAplicares == "1") {
                UpdateToRoomAplicares();
                UpdateFromRoomAplicares();
            }
        }
...
<asp:Content ID="ctnEntry" ContentPlaceHolderID="plhEntry" runat="server">
    ...
    <input type="hidden" id="hdnIsAutoConfirm" runat="server" />
