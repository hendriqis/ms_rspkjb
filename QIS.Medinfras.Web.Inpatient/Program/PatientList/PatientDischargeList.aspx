<%@ Page Language="C#" MasterPageFile="~/Libs/MasterPage/MPMain.master" AutoEventWireup="true"
    CodeBehind="PatientDischargeList.aspx.cs" Inherits="QIS.Medinfras.Web.Inpatient.Program.PatientDischargeList" %>
    ...
    <script type="text/javascript">
        ...
        $('.lvwView tr:gt(0):not(.trEmpty)').live('click', function () {
            if (!isHoverTdExpand) {
                ...
                if ($('#<%=hdnIsPatientDischarge.ClientID %>').val() == '1') {
                    var id = $('#<%=hdnTransactionNo.ClientID %>').val();
                    var url = ResolveUrl("~/Program/PatientDischarge/PatientDischargeEntryCtl.ascx");
                    openUserControlPopup(url, id, 'Pasien Pulang', 900, 450);
                }
                else {
                    //var id = $('#<%=hdnTransactionNo.ClientID %>').val();
                    var consultVisitID = $(this).find('.hdnConsultVisitID').val();
                    if ($(this).hasClass("trDetail")) consultVisitID = $(this).prev().find('.hdnConsultVisitID').val();
                    var url = ResolveUrl("~/Program/PatientDischargePlan/PatientDischargePlanEntryCtl.ascx");
                    openUserControlPopup(url, consultVisitID, 'Rencana Pasien Pulang', 900, 450);
                }
            }
        });
                                            ...
                                            <tr>
                                                ...
                                                <td align="center">
                                                    ...
                                                    <img id="imgPrescriptionOrder" src='<%= ResolveUrl("~/Libs/Images/Status/prescription_order.png")%>'
                                                        class="blink-icon" title="<%=GetLabel("Outstanding/Pending Farmasi/Retur")%>"
                                                        alt="" style='<%# Eval("IsHasOutstandingPrescriptionOrder").ToString() == "True" || Eval("IsHasOutstandingPrescriptionReturnOrder").ToString() == "True" ? "cursor: pointer;width:24px;height:24px;": "display:none" %>' />
                                                </td>
                                                <td align="center">
                                                ...
                                                <td align="center">
                                                    <img class="imgLock <%# Eval("IsHasPayment").ToString() == "1" ? "imgDisabled" : "imgLink"%>"
                                                        title='<%=GetLabel("Patient Payment")%>' src=' <%# ResolveUrl("~/Libs/Images/Status/coverage_ok.png") %>'
                                                        style='<%# Eval("IsHasPayment").ToString() == "True" ? "width:25px": "width:25px;display:none" %>'
                                                        alt="" />
                                                </td>
                                            </tr>
