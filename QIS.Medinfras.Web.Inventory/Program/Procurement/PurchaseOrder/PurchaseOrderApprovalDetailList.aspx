<%@ Page Title="" Language="C#" MasterPageFile="~/libs/MasterPage/MPTrx.master" AutoEventWireup="true"
    CodeBehind="PurchaseOrderApprovalDetailList.aspx.cs" Inherits="QIS.Medinfras.Web.Inventory.Program.PurchaseOrderApprovalDetailList" %>
...
<asp:Content ID="Content3" ContentPlaceHolderID="plhCustomButtonToolbar" runat="server">
    ...
    <li id="btnPurchaseOrderDetailSave" crudmode="R" runat="server">
        <img src='<%=ResolveUrl("~/Libs/Images/Icon/tbsave.png")%>' alt="" /><br style="clear: both" />
        <div>
            <%=GetLabel("Save")%></div>
    </li>
    <li id="btnPurchaseOrderDetailApprove" crudmode="R" runat="server">
        <img src='<%=ResolveUrl("~/Libs/Images/Icon/tbapprove.png")%>' alt="" /><br style="clear: both" />
        <div>
            <%=GetLabel("Approve")%></div>
    </li>
    <li id="btnPurchaseOrderDetailApproveAll" crudmode="R" runat="server">
        <img src='<%=ResolveUrl("~/Libs/Images/Icon/tbapprove.png")%>' alt="" /><br style="clear: both" />
        <div>
...
<asp:Content ID="Content1" ContentPlaceHolderID="plhEntry" runat="server">
    <script type="text/javascript">
        function onLoad() {
            $('#<%=btnPurchaseRequestBack.ClientID %>').click(function () {
                showLoadingPanel();
                document.location = ResolveUrl('~/Program/Procurement/PurchaseOrder/PurchaseOrderApprovalList.aspx?id=to');
            });

            $('#<%=btnPurchaseOrderDetailSave.ClientID %>').click(function () {
                getCheckedMember();
            ...
            $('#<%=btnPurchaseOrderDetailApprove.ClientID %>').click(function () {
                getCheckedMember();
                if ($('#<%=hdnSelectedMember.ClientID %>').val() == '') {
                    showToast('Warning', 'Please Select Item First');
                }
            ...
            $('#<%=btnPurchaseOrderDetailApproveAll.ClientID %>').click(function () {
                if (IsValid(null, 'fsMPEntry', 'mpEntry')) {
                    var message = "Apakah anda yakin ingin APPROVE semua barang?";
        ...
        function onAfterCustomClickSuccess(type, retval) {
            if (type == "save") {
                showToast('SAVE SUCCESS', 'Proses SIMPAN data pemesanan detail berhasil.');
            } else if (type == "approve") {
                showToast('APPROVED SUCCESS', 'Proses APPROVED detail berhasil.');
            } else if (type == "approveall") {
                showToast('APPROVED ALL SUCCESS', 'Proses APPROVED ALL detail berhasil.');
            }
        ...
        $('.chkIsSelected').live('change', function () {
            ...
            if ($(this).find('input').is(':checked')) {
                $tr.find('.txtPOQty').removeAttr('readonly');
            } else {
                $tr.find('.txtPOQty').attr('readonly', 'readonly');
            }
        });

        function getCheckedMember() {
            var lstSelectedMember = $('#<%=hdnSelectedMember.ClientID %>').val().split(',');
            var lstSelectedMemberPOQty = $('#<%=hdnSelectedMemberPOQty.ClientID %>').val().split('|');
            var result = '';
            $('#<%=grdView.ClientID %> .chkIsSelected input').each(function () {
                if ($(this).is(':checked')) {
                    var key = $(this).closest('tr').find('.keyField').html();
                    var poQty = $(this).closest('tr').find('.txtPOQty').val();

                    if (lstSelectedMember.indexOf(key) < 0) {
                        lstSelectedMember.push(key);
                        lstSelectedMemberPOQty.push(poQty);
                    }
                }
                else {
                    var key = $(this).closest('tr').find('.keyField').html();
                    ...
                    if (lstSelectedMember.indexOf(key) > -1) {
                        ...
                        lstSelectedMemberPOQty.splice(lstSelectedMemberPOQty.indexOf(key), 1);
                    }
                }
            });
            $('#<%=hdnSelectedMember.ClientID %>').val(lstSelectedMember.join(','));
            $('#<%=hdnSelectedMemberPOQty.ClientID %>').val(lstSelectedMemberPOQty.join('|'));
        }
    ...
    <input type="hidden" id="hdnSelectedMemberPOQty" runat="server" value="" />
                                ...
                                <asp:Panel runat="server" ID="pnlView" Style="width: 100%; margin-left: auto; margin-right: auto;
                                    ...
                                    <asp:GridView ID="grdView" runat="server" CssClass="grdService grdNormal notAllowSelect"
                                        ...
                                        <Columns>
                                            ...
                                            <asp:BoundField DataField="DraftQuantity" ItemStyle-HorizontalAlign="Center" HeaderText="Draft PO"
                                                HeaderStyle-Width="50px" />
                                            <asp:TemplateField ItemStyle-HorizontalAlign="Center" HeaderStyle-HorizontalAlign="Center"
                                                HeaderText="PO" HeaderStyle-Width="60px">
                                                <ItemTemplate>
                                                    <asp:TextBox ID="txtPOQty" Width="50px" runat="server" CssClass="number txtPOQty" ReadOnly="true" />
                                                </ItemTemplate>
                                            </asp:TemplateField>
