<%@ Page Title="" Language="C#" MasterPageFile="~/libs/MasterPage/MPTrx.master" AutoEventWireup="true"
    CodeBehind="DirectPurchaseReturnApprovalList.aspx.cs" Inherits="QIS.Medinfras.Web.Inventory.Program.DirectPurchaseReturnApprovalList" %>
...
<asp:Content ID="Content3" ContentPlaceHolderID="plhCustomButtonToolbar" runat="server">
    <li id="btnDirectPurchaseReturnHdItem" crudmode="R" runat="server">
        <img src='<%=ResolveUrl("~/Libs/Images/Icon/tbnew.png")%>' alt="" /><br style="clear: both" />
        <div>
            <%=GetLabel("Item")%></div>
    </li>
    <li id="btnDirectPurchaseReturnHdApprove" crudmode="R" runat="server">
        <img src='<%=ResolveUrl("~/Libs/Images/Icon/tbapprove.png")%>' alt="" /><br style="clear: both" />
        <div>
            <%=GetLabel("Approve")%></div>
    </li>
    <li id="btnDirectPurchaseReturnHdDecline" crudmode="R" runat="server">
...
<asp:Content ID="Content1" ContentPlaceHolderID="plhEntry" runat="server">
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $('#<%=btnDirectPurchaseReturnHdItem.ClientID %>').click(function () {
                showLoadingPanel();
                var id = $('#<%=hdnID.ClientID %>').val();
                var url = ResolveUrl('~/Program/Procurement/DirectPurchase/DirectPurchaseReturnApprovalDetailList.aspx?id=' + id);
                document.location = url;
            });

            $('#<%=btnDirectPurchaseReturnHdApprove.ClientID %>').click(function () {
                if ($('.chkIsSelected input:checked').length < 1) {
                    showToast('Warning', 'Please Select Direct Purchase Return First');
                }
                else {
                    var param = '';
                    $('.chkIsSelected input:checked').each(function () {
                        var DirectPurchaseReturnHdID = $(this).closest('tr').find('.keyField').html();
                        if (param != '')
                            param += ',';
                        param += DirectPurchaseReturnHdID;
                    });
            ...
            $('#<%=btnDirectPurchaseReturnHdDecline.ClientID %>').click(function () {
                if ($('.chkIsSelected input:checked').length < 1) {
                    showToast('Warning', 'Please Select Direct Purchase Return First');
                }
                else {
                            ...
                            <dx:PanelContent ID="PanelContent1" runat="server">
                                <asp:Panel runat="server" ID="pnlView" CssClass="pnlContainerGrid">
                                    <asp:GridView ID="grdView" runat="server" CssClass="grdSelected" AutoGenerateColumns="false"
                                        ...
                                        <Columns>
                                            <asp:BoundField DataField="DirectPurchaseReturnID" HeaderStyle-CssClass="keyField"
                                            ...
                                            <asp:BoundField DataField="DirectPurchaseReturnNo" HeaderText="No Retur Pembelian Tunai"
                                                HeaderStyle-Width="150px" />
                                            <asp:BoundField DataField="SupplierName" HeaderText="Nama Supplier" HeaderStyle-Width="300px" />
                                            <asp:BoundField DataField="PurchaseReturnType" HeaderText="Penggantian" HeaderStyle-Width="200px" />
                                            <asp:BoundField DataField="DirectPurchaseNo" HeaderText="No Pembelian Tunai" HeaderStyle-Width="150px" />
                                            <asp:BoundField DataField="Remarks" HeaderText="Keterangan" />
                                            <asp:BoundField DataField="cfTransactionAmountInString" HeaderText="Total Nilai Retur"
