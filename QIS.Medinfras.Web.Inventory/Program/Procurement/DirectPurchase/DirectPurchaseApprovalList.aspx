<%@ Page Title="" Language="C#" MasterPageFile="~/libs/MasterPage/MPTrx.master" AutoEventWireup="true"
    CodeBehind="DirectPurchaseApprovalList.aspx.cs" Inherits="QIS.Medinfras.Web.Inventory.Program.DirectPurchaseApprovalList" %>
...
<asp:Content ID="Content3" ContentPlaceHolderID="plhCustomButtonToolbar" runat="server">
    <li id="btnDirectPurchaseHdItem" crudmode="R" runat="server">
        <img src='<%=ResolveUrl("~/Libs/Images/Icon/tbnew.png")%>' alt="" /><br style="clear: both" />
        <div>
            <%=GetLabel("Item")%></div>
    </li>
    <li id="btnDirectPurchaseHdApprove" crudmode="R" runat="server">
        <img src='<%=ResolveUrl("~/Libs/Images/Icon/tbapprove.png")%>' alt="" /><br style="clear: both" />
        <div>
            <%=GetLabel("Approve")%></div>
    </li>
    <li id="btnDirectPurchaseHdDecline" crudmode="R" runat="server">
...
<asp:Content ID="Content1" ContentPlaceHolderID="plhEntry" runat="server">
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $('#<%=btnDirectPurchaseHdItem.ClientID %>').click(function () {
                showLoadingPanel();
                var id = $('#<%=hdnID.ClientID %>').val();
                var url = ResolveUrl('~/Program/Procurement/DirectPurchase/DirectPurchaseApprovalDetailList.aspx?id=' + id);
                document.location = url;
            });

            $('#<%=btnDirectPurchaseHdApprove.ClientID %>').click(function () {
                if ($('.chkIsSelected input:checked').length < 1) {
                    showToast('Warning', 'Please Select Direct Purchase First');
                }
                else {
                    var param = '';
                    $('.chkIsSelected input:checked').each(function () {
                        var DirectPurchaseHdID = $(this).closest('tr').find('.keyField').html();
                        if (param != '')
                            param += ',';
                        param += DirectPurchaseHdID;
                    });
            ...
            $('#<%=btnDirectPurchaseHdDecline.ClientID %>').click(function () {
                if ($('.chkIsSelected input:checked').length < 1) {
                    showToast('Warning', 'Please Select Direct Purchase First');
                }
                else {
                                    ...
                                    <asp:GridView ID="grdView" runat="server" CssClass="grdSelected" AutoGenerateColumns="false"
                                        ...
                                        <Columns>
                                            ...
                                            <asp:BoundField DataField="BusinessPartnerName" HeaderText="Supplier" HeaderStyle-Width="300px"
                                                HeaderStyle-HorizontalAlign="Left" ItemStyle-HorizontalAlign="Left" />
                                            <asp:BoundField DataField="ProductLineName" HeaderText="Product Line" HeaderStyle-Width="230px"
                                                HeaderStyle-HorizontalAlign="Left" ItemStyle-HorizontalAlign="Left" />
                                            <asp:BoundField DataField="DirectPurchaseType" HeaderText="Jenis Pembelian Tunai" HeaderStyle-Width="150px"
                                                HeaderStyle-HorizontalAlign="Center" ItemStyle-HorizontalAlign="Center" />
                                            <asp:BoundField DataField="Remarks" HeaderText="Keterangan" />
                                            <asp:BoundField DataField="cfTotalDirectPurchaseInString" HeaderStyle-Width="200px" HeaderStyle-HorizontalAlign="Right"
                                                ItemStyle-HorizontalAlign="Right" HeaderText="Jumlah Transaksi" />
                                        </Columns>
                                        <EmptyDataTemplate>
