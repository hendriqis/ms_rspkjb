<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="PurchaseRequestDtReceivedDetailCtl.ascx.cs"
    Inherits="QIS.Medinfras.Web.Inventory.Program.PurchaseRequestDtReceivedDetailCtl" %>
...
<input type="hidden" value="" id="hdnItemCode" runat="server" />
                            ...
                            <asp:Panel runat="server" ID="pnlView" CssClass="pnlContainerGridPatientPage">
                                <table style="width: 100%">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <asp:TextBox ID="txtPurchaseRequestNo" ReadOnly="true" Width="50%" runat="server" />
                                        </td>
                                    </tr>
                                </table>
                                    ...
                                    <Columns>
                                        ...
                                        <asp:TemplateField HeaderStyle-Width="10px" ItemStyle-HorizontalAlign="Center">
                                            <ItemTemplate>
                                                ...
                                                <input type="hidden" value="<%#:Eval("ItemCode") %>" class="ItemCode" bindingfield="ItemCode" />
                                        ...
                                        <asp:BoundField DataField="PurchaseUnit" HeaderText="Satuan Minta" HeaderStyle-Width="100px"
                            ...
                            <asp:Panel runat="server" ID="Panel1" CssClass="pnlContainerGridPatientPage">
                                <table style="width: 100%">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <asp:TextBox ID="txtItemNameCode" ReadOnly="true" Width="90%" runat="server" />
                                        </td>
                                    </tr>
                                </table>
                                <asp:GridView ID="grdViewPODT" runat="server" CssClass="grdSelected grdPRDT" AutoGenerateColumns="false"
                                    ShowHeaderWhenEmpty="true" EmptyDataRowStyle-CssClass="trEmpty">
                                    <Columns>
                                        <asp:BoundField DataField="ID" HeaderStyle-CssClass="keyField" ItemStyle-CssClass="keyField" />
                                        <asp:BoundField DataField="PurchaseOrderNo" HeaderText="No. Pemesanan" HeaderStyle-Width="200px"
                                            ItemStyle-HorizontalAlign="Left" HeaderStyle-HorizontalAlign="Left" />
                                        <asp:BoundField DataField="OrderDateInString" HeaderText="Tanggal Permintaan" HeaderStyle-Width="120px"
                                            ItemStyle-HorizontalAlign="Center" HeaderStyle-HorizontalAlign="Center" />
...
<script type="text/javascript" id="dxss_infopurchaseorderctl">
    $(function () {
        $('#<%=grdViewPRDT.ClientID %> > tbody > tr:gt(0):not(.trEmpty)').live('click', function () {
            if ($(this).attr('class') != 'selected') {
                ...
                $('#<%=hdnItemCode.ClientID %>').val($(this).find('.ItemCode').val());
                $('#<%=hdnItemName.ClientID %>').val($(this).find('.ItemName1').val());
                cbpViewPRDT.PerformCallback('refresh');
            }
        });

        $('#<%=grdViewPODT.ClientID %> tr:gt(0):not(.trEmpty)').live('click', function () {
            if ($(this).attr('class') != 'selected') {
    ...
    function oncbpViewPRDTEndCallback(s) {
        ...
        $('#<%=txtItemNameCode.ClientID %>').val("(" + $('#<%=hdnItemCode.ClientID %>').val() + ") " + $('#<%=hdnItemName.ClientID %>').val());
        var param = s.cpResult.split('|');
        if (param[0] == 'refresh') {
