<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ItemRequestPurchaseRequestCtl.ascx.cs"
    Inherits="QIS.Medinfras.Web.Inventory.Program.ItemRequestPurchaseRequestCtl" %>
...
<table style="width: 100%">
    ...
    <tr>
                    ...
                    <PanelCollection>
                        <dx:PanelContent ID="PanelContent1" runat="server">
                            <asp:Panel runat="server" ID="pnlView" CssClass="pnlContainerGridPatientPage">
                                ...
                                <asp:GridView ID="grdViewPRDT" runat="server" CssClass="grdSelected grdIRDT" AutoGenerateColumns="false"
        ...
        <td valign="top">
            <div style="position: relative;">
                <dxcp:ASPxCallbackPanel ID="cbpViewPurchaseRequestDt" runat="server" Width="100%" ClientInstanceName="cbpViewPurchaseRequestDt"
                    ShowLoadingPanel="false" OnCallback="cbpViewPurchaseRequestDt_Callback">
                    <ClientSideEvents BeginCallback="function(s,e){ $('#containerImgLoadingViewDt').show(); }"
                        EndCallback="function(s,e){ oncbpViewPurchaseRequestDtEndCallback(s); }" />
                    <PanelCollection>
                        <dx:PanelContent ID="PanelContent2" runat="server">
                            <asp:Panel runat="server" ID="Panel1" CssClass="pnlContainerGridPatientPage">
                                ...
                                <asp:GridView ID="grdViewPurchaseRequestDt" runat="server" CssClass="grdSelected grdDistributionDt" AutoGenerateColumns="false"
                                    ShowHeaderWhenEmpty="true" EmptyDataRowStyle-CssClass="trEmpty">
                                    <Columns>
                                        <asp:BoundField DataField="ID" HeaderStyle-CssClass="keyField" ItemStyle-CssClass="keyField" />
                                        <asp:BoundField DataField="PurchaseRequestNo" HeaderText="No. Permintaan Pembelian" HeaderStyle-Width="200px"
                                            ItemStyle-HorizontalAlign="Left" HeaderStyle-HorizontalAlign="Left" />
                                        <asp:BoundField DataField="TransactionDateInString" HeaderText="Tanggal Transaksi" HeaderStyle-Width="120px"
                                            ItemStyle-HorizontalAlign="Center" HeaderStyle-HorizontalAlign="Center" />
                                        <asp:BoundField DataField="Quantity" HeaderText="Qty Minta Beli" HeaderStyle-Width="80px"
                                            ItemStyle-HorizontalAlign="Right" HeaderStyle-HorizontalAlign="Right" />
                                        <asp:BoundField DataField="PurchaseUnit" HeaderText="Satuan Minta Beli" HeaderStyle-Width="100px"
                                            ItemStyle-HorizontalAlign="Center" HeaderStyle-HorizontalAlign="Center" />
                                    </Columns>
                                    <EmptyDataTemplate>
                                        <%=GetLabel("No Data To Display") %>
...
<script type="text/javascript" id="dxss_infopurchaseorderctl">
    $(function () {
        $('#<%=grdViewPRDT.ClientID %> > tbody > tr:gt(0):not(.trEmpty)').live('click', function () {
            if ($(this).attr('class') != 'selected') {
                ...
                cbpViewPurchaseRequestDt.PerformCallback('refresh');
            }
        });

        $('#<%=grdViewPurchaseRequestDt.ClientID %> tr:gt(0):not(.trEmpty)').live('click', function () {
            if ($(this).attr('class') != 'selected') {
                $('#<%=grdViewPurchaseRequestDt.ClientID %> tr.selected').removeClass('selected');
                $(this).addClass('selected');
            }
        });
    ...
    //#region Paging Dt
    function oncbpViewPurchaseRequestDtEndCallback(s) {
        ...
        if (param[0] == 'refresh') {
            ...
            var pageCount = parseInt(param[1]);
            if (pageCount > 0)
                $('#<%=grdViewPurchaseRequestDt.ClientID %> tr:eq(1)').click();

            setPaging($("#pagingDt"), pageCount, function (page) {
                cbpViewPurchaseRequestDt.PerformCallback('changepage|' + page);
            });
        }
        else
            $('#<%=grdViewPurchaseRequestDt.ClientID %> tr:eq(1)').click();
