...
CodeBehind="ItemRequestApprovalList.aspx.cs" Inherits="QIS.Medinfras.Web.Inventory.Program.ItemRequestApprovalList" %>
...
<asp:Content ID="Content3" ContentPlaceHolderID="plhCustomButtonToolbar" runat="server">
    <li id="btnItemRequestHdItem" CRUDMode="R" runat="server"><img src='<%=ResolveUrl("~/Libs/Images/Icon/tbnew.png")%>' alt="" /><br style="clear:both"/><div><%=GetLabel("Item")%></div></li>
    <li id="btnItemRequestHdApprove" CRUDMode="R" runat="server"><img src='<%=ResolveUrl("~/Libs/Images/Icon/tbapprove.png")%>' alt="" /><br style="clear:both"/><div><%=GetLabel("Approve")%></div></li>
    <li id="btnItemRequestHdDecline" CRUDMode="R" runat="server"><img src='<%=ResolveUrl("~/Libs/Images/Icon/tbvoid.png")%>' alt="" /><br style="clear:both"/><div><%=GetLabel("Decline")%></div></li>
</asp:Content>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $('#<%=btnItemRequestHdItem.ClientID %>').click(function () {
                ...
                var url = ResolveUrl('~/Program/Warehouse/ItemOrder/ItemRequestApprovalDetailList.aspx?id=' + id);
                document.location = url;
            });

            $('#<%=btnItemRequestHdApprove.ClientID %>').click(function () {
            ...
            $('#<%=btnItemRequestHdDecline.ClientID %>').click(function () {
                if ($('.chkIsSelected input:checked').length < 1) {
                    showToast('Warning', 'Please Select Item Request First');
                }
                else {
