<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ItemRequestQuickPicksCtl.ascx.cs"
    Inherits="QIS.Medinfras.Web.Inventory.Program.ItemRequestQuickPicksCtl" %>
...
<script type="text/javascript" id="dxss_drugslogisticsquickpicksctl">
    ...
    $(function () {
        ...
        function onGetItemTypeToFilterExpression() {
            ...
            var GCItemType = $('#<%=hdnProductLineItemTypeCtl.ClientID %>').val();
            var GCLocationGroup = $('#<%=hdnGCLocationGroupToCtl.ClientID %>').val();
        ...
        function onGetItemTypeFromFilterExpression() {
            var ProductLineID = $('#<%=hdnProductLineIDCtl.ClientID %>').val();
            var GCItemType = $('#<%=hdnProductLineItemTypeCtl.ClientID %>').val();
            var GCLocationGroup = $('#<%=hdnGCLocationGroupFromCtl.ClientID %>').val();
            var filterExpression = "IsDeleted = 0";
        ...
        function onGetItemGroupFilterExpression() {
            ...
            if (chk == 'from') {
                if ($('#<%=hdnFromLocationItemGroupIDCtl.ClientID %>').val() != '' && $('#<%=hdnFromLocationItemGroupIDCtl.ClientID %>').val() != '0') {
                    filterExpression += " AND ItemGroupID IN (SELECT ItemGroupID FROM vItemGroupMaster1 WITH(NOLOCK) WHERE DisplayPath like '%/" + $('#<%=hdnFromLocationItemGroupIDCtl.ClientID %>').val() + "/%')";
                }
            } else {
                if ($('#<%=hdnToLocationItemGroupIDCtl.ClientID %>').val() != '' && $('#<%=hdnToLocationItemGroupIDCtl.ClientID %>').val() != '0') {
                    filterExpression += " AND ItemGroupID IN (SELECT ItemGroupID FROM vItemGroupMaster1 WITH(NOLOCK) WHERE DisplayPath like '%/" + $('#<%=hdnToLocationItemGroupIDCtl.ClientID %>').val() + "/%')";
                }
            }
...
<div style="padding: 10px;">
    ...
    <input type="hidden" id="hdnSelectedMemberLocation" runat="server" value="" />
    <input type="hidden" id="hdnSelectedMemberQty" runat="server" value="" />
    <input type="hidden" id="hdnLocationIDFromCtl" runat="server" value="" />
    <input type="hidden" id="hdnGCLocationGroupFromCtl" runat="server" value="" />
    <input type="hidden" id="hdnFromLocationItemGroupIDCtl" value="" runat="server" />
    <input type="hidden" id="hdnLocationIDToCtl" runat="server" value="" />
    <input type="hidden" id="hdnGCLocationGroupToCtl" runat="server" value="" />
    <input type="hidden" id="hdnToLocationItemGroupIDCtl" value="" runat="server" />
    <input type="hidden" id="hdnItemLocationIs" runat="server" value="" />
    ...
    <input type="hidden" id="hdnIsFilterQtyOnHand" runat="server" value="" />
    <table>
        <colgroup>
    ...
    <table style="width: 100%">
        ...
        <tr>
                            ...
                            <asp:Panel runat="server" ID="pnlView" Style="width: 100%; margin-left: auto; margin-right: auto;
                                ...
                                <asp:GridView ID="grdView" runat="server" CssClass="grdView notAllowSelect" AutoGenerateColumns="false"
                                    ...
                                    <Columns>
                                        ...
                                        <asp:BoundField HeaderStyle-Width="40px" DataField="BinLocationName" HeaderText="Rak" HeaderStyle-HorizontalAlign="Left"
                                            ItemStyle-CssClass="tdBinLocationName" />
                                        <asp:BoundField HeaderStyle-Width="50px" DataField="QuantityEND" HeaderText="Stok"
                                            HeaderStyle-HorizontalAlign="Right" ItemStyle-HorizontalAlign="Right" DataFormatString="{0:N}" />
                                        <asp:BoundField HeaderStyle-Width="40px" DataField="ItemUnit" HeaderText="Satuan"
                                            HeaderStyle-HorizontalAlign="Left" ItemStyle-HorizontalAlign="Left" />
                                        <asp:BoundField HeaderStyle-Width="120px" DataField="cfConversionFactor" HeaderText="Satuan Beli"
                                            HeaderStyle-HorizontalAlign="Left" ItemStyle-HorizontalAlign="Left" />
                                    </Columns>
                                    <EmptyDataTemplate>
                                        <%=GetLabel("Tidak ada data persediaan di lokasi yang dipilih")%>
                                    </EmptyDataTemplate>
                                </asp:GridView>
                            </asp:Panel>
                        </dx:PanelContent>
            ...
            <td style="padding: 5px; vertical-align: top">
                <h4>
                    <%=GetLabel("Dipilih (Jumlah dalam Satuan Kecil)")%></h4>
