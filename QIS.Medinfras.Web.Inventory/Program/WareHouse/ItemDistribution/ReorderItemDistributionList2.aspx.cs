...
{
    public partial class ReorderItemDistributionList2 : BasePageTrx
    {
        ...
        private void BindGridView(int pageIndex, bool isCountPageCount, ref int pageCount)
        {
            ...
            if (hdnLocationIDTo.Value != "" && hdnLocationIDFrom.Value != "")
                filterExpression = string.Format("LocationID = {0} AND IsUsingDynamicROP = {2} AND QuantityEND <= QuantityMIN AND IsDeleted = 0 AND ItemID IN (SELECT ItemID FROM ItemBalance WHERE LocationID = {1} AND IsDeleted = 0)", hdnLocationIDTo.Value, hdnLocationIDFrom.Value, isUsingDynamic);
        ...
        protected void grdView_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            else grdView.Columns[7].Visible = true;
