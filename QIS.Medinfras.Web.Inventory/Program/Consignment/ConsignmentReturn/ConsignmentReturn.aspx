<%@ Page Title="" Language="C#" MasterPageFile="~/libs/MasterPage/MPTrx2.master"
    AutoEventWireup="true" CodeBehind="ConsignmentReturn.aspx.cs" Inherits="QIS.Medinfras.Web.Inventory.Program.ConsignmentReturn" %>
    ...
    <script type="text/javascript">
        ...
        function onLoad() {
            ...
            function getItemFilterExpression() {
                ...
                if (receiveID != '') {
                    filterExpression += "PurchaseReceiveID = " + receiveID + " AND GCItemDetailStatus != '<%=GetTransactionStatusVoid() %>' AND (ISNULL(Quantity,0) - ISNULL(ReturnedQuantity,0) - ISNULL(OrderQuantity,0)) > 0";
                }
                if (returnID != '') {
                    filterExpression += " AND ID NOT IN (SELECT PurchaseReceiveDtID FROM PurchaseReturnDt WHERE PurchaseReturnID = " + returnID + " AND GCItemDetailStatus != '<%=GetTransactionStatusVoid() %>')";
                }
            ...
            function onTxtItemCodeChanged(value) {
                var isFromOrder = $('#<%=hdnIsAPConsignmentFromOrder.ClientID %>').val();
                ...
                Methods.getObject('GetvPurchaseReceiveDtList', filterExpression, function (result) {
                    if (result != null) {
                        ...
                        $('#<%=hdnOrderQuantity.ClientID %>').val(result.OrderQuantity);

                        $('#<%=txtQuantity.ClientID %>').attr('max', result.ReceiveForReturnQuantity);
                        $('#<%=hdnReceiveQtyBaseUnit.ClientID %>').val(result.ReceiveForReturnQuantity);
                        ...
                        $('#<%=hdnOrderQuantity.ClientID %>').val('0');
            ...
            function onGetPurchaseReceiveFilterExpression() {
                //var filterExpression = "TransactionCode = '4214' AND PurchaseReceiveID NOT IN (SELECT PurchaseReceiveID FROM vPurchaseInvoiceDt WHERE IsDeleted = 0 AND GCTransactionStatus != 'X121^999' AND PurchaseReceiveID IS NOT NULL) AND PurchaseReceiveID NOT IN (SELECT PurchaseReceiveID FROM PurchaseReturnHd WHERE GCTransactionStatus != 'X121^999') AND GCTransactionStatus = 'X121^003'";               
                var filterExpression = "TransactionCode = '4214' AND GCTransactionStatus IN ('X121^003','X121^004')";
            ...
            $('#<%=txtQuantity.ClientID %>').change(function () {
                ...
                var maxQty = $('#<%=hdnReceiveQtyBaseUnit.ClientID %>').val();

                if (qty <= maxQty) {
                    calculateSubTotal();
                } else {
                    alert("Maaf, batas maksimal retur adalah sejumlah " + maxQty);
                    $('#<%=txtQuantity.ClientID %>').val("0").trigger('changeValue');
                    calculateSubTotal();
                }
            });

            function onBeforeSaveRecord() {
                var qtyPO = $('#<%=hdnOrderQuantity.ClientID %>').val();
                var qtyReceive = $('#<%=txtReceivedQty.ClientID %>').val();
                var qtyAllow = qtyReceive - qtyPO;
                var qtyInput = $('#<%=txtQuantity.ClientID %>').val();
                var itemName1 = $('#<%=txtItemName.ClientID %>').val();

                if (qtyInput > qtyAllow) {
                    displayErrorMessageBox('ERROR','Jumlah <b>' + itemName1 + '</b> yang diproses melebihi jumlah yang diperbolehkan, yaitu maksimal sejumlah <b>' + qtyAllow + '</b> <br/>');
                    return false;
                }
            }
        ...
        $('#<%=grdView.ClientID %> .imgEdit.imgLink').live('click', function () {
            ...
            $('#<%=hdnOrderQuantity.ClientID %>').val(entity.OrderQuantity);
        ...
        function calculateTotal() {
            ...
            if ($('#<%=chkPPN.ClientID %>').is(':checked')) {
                var PPN = parseFloat($('#<%=hdnVATPercentage.ClientID %>').val()) / 100 * totalKotor;
        ...
        function onCboItemUnitChanged() {
            ...
            if (baseValue == toUnitItem) {
                ...
                Methods.getObjectValue('GetvItemAlternateUnitList', filterExpression, 'ConversionFactor', function (result) {
                    ...
                    var qty = parseFloat($('#<%=hdnReceiveQtyBaseUnit.ClientID %>').val()) / parseFloat(result);
                                        ...
                                        <table style="width: 100%">
                                            ...
                                            <tr>
                                                ...
                                                <td colspan="2">
                                                    ...
                                                    <input type="hidden" value="" id="hdnOrderQuantity" runat="server" />
                                    ...
                                    <td valign="top">
                                        <table style="width: 100%;">
                                            ...
                                            <tr>
                                                <td class="tdLabel">
                                                    <label class="lblNormal">
                                                        <%=GetLabel("PPN")%><asp:TextBox ID="txtVATPercentageDefault" CssClass="txtCurrency"
                                                            ReadOnly="true" Width="40px" runat="server" />
                                                        %</label>
                                                </td>
                                                <td>
