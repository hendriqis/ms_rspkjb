...
{
    public partial class LaboratoryTestResultDetail1 : BasePageTrx
    {
        ...
        protected override void InitializeDataControl()
        {
            if (Page.Request.QueryString.Count > 0)
            {
                ...
                if (entityChargesHd != null)
                {
                    ...
                    {
                        trPA.Style.Remove("display");
                        txtPAReferenceNo.Text = entityChargesHdInfo.PAReferenceNo;
                    }
                }
        ...
        {
            return AppSession.IsBridgingToLIS.ToString();
        }
        ...
        public void SaveLaboratoryResultHd(IDbContext ctx, ref int labResultID, ref string referenceNo)
        {
            ...
            if (hdnLabResultID.Value == "0")
            {
                ...
                if (cboResultDeliveryPlan.Value != null)
                {
                    ...
                    {
                        entityHd.GCResultDeliveryPlan = cboResultDeliveryPlan.Value.ToString();
                        if (cboResultDeliveryPlan.Value.ToString() == Constant.ResultDeliveryPlan.OTHERS)
                        {
                            entityHd.ResultDeliveryPlanOthers = Request.Form[txtResultDeliveryPlanOthers.UniqueID];
                        }
                        else
                        {
                            entityHd.ResultDeliveryPlanOthers = null;
                        }
                    }
                    else
                    {
                        entityHd.GCResultDeliveryPlan = null;
                        entityHd.ResultDeliveryPlanOthers = null;
                    }
                }
                else
        ...
        protected override bool OnProposeRecord(ref string errMessage)
        {
            ...
            IDbContext ctx = DbFactory.Configure(true);
            LaboratoryResultHdDao entityHdDao = new LaboratoryResultHdDao(ctx);
            PatientChargesHdInfoDao pcHDinfoDao = new PatientChargesHdInfoDao(ctx);
            try
            {
                if (hdnLabResultID.Value != null && hdnLabResultID.Value != "")
                {
                    ...
                    if (entityResult != null)
                    {
                        ...
                        if (AppSession.LB0034 == "1" && hdnIsPathologicalAnatomyTest.Value == "1")
                        {
                            PatientChargesHdInfo oHDInfo = BusinessLayer.GetPatientChargesHdInfo(Convert.ToInt32(hdnTransactionHdID.Value));
                            if (oHDInfo != null)
                            {
                                oHDInfo.PAReferenceNo = txtPAReferenceNo.Text;
                                pcHDinfoDao.Update(oHDInfo);
                            }
                        }
                        ctx.CommandType = CommandType.Text;
                        ctx.Command.Parameters.Clear();
            ...
            else
            {
                ...
                else if (type == "reopen_result")
                {
                    ...
                    try
                    {
                        ...
                        {
                            string filterLaboratoryHd = string.Format("ChargeTransactionID = {0} AND ID = {1} AND GCTransactionStatus = '{2}' AND IsDeleted = 0",
                                                                                                                hdnTransactionHdID.Value, hdnLabResultID.Value, Constant.TransactionStatus.WAIT_FOR_APPROVAL);
                            LaboratoryResultHd entityResult = BusinessLayer.GetLaboratoryResultHdList(filterLaboratoryHd, ctx).FirstOrDefault();
                            if (entityResult != null)
                            {
