<%@ Page Language="C#" MasterPageFile="~/Libs/MasterPage/MPTrx.master" AutoEventWireup="true" 
    CodeBehind="PurchaseRequestEntry.aspx.cs" Inherits="QIS.Medinfras.Web.Pharmacy.Program.PurchaseRequestEntry" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="plhEntry" runat="server">   
    <script type="text/javascript">
        function onLoad() {
            ...
            $('#lblTransactionNo.lblLink').click(function () {
                openSearchDialog('patientchargeshd', '', function (value) {
            ...
            $('#<%=txtQty.ClientID %>').change(function () {
                var conversionValue = parseFloat($('#<%=hdnConversionValue.ClientID %>').val());
                var qty = parseFloat($(this).val());
                $('#<%=txtBaseQty.ClientID %>').val(conversionValue * qty);
            });
        }
        ...
        function onLedItemLostFocus(led) {
            var id = led.GetValueText();
            $('#<%=hdnItemID.ClientID %>').val(id);
            if (id != '') {
                showLoadingPanel();
                cboUoM.PerformCallback();
            }
        }

        function onCboUoMEndCallback() {
            hideLoadingPanel();

            var filterExpression = "HealthcareID = '" + AppSession.healthcareID + "' AND ItemID = " + $('#<%=hdnItemID.ClientID %>').val();
            Methods.getObject('GetvItemPlanningList', filterExpression, function (result) {
                ledSupplier.SetValue(result.BusinessPartnerID);
                cboUoM.SetValue(result.GCItemUnit);
                $('#<%=hdnDefaultUoM.ClientID %>').val(result.GCItemUnit);
                $('#<%=txtQtyOnOrder.ClientID %>').val('0');
                $('#<%=txtQty.ClientID %>').val('1');
                $('#<%=txtDiscount.ClientID %>').val('0');
                setItemUnitConversionText();
            });
        }

        function getItemUnitName(itemUnitID) {
            var value = cboUoM.GetValue();
            cboUoM.SetValue(itemUnitID);
            var text = cboUoM.GetText();
            cboUoM.SetValue(value);
            return text;
        }

        function setItemUnitConversionText() {
            var defaultUoM = $('#<%=hdnDefaultUoM.ClientID %>').val();
            var currUoM = cboUoM.GetValue();
            var fromConversion = getItemUnitName(defaultUoM);
            if (defaultUoM == currUoM) {
                $('#<%=hdnConversionValue.ClientID %>').val('1');
                var conversion = "1 " + fromConversion + " = 1 " + fromConversion;
                $('#<%=txtConversion.ClientID %>').val(conversion);
                $('#<%=txtQty.ClientID %>').change();
            }
            else {
                ...
                Methods.getObjectValue('GetvItemAlternateUnitList', filterExpression, 'ConversionFactor', function (result) {
                    var toConversion = getItemUnitName(currUoM);
                    $('#<%=hdnConversionValue.ClientID %>').val(result);
                    var conversion = "1 " + fromConversion + " = " + result + " " + toConversion;
                    $('#<%=txtConversion.ClientID %>').val(conversion);
                    $('#<%=txtQty.ClientID %>').change();
                });
            }
        ...
        function onLedSupplierLostFocus(led) {
            var id = led.GetValueText();
            $('#<%=hdnSupplierID.ClientID %>').val(id);
        }
        ...
        function onCboLocationValueChanged() {
            ...
            var filterExpression = $('#<%=hdnLedItemFilterExpression.ClientID %>').val().replace('[LocationID]', locationID);
    ...
    <div style="height:500px;overflow-y:auto;">
        <div class="pageTitle">
            <div style="font-size: 1em"><%=GetLabel("Purchase Request")%></div>
        </div>
            ...
            <tr>
                    ...
                    <table class="tblEntryContent" style="width:100%">
                        ...
                        <tr>
                            ...
                            <table cellpadding="0" cellspacing="0">
                                <tr>
                                    <td style="padding-right: 1px;width:140px"><asp:TextBox ID="txtTransactionDate" Width="120px" CssClass="datepicker" runat="server" /></td>
                ...
                <td style="padding:5px;vertical-align:top">
                    <table class="tblEntryContent" style="width:100%">
                        ...
                        <tr>
                            <td class="tdLabel"><%=GetLabel("Notes") %></td>
                    ...
                    <div id="containerEntry" style="margin-top:4px;display:none;">
                        ...
                        <fieldset id="fsTrxService" style="margin:0"> 
                            <table class="tblEntryDetail" style="width:100%">
                                ...
                                <tr>
                                    <td class="tdLabel"><label><%=GetLabel("Item")%></label></td>
                                    <td colspan="2">
                                        <input type="hidden" value="" id="hdnItemID" runat="server" />
                                        <input type="hidden" value="" id="hdnConversionValue" runat="server" />
                                        <input type="hidden" value="" id="hdnLedItemFilterExpression" runat="server" />
                                        <div id="divLedItemName">
                                            <qis:QISSearchTextBox ID="ledItem" ClientInstanceName="ledItem" runat="server" Width="99%"
                                                ValueText="ItemID" FilterExpression="IsDeleted = 0" DisplayText="ItemName1" MethodName="GetvItemBalanceList" >
                                                <ClientSideEvents ValueChanged="function(s){ onLedItemLostFocus(s); }" />
                                                <Columns>
                                                    <qis:QISSearchTextBoxColumn Caption="Item Code" FieldName="ItemCode" Description="i.e. ABA00001" Width="100px" />
                                                    <qis:QISSearchTextBoxColumn Caption="Item Name" FieldName="ItemName1" Description="i.e. ABAJOS CAP" Width="300px" />
                                                </Columns>
                                            </qis:QISSearchTextBox>
                                        </div>
                                        <div id="divTxtItemName" style="display:none">
                                ...
                                <tr>
                                    <td class="tdLabel"><label><%=GetLabel("Supplier")%></label></td>
                                    <td colspan="2">
                                        <input type="hidden" value="" id="hdnSupplierID" runat="server" />
                                        <qis:QISSearchTextBox ID="ledSupplier" ClientInstanceName="ledSupplier" runat="server" Width="99%"
                                            ValueText="BusinessPartnerID" FilterExpression="IsDeleted = 0" DisplayText="BusinessPartnerName" MethodName="GetBusinessPartnersList" >
                                            <ClientSideEvents ValueChanged="function(s){ onLedSupplierLostFocus(s); }" />
                                            <Columns>
                                                <qis:QISSearchTextBoxColumn Caption="Supplier Code" FieldName="BusinessPartnerCode" Description="i.e. HCP0001" Width="100px" />
                                                <qis:QISSearchTextBoxColumn Caption="Supplier Name" FieldName="BusinessPartnerName" Description="i.e. Johnson" Width="300px" />
                                            </Columns>
                                        </qis:QISSearchTextBox>
                                    </td>
                                </tr>                                   
                                <tr>
                                    <td class="tdLabel"><label><%=GetLabel("Qty On Order")%></label></td>
                                    <td colspan="2"><asp:TextBox ID="txtQtyOnOrder" Width="100px" CssClass="number" runat="server" /></td>
                                </tr>    
                                <tr>
                                    <td class="tdLabel"><label><%=GetLabel("Qty")%></label></td>
                                    <td><asp:TextBox ID="txtQty" Width="100%" CssClass="number" runat="server" /></td>
                                    <td>
                                        <input type="hidden" value="" id="hdnDefaultUoM" runat="server" />
                                        <dxe:ASPxComboBox ID="cboUoM" runat="server" ClientInstanceName="cboUoM" Width="200px" OnCallback="cboUoM_Callback">
                                            <ClientSideEvents EndCallback="function(s,e) { onCboUoMEndCallback(); }" 
                                                ValueChanged="function(s,e){ setItemUnitConversionText(); }" />
                                        </dxe:ASPxComboBox>
                                    </td>
                                </tr>
                                <tr>
                                    <td>&nbsp;</td>
                                    <td><div class="lblComponent"><%=GetLabel("Qty")%></div></td>
                                    <td><div class="lblComponent"><%=GetLabel("Conversion")%></div></td>
                                </tr>
                                <tr>
                                    <td class="tdLabel"><label><%=GetLabel("Base Quantity")%></label></td>
                                    <td><asp:TextBox ID="txtBaseQty" ReadOnly="true" CssClass="number" Width="100%" runat="server" /></td>
                                    <td><asp:TextBox ID="txtConversion" ReadOnly="true" Width="100%" runat="server" /></td>
                                </tr>                                 
                                <tr>
                                    <td class="tdLabel"><label><%=GetLabel("Price")%></label></td>
                                    <td colspan="2"><asp:TextBox ID="txtPrice" Width="200px" CssClass="txtCurrency" runat="server" /></td>
                                </tr>                                   
                                <tr>
                                    <td class="tdLabel"><label><%=GetLabel("Discount")%> (%)</label></td>
                                    <td colspan="2"><asp:TextBox ID="txtDiscount" Width="100px" CssClass="number" runat="server" /></td>
                                </tr>    
                                <tr>
                                    <td colspan="2">
                                        <table>
        ...
        <table style="width:100%" cellpadding="0" cellspacing="0">
            ...
            <tr>
                <td></td>
                <td></td>
                <td><div class="lblComponent" style="text-align: left; padding-left: 5px;"><%=GetLabel("Total") %>  : </div></td>
                <td style="text-align:right;padding-right: 10px;">
                    Rp. <asp:TextBox ID="txtTotal" ReadOnly="true" CssClass="txtCurrency" runat="server" Width="200px" />
                </td>
            </tr>
        </table> 
