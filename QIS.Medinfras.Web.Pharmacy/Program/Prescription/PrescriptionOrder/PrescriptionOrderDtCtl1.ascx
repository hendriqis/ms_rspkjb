<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="PrescriptionOrderDtCtl1.ascx.cs"
    Inherits="QIS.Medinfras.Web.Pharmacy.Program.PrescriptionOrderDt1Ctl" %>
...
<script type="text/javascript" id="dxss_PrescriptionOrderDt1Ctl">
    ...
    if ($('#<%=hdnIsGenerateTicket.ClientID %>').val() == "1") {
        $('#<%=txtReferenceNo2.ClientID %>').focus();
    }
            ...
            if (processType == "scheduled") {
                ...
                if ($('#<%=hdnIsOrderDetailExists.ClientID %>').val() == "1") {
                    ...
                    if ($('#<%=hdnLstSelected1.ClientID %>').val() == "") {
                        ...
                        displayMessageBox('Konfirmasi Order', messageBody);
                    }
                    else {
                        cbpEntryPopupView.PerformCallback(processType);
                    }
    ...
    $('#btnStockInfo').click(function (evt) {
        displayMessageBox('Informasi Stock', $('#<%=hdnBalanceInfo.ClientID %>').val());
    });
    ...
    $('.txtTakenQty1').live('change', function () {
        ...
        var takenqty = parseInt($tr.find('.txtTakenQty1').val());
        if (takenqty > orderQty) {
            ...
            $tr.find('.txtTakenQty1').val(orderQty);
        }
    });
    ...
    function onCbpEntryPopupViewEndCallback(s) {
        ...
        if (param[0] == 'approve' || param[0] == 'nottaken') {
            if (param[1] == 'fail') {
                ...
                displayErrorMessageBox('Konfirmasi Order', messageBody);
            }
            else {
            ...
            if (param[1] == 'fail') {
                var messageBody = param[2];
                displayMessageBox('ERROR : Proses Pembatalan', messageBody);
            }
            else {
                pcRightPanelContent.Hide();
            }
        }
        else if (param[0] == 'scheduled') {
            if (param[1] == 'fail') {
                var messageBody = param[2];
                displayMessageBox('ERROR : Proses Pembatalan', messageBody);
            }
    ...
    function onCbpCalculateItemViewEndCallback(s) {
        ...
        if (param[0] == 'calculate') {
            if (param[1] == 'fail') {
                ...
                $('#<%=hdnBalanceInfo.ClientID %>').val(retVal[3]);
                $('#btnStockInfo').show();
            }
        }
    }
    ...
    $('#<%=chkIsCorrectAlowAll.ClientID %>').change(function () {
        if ($('#<%=chkIsCorrectAlowAll.ClientID %>').is(':checked')) {
            $('#<%=chkIsCorrectPatient.ClientID%>').attr('checked', 'true');
            $('#<%=chkIsCorrectMedication.ClientID%>').attr('checked', 'true');
            $('#<%=chkIsCorrectStrength.ClientID%>').attr('checked', 'true');
            $('#<%=chkIsCorrectFrequency.ClientID%>').attr('checked', 'true');
            $('#<%=chkIsCorrectDosage.ClientID%>').attr('checked', 'true');
            $('#<%=chkIsCorrectRoute.ClientID%>').attr('checked', 'true');
            $('#<%=chkIsHasDrugInteraction.ClientID%>').attr('checked', 'true');
            $('#<%=chkIsHasDuplication.ClientID%>').attr('checked', 'true');
            $('#<%=chkIsCorrectTimeOfGiving.ClientID%>').attr('checked', 'true');
        } else {
            $('#<%=chkIsCorrectPatient.ClientID%>').removeAttr('checked');
            $('#<%=chkIsCorrectMedication.ClientID%>').removeAttr('checked');
            $('#<%=chkIsCorrectStrength.ClientID%>').removeAttr('checked');
            $('#<%=chkIsCorrectFrequency.ClientID%>').removeAttr('checked');
            $('#<%=chkIsCorrectDosage.ClientID%>').removeAttr('checked');
            $('#<%=chkIsCorrectRoute.ClientID%>').removeAttr('checked');
            $('#<%=chkIsHasDrugInteraction.ClientID%>').removeAttr('checked');
            $('#<%=chkIsHasDuplication.ClientID%>').removeAttr('checked');
            $('#<%=chkIsCorrectTimeOfGiving.ClientID%>').removeAttr('checked');
        }
    });

    $('#<%=chkSkringResep.ClientID %>').change(function () {
        if ($('#<%=chkSkringResep.ClientID %>').is(':checked')) {
            $('#<%=chkIsADChecked.ClientID%>').attr('checked', 'true');
            $('#<%=chkIsFARChecked.ClientID%>').attr('checked', 'true');
            $('#<%=chkIsKLNChecked.ClientID%>').attr('checked', 'true');
        } else {
            $('#<%=chkIsADChecked.ClientID%>').removeAttr('checked');
            $('#<%=chkIsFARChecked.ClientID%>').removeAttr('checked');
            $('#<%=chkIsKLNChecked.ClientID%>').removeAttr('checked');
        }
    });
    ...
    $('.imgViewPPRA.imgLink').click(function () {
        pcPPRAForm.Show();
        var id = $('#<%=hdnPrescriptionOrderID.ClientID %>').val();
        var url = "~/Libs/Controls/EMR/Information/CPOE/ViewPPRAFormCtl.ascx";
        $('#ppraContent').html('');
        if (url != '#') {
            $('#ppraContentLoading').show();
            Methods.getHtmlControl(ResolveUrl(url), id, function (result) {
                $('#ppraContentLoading').hide();
                $('#ppraContent').html(result.replace(/\VIEWSTATE/g, ''));
            }, function () {
                $('#ppraContentLoading').hide();
            });
        }
...
<input type="hidden" id="hdnIsCustomerTypeIsBPJS" value="" runat="server" />
...
<input type="hidden" id="hdnBalanceInfo" value="0" runat="server" />
...
<input type="hidden" id="hdnIsBridgingMedinlink" runat="server" value="0" />
<input type="hidden" id="hdnIsGetQueueNoMedinlink" runat="server" value="0" />
                ...
                <fieldset id="fsMPPopupEntry">
                    <table class="tblEntryContent" style="width: 100%">
                        ...
                        <tr>
                            <td class="tdLabel">
                                <label class="lblNormal">
                                    <%=GetLabel("Lokasi Pengambilan")%></label>
                            </td>
                            <td colspan="3">
                                <dxe:ASPxComboBox runat="server" ID="cboPrescriptionOrderLocation" ClientInstanceName="cboPrescriptionOrderLocation"
                                    Width="100%" />
                            </td>
                            <td class="tdLabel">
                                <label class="lblNormal">
                            ...
                            <td colspan="2">
                                <asp:TextBox ID="txtPayerName" Width="100%" runat="server" ReadOnly="true" />
                            </td>
                        </tr>
                        <tr>
                            <td />
                        ...
                        <tr>
                            ...
                            <td colspan="2">
                                <asp:TextBox ID="txtPrescriptionOrderNo" Width="100%" runat="server" />
                            </td>
                            <td>
                                ...
                                <div id="divViewPPRAForm" runat="server">
                                    <img class="imgViewPPRA imgLink blink-alert" id="img1" height="25px" src='<%= ResolveUrl("~/Libs/Images/Status/ppra.png")%>'
                                        alt='' title='View Formulir PPRA' />
                                </div>
                            </td>
                            <td class="tdLabel">
                            ...
                            <td>
                                <asp:TextBox ID="txtPrescriptionOrderDate" Width="120px" runat="server" Style="text-align: center"
                                    ReadOnly="true" />
                            </td>
                            <td style="">
                                <asp:TextBox ID="txtPrescriptionOrderTime" ReadOnly="true" Width="60px" runat="server"
                        ...
                        <tr>
                            ...
                            <td class="tdLabel">
                                <label class="lblNormal" id="lblSendOrderInfo">
                                    <%=GetLabel("Order Dikirim")%></label>
                            </td>
                            <td>
                                <asp:TextBox ID="txtSendOrderDate" Width="120px" runat="server" Style="text-align: center"
                                    ReadOnly="true" />
                            </td>
                            <td style="">
                                <asp:TextBox ID="txtSendOrderTime" ReadOnly="true" Width="60px" runat="server" Style="text-align: center" />
                            </td>
                        </tr>
                        <tr>
                            <td>
                        ...
                        <tr>
                            ...
                            <td colspan="6">
                                <asp:TextBox ID="txtNotes" ReadOnly="true" Width="100%" TextMode="MultiLine" Height="80px"
                                    runat="server" />
                            </td>
                        </tr>
                        <tr>
                        ...
                        <tr id="trTransactionDateTime" runat="server">
                            <td class="tdLabel">
                                <%=GetLabel("Tanggal") %>
                                /
                                <%=GetLabel("Jam Diproses") %>
                            </td>
                            <td>
                                <asp:TextBox ID="txtTransactionDate" Width="100px" runat="server" CssClass="datepicker"
                                    Style="text-align: center" />
                            </td>
                            <td>
                                <asp:TextBox ID="txtTransactionTime" Width="60px" CssClass="time" runat="server"
                                    Style="text-align: center" />
                            </td>
                            <td colspan="4">
                                    ...
                                    <tr>
                                        <td>
                                            <label class="lblNormal" id="Label2">
                                                <%=GetLabel("Nomor Itter")%></label>
                                        </td>
                                        <td>
                                            <asp:TextBox ID="txtReferenceNo1" Width="60px" runat="server" />
                                        </td>
                                        <td class="tdLabel">
                                            <label class="lblNormal" id="lblReferenceNo">
                                                <%=GetLabel("Nomor Antrian")%></label>
                                        </td>
                                        <td>
                                            <asp:TextBox ID="txtReferenceNo2" Width="60px" runat="server" />
                                        </td>
                                    </tr>
                                </table>
                            </td>
            ...
            <EmptyDataTemplate>
                <table id="tblView" runat="server" class="grdNormal notAllowSelect" cellspacing="0"
                    ...
                    <tr>
                        ...
                        <th style="width: 100px;" align="left">
                            <div>
                                <%=GetLabel("Jumlah Order")%></div>
                        </th>
                        <th style="width: 100px;" align="left">
                            <div>
                                <%=GetLabel("Jumlah Diambil")%></div>
                        </th>
                    ...
                    <tr align="center" style="height: 50px; vertical-align: middle;">
                        <td colspan="3">
                            <%=GetLabel("Tidak ada informasi order")%>
                        </td>
                    </tr>
                </table>
            </EmptyDataTemplate>
            ...
            <ItemTemplate>
                <tr>
                    ...
                    <td style="vertical-align: middle; text-align: right">
                        <asp:TextBox ID="txtOrderQuantity" Width="60px" runat="server" CssClass="txtOrderQuantity txtNumeric"
                            ReadOnly="true" Text='<%#: Eval("DispenseQtyInString")%>' />
                    </td>
                    <td style="vertical-align: middle; text-align: right;">
                        <asp:TextBox ID="txtTakenQty1" Width="60px" runat="server" CssClass="txtTakenQty1 txtNumeric"
                            ReadOnly="true" />
                    </td>
                    <td style="vertical-align: middle; padding: 3px">
                        <%#: Eval("ItemUnit")%>
...
<div id="divProcessButton" runat="server" style="width: 100%; text-align: right;
    padding-top: 10px">
    <table id="tblApproveDecline" runat="server" width="100%">
        <tr id="trChecklist">
            <td colspan="4">
                <table border="0" cellpadding="1" cellspacing="0">
                    ...
                    <tr>
                        <td align="right">
                            <asp:CheckBox ID="chkSkringResep" runat="server" ToolTip="Centang Semua" />
                        </td>
                        <td align="left" style="font-weight: bold;">
                            <asp:Label ID="Label4" runat="server" ToolTip="Skrining Resep" Text="Skrining Resep">Skrining Resep</asp:Label>
                        </td>
                        <td align="right">
                            <asp:CheckBox ID="chkIsADChecked" runat="server" ToolTip="(Persyatratan Administrasi)" />
                        </td>
                        <td align="left" style="font-weight: bold;">
                            <asp:Label ID="Label5" runat="server" ToolTip="(Persyatratan Administrasi)" Text="AD">AD</asp:Label>
                        </td>
                        <td align="right">
                            <asp:CheckBox ID="chkIsFARChecked" runat="server" ToolTip="(Persyaratan Farmasetik)" />
                        </td>
                        <td align="left" style="font-weight: bold;">
                            <asp:Label ID="Label6" runat="server" ToolTip="(Persyaratan Farmasetik)" Text="FAR">FAR</asp:Label>
                        </td>
                        <td align="right">
                            <asp:CheckBox ID="chkIsKLNChecked" runat="server" ToolTip="(Persyaratan Klinis)" />
                        </td>
                        <td align="left" style="font-weight: bold;">
                            <asp:Label ID="Label7" runat="server" ToolTip="(Persyaratan Klinis)" Text="FAR">KLN</asp:Label>
                        </td>
                    </tr>
                    <tr>
                        <td align="right">
                            <asp:CheckBox ID="chkIsCorrectAlowAll" runat="server" ToolTip="Centang Semua" />
                        </td>
                        <td align="left" style="font-weight: bold;">
                            <asp:Label runat="server" ToolTip="Telaah Resep" Text="Telaah Resep">Telaah Resep</asp:Label>
                        </td>
                        <td align="right">
                            <asp:CheckBox ID="chkIsCorrectPatient" runat="server" ToolTip="Benar Pasien" />
                        </td>
                        <td align="left" style="font-weight: bold;">
                            <asp:Label runat="server" ToolTip="Benar Pasien" Text="Px">Px</asp:Label>
                        </td>
                        <td align="right">
                            <asp:CheckBox ID="chkIsCorrectMedication" runat="server" ToolTip="Benar Obat" />
                        </td>
                        <td align="left" style="font-weight: bold;">
                            <asp:Label runat="server" ToolTip="Benar Obat" Text="OB">OB</asp:Label>
                        </td>
                        <td align="right">
                            <asp:CheckBox ID="chkIsCorrectStrength" runat="server" ToolTip="Benar Kekuatan Obat" />
                        </td>
                        <td align="left" style="font-weight: bold;">
                            <asp:Label runat="server" ToolTip="Benar Kekuatan Obat" Text="KE">KE</asp:Label>
                        </td>
                        <td align="right">
                            <asp:CheckBox ID="chkIsCorrectFrequency" runat="server" ToolTip="Benar Frekuensi Pemberian" />
                        </td>
                        <td align="left" style="font-weight: bold;">
                            <asp:Label runat="server" ToolTip="Benar Frekuensi Pemberian" Text="FRE">FRE</asp:Label>
                        </td>
                        <td align="right">
                            <asp:CheckBox ID="chkIsCorrectDosage" runat="server" ToolTip="Benar Dosis" />
                        </td>
                        <td align="left" style="font-weight: bold;">
                            <asp:Label runat="server" ToolTip="Benar Dosis" Text="DO">DO</asp:Label>
                        </td>
                        <td align="right">
                            <asp:CheckBox ID="chkIsCorrectRoute" runat="server" ToolTip="Benar Rute Pemberian" />
                        </td>
                        <td align="left" style="font-weight: bold;">
                            <asp:Label runat="server" ToolTip="Benar Rute Pemberian" Text="RP">RP</asp:Label>
                        </td>
                        <td align="right">
                            <asp:CheckBox ID="chkIsHasDrugInteraction" runat="server" ToolTip="ada tidaknya interaksi obat" />
                        </td>
                        <td align="left" style="font-weight: bold;">
                            <asp:Label runat="server" ToolTip="ada tidaknya interaksi obat" Text="IO">IO</asp:Label>
                        </td>
                        <td align="right">
                            <asp:CheckBox ID="chkIsHasDuplication" runat="server" ToolTip="ada tidaknya duplikasi obat" />
                        </td>
                        <td align="left" style="font-weight: bold;">
                            <asp:Label runat="server" ToolTip="ada tidaknya duplikasi obat" Text="DUP">DUP</asp:Label>
                        </td>
                        <td align="right">
                            <asp:CheckBox ID="chkIsCorrectTimeOfGiving" runat="server" ToolTip="Benar Waktu Pemberian" />
                        </td>
                        <td align="left" style="font-weight: bold;">
                            <asp:Label runat="server" ToolTip="Benar Waktu Pemberian" Text="WP">WP</asp:Label>
                        </td>
                    </tr>
                    <tr>
                        <td>
                        ...
                        <td colspan="24" align="left">
                            <asp:TextBox ID="txtPrescriptionReviewText" Width="100%" runat="server" />
                        </td>
                    </tr>
                </table>
            </td>
        ...
        <tr>
            ...
            <td style="width: 350px;">
                <asp:RadioButtonList ID="rblProcessType" runat="server" RepeatDirection="Horizontal">
                    ...
                    <asp:ListItem Text=" Dijadwalkan" Value="4" />
                </asp:RadioButtonList>
            </td>
            <td colspan="3">
                <table id="tblVoidReason" runat="server" style="width: 100%;">
                    <tr>
                        <td class="tdLabel" style="font-weight: bold; color: Red">
        ...
        <tr>
            <td style="width: 80px;">
                <input class="w3-btn w3-hover-blue" type="button" value='<%= GetLabel("Kalkulasi")%>'
                    id="btnCalculate" style="background-color: Green; width: 100px;" />
            </td>
            <td>
                    ...
                    <table border="0" cellpadding="0" cellspacing="0" width="100%">
                        ...
                        <tr>
                            ...
                            <td style="padding-left: 2px; padding-left: 5px">
                                <asp:TextBox ID="txtTotalAmount" Width="100px" runat="server" CssClass="txtCurrency"
                            ...
                            <td style="padding-left: 2px; padding-left: 5px">
                                <asp:TextBox ID="txtTotalPayer" Width="100px" runat="server" CssClass="txtCurrency"
                                    ReadOnly="true" TabIndex="999" />
                            </td>
                            <td class="tdLabel">
                                <%=GetLabel("Pasien") %>
                            ...
                            <td style="padding-left: 2px; padding-left: 5px">
                                <asp:TextBox ID="txtTotalPatient" Width="100px" runat="server" CssClass="txtCurrency"
                                    ReadOnly="true" TabIndex="999" />
                            </td>
                            <td style="padding: 10px">
                                <input class="w3-btn w3-hover-blue" type="button" value='<%= GetLabel("Informasi Stok")%>'
                                    id="btnStockInfo" style="background-color: Green; width: 150px; display: none" />
                            </td>
                        </tr>
                    </table>
                </div>
            ...
            <td style="width: 80px;">
                <input class="w3-btn w3-hover-blue" type="button" value='<%= GetLabel("Close")%>'
                    id="btnTestOrderClose" style="width: 100px;" />
            </td>
        </tr>
    </table>
    ...
    <div style="display: none">
        <dxcp:ASPxCallbackPanel ID="cbpCalculateItem" runat="server" Width="100%" ClientInstanceName="cbpCalculateItem"
            ...
            <ClientSideEvents BeginCallback="function(s,e){ showLoadingPanel(); }" EndCallback="function(s,e){ onCbpCalculateItemViewEndCallback(s); }" />
        </dxcp:ASPxCallbackPanel>
    </div>
</div>
<dx:ASPxPopupControl ID="pcNotes" runat="server" ClientInstanceName="pcNotes" Height="150px"
    HeaderText="Drug Alert Information" CloseAction="None" Width="650px" Modal="True"
    PopupAction="None" PopupHorizontalAlign="WindowCenter" PopupVerticalAlign="WindowCenter"
    CloseButtonImage-Width="0">
    <ContentCollection>
                                    ...
                                    <td style="height: 450px; width: 100%; vertical-align: top">
                                        <div id="rightPanelCtl">
                                            <ul style="display: none">
                                                <li url="~/Libs/Program/Information/DrugInteractionSummaryViewCtl.ascx" title="Drug Interaction">
                                                    Drug Alert</li>
                                            </ul>
...
<dx:ASPxPopupControl ID="pcPPRAForm" runat="server" ClientInstanceName="pcPPRAForm" Height="150px"
    HeaderText="Program Pengendalian Resistensi Antimikroba (PPRA)" CloseAction="None" Width="800px" Modal="True"
    PopupAction="None" PopupElementID = "imgViewPPRA" PopupHorizontalAlign="WindowCenter" PopupVerticalAlign="WindowCenter" AllowDragging="True"
    CloseButtonImage-Width="0">
    <ContentCollection>
        <dx:PopupControlContentControl runat="server" ID="popupContent2">
            <dx:ASPxPanel ID="panPopupContent2" runat="server" Width="100%">
                <PanelCollection>
                    <dx:PanelContent ID="PanelContent2" runat="server">
                        <div style="width: 100%;">
                            <div id="ppraContent">
                            </div>
                            <div id="ppraContentLoading" style="position: absolute; top: 30%; left: 48%;
                                display: none">
                                <div style="margin: 0 auto">
                                    <img src="<%= ResolveUrl("~/Libs/Images/Loading.gif")%>" alt="" />
                                </div>
                        ...
                        <div style="text-align: center; width: 100%;">
                            <table style="margin-left: auto; margin-right: auto; margin-top: 5px;">
                                <tr>
                                    <td>
                                        <input type="button" id="btnClosePPRA" class="btnPropose w3-btn w3-hover-blue" value="CLOSE" style="background-color: Red;
                                              color: White; width: 100px;" value='<%= GetLabel("Close")%>' />
                                    </td>
                                </tr>
                            </table>
                        </div>
