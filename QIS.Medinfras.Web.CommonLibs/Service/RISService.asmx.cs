...
namespace QIS.Medinfras.Web.CommonLibs.Service
{
    ...
    public class RISService : System.Web.Services.WebService
    {
        ...
        #region Centerback
        #region Send Order to RIS (MedinfrasAPI)
        [WebMethod(EnableSession = true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public string SendOrderToRIS_MedinfrasApi(int transactionID, string lstItemID)
        {
            try
            {
                string result = "";
                HttpWebRequest request = (HttpWebRequest)WebRequest.Create(string.Format("{0}/imaging/send/order", AppSession.SA0132));
                request.Method = "POST";
                request.ContentType = "application/json";
                SetRequestHeader(request);

                SendOrderToRIS obj = new SendOrderToRIS();
                List<SendOrderToRIS_Detail> lst = new List<SendOrderToRIS_Detail>();
                List<vPatientChargesDt> lstChargesDt = BusinessLayer.GetvPatientChargesDtList(string.Format("TransactionID = {0} AND ID IN ({1})", transactionID, lstItemID));
                obj.TransactionNo = lstChargesDt.FirstOrDefault().TransactionNo;
                foreach (vPatientChargesDt dt in lstChargesDt)
                {
                    SendOrderToRIS_Detail item = new SendOrderToRIS_Detail();
                    item.ItemCode = dt.ItemCode;
                    lst.Add(item);
                }
                obj.ListItem = lst;
        ...
        #region Send Result to RIS (MedinfrasAPI)
        [WebMethod(EnableSession = true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public string SendResultToRIS_MedinfrasApi(int transactionID)
        {
            try
            {
                string result = "";
                HttpWebRequest request = (HttpWebRequest)WebRequest.Create(string.Format("{0}/imaging/send/result", AppSession.SA0132));
                request.Method = "POST";
                request.ContentType = "application/json";
                SetRequestHeader(request);
                ...
                SendOrderToRIS obj = new SendOrderToRIS();
                List<SendOrderToRIS_Detail> lst = new List<SendOrderToRIS_Detail>();
                List<vPatientChargesDt> lstChargesDt = BusinessLayer.GetvPatientChargesDtList(string.Format("TransactionID = {0} AND IsDeleted = 0", transactionID));
                obj.TransactionNo = lstChargesDt.FirstOrDefault().TransactionNo;
    ...
    public class SendOrderToRIS
    {
        public string TransactionNo { get; set; }
        public List<SendOrderToRIS_Detail> ListItem { get; set; }
    }
    public class SendOrderToRIS_Detail
    {
