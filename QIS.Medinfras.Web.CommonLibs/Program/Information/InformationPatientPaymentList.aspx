...
CodeBehind="InformationPatientPaymentList.aspx.cs" Inherits="QIS.Medinfras.Web.CommonLibs.Program.InformationPatientPaymentList" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="plhMPMain" runat="server">
    <script type="text/javascript">
        $(function () {
            ...
            function getHealthcareServiceUnitFilterExpression() {
                if ($('#<%=hdnModuleID.ClientID %>').val() == 'LB') {
                    var filterExpression = "IsUsingRegistration = 1 AND HealthcareID = '" + AppSession.healthcareID + "' AND DepartmentID = '" + cboDepartment.GetValue() + "'" + "AND HealthCareServiceUnitID = '" + $('#<%=hdnLB.ClientID %>').val() + "'";
                } 
                else if ($('#<%=hdnModuleID.ClientID %>').val() == 'IS') {
                    var filterExpression = "IsUsingRegistration = 1 AND HealthcareID = '" + AppSession.healthcareID + "' AND DepartmentID = '" + cboDepartment.GetValue() + "'" + "AND HealthCareServiceUnitID =  '" + $('#<%=hdnIS.ClientID %>').val() + "'";
                }
                else if ($('#<%=hdnModuleID.ClientID %>').val() == 'MD') {
                    var filterExpression = "IsUsingRegistration = 1 AND HealthcareID = '" + AppSession.healthcareID + "' AND DepartmentID = '" + cboDepartment.GetValue() + "'" + "AND HealthCareServiceUnitID NOT IN ('" + $('#<%=hdnIS.ClientID %>').val() + "'" + "," + "'"+ $('#<%=hdnLB.ClientID %>').val() + "')" ;
                }  
                else {
        ...
        function onCboMedicalDiagnosticValueChanged() {
            $('#<%=hdnServiceUnitID.ClientID %>').val(cboMedicalDiagnostic.GetValue());
            onRefreshGrdReg();
        }
        ...
        function oncboCheckinStatusValueChanged() {
            $('#<%=hdnStatus.ClientID %>').val(cboCheckinStatus.GetValue());
            onRefreshGrdReg();
        }
    ...
    <input type="hidden" id="hdnLB" runat="server" value="" />
    <input type="hidden" id="hdnIS" runat="server" value="" />
    <div style="padding:15px">
        <input type="hidden" id="hdnFilterExpression" runat="server" value="" />
        <div class="pageTitle"><%=GetLabel("Informasi Pembayaran Pasien")%></div>
                            ...
                            <tr>
                                ...
                                <td>
                                    <dxe:ASPxComboBox ID="cboCheckinStatus" ClientInstanceName="cboCheckinStatus" Width="350px"
                                        runat="server">
                                        <ClientSideEvents ValueChanged="function(s,e) { oncboCheckinStatusValueChanged(); }" />
                                    </dxe:ASPxComboBox>
                                </td>
                            </tr>
                            <tr>
