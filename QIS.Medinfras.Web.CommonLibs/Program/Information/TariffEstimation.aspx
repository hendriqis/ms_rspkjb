<%@ Page Language="C#" MasterPageFile="~/Libs/MasterPage/MPTrx.master" AutoEventWireup="true"
    CodeBehind="TariffEstimation.aspx.cs" Inherits="QIS.Medinfras.Web.CommonLibs.Program.TariffEstimation" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="plhEntry" runat="server">
    <script type="text/javascript">
        ...
        function calculateTariffEstimation($row) {
            ...
            if ($row.find('.hdnIsDiscountUsedComp').val() == 'true') {
                discount = $row.find('.hdnDiscountAmount').val();
            }
            else {
                if ($row.find('.hdnIsDiscountInPercentage').val() == 'true') {
...
//            });
            ...
            $('#<%=btnCalculate.ClientID %>').click(function () {
                ...
                if (selectedItem == '')
                    ...
                    Methods.getTariffEstimation(cboClass.GetValue(), businessPartnerID, coverageTypeID, selectedItem, getTrxDate(), department, 2, function (result) {
                        if (result != null) {
                            totalPatient = 0;
                            totalPayer = 0;
                            grandTotal = 0;
                            for (var i = 0; i < result.length; i++) {
                                ...
                                $('#tblSelectedItem .trSelectedItem').each(function () {
                                    ...
                                    if ($(this).find('.keyField').val() == result[i].ItemID) {
                                        ...
                                        $row.find('.hdnIsDiscountUsedComp').val(result[i].IsDiscountUsedComp);
        ...
        function SetCboClass(type) {
            if (type == "RJ") {
                cboClass.SetValue($('#<%=hdnOutpatientClass.ClientID %>').val());
        ...
        $(function () {
            ...
            $('#lblItemGroup.lblLink').click(function () {
                var filterExpression = "GCItemType = '" + cboItemType.GetValue().toString() + "' AND IsDeleted = 0";
        ...
        function cboDepartmentValueChanged(e) {
            var type = cboDepartment.GetValue();
            $('#<%=hdnDepartmentID.ClientID %>').val(cboDepartment.GetValue());
            if (type != "INPATIENT" && type != "DIAGNOSTIC") {
                if (type == "EMERGENCY") {
                    if ($('#<%=hdnEmergencyClass.ClientID %>').val() == "" || $('#<%=hdnEmergencyClass.ClientID %>').val() == null) {
                        cboClass.SetValue($('#<%=hdnOutpatientClass.ClientID %>').val());
                    }
                    else {
                        cboClass.SetValue($('#<%=hdnEmergencyClass.ClientID %>').val());
                    }
                }
                else if (type == "MCU") {
                    if ($('#<%=hdnMedicalCheckupClass.ClientID %>').val() == "" || $('#<%=hdnMedicalCheckupClass.ClientID %>').val() == null) {
                        cboClass.SetValue($('#<%=hdnOutpatientClass.ClientID %>').val());
                    }
                    else {
                        cboClass.SetValue($('#<%=hdnMedicalCheckupClass.ClientID %>').val());
                    }
                }
                else if (type == "OUTPATIENT") {
                    if ($('#<%=hdnOutpatientClass.ClientID %>').val() == "" || $('#<%=hdnOutpatientClass.ClientID %>').val() == null) {
                        cboClass.SetValue($('#<%=hdnOutpatientClass.ClientID %>').val());
                    }
                    else {
                        cboClass.SetValue($('#<%=hdnOutpatientClass.ClientID %>').val());
                    ...
                    if ($('#<%=hdnPharmacyClass.ClientID %>').val() == "" || $('#<%=hdnPharmacyClass.ClientID %>').val() == null) {
                        cboClass.SetValue($('#<%=hdnOutpatientClass.ClientID %>').val());
                    }
                    else {
                        cboClass.SetValue($('#<%=hdnPharmacyClass.ClientID %>').val());
                    }
                }
                else {
                    cboClass.SetValue($('#<%=hdnOutpatientClass.ClientID %>').val());
    ...
    <div style="padding: 10px;">
        <script id="tmplSelectedTestItem" type="text/x-jquery-tmpl">
            <tr class="trSelectedItem">
                <td align="center">
                    ...
                    <input type="hidden" class="hdnIsDiscountUsedComp" value="0" />
        ...
        <input type="hidden" id="hdnEmergencyClass" runat="server" />
        <input type="hidden" id="hdnMedicalCheckupClass" runat="server" />
        <input type="hidden" id="hdnOutpatientClass" runat="server" />
        <input type="hidden" id="hdnPharmacyClass" runat="server" />
                         ...
                         <td class="tdLabel">
                            ...
                            <td>
                                 ...
                                 <dxe:ASPxComboBox ID="cboDepartment" ClientInstanceName="cboDepartment" Width="100%" runat="server" >
                                    <ClientSideEvents ValueChanged="function(s,e) { cboDepartmentValueChanged(s); }" />
                                 </dxe:ASPxComboBox>
                            </td>
                        </tr>
                        <tr>
                                ...
                                <asp:Panel runat="server" ID="pnlView" Style="width: 100%; margin-left: auto; margin-right: auto;
                                    ...
                                    <asp:GridView ID="grdView" runat="server" CssClass="grdView notAllowSelect" AutoGenerateColumns="false"
                                        ...
                                        <Columns>
                                            ...
                                            <asp:BoundField DataField="cfItemName" HeaderText="Item" ItemStyle-CssClass="tdItemName1" />
                                        </Columns>
                                        <EmptyDataTemplate>
