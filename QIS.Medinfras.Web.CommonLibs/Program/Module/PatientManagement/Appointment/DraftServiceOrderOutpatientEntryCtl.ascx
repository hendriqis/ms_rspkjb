<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="DraftServiceOrderOutpatientEntryCtl.ascx.cs"
    Inherits="QIS.Medinfras.Web.CommonLibs.Program.DraftServiceOrderOutpatientEntryCtl" %>
...
<script type="text/javascript" id="dxss_draftserviceorderemergencyctl">
    setDatePicker('<%=txtDraftServiceOrderOutpatientDate.ClientID %>');
    ...
    window.onCbpViewCtlEndCallback = function (s, e) {
        ...
        if (param[0] == 'save') {
            if (param[1] == 'fail') {
                ...
                $('#containerEntry').hide();
                $('#<%=hdnDraftServiceOrderOutpatientID.ClientID %>').val(s.cpServiceOrderID);
            ...
            if (param[1] == 'fail') {
                ...
                $('#containerEntry').hide();
                $('#<%=hdnDraftServiceOrderOutpatientID.ClientID %>').val(s.cpTestOrderID);
                cbpMainPopup.PerformCallback('loadaftersave');
            }
        }
    }
    ...
    $('#lblDraftServiceOrderOutpatientNo.lblLink').live('click', function () {
        ...
        openSearchDialog('draftserviceorderhd', filterExpression, function (value) {
            $('#<%=txtDraftServiceOrderOutpatientNo.ClientID %>').val(value);
            ontxtDraftServiceOrderOutpatientNoChanged(value);
        });
    });

    $('#<%=txtDraftServiceOrderOutpatientNo.ClientID %>').change(function () {
        ontxtDraftServiceOrderOutpatientNoChanged($(this).val());
    });

    function ontxtDraftServiceOrderOutpatientNoChanged(value) {
        ...
        Methods.getObject('GetvDraftServiceOrderHdList', filterExpression, function (result) {
            if (result != null) {
                $('#<%=hdnDraftServiceOrderOutpatientID.ClientID %>').val(result.DraftServiceOrderID);
                cbpMainPopup.PerformCallback('load');
            }
            else {
                $('#<%=hdnDraftServiceOrderOutpatientID.ClientID %>').val('');
            }
        });
    }
    //#endregion
    ...
    $('#<%=lblServiceUnitOutpatient.ClientID %>.lblLink').live('click', function () {
        if ($('#<%=txtDraftServiceOrderOutpatientNo.ClientID %>').val() != '')
            return;
    ...
    $('#<%=txtDraftServiceOrderOutpatientNo.ClientID %>').live('change', function () {
    ...
    function getItemMasterFilterExpression() {
        var testOrderID = $('#<%=hdnDraftServiceOrderOutpatientID.ClientID %>').val();
        ...
        if (testOrderID != '') {
            if ($('#<%=hdnHealthcareServiceUnitID.ClientID %>').val() == $('#<%=hdnLaboratoryServiceUnitID.ClientID %>').val() || $('#<%=hdnHealthcareServiceUnitID.ClientID %>').val() == $('#<%=hdnImagingServiceUnitID.ClientID %>').val())
                filterExpression = 'ItemID IN (SELECT ItemID FROM vServiceUnitItem WHERE HealthcareServiceUnitID = ' + $('#<%=hdnHealthcareServiceUnitID.ClientID %>').val() + ' AND IsTestItem = 1) AND ItemID NOT IN (SELECT ItemID FROM TestOrderDt WHERE TestOrderID = ' + $('#<%=hdnDraftServiceOrderOutpatientID.ClientID %>').val() + ' AND IsDeleted = 0) AND IsDeleted = 0';
            else filterExpression = 'ItemID IN (SELECT ItemID FROM vServiceUnitItem WHERE HealthcareServiceUnitID = ' + $('#<%=hdnHealthcareServiceUnitID.ClientID %>').val() + ') AND ItemID NOT IN (SELECT ItemID FROM TestOrderDt WHERE TestOrderID = ' + $('#<%=hdnDraftServiceOrderOutpatientID.ClientID %>').val() + ' AND IsDeleted = 0) AND IsDeleted = 0';
        }
        else {
            if ($('#<%=hdnHealthcareServiceUnitID.ClientID %>').val() == $('#<%=hdnLaboratoryServiceUnitID.ClientID %>').val() || $('#<%=hdnHealthcareServiceUnitID.ClientID %>').val() == $('#<%=hdnImagingServiceUnitID.ClientID %>').val())
    ...
    function onAfterSaveAddRecordEntryPopup(param) {
        $('#containerEntry').hide();
        $('#<%=hdnDraftServiceOrderOutpatientID.ClientID %>').val(param);
        cbpMainPopup.PerformCallback('loadaftersave');
    }
            ...
            <dx:PanelContent ID="PanelContent2">
                <fieldset id="fsEntryPopup" style="margin: 0">
                    <asp:Panel runat="server" ID="Panel1" Style="width: 100%; margin-left: auto; margin-right: auto;
                        ...
                        <input type="hidden" value="" id="hdnDraftServiceOrderOutpatientID" runat="server" />
                        ...
                        <table class="tblContentArea">
                            ...
                            <tr>
                                <td style="padding: 5px; vertical-align: top">
                                    <table class="tblEntryContent" style="width: 100%">
                                        ...
                                        <tr>
                                            <td class="tdLabel">
                                                <label class="lblLink" id="lblDraftServiceOrderOutpatientNo">
                                                    <%=GetLabel("No. Draft Order")%></label>
                                            </td>
                                            <td>
                                                <asp:TextBox ID="txtDraftServiceOrderOutpatientNo" Width="232px" runat="server" />
                                            </td>
                                        </tr>
                                        <tr>
                                            <td class="tdLabel">
                                                ...
                                                <table cellpadding="0" cellspacing="0">
                                                    <tr>
                                                        <td style="padding-right: 1px; width: 145px">
                                                            <asp:TextBox ID="txtDraftServiceOrderOutpatientDate" Width="120px" CssClass="datepicker"
                                                                runat="server" />
                                                        </td>
                                                        <td style="width: 5px">
                                                            &nbsp;
                                                        ...
                                                        <td>
                                                            <asp:TextBox ID="txtDraftServiceOrderOutpatientTime" Width="80px" CssClass="time"
                                ...
                                <td style="padding: 5px; vertical-align: top">
                                    <table class="tblEntryContent" style="width: 100%">
                                        ...
                                        <tr>
                                            <td class="tdLabel">
                                                <label class="lblLink lblMandatory" runat="server" id="lblServiceUnitOutpatient">
                            ...
                            <tr>
                                <td colspan="2">
                                    ...
                                    <div id="divAddDataOutpatientDraftOrder" style="display: none; width: 100%; text-align: center;" runat="server">
