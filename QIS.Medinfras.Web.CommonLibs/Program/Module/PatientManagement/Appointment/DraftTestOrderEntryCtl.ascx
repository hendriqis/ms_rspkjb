<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="DraftTestOrderEntryCtl.ascx.cs"
    Inherits="QIS.Medinfras.Web.CommonLibs.Program.DraftTestOrderEntryCtl" %>
...
<script type="text/javascript" id="dxss_drafttestorderctl">
    setDatePicker('<%=txtDraftTestOrderDate.ClientID %>');
    ...
    window.onCbpViewCtlEndCallback = function (s, e) {
        ...
        if (param[0] == 'save') {
            if (param[1] == 'fail') {
                ...
                $('#containerEntry').hide();
                $('#<%=hdnDraftTestOrderID.ClientID %>').val(s.cpTestOrderID);
    ...
    $('#lblDraftTestOrderNo.lblLink').live('click', function () {
        var filterExpression = "AppointmentID = '" + $('#<%=hdnAppointmentID.ClientID %>').val() + "'";
        openSearchDialog('drafttestorderhd', filterExpression, function (value) {
            $('#<%=txtDraftTestOrderNo.ClientID %>').val(value);
            ontxtDraftTestOrderNoChanged(value);
        });
    });

    $('#<%=txtDraftTestOrderNo.ClientID %>').change(function () {
        ontxtDraftTestOrderNoChanged($(this).val());
    });

    function ontxtDraftTestOrderNoChanged(value) {
    ...
    $('#<%=lblServiceUnit.ClientID %>.lblLink').live('click', function () {
        if ($('#<%=txtDraftTestOrderNo.ClientID %>').val() != '') {
            return;
        }
    ...
    function getItemMasterFilterExpression() {
        var testOrderID = $('#<%=hdnDraftTestOrderID.ClientID %>').val();
        ...
        if (testOrderID != '') {
            if ($('#<%=hdnHealthcareServiceUnitID.ClientID %>').val() == $('#<%=hdnLaboratoryServiceUnitID.ClientID %>').val() || $('#<%=hdnHealthcareServiceUnitID.ClientID %>').val() == $('#<%=hdnImagingServiceUnitID.ClientID %>').val())
                filterExpression = 'ItemID IN (SELECT ItemID FROM vServiceUnitItem WHERE HealthcareServiceUnitID = ' + $('#<%=hdnHealthcareServiceUnitID.ClientID %>').val() + ' AND IsTestItem = 1) AND ItemID NOT IN (SELECT ItemID FROM TestOrderDt WHERE TestOrderID = ' + $('#<%=hdnDraftTestOrderID.ClientID %>').val() + ' AND IsDeleted = 0) AND IsDeleted = 0';
            else filterExpression = 'ItemID IN (SELECT ItemID FROM vServiceUnitItem WHERE HealthcareServiceUnitID = ' + $('#<%=hdnHealthcareServiceUnitID.ClientID %>').val() + ') AND ItemID NOT IN (SELECT ItemID FROM TestOrderDt WHERE TestOrderID = ' + $('#<%=hdnDraftTestOrderID.ClientID %>').val() + ' AND IsDeleted = 0) AND IsDeleted = 0';
        }
        else {
            if ($('#<%=hdnHealthcareServiceUnitID.ClientID %>').val() == $('#<%=hdnLaboratoryServiceUnitID.ClientID %>').val() || $('#<%=hdnHealthcareServiceUnitID.ClientID %>').val() == $('#<%=hdnImagingServiceUnitID.ClientID %>').val())
    ...
    function onAfterSaveAddRecordEntryPopup(param) {
        $('#containerEntry').hide();
        $('#<%=hdnDraftTestOrderID.ClientID %>').val(param);
        cbpMainPopup.PerformCallback('loadaftersave');
    }
            ...
            <PanelCollection>
                ...
                <fieldset id="fsEntryPopup" style="margin: 0">
                    <asp:Panel runat="server" ID="Panel1" Style="width: 100%; margin-left: auto; margin-right: auto;
                        ...
                        <input type="hidden" value="" id="hdnDraftTestOrderID" runat="server" />
                            ...
                            <tr>
                                <td style="padding: 5px; vertical-align: top">
                                    <table class="tblEntryContent" style="width: 100%">
                                        ...
                                        <tr>
                                            <td class="tdLabel">
                                                <label class="lblLink" id="lblDraftTestOrderNo">
                                                    <%=GetLabel("No. Draft Order")%></label>
                                            </td>
                                            <td>
                                                <asp:TextBox ID="txtDraftTestOrderNo" Width="232px" runat="server" />
                                            </td>
                                        </tr>
                                        <tr>
                                            <td class="tdLabel">
                                                ...
                                                <table cellpadding="0" cellspacing="0">
                                                    <tr>
                                                        <td style="padding-right: 1px; width: 145px">
                                                            <asp:TextBox ID="txtDraftTestOrderDate" Width="120px" CssClass="datepicker" runat="server" />
                                                        </td>
                                                        <td style="width: 5px">
                                                            &nbsp;
                                                        </td>
                                                        <td>
                                                            <asp:TextBox ID="txtDraftTestOrderTime" Width="80px" CssClass="time" runat="server" Style="text-align: center" />
                                                        </td>
                                                    </tr>
                                                </table>
                                            </td>
                                ...
                                <td style="padding: 5px; vertical-align: top">
                                    <table class="tblEntryContent" style="width: 100%">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <asp:TextBox ID="txtStatus" Width="120px" runat="server" style="text-align: center; color:Red" />
                                            </td>
                                        </tr>
                                    </table>
                                </td>
                                ...
                                <td colspan="2">
                                    ...
                                    <dxcp:ASPxCallbackPanel ID="cbpViewCtl" runat="server" Width="100%" ClientInstanceName="cbpViewCtl"
                                        ...
                                        <ClientSideEvents BeginCallback="function(s,e){ showLoadingPanel(); }" 
                                            EndCallback="function(s,e){ onCbpViewCtlEndCallback(s,e); }" />
                                        <PanelCollection>
                                    ...
                                    <div id="divAddDataDraftTestOrder" style="display: none; width: 100%; text-align: center;" runat="server">
