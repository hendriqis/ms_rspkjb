<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="AppointmentChangeDateV2Ctl.ascx.cs"
    Inherits="QIS.Medinfras.Web.CommonLibs.Program.AppointmentChangeDateV2Ctl" %>
        ...
        if (appointmentID < 0) {
            if (IsValid(evt, 'fsEntryPopup', 'mpEntryPopup')) {
                if ($('#<%=hdnIsWaitingListCtl.ClientID %>').val() == '0') {
                    if ($('#<%=hdnIsValidAppoMessageCtl.ClientID %>').val() == '1') {
                        cbpPhysicianChangeAppointment.PerformCallback('save');
                    }
                    else {
                        if ($('#<%=hdnMaxAppoMessageCtl.ClientID %>').val() > 0) {
                            var message = 'Maximum Appointment (Untuk Sesi Yang Dipilih) Adalah ' + '<b>' + $('#<%=hdnMaxAppoMessageCtl.ClientID %>').val() + '</b>' + ' Dan Jumlah Appointment (Untuk Sesi Yang Dipilih) Yang Ada Saat Ini Adalah ' + '<b>' + $('#<%=hdnTotalAppoMessageCtl.ClientID %>').val() + '</b>' + ' , Apakah Ingin Melanjutkan Proses ?'
                    ...
                    if ($('#<%=hdnIsValidWaitingMessageCtl.ClientID %>').val() == '1') {
                        ...
                        if ($('#<%=hdnMaxWaitingMessageCtl.ClientID %>').val() > 0) {
                            var message = 'Maximum Waiting List (Untuk Sesi Yang Dipilih) Adalah ' + '<b>' + $('#<%=hdnMaxWaitingMessageCtl.ClientID %>').val() + '</b>' + ' Dan Jumlah Waiting List (Untuk Sesi Yang Dipilih) Yang Ada Saat Ini Adalah ' + '<b>' + $('#<%=hdnTotalWaitingMessageCtl.ClientID %>').val() + '</b>' + ' , Apakah Ingin Melanjutkan Proses ?'
                            showToastConfirmation(message, function (result) {
                                if (result) {
                                    cbpPhysicianChangeAppointment.PerformCallback('save');
                                }
