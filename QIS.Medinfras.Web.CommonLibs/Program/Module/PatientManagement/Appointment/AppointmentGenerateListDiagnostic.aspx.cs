...
{
    public partial class AppointmentGenerateListDiagnostic : BasePageTrx
    {
        ...
        protected string GetMCUHealthcareServiceUnitID()
        {
            vHealthcareServiceUnit entity = BusinessLayer.GetvHealthcareServiceUnitList(string.Format("DepartmentID = '{0}' AND IsDeleted = 0", Constant.Facility.MEDICAL_CHECKUP)).FirstOrDefault();
            return entity.HealthcareServiceUnitID.ToString();
        }
        ...
        protected override void InitializeDataControl()
        {
            if (Page.Request.QueryString["id"] != null)
            {
                ...
                Methods.SetComboBoxField<StandardCode>(cboRegistrationPayer, lstStandardCode.Where(sc => sc.ParentID == Constant.StandardCode.CUSTOMER_TYPE && sc.StandardCodeID != Constant.CustomerType.PERSONAL).ToList(), "StandardCodeName", "StandardCodeID");
                cboRegistrationPayer.SelectedIndex = 0;
                cboRegistrationPayer.Value = Constant.CustomerType.INSURANCE;

                Methods.SetComboBoxField<StandardCode>(cboRegistrationPayerApm, lstStandardCode.Where(sc => sc.ParentID == Constant.StandardCode.CUSTOMER_TYPE && sc.StandardCodeID != Constant.CustomerType.PERSONAL).ToList(), "StandardCodeName", "StandardCodeID");
                cboRegistrationPayerApm.SelectedIndex = 0;
                cboRegistrationPayerApm.Value = Constant.CustomerType.INSURANCE;

                Methods.SetComboBoxField<StandardCode>(cboRegistrationPayerReg, lstStandardCode.Where(sc => sc.ParentID == Constant.StandardCode.CUSTOMER_TYPE && sc.StandardCodeID != Constant.CustomerType.PERSONAL).ToList(), "StandardCodeName", "StandardCodeID");
                cboRegistrationPayerReg.SelectedIndex = 0;
                cboRegistrationPayerReg.Value = Constant.CustomerType.INSURANCE;
        ...
        private void OnGetAccessMenuTab()
        {
            List<GetUserRoleTabMenuList> lstMenuTab = Helper.OnGetMenuTabAccess(Constant.Module.MEDICAL_CHECKUP, Constant.MenuCode.MedicalCheckup.MCU_APPOINTMENT_REQUEST);
            bool isSelected = false;
            if (lstMenuTab.Where(w => w.MenuCode == Constant.MenuCodeTab.MC080300.BELUM_DIPROSES).ToList().Count > 0) //Belum Diproses
            {
                listPatientUpload.Attributes.Remove("style");
                if (!isSelected)
                {
                    OnSetAttributes(containerUpload, listPatientUpload);
                    isSelected = true;
                }
            }
            if (lstMenuTab.Where(w => w.MenuCode == Constant.MenuCodeTab.MC080300.SUDAH_PERJANJIAN).ToList().Count > 0) //Sudah Perjanjian
            {
                listPatientAppointment.Attributes.Remove("style");
                if (!isSelected)
                {
                    OnSetAttributes(containerAppointment, listPatientAppointment);
                    isSelected = true;
                }
                
            }
            if (lstMenuTab.Where(w => w.MenuCode == Constant.MenuCodeTab.MC080300.SUDAH_REGISTRASI).ToList().Count > 0) //Sudah Registrasi
            {
                listPatientRegistration.Attributes.Remove("style");
                if (!isSelected)
                {
                    OnSetAttributes(containerRegistration, listPatientRegistration);
                    isSelected = true;
                }
            }
        }

        private void OnSetAttributes(HtmlGenericControl container, HtmlGenericControl list)
        {
        ...
        private void InitializeFilterParameter()
        {
            ...
            txtFromAppointmentDateApm.Text = DateTime.Today.ToString(Constant.FormatString.DATE_PICKER_FORMAT);
            txtFromAppointmentDateReg.Text = DateTime.Today.ToString(Constant.FormatString.DATE_PICKER_FORMAT);
        }
        ...
        private void BindGridView(int pageIndex, bool isCountPageCount, ref int pageCount)
        {
            ...
            string filterExpression = string.Format("IsDeleted = 0 AND GCCustomerType = '{0}'", cboRegistrationPayer.Value.ToString());
            ...
            {
                if (!string.IsNullOrEmpty(hdnPayerID.Value.ToString()))
                {
            ...
            List<vAppointmentRequest> lstEntity = BusinessLayer.GetvAppointmentRequestList(filterExpression, Constant.GridViewPageSize.GRID_TEMP_MAX, pageIndex, "AppointmentRequestID");
        ...
        protected void lvwView_ItemDataBound(object sender, ListViewItemEventArgs e)
        {
            if (e.Item.ItemType == ListViewItemType.DataItem)
            {
                ...
                {
                    lblCustomerType.InnerText = entity.BusinessPartnerName;
                }
                else
                {
        ...
        protected void cbpView_Callback(object sender, DevExpress.Web.ASPxClasses.CallbackEventArgsBase e)
        {
            ...
            string lstApmReqID = string.Empty;
            this.tab = "1";
            ...
            if (e.Parameter != null && e.Parameter != "")
            {
                ...
                else if (param[0] == "update")
                {
                    if (param[1] == "single")
                    {
                        if (param[2] == "paramedic")
                        {
                            string apmReqID = param[3];
                            int paramedicID = Convert.ToInt32(param[4]);
                            result = "update|paramedic|";
                            if (OnUpdateParamedicAppointmentRequest(param[1], apmReqID, paramedicID, ref errMessage))
                            {
                                result += "success|";
                            }
                            else
                ...
                {
                    result = "void|";
                    if (ValidateVoidAppointment(ref errMessage, ref lstApmReqID))
                    {
                        result += "success|" + lstApmReqID;
                    }
                    else
                    {
                        result += "fail|" + errMessage;
        ...
        #region Sudah Appointment
        private void BindGridViewApm(int pageIndex, bool isCountPageCount, ref int pageCount)
        {
            string apmFromDateYear = txtFromAppointmentDateApm.Text.Substring(6, 4);
            string apmFromDateMonth = txtFromAppointmentDateApm.Text.Substring(3, 2);
            string apmFromDateDay = txtFromAppointmentDateApm.Text.Substring(0, 2);
            string apmFromDateFormat = string.Format("{0}-{1}-{2}", apmFromDateYear, apmFromDateMonth, apmFromDateDay);
            DateTime apmFromDate = DateTime.ParseExact(apmFromDateFormat, Constant.FormatString.DATE_PICKER_FORMAT2, null);

            string filterExpression = string.Format("IsDeleted = 0 AND GCCustomerType = '{0}'", cboRegistrationPayerApm.Value.ToString());
            ...
            if (cboRegistrationPayerApm.Value.ToString() == Constant.CustomerType.PERSONAL)
            {
                filterExpression += string.Format(" AND BusinessPartnerID = 1");
            }
            else
            {
                if (!string.IsNullOrEmpty(hdnPayerIDApm.Value.ToString()))
                {
                    filterExpression += string.Format(" AND BusinessPartnerID = {0}", hdnPayerIDApm.Value);
                }
            }
            ...
            List<vAppointmentRequest> lstEntity = BusinessLayer.GetvAppointmentRequestList(filterExpression, Constant.GridViewPageSize.GRID_TEMP_MAX, pageIndex, "AppointmentRequestID");
            lvwViewApm.DataSource = lstEntity;
            lvwViewApm.DataBind();
        }
        protected void lvwViewApm_ItemDataBound(object sender, ListViewItemEventArgs e)
        {
            if (e.Item.ItemType == ListViewItemType.DataItem)
            {
                vAppointmentRequest entity = e.Item.DataItem as vAppointmentRequest;
                HtmlInputHidden hdnKey = (HtmlInputHidden)e.Item.FindControl("hdnKeyApm");
                HtmlInputHidden hdnParamedicID = (HtmlInputHidden)e.Item.FindControl("hdnParamedicIDApm");
                HtmlInputHidden hdnCorporateAccountNo = (HtmlInputHidden)e.Item.FindControl("hdnCorporateAccountNoApm");
                HtmlInputHidden hdnCorporateAccountName = (HtmlInputHidden)e.Item.FindControl("hdnCorporateAccountNameApm");
                HtmlInputHidden hdnAppointmentID = (HtmlInputHidden)e.Item.FindControl("hdnAppointmentIDApm");
                HtmlInputHidden hdnRegistrationID = (HtmlInputHidden)e.Item.FindControl("hdnRegistrationIDApm");
                HtmlInputHidden hdnHealthcareServiceUnitPerRowID = (HtmlInputHidden)e.Item.FindControl("hdnHealthcareServiceUnitPerRowIDApm");
                HtmlInputHidden hdnGCCustomerType = (HtmlInputHidden)e.Item.FindControl("hdnGCCustomerTypeApm");
                HtmlGenericControl lblParamedicName = (HtmlGenericControl)e.Item.FindControl("lblParamedicNameApm");
                HtmlGenericControl lblCustomerType = (HtmlGenericControl)e.Item.FindControl("lblCustomerTypeApm");
                TextBox txtAppointmentDate = e.Item.FindControl("txtAppointmentDateApm") as TextBox;
                txtAppointmentDate.Text = entity.AppointmentRequestDate.ToString(Constant.FormatString.DATE_PICKER_FORMAT);
                hdnKey.Value = entity.AppointmentRequestID.ToString();
                if (!string.IsNullOrEmpty(entity.ParamedicName))
                {
        ...
        protected void cbpViewApm_Callback(object sender, DevExpress.Web.ASPxClasses.CallbackEventArgsBase e)
        {
            int pageCount = 1;
            string result = "";
            string errMessage = "";
            ...
            if (e.Parameter != null && e.Parameter != "")
            {
                ...
                if (param[0] == "changepage")
                {
                    BindGridViewApm(Convert.ToInt32(param[1]), false, ref pageCount);
                    result = "changepage";
                }
                else if (param[0] == "print")
                {
                ...
                else if (param[0] == "multiple")
                {
                    if (param[1] == "process")
                    {
                        ...
                        if (txtFromAppointmentDateApm.Text == hdnTodayDate.Value)
                        {
                            if (ValidateAppointmentRequest(ref errMessage))
                            {
                                if (createAppointmentToRegistration(ref errMessage, ref appointmentRegID))
                ...
                {
                    BindGridViewApm(1, true, ref pageCount);
                    result = "refresh|" + pageCount;
                }
            }
        ...
        #region Sudah Registrasi
        private void BindGridViewReg(int pageIndex, bool isCountPageCount, ref int pageCount)
        {
            string apmFromDateYear = txtFromAppointmentDateReg.Text.Substring(6, 4);
            string apmFromDateMonth = txtFromAppointmentDateReg.Text.Substring(3, 2);
            string apmFromDateDay = txtFromAppointmentDateReg.Text.Substring(0, 2);
            string apmFromDateFormat = string.Format("{0}-{1}-{2}", apmFromDateYear, apmFromDateMonth, apmFromDateDay);
            DateTime apmFromDate = DateTime.ParseExact(apmFromDateFormat, Constant.FormatString.DATE_PICKER_FORMAT2, null);

            string filterExpression = string.Format("IsDeleted = 0 AND GCCustomerType = '{0}'", cboRegistrationPayerReg.Value.ToString());
            ...
            if (cboRegistrationPayerReg.Value.ToString() == Constant.CustomerType.PERSONAL)
            {
                filterExpression += string.Format(" AND BusinessPartnerID = 1");
            }
            else
            {
                if (!string.IsNullOrEmpty(hdnPayerIDReg.Value.ToString()))
                {
                    filterExpression += string.Format(" AND BusinessPartnerID = {0}", hdnPayerIDReg.Value);
                }
            }
            ...
            List<vAppointmentRequest> lstEntity = BusinessLayer.GetvAppointmentRequestList(filterExpression, Constant.GridViewPageSize.GRID_TEMP_MAX, pageIndex, "AppointmentRequestID");
            lvwViewReg.DataSource = lstEntity;
            lvwViewReg.DataBind();
        }
        protected void lvwViewReg_ItemDataBound(object sender, ListViewItemEventArgs e)
        {
            if (e.Item.ItemType == ListViewItemType.DataItem)
            {
                vAppointmentRequest entity = e.Item.DataItem as vAppointmentRequest;
                HtmlInputHidden hdnKey = (HtmlInputHidden)e.Item.FindControl("hdnKeyReg");
                HtmlInputHidden hdnParamedicID = (HtmlInputHidden)e.Item.FindControl("hdnParamedicIDReg");
                HtmlInputHidden hdnCorporateAccountNo = (HtmlInputHidden)e.Item.FindControl("hdnCorporateAccountNoReg");
                HtmlInputHidden hdnCorporateAccountName = (HtmlInputHidden)e.Item.FindControl("hdnCorporateAccountNameReg");
                HtmlInputHidden hdnAppointmentID = (HtmlInputHidden)e.Item.FindControl("hdnAppointmentIDReg");
                HtmlInputHidden hdnRegistrationID = (HtmlInputHidden)e.Item.FindControl("hdnRegistrationIDReg");
                HtmlInputHidden hdnHealthcareServiceUnitPerRowID = (HtmlInputHidden)e.Item.FindControl("hdnHealthcareServiceUnitPerRowIDReg");
                HtmlInputHidden hdnGCCustomerType = (HtmlInputHidden)e.Item.FindControl("hdnGCCustomerTypeReg");
                HtmlGenericControl lblParamedicName = (HtmlGenericControl)e.Item.FindControl("lblParamedicNameReg");
                HtmlGenericControl lblCustomerType = (HtmlGenericControl)e.Item.FindControl("lblCustomerTypeReg");
                TextBox txtAppointmentDate = e.Item.FindControl("txtAppointmentDateReg") as TextBox;
                txtAppointmentDate.Text = entity.AppointmentRequestDate.ToString(Constant.FormatString.DATE_PICKER_FORMAT);
                hdnKey.Value = entity.AppointmentRequestID.ToString();
                if (!string.IsNullOrEmpty(entity.ParamedicName))
                {
        ...
        protected void cbpViewReg_Callback(object sender, DevExpress.Web.ASPxClasses.CallbackEventArgsBase e)
        {
            int pageCount = 1;
            string result = "";
            string errMessage = "";
            ...
            if (e.Parameter != null && e.Parameter != "")
            {
                ...
                if (param[0] == "changepage")
                {
                    BindGridViewReg(Convert.ToInt32(param[1]), false, ref pageCount);
                    result = "changepage";
                }
                else if (param[0] == "print")
                {
                ...
                //        if (txtFromAppointmentDateReg.Text == hdnTodayDate.Value)
                //        {
                //            if (ValidateAppointmentRequest(ref errMessage))
                //            {
                //                if (createAppointmentToRegistration(ref errMessage, ref appointmentRegID))
                ...
                {
                    BindGridViewReg(1, true, ref pageCount);
                    result = "refresh|" + pageCount;
                }
            }
        ...
        private void GetIDs(ref string[] lstApmReqID, ref string[] lstParamedicID, ref string[] lstApmReqDate, ref string[] lstCustomerType)
        {
            string lstParam = string.Empty;
            string lstPhysician = string.Empty;
            string lstDate = string.Empty;
            string lstCustomer = string.Empty;

            if (this.tab == "1")
            {
                lstParam = hdnParam.Value;
                lstPhysician = hdnLstParamedicID.Value;
                lstDate = hdnLstApmDate.Value;
                lstCustomer = hdnLstCustomerType.Value;
            }
            else if (this.tab == "2")
            {
                lstParam = hdnParamApm.Value;
                lstPhysician = hdnLstParamedicIDApm.Value;
                lstDate = hdnLstApmDateApm.Value;
                lstCustomer = hdnLstCustomerTypeApm.Value;
            }

            if (!String.IsNullOrEmpty(lstParam))
            {
                #region Appointment Request ID
                if (lstParam.Contains(','))
                {
                    lstApmReqID = lstParam.Split(',');
                }
                else
                {
                ...
                {
                    #region Paramedic ID
                    if (lstPhysician.Contains(','))
                    {
                        lstParamedicID = lstPhysician.Split(',');
                    }
                    else
                    {
                        List<string> lst = new List<string>();
                        lst.Add(lstPhysician);
                ...
                if (lstDate != null)
                {
                    #region Appointment Request Date
                    if (lstDate.Contains(','))
                    {
                        lstApmReqDate = lstDate.Split(',');
                    }
                    else
                    {
                        List<string> lst = new List<string>();
                ...
                if (lstCustomer != null)
                {
                    ...
                    if (lstCustomer.Contains(','))
                    {
                        lstCustomerType = lstCustomer.Split(',');
                    }
                    else
                    {
                        List<string> lst = new List<string>();
        ...
        private bool ValidateAppointmentRequest(ref string errMessage)
        {
            ...
            string[] lstPhysician = null;
            string[] lstDate = null;
            string[] lstCustomer = null;

            GetIDs(ref lstApmReqID, ref lstPhysician, ref lstDate, ref lstCustomer);
        ...
        private bool ValidateVoidAppointment(ref string errMessage, ref string lstApmReqID)
        {
            bool result = true;
            StringBuilder err = new StringBuilder();
            ...
            if (this.tab == "1")
            {
                lstApmReqID = hdnParam.Value;
            }
            else if (this.tab == "2")
            {
                lstApmReqID = hdnParamApm.Value;
            }

            List<vAppointmentRequest> lst = BusinessLayer.GetvAppointmentRequestList(string.Format("AppointmentRequestID IN ({0})", lstApmReqID));
            if (lst.Count > 0)
            {
        ...
        private bool createAppointment(ref string errMessage, ref int appointmentRegID)
        {
            ...
            try
            {
                GetIDs(ref lstApmReqID, ref lstParamedicID, ref lstApmReqDate, ref lstCustomerType);
                if (lstApmReqID.Length > 0)
                {
                    for (int i = 0; i < lstApmReqID.Length; i++)
                    {
                        ...
                        if (appRequest != null)
                        {
                            if (appRequest.AppointmentID == null)
                            {
                                ...
                                if (lstVisitTypeParamedic.Count > 0)
                                {
                                    ...
                                    appointment.VisitTypeID = appRequest.VisitTypeID;
                                }
                                else
                                {
                                ...
                                if (isValid)
                                {
                                    ...
                                    else
                                    {
                                            ...
                                            else
                                            {
                                                if (appRequest.GuestID != null && appRequest.GuestID > 0)
                                                {
                                                    ...
                                                    if (entityGuestCheck != null)
                                                    {
                                                        ...
                                                        appointment.FirstName = entityGuestCheck.FirstName;
                                                        appointment.MiddleName = entityGuestCheck.MiddleName;
                                                        appointment.LastName = entityGuestCheck.LastName;
                                                    ...
                                                    else
                                                    {
                                                        appointment.IsNewPatient = false;
                                                        appointment.MRN = entityPatient.MRN;
                                                        appointment.FirstName = entityPatient.FirstName;
                                                        appointment.MiddleName = entityPatient.MiddleName;
                                                        appointment.LastName = entityPatient.LastName;
                                                        appointment.Name = entityPatient.PatientName;
                                                        appointment.GCGender = entityPatient.GCGender;
                                                        appointment.StreetName = entityPatient.StreetName;
                                                        appointment.PhoneNo = entityPatient.PhoneNo1;
                                        ...
                                        appointment.ReferenceNo = string.IsNullOrEmpty(appRequest.RequestBatchNo) ? string.Empty : appRequest.RequestBatchNo;
                                        ...
                                        if (appointment.BusinessPartnerID != 1)
                                        {
                ...
                if (lstApmReqID.Length > 0)
                {
                    for (int i = 0; i < lstApmReqID.Length; i++)
                    {
                        ...
                        if (appRequest != null)
                        {
                            ...
                            if ((this.tab == "1" && rblProcessType.SelectedValue == "1"))
                            {
                                #region validate paramedic Schedule
                                vParamedicSchedule obj = BusinessLayer.GetvParamedicScheduleList(string.Format(
                                                                            "HealthcareServiceUnitID = {0} AND ParamedicID = {1} AND DayNumber = {2}",
...
(appointment.ParamedicID), appointment.StartDate, session, 1, ctx));
                            ...
                            {
                                entityPatient = BusinessLayer.GetvPatientList(string.Format("MRN = {0}", appointment.MRN), ctx).FirstOrDefault();
                            ...
                            if (entityRegistration.BusinessPartnerID != 1)
                            {
                                ...
                                entityRegistrationPayer.BusinessPartnerID = Convert.ToInt32(appointment.BusinessPartnerID);
                                entityRegistrationPayer.ContractID = Convert.ToInt32(appointment.ContractID);
                                entityRegistrationPayer.CoverageTypeID = Convert.ToInt32(appointment.CoverageTypeID);
                                entityRegistrationPayer.GCCustomerType = appointment.GCCustomerType;
                                entityRegistrationPayer.CoverageLimitAmount = 0;
                                entityRegistrationPayer.IsPrimaryPayer = true;
                                entityRegistrationPayer.RegistrationID = entityRegistration.RegistrationID;
                            ...
                            entityVisit.VisitTypeID = appRequest.VisitTypeID;
        ...
        private bool PrintAppointmentRequestLabel(ref string errMessage)
        {
            ...
            {
                lstApmReqID = hdnParamReg.Value;
            }

            //string ipAddress = HttpContext.Current.Request.UserHostAddress;
            string ipAddress = "192.168.1.24";
            ...
            List<vAppointmentRequest> lstApmReq = BusinessLayer.GetvAppointmentRequestList(string.Format("AppointmentRequestID IN ({0})", lstApmReqID));
            ...
            if (lstPrinter.Count > 0)
            {
                if (lstApmReq.Count > 0)
                {
                    ...
                    {
                        List<AppointmentRequest> lstEntity = BusinessLayer.GetAppointmentRequestList(string.Format("AppointmentRequestID IN ({0})", lstApmReqID));
        ...
        private bool OnUpdateParamedicAppointmentRequest(string type, string apmReqID, int paramedicID, ref string errMessage)
        {
            bool result = true;

            if (type == "single")
            {
                AppointmentRequest entity = BusinessLayer.GetAppointmentRequest(Convert.ToInt32(apmReqID));
                if (entity != null)
                {
                    IDbContext ctx = DbFactory.Configure(true);
                    AppointmentRequestDao entityDao = new AppointmentRequestDao(ctx);
            ...
            {
                List<AppointmentRequest> lstEntity = BusinessLayer.GetAppointmentRequestList(string.Format("AppointmentRequestID IN ({0})", apmReqID));
