...
namespace QIS.Medinfras.Web.CommonLibs.Program
{
    public partial class TestOrderDtCtl : BaseViewPopupCtl
    {
        ...
        public override void InitializeDataControl(string param)
        {
            ...
            if (reg.IsPregnant)
            {
                DivIsPregnant.Style.Remove("display");
            }
            else
            {
                DivIsPregnant.Style.Add("display", "none");
            }

            if (entity.IsBloodBankOrder)
            {
                trBloodBankOrder.Style.Add("display", "table-row");
                txtBloodBankNotes.Text = entity.cfBloodBankNotes;
            }
            else
            {
                trBloodBankOrder.Style.Add("display", "none");
                txtBloodBankNotes.Text = string.Empty;
            }
            ...
            vHealthcareServiceUnitCustom vsu = BusinessLayer.GetvHealthcareServiceUnitCustomList(string.Format("HealthcareServiceUnitID = {0}", entity.HealthcareServiceUnitID)).FirstOrDefault();
            ...
            if (!string.IsNullOrEmpty(hdnHSUOperatingTheatreID.Value))
            {
                if (entity.HealthcareServiceUnitID == Convert.ToInt32(hdnHSUOperatingTheatreID.Value))
                {
                    if (!String.IsNullOrEmpty(entity.ChargesGCTransactionStatus))
                    {
                        tblApproveDecline.Style.Add("display", "none");
                    }
                    else
                    {
                        tblApproveDecline.Attributes.Remove("style");
                    }
                }

            }

            if (hdnIsUsingMultiVisitScheduleOrder.Value == "1" && vsu.IsAllowMultiVisitSchedule)
            {
                tdMultiVisitScheduleOrder.Style.Remove("display");
            }
            else
            {
                tdMultiVisitScheduleOrder.Style.Add("display", "none");
            }
        ...
        private void GetSettingParameter()
        {
            List<SettingParameterDt> lstSettingParameterDt = BusinessLayer.GetSettingParameterDtList(string.Format(
                                                                            ...
                                                                            Constant.SettingParameter.SA_KONTROL_PEMBATALAN_ORDER, //1
                                                                            Constant.SettingParameter.LB_KODE_DEFAULT_DOKTER, //2
                                                                            Constant.SettingParameter.OP0040, //3
                                                                            Constant.SettingParameter.IS_KODE_PENUNJANG_RADIOLOGI, //4
                                                                            Constant.SettingParameter.LB_KODE_PENUNJANG_LABORATORIUM, //5
                                                                            Constant.SettingParameter.MD_SERVICE_UNIT_OPERATING_THEATRE, //6
                                                                            Constant.SettingParameter.FN_IS_END_AMOUNT_ROUNDING_TO_1, //7
                                                                            Constant.SettingParameter.MD_IS_USING_MULTIVISIT_SCHEDULE, //8
                                                                            Constant.SettingParameter.RT0001 //9
                                                                        ));
            ...
            hdnOrderToAppointmentUsingSamePayerAsRegistration.Value = lstSettingParameterDt.Where(t => t.ParameterCode == Constant.SettingParameter.OP0040).FirstOrDefault().ParameterValue;

            hdnHSUImagingID.Value = lstSettingParameterDt.Where(t => t.ParameterCode == Constant.SettingParameter.IS_KODE_PENUNJANG_RADIOLOGI).FirstOrDefault().ParameterValue;
            hdnHSULaboratoryID.Value = lstSettingParameterDt.Where(t => t.ParameterCode == Constant.SettingParameter.LB_KODE_PENUNJANG_LABORATORIUM).FirstOrDefault().ParameterValue;
            hdnHSUOperatingTheatreID.Value = lstSettingParameterDt.Where(t => t.ParameterCode == Constant.SettingParameter.MD_SERVICE_UNIT_OPERATING_THEATRE).FirstOrDefault().ParameterValue;
            hdnHSURadiotheraphyID.Value = lstSettingParameterDt.Where(t => t.ParameterCode == Constant.SettingParameter.RT0001).FirstOrDefault().ParameterValue;
            ...
            hdnIsUsingMultiVisitScheduleOrder.Value = lstSettingParameterDt.Where(a => a.ParameterCode == Constant.SettingParameter.MD_IS_USING_MULTIVISIT_SCHEDULE).FirstOrDefault().ParameterValue;
        ...
        {
            return string.Format("TestOrderID = {0} AND IsDeleted = 0 AND ((GCTestOrderStatus = '{1}' AND IsCreatedFromOrder = 1) OR (GCTestOrderStatus != '{1}' AND IsCreatedFromOrder = 1) OR (GCTestOrderStatus != '{1}' AND IsCreatedFromOrder = 0))",
                                    hdnTestOrderID.Value, Constant.TestOrderStatus.CANCELLED);
        }
        ...
        private void BindGridView(int pageIndex, bool isCountPageCount, ref int pageCount)
        {
            ...
            hdnCountDt.Value = lstEntity.Count.ToString();
        ...
        protected void cbpEntryPopupView_Callback(object sender, DevExpress.Web.ASPxClasses.CallbackEventArgsBase e)
        {
            ...
            else if (param == "approveMultiVisit")
            {
                if (OnProcessMultiVisitRecord(ref errMessage))
                    result += "success";
                else
                    result += "fail|" + errMessage;
            }
        ...
        private bool OnCheckAIOBalance(IDbContext ctx, int regID, ref string errMessage)
        {
            bool result = true;

            if (hdnIsHasAIOPackage.Value == "1")
            {
                List<TestOrderDt> lstOrderDt = BusinessLayer.GetTestOrderDtList(string.Format("TestOrderID = {0} AND GCTestOrderStatus != '{1}' AND IsDeleted = 0", hdnTestOrderID.Value, Constant.OrderStatus.CANCELLED), ctx);
                foreach (TestOrderDt dt in lstOrderDt)
                {
                    List<vConsultVisitItemPackageBalance> balance = BusinessLayer.GetvConsultVisitItemPackageBalanceList(string.Format("DtHealthcareServiceUnitID = {0} AND RegistrationID = {1} AND DtItemID = {2} AND QuantityEnd > 0", hdnHealthcareServiceUnitID.Value, regID, dt.ItemID), ctx);
                    if (balance.Count > 0)
                    {
                        result = false;
        ...
        private bool OnApproveRecord(ref string errMessage, ref string retval)
        {
            ...
            try
            {
                ...
                {
                    if (OnCheckAIOBalance(ctx, registrationID, ref errMessage))
                    {
        ...
        private bool OnRescheduleRecord(ref string errMessage, ref string retval)
        {
            ...
            try
            {
                ...
                vConsultVisit10 visitInfo = BusinessLayer.GetvConsultVisit10List(string.Format("VisitID={0}", visitID)).FirstOrDefault();
                if (visitInfo.DepartmentID == Constant.Facility.INPATIENT)
                {
                    Int32 TestOrderID = Convert.ToInt32(hdnTestOrderID.Value);
                    TestOrderHd oTestOrderHD = TestOrderHdDao.Get(TestOrderID);
                    if (!string.IsNullOrEmpty(hdnListTestOrderDtID.Value))
                    {
                        string[] tempListOrderDt = hdnListTestOrderDtID.Value.Split(',');
                        if (oTestOrderHD != null)
                        {

                            catatanKlinis = oTestOrderHD.Remarks;

                            List<TestOrderDt> lstTestOrderDtData = BusinessLayer.GetTestOrderDtList(string.Format("TestOrderID='{0}' AND Isdeleted = 0", oTestOrderHD.TestOrderID));

                            if (lstTestOrderDtData.Count == tempListOrderDt.Length)
                            {
                                oTestOrderHD.ScheduledDate = scheduleDate;
                                oTestOrderHD.ScheduledTime = txtRealizationTime.Text;
                                oTestOrderHD.LastUpdatedBy = AppSession.UserLogin.UserID;
                                oTestOrderHD.LastUpdatedDate = DateTime.Now;
                                oTestOrderHD.GCToBePerformed = Constant.ToBePerformed.SCHEDULLED;
                                TestOrderHdDao.Update(oTestOrderHD);
                                ctx.CommitTransaction();
                            }
                            else
                            {
                                result = false;
                                errMessage = "Maaf, untuk rawat inap berlaku semua item yang di reschedule mohon di pilih semua item yang ada";
                            }
                        ...
                        {
                            result = false;
                            errMessage = "Maaf, Nomor order tidak ditemukan";
                    ...
                    {
                        result = false;
                        errMessage = "Maaf, mohon dipilih item yang akan di reschedule";
                    }
                }
                else
                {
                    ...
                    if (objLeave.Count > 0)
                    {
                        ...
                        if (objLeave.FirstOrDefault().Date.ToString(Constant.FormatString.DATE_PICKER_FORMAT) != Constant.ConstantDate.DEFAULT_NULL)
                        {
                            errMessage = string.Format("Maaf, untuk tanggal {0} dokter tersebut sedang cuti", txtRescheduleDate.Text);
                            return false;
                        }
                    }
                    ...
                    if (!string.IsNullOrEmpty(hdnListTestOrderDtID.Value))
                    {
                        ...
                        if (testOrderDtCount < 1)
                        {
                            ...
                            testOrderHd.LastUpdatedBy = AppSession.UserLogin.UserID;
                            catatanKlinis = testOrderHd.Remarks;
                            TestOrderHdDao.Update(testOrderHd);
                        }
                    }
                    ...
                    if (isValid)
                    {
                        ...
                        if (hdnOrderToAppointmentUsingSamePayerAsRegistration.Value == "1")
                        {
                            ...
                            if (reg.CoverageTypeID != null && reg.CoverageTypeID != 0)
                            {
                        ...
                        if (lstAppointmentCheck.Count < 1)
                        {
                            if (lstTestOrderDtToDfart.Count > 0)
                            {
                                ...
                                string remarks = string.Format("From Test Order {0} (Date : {1} {2})", Request.Form[txtTransactionNo.UniqueID], Request.Form[txtTestOrderDate.UniqueID], Request.Form[txtTestOrderTime.UniqueID]);
                                if (!string.IsNullOrEmpty(catatanKlinis))
                                {
                                    remarks += string.Format("|{0}", catatanKlinis);
                                }
                                entityHd.Remarks = remarks;
        ...
        private bool OnProcessMultiVisitRecord(ref string errMessage)
        {
            bool result = true;

            TestOrderHd entity = BusinessLayer.GetTestOrderHdList(string.Format("TestOrderID = {0}", hdnTestOrderID.Value)).FirstOrDefault();
            ...
            if (entity != null)
            {
                ...
                try
                {
                    ...
                    entity.GCOrderStatus = Constant.OrderStatus.CLOSED;
