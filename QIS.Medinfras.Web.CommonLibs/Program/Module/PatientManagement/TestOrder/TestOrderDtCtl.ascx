<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="TestOrderDtCtl.ascx.cs"
    Inherits="QIS.Medinfras.Web.CommonLibs.Program.TestOrderDtCtl" %>
...
<script type="text/javascript" id="dxss_TestOrderDtCtl">
    ...
    $('#btnTestOrderApprove').click(function (evt) {
        ...
        if (countDt == "0") {
            if ($('#<%=chkIsMultiVisitScheduleOrder.ClientID %>').is(":checked")) {
                showToast('Failed', 'Error Message : Tidak ada order yang diproses.');
            }
            else {
            ...
            if ($('#<%=chkIsMultiVisitScheduleOrder.ClientID %>').is(":checked")) {
                if (lstOrderDt != "" && lstOrderDt != "0") {
                    cbpEntryPopupView.PerformCallback('approveMultiVisit');
                }
                else {
    ...
    function onCbpEntryPopupViewEndCallback(s) {
        ...
        else if (param[0] == 'approveMultiVisit') {
            if (param[1] == 'fail')
                showToast('Approve Multi Schedule Order Failed', 'Error Message : ' + param[2]);
            else {
                pcRightPanelContent.Hide();
            }
        }
    ...
    $('#<%=chkIsMultiVisitScheduleOrder.ClientID %>').live('click', function (evt) {
        if ($('#<%=chkIsMultiVisitScheduleOrder.ClientID %>').is(":checked")) {
            $('#<%=trDataSource.ClientID %>').attr("style", "display:none");
        }
        else {
            $('#<%=trDataSource.ClientID %>').removeAttr("style");
        }
    });
...
<div>
    ...
    <input type="hidden" id="hdnHSUOperatingTheatreID" value="" runat="server" />
    <input type="hidden" id="hdnHSURadiotheraphyID" value="" runat="server" />
    ...
    <input type="hidden" id="hdnIsHasAIOPackage" runat="server" value="" />
    <input type="hidden" id="hdnOrderToAppointmentUsingSamePayerAsRegistration" runat="server"
        value="" />
                    ...
                    <fieldset id="fsMPPopupEntry">
                        <table class="tblEntryContent" style="width: 100%">
                            ...
                            <tr id="trTransactionNo" runat="server">
                                ...
                                <td style="display: none" id="trPATest" runat="server">
                                    <asp:CheckBox ID="chkIsPathologicalAnatomyTest" runat="server" CssClass="chkIsPathologicalAnatomyTest"
                                        Checked='<%#Eval("IsPathologicalAnatomyTest") %>' />
                                    <label class="lblNormal" id="Label2">
                                        <%=GetLabel("Pemeriksaan PA")%>
                                    </label>
                                </td>
                                <td>
                                    <div id="DivIsPregnant" runat="server">
                                        <img class="imgPregnant" height="25px" src='<%= ResolveUrl("~/libs/Images/Toolbar/obsgyn_history.png")%>'
                                            alt='' title='Pregannt' />
                                    </div>
                                </td>
                            </tr>
                            <tr id="trDiorderOleh" runat="server">
                                ...
                                <td id="tdMultiVisitScheduleOrder" runat="server" style="display: none">
                                    <asp:CheckBox ID="chkIsMultiVisitScheduleOrder" runat="server" />
                                    <label class="lblNormal" id="Label4">
                                        <%=GetLabel("Penjadwalan Multi Kunjungan")%>
                                    </label>
                                </td>
                            </tr>
                            ...
                            <tr id="trBloodBankOrder" runat="server" style="display: none">
                                <td class="tdLabel" valign="top" style="padding-top: 5px;">
                                    <label>
                                        <%=GetLabel("Catatan Order Bank Darah")%></label>
                                </td>
                                <td colspan="5">
                                    <asp:TextBox ID="txtBloodBankNotes" ReadOnly="true" Width="100%" TextMode="MultiLine"
                            ...
                            <tr>
                                ...
                                <td>
                                    <asp:TextBox ID="txtPatientVisaNumber" Width="120px" runat="server" ReadOnly="true" />
                                </td>
                            </tr>
    ...
    <div style="width: 100%; text-align: right; padding-top: 10px">
        <table id="tblProcess" runat="server" width="100%">
            <tr id="trDataSource" runat="server">
                <td style="width: 100%">
