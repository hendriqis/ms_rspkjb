<%@ Page Language="C#" MasterPageFile="~/Libs/MasterPage/PatientPage/MPBasePatientPageTrx.master"
    AutoEventWireup="true" CodeBehind="PatientBillSummaryRecalculationBillDtPackage.aspx.cs"
    Inherits="QIS.Medinfras.Web.CommonLibs.Program.PatientBillSummaryRecalculationBillDtPackage" %>
...
<asp:Content ID="Content3" ContentPlaceHolderID="plhCustomButtonToolbar" runat="server">
    <li id="btnRecalculationPatientBillRecalculate" runat="server" crudmode="R">
        <img src='<%=ResolveUrl("~/Libs/Images/Icon/tbrecalculate.png")%>' alt="" /><div>
            <%=GetLabel("Rekalkulasi")%></div>
    </li>
    <li id="btnRecalculationPatientBillUpdatePayer" runat="server" crudmode="R">
        <img src='<%=ResolveUrl("~/Libs/Images/Icon/tbpayorupdate.png")%>' alt="" /><div>
            <%=GetLabel("Update Pembayar")%></div>
    </li>
</asp:Content>
...
<asp:Content ID="Content1" ContentPlaceHolderID="plhEntry" runat="server">
    <script type="text/javascript" id="dxss_generatebilldtctl">
        function onLoad() {
            ...
            $('#<%=btnRecalculationPatientBillRecalculate.ClientID %>').click(function () {
                ...
                var url = ResolveUrl("~/Libs/Program/Module/PatientManagement/PatientBillSummary/RecalculationBill/PatientBillSummaryRecalculationBillProcessCtl2.ascx");
        ...
        function setRightPanelButtonEnabled() {
            ...
            if ($('#<%=hdnGCCustomerType.ClientID %>').val() != "X004^999") {
                ...
                if($('#<%=hdnGCCustomerType.ClientID %>').val() == "X004^500") {
                    ...
                    Methods.getObject('GetRegistrationBPJSList', filterExpression, function (result) {
                        if (result != null) {
                            $('#btnBPJSProcessData').removeAttr('enabled');
                        }
                        else {
                            $('#btnBPJSProcessData').attr('enabled', 'false');
                        }
                    });                
                }
            } 
        ...
        function onBeforeLoadRightPanelContent(code) {
            if (code == 'bpjsProcessData') {
                return $('#<%:hdnRegistrationID.ClientID %>').val() + '|' + $('#<%:hdnRegistrationNo.ClientID %>').val();            
            }
            else {
        ...
        function onAfterSaveRightPanelContent(code, value, isAdd) {
            ...
            Methods.getObject('GetvRegistrationList', filterExpression, function (result) {
                ...
                $('#<%=hdnGCCustomerType.ClientID %>').val(result.GCCustomerType);
            });

            if (userControlType == 'updatepayer') {
                if (businessPartnerID != 1) {
                    setLblPayerText(businessPartner);
    ...
    <div>
        <input type="hidden" runat="server" id="hdnGCCustomerType" value="" />
        <table class="tblContentArea">
            <tr>
                <td style="padding-top: 5px;">
                    <dxcp:ASPxCallbackPanel ID="cbpRecalculationPatientBill" runat="server" Width="100%"
                        ClientInstanceName="cbpRecalculationPatientBill" ShowLoadingPanel="false" OnCallback="cbpRecalculationPatientBill_Callback">
                        <ClientSideEvents BeginCallback="function(s,e){ showLoadingPanel(); }" EndCallback="function(s,e){ onCbpRecalculationPatientBillEndCallback(s); }" />
                        <PanelCollection>
