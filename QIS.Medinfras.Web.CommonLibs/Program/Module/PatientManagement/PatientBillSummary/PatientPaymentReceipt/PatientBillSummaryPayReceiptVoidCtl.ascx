<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="PatientBillSummaryPayReceiptVoidCtl.ascx.cs" 
    Inherits="QIS.Medinfras.Web.CommonLibs.Program.PatientBillSummaryPayReceiptVoidCtl" %>
...
<script type="text/javascript" id="dxss_patientpaymentreceiptvoidctl">
    ...
    $('#<%=btnVoidReceipt.ClientID %>').click(function (evt) {
        if (IsValid(evt, 'fsTrxPopup', 'mpTrxPopup')) {
            cbpPatientPaymentReceiptVoid.PerformCallback('void');
        }
    });

    function onVoidPatientPaymentReceiptSuccess() {
...
<div class="toolbarArea">
    <ul id="ulMPListToolbar" runat="server">
        <li id="btnVoidReceipt" runat="server"><img src='<%=ResolveUrl("~/Libs/Images/Icon/tbset.png")%>' alt="" /><br style="clear:both"/><div><%=GetLabel("Process")%></div></li>
    </ul>
</div>
...
<div style="padding:10px;">
    <fieldset id="fsTrxPopup" style="margin:0"> 
        ...
        <dxcp:ASPxCallbackPanel ID="cbpPatientPaymentReceiptVoid" runat="server" Width="100%" ClientInstanceName="cbpPatientPaymentReceiptVoid"
            ShowLoadingPanel="false" OnCallback="cbpPatientPaymentReceiptVoid_Callback">
            <ClientSideEvents BeginCallback="function(s,e){ showLoadingPanel();}" 
                EndCallback="function(s,e){
                    ...
                    if(result[0] == 'fail')
                        ...
                        onVoidPatientPaymentReceiptSuccess();
                    hideLoadingPanel();
                }" />
