<%@ Page Title="" Language="C#" MasterPageFile="~/Libs/MasterPage/PatientPage/MPBasePatientPageTrx.Master"
    AutoEventWireup="true" CodeBehind="PatientBillSummaryPharmacy.aspx.cs" Inherits="QIS.Medinfras.Web.CommonLibs.Program.PatientBillSummaryPharmacy" %>
    ...
    <script type="text/javascript">
        ...
        $('#<%=btnVoid.ClientID %>').live('click', function (evt) {
            if (IsValid(evt, 'fsMPEntry', 'mpEntry')) {
                //                var url = ResolveUrl('~/Libs/Program/Module/PatientManagement/PatientBillSummary/PatientChargesPrescriptionVoidCtl.ascx');
        ...
        function onCboChargeClassValueChanged() {
            ...
            Methods.getItemTariff(registrationID, visitID, classID, itemID, trxDate, function (result) {
                ...
                var tariff = parseFloat($('#<%=txtItemUnitPrice.ClientID %>').attr('hiddenVal'));
                var qty = parseFloat($('#<%=txtItemQty.ClientID %>').val());
                $('#<%=txtItemPrice.ClientID %>').val(tariff * qty).trigger('changeValue');
                //#endregion

                //#region calculate Discount
                var discountAmount = parseInt($('#<%=hdnDiscountAmount.ClientID %>').val());
                var isDicountInPercentage = ($('#<%=hdnIsDicountInPercentage.ClientID %>').val() == '1');

                var discountTotal = 0;
                if (discountAmount > 0) {
                    var tariff = parseFloat($('#<%=txtItemPrice.ClientID %>').attr('hiddenVal'));
                ...
                var tariff = parseInt($('#<%=txtItemPrice.ClientID %>').attr('hiddenVal'));
                var discount = parseInt($('#<%=txtDiscountAmount.ClientID %>').attr('hiddenVal'));
        ...
        $('.imgEdit.imgLink').live('click', function () {
            ...
            $('#<%=txtItemName.ClientID %>').val(entity.DrugName);
        ...
        function onBeforeLoadRightPanelContent(code) {
            ...
            else if (code == 'changePrescriptionType') {
                var transactionID = $('#<%:hdnTransactionID.ClientID %>').val();
                                ...
                                <td>
                                    ...
                                    <table>
                                        <tr>
                                            ...
                                            <td>
                                                <asp:TextBox runat="server" ID="txtItemName" Width="300px" ReadOnly="true" />
                                            </td>
                                        </tr>
                                    </table>
                                </td>
                                ...
                                <td>
                                    <table>
                                        <tr>
                                            ...
                                            <td>
                                                <asp:TextBox runat="server" ID="txtItemUnit" Width="200px" ReadOnly="true" />
                                            </td>
                                        </tr>
                                    </table>
                                </td>
