...
{
    public partial class ResendImagingOrderDetail : BasePageTrx
    {
        ...
        protected override void InitializeDataControl()
        {
            ...
            if (param[0] == "to")
            {
                PatientChargesHd entityHd = BusinessLayer.GetPatientChargesHdList(string.Format("TestOrderID = '{0}' AND GCTransactionStatus <> '{1}'", param[1], Constant.TransactionStatus.VOID)).FirstOrDefault();
                if (entityHd != null)
                {
            ...
            if (!string.IsNullOrEmpty(transactionNo))
            {
                ...
                pageIndexFirstLoad = BusinessLayer.GetvPatientChargesHdRowIndex(GetFilterExpression(), transactionNo, "TransactionID DESC");
            }
        ...
        protected string GetFilterExpression()
        {
            ...
            filterExpression += string.Format(" AND GCTransactionStatus != '{0}' AND HealthcareServiceUnitID = {1}", Constant.TransactionStatus.VOID, hdnHsuImaging.Value);
        ...
        {
            string filterExpression = string.Format("TransactionID = {0} AND GCItemType IN ('{1}') AND IsDeleted = 0 ORDER BY ID", hdnTransactionID.Value, Constant.ItemGroupMaster.RADIOLOGY);
