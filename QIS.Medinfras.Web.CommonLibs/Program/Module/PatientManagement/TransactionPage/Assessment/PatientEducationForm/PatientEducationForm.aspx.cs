...
{
    public partial class PatientEducationForm : BasePagePatientPageList
    {
            ...
            else
            {
                ...
                switch (param[0])
                {
                    ...
                    case Constant.Facility.DIAGNOSTIC:
                        return Constant.MenuCode.MedicalDiagnostic.PATIENT_PAGE_EDUCATION_FORM;
                    default:
                        return Constant.MenuCode.MedicalDiagnostic.PATIENT_PAGE_EDUCATION_FORM;
                }
                #endregion
        ...
        protected override void InitializeDataControl()
        {
            ...
            if (param.Count() > 1)
            {
                deptType = param[0];
                menuType = param[1];
                formType = string.Format("X397^{0}",param[2]);
            }
            else
            {
                deptType = param[0];
                formType = string.Format("X397^001");
            }
        ...
        {
            string filterExpression = string.Format("ParentID = '{0}' ORDER BY StandardCodeID", Constant.StandardCode.PATIENT_ASSESSMENT_FORM);

            switch (formType)
            {
                case Constant.AssessmentFormGroup.KEPERAWATAN_UMUM:
                    filterExpression = string.Format("ParentID = '{0}' AND IsActive = 1 ORDER BY StandardCodeID", Constant.StandardCode.PATIENT_ASSESSMENT_FORM);
                    break;
                case Constant.AssessmentFormGroup.KEPERAWATAN_KAMAR_OPERASI:
                    filterExpression = string.Format("ParentID = '{0}' AND IsActive = 1 ORDER BY StandardCodeID", Constant.StandardCode.FORM_PENGKAJIAN_KAMAR_OPERASI);
                    break;
                case Constant.AssessmentFormGroup.KEPERAWATAN_HEMODIALISA:
                    filterExpression = string.Format("ParentID = '{0}' AND IsActive = 1 ORDER BY StandardCodeID", Constant.StandardCode.FORM_PENGKAJIAN_HEMODIALISA);
                    break;
                case Constant.AssessmentFormGroup.KEPERAWATAN_KAMAR_BERSALIN:
                    filterExpression = string.Format("ParentID = '{0}' AND IsActive = 1 ORDER BY StandardCodeID", Constant.StandardCode.FORM_PENGKAJIAN_KAMAR_BERSALIN);
                    break;
                case Constant.AssessmentFormGroup.KEPERAWATAN_GIZI:
                    filterExpression = string.Format("ParentID = '{0}' AND IsActive = 1 ORDER BY StandardCodeID", Constant.StandardCode.FORM_PENGKAJIAN_GIZI);
                    break;   
                default:
                    filterExpression = string.Format("ParentID = '{0}' AND IsActive = 1 ORDER BY StandardCodeID", Constant.StandardCode.PATIENT_ASSESSMENT_FORM);
                    break;
        ...
        private void BindPrintout()
        {
            if (!string.IsNullOrEmpty(hdnID.Value) && hdnID.Value != "0")
            {
                ...
                List<vPatientAssessment> lstPatient = BusinessLayer.GetvPatientAssessmentList(string.Format("RegistrationID = '{0}' AND IsDeleted = 0 ORDER BY AssessmentID DESC", AppSession.RegisteredPatient.RegistrationID));
        ...
        protected override bool OnAddRecord(ref string url, ref string errMessage, ref string queryString, ref int popupWidth, ref int popupHeight, ref string popupHeaderText)
        {
            ...
            queryString = string.Format("{0}|{1}|{2}|{3}|{4}|{5}|{6}|{7}", "1", hdnGCAssessmentType.Value, hdnID.Value, hdnAssessmentDate.Value, hdnAssessmentTime.Value, hdnParamedicID.Value, hdnAssessmentValues.Value, hdnIsInitialAssessment.Value); popupWidth = 800;
            popupHeight = 600;
        ...
        protected override bool OnEditRecord(ref string url, ref string errMessage, ref string queryString, ref int popupWidth, ref int popupHeight, ref string popupHeaderText)
        {
            if (hdnID.Value != "")
            {
                ...
                queryString = string.Format("{0}|{1}|{2}|{3}|{4}|{5}|{6}|{7}", "0", hdnGCAssessmentType.Value, hdnID.Value, hdnAssessmentDate.Value, hdnAssessmentTime.Value, hdnParamedicID.Value, hdnAssessmentValues.Value, hdnIsInitialAssessment.Value); popupWidth = 800;
                popupHeight = 600;
                popupHeaderText = string.Format("Pengkajian Pasien : {0} (MRN = {1}, REG = {2})", AppSession.RegisteredPatient.PatientName, AppSession.RegisteredPatient.MedicalNo, AppSession.RegisteredPatient.RegistrationNo);
                return true;
            }
