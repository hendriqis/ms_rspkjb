<%@ Page Language="C#" MasterPageFile="~/libs/MasterPage/PatientPage/MPBasePatientPageList.master"
    ValidateRequest="false" AutoEventWireup="true" CodeBehind="PopulationAssessmentForm.aspx.cs"
    Inherits="QIS.Medinfras.Web.CommonLibs.Program.PopulationAssessmentForm" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="plhList" runat="server">
    <script type="text/javascript" id="dxss_medicalFormlist">
        ...
        $('.lnkCopy').live('click', function () {
            ...
            displayConfirmationMessageBox('COPY PENGKAJIAN PASIEN :', message, function (result) {
                if (result) {
                    var url = ResolveUrl("~/libs/Controls/EMR/Nursing/CopyPopulationAssessmentFormEntry.ascx");
        ...
        function onCbpViewEndCallback(s) {
            ...
            if (param[0] == 'refresh') {
                ...
                if (FileString != "") {
                    ...
                    newWin.document.write('<html><head><title>Asesmen Tambahan (Populasi Khusus)</title><link rel="stylesheet" type="text/css" href="' + css + '"> </head><style type="text/css" media="print"> .noPrint{display:none;} </style>');
                                ...
                                <dx:PanelContent ID="PanelContent2" runat="server">
                                    <asp:Panel runat="server" ID="panFormList" CssClass="pnlContainerGridPatientPage">
                                        <asp:GridView ID="grdFormList" runat="server" CssClass="grdSelected grdPatientPage"
                                            ...
                                            <EmptyDataTemplate>
                                                <%=GetLabel("Tidak ada template form pengkajian populasi khusus yang bisa digunakan")%>
                                            </EmptyDataTemplate>
                                        </asp:GridView>
                                    </asp:Panel>
                                </dx:PanelContent>
                                    ...
                                    <asp:GridView ID="grdView" runat="server" CssClass="grdSelected grdPatientPage" AutoGenerateColumns="false"
                                        ...
                                        <Columns>
                                            ...
                                            <asp:TemplateField HeaderStyle-HorizontalAlign="Center" ItemStyle-HorizontalAlign="Center"
                                                ...
                                                <ItemTemplate>
                                                    <div>
                                                        <a class="lnkCopy" recordid="<%#:Eval("AssessmentID") %>" visitid="<%#:Eval("VisitID") %>"
                                                            assessmenttype="<%#:Eval("GCAssessmentType") %>" assessmentdate="<%#:Eval("cfAssessmentDatePickerFormat") %>"
                                                            assessmenttime="<%#:Eval("AssessmentTime") %>" assessmentlayout="<%#:Eval("AssessmentFormLayout") %>"
                                                            assessmentvalue="<%#:Eval("AssessmentFormValue") %>" paramedicname="<%#:Eval("ParamedicName") %>"
                                                            isinitialassessment="<%#:Eval("IsInitialAssessment") %>">Copy</a>
                                                    </div>
                                                </ItemTemplate>
                                            </asp:TemplateField>
                                        </Columns>
