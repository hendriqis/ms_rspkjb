<%@ Page Language="C#" MasterPageFile="~/libs/MasterPage/PatientPage/MPBasePatientPageList.master"
    AutoEventWireup="true" CodeBehind="EWSAssessmentForm.aspx.cs" Inherits="QIS.Medinfras.Web.CommonLibs.Program.EWSAssessmentForm" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="plhList" runat="server">
    <script type="text/javascript" id="dxss_vitalsignlist">
        ...
        $(function () {
            ...
            $('#<%=grdView.ClientID %> tr:gt(0):not(.trEmpty)').live('click', function () {
                ...
                $('#<%=hdnIsEWSAlert.ClientID %>').val($(this).find('.cfIsEWSAlert').html());
                $('#<%=hdnEWSScore.ClientID %>').val($(this).find('.ewsScore').html());
                $('#<%=hdnGCEWSScoreType.ClientID %>').val($(this).find('.gcEWSScoreType').html());
        ...
        $('.lnkView').live('click', function () {
            ...
            var isEWSAlert = $(this).attr("isEWSAlert");
            ...
            var param = formType + '|' + id + '|' + patientInfo;
            var url = ResolveUrl("~/libs/Controls/EMR/Nursing/Assessment/ViewEWSAssessmentCtl.ascx");
        ...
        $('.lnkCopy').live('click', function () {
            ...
            displayConfirmationMessageBox('COPY PENGKAJIAN PASIEN :', message, function (result) {
                if (result) {
                    var url = ResolveUrl("~/libs/Controls/EMR/Nursing/CopyEWSAssessmentFormEntry.ascx");
                     openUserControlPopup(url, param, $('#<%=hdnPopupTitle.ClientID %>').val(), 800, 600);
                }
            });
        });
    ...
    <input type="hidden" runat="server" id="hdnIsEWSAlert" value="" />
    <input type="hidden" runat="server" id="hdnEWSScore" value="" />
    <input type="hidden" runat="server" id="hdnGCEWSScoreType" value="" />
                                ...
                                <asp:Panel runat="server" ID="pnlView" CssClass="pnlContainerGridPatientPage">
                                    <asp:GridView ID="grdView" runat="server" CssClass="grdSelected grdPatientPage" AutoGenerateColumns="false"
                                        ...
                                        <Columns>
                                            ...
                                            <asp:BoundField DataField="EWSScore" HeaderText="Skor" HeaderStyle-HorizontalAlign="Right"
                                                HeaderStyle-Width="100px" ItemStyle-HorizontalAlign="Right" />
<%--                                            <asp:BoundField DataField="EWSScoreType" HeaderText="Kesimpulan" HeaderStyle-HorizontalAlign="Left"
                                            ...
                                            <asp:BoundField DataField="GCEWSAssessmentType" HeaderText="Values" HeaderStyle-HorizontalAlign="Left"
                                            ...
                                            <asp:BoundField DataField="cfIsEWSAlert" HeaderText="Values" HeaderStyle-HorizontalAlign="Left"
                                                HeaderStyle-CssClass="cfIsEWSAlert hiddenColumn" ItemStyle-CssClass="cfIsEWSAlert hiddenColumn" />
                                            <asp:BoundField DataField="EWSScore" HeaderText="Values" HeaderStyle-HorizontalAlign="Left"
                                                HeaderStyle-CssClass="ewsScore hiddenColumn" ItemStyle-CssClass="ewsScore hiddenColumn" />
                                            <asp:BoundField DataField="GCEWSScoreType" HeaderText="Values" HeaderStyle-HorizontalAlign="Left"
                                                HeaderStyle-CssClass="gcEWSScoreType hiddenColumn" ItemStyle-CssClass="gcEWSScoreType hiddenColumn" />
                                                ...
                                                <ItemTemplate>
                                                    <div>
                                                        <a class="lnkView" recordid="<%#:Eval("AssessmentID") %>"
                                                            visitID="<%#:Eval("VisitID") %>" assessmentType="<%#:Eval("GCEWSAssessmentType") %>" assessmentDate="<%#:Eval("cfAssessmentDatePickerFormat") %>" assessmentTime="<%#:Eval("AssessmentTime") %>" assessmentLayout="<%#:Eval("AssessmentFormLayout") %>" assessmentValue="<%#:Eval("AssessmentFormValue") %>"
                                                            paramedicName="<%#:Eval("ParamedicName") %>" isInitialAssessment="<%#:Eval("IsInitialAssessment") %>" isEWSAlert="<%#:Eval("cfIsEWSAlert") %>" riskScoreType="<%#:Eval("GCEWSScoreType") %>">View</a>
                                                    </div>
                                                </ItemTemplate>
                                            </asp:TemplateField>
                                            <asp:TemplateField HeaderStyle-HorizontalAlign="Center" ItemStyle-HorizontalAlign="Center" ItemStyle-Width="60px">
                                                <ItemTemplate>
                                                    <div>
                                                        <a class="lnkCopy" recordid="<%#:Eval("AssessmentID") %>"
                                                            visitID="<%#:Eval("VisitID") %>" assessmentType="<%#:Eval("GCEWSAssessmentType") %>" assessmentDate="<%#:Eval("cfAssessmentDatePickerFormat") %>" assessmentTime="<%#:Eval("AssessmentTime") %>" assessmentLayout="<%#:Eval("AssessmentFormLayout") %>" assessmentValue="<%#:Eval("AssessmentFormValue") %>"
                                                            paramedicName="<%#:Eval("ParamedicName") %>" isInitialAssessment="<%#:Eval("IsInitialAssessment") %>" isEWSAlert="<%#:Eval("cfIsEWSAlert") %>" riskScoreType="<%#:Eval("GCEWSScoreType") %>">Copy</a>
                                                    </div>
                                                </ItemTemplate>
                                            </asp:TemplateField>
                                        </Columns>
