<%@ Page Language="C#" MasterPageFile="~/libs/MasterPage/PatientPage/MPBasePatientPageList.master" ValidateRequest="false" 
    AutoEventWireup="true" CodeBehind="PatientAssessmentForm.aspx.cs" Inherits="QIS.Medinfras.Web.CommonLibs.Program.PatientAssessmentForm" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="plhList" runat="server">
    <script type="text/javascript" id="dxss_vitalsignlist">
        ...
        $(function () {
            ...
            $('#<%=grdView.ClientID %> tr:gt(0):not(.trEmpty)').live('click', function () {
                ...
                $('#<%=hdnIsVerified.ClientID %>').val($(this).find('.isVerified').html());
                ...
                var isNutritionMenu = $('#<%=hdnNutritionMenu.ClientID %>').val();
            });
        });
        ...
        $('.lnkView').live('click', function () {
            ...
            var param = formType + '|' + id + '|' + date + '|' + time + '|' + ppa + '|' + isInitialAssessment + '|' + layout + '|' + values + '|' + formGroup + "|" + visitID + '|' + patientInfo + "|" + isNeedVerified + '|' + isNutritionMenu;
        ...
        $('.lnkCopy').live('click', function () {
            ...
            displayConfirmationMessageBox('COPY PENGKAJIAN PASIEN :', message, function (result) {
                if (result) {
                    ...
                    var param = formType + '|' + visitID + "|" + id + '|' + layout + '|' + values + '|' + patientInfo + '|' + isNutritionMenu;
                    if (formType == 'X401^113' || formType == 'X401^003' || formType == 'X401^004' || formType == 'X401^013' || formType == 'X401^017') {
                        patientInfo = medicalNo + "@" + patientName + "@" + patientDOB + "@" + registrationNo;
                        param = formType + '@' + visitID + "@" + id + '@' + layout + '@' + values + '@' + patientInfo;
                    }
                    openUserControlPopup(url, param, 'Pengkajian Pasien', 800, 600);
                }
            });
        ...
        function onBeforeEditRecord(entity, errMessage) {
            if (entity.IsVerified == true) {
                errMessage.text = 'Maaf Data tidak bisa diubah, karena data sudah dikonfirmasi.';
                return false;
            }
            return true;
        }
        ...
        function onBeforeDeleteRecord(entity, errMessage) {
            if (entity.IsVerified == true) {
                errMessage.text = 'Maaf Data tidak bisa dihapus, karena data sudah dikonfirmasi.';
                return false;
            }
            return true;
        }
        ...
        function onCbpViewEndCallback(s) {
            ...
            if (param[0] == 'refresh') {
                ...
                if (FileString != "") {
                    ...
                    newWin.document.write('<html><head><title>Form Pengkajian Pasien</title><link rel="stylesheet" type="text/css" href="' + css + '"> </head><style type="text/css" media="print"> .noPrint{display:none;} </style>');
    ...
    <input type="hidden" id="hdnIsVerified" runat="server" value="0" />
                                ...
                                <asp:Panel runat="server" ID="pnlView" CssClass="pnlContainerGridPatientPage">
                                    <asp:GridView ID="grdView" runat="server" CssClass="grdSelected grdPatientPage" AutoGenerateColumns="false"
                                        ...
                                        <Columns>
                                            ...
                                            <asp:BoundField DataField="IsVerified" HeaderText = "Values" HeaderStyle-HorizontalAlign="Left" ItemStyle-HorizontalAlign="Left" HeaderStyle-CssClass="isVerified hiddenColumn" ItemStyle-CssClass="isVerified hiddenColumn"/>
                                            ...
                                            <asp:TemplateField HeaderStyle-Width="250px" ItemStyle-HorizontalAlign="Left" HeaderStyle-HorizontalAlign="Left">
                                                <HeaderTemplate>
                                                    <div><%#: IsNutritionMenu() == "0" ? "" : GetLabel("Diverifikasi Oleh")%></div>
                                                </HeaderTemplate>
                                                <ItemTemplate>
                                                    <asp:Label ID="txtVerifiedByName" runat="server" Enabled="False"></asp:Label>
                                                </ItemTemplate>
                                            </asp:TemplateField>
                                            <asp:TemplateField HeaderStyle-Width="120px" ItemStyle-HorizontalAlign="Center" HeaderStyle-HorizontalAlign="Center">
                                                <HeaderTemplate>
                                                    <div><%#: IsNutritionMenu() == "0" ? "" : GetLabel("Tgl. Verifikasi")%></div>
                                                </HeaderTemplate>
                                                <ItemTemplate>
                                                    <asp:Label ID="txtVerifiedDateTime" runat="server" Enabled="False"></asp:Label>
                                                </ItemTemplate>
                                            </asp:TemplateField>
                                                ...
                                                <ItemTemplate>
                                                    <div>
                                                        <a class="lnkView" recordid="<%#:Eval("AssessmentID") %>"
                                                            ...
                                                            paramedicName="<%#:Eval("ParamedicName") %>" isInitialAssessment="<%#:Eval("IsInitialAssessment") %>" isNeedVerified="<%#:Eval("IsNeedVerified") %>" isVerified="<%#:Eval("IsVerified") %>">View</a>
                                                    </div>
                                                </ItemTemplate>
                                            </asp:TemplateField>
                                            <asp:TemplateField HeaderStyle-HorizontalAlign="Center" ItemStyle-HorizontalAlign="Center" ItemStyle-Width="60px">
                                                <ItemTemplate>
                                                    <div>
                                                        <a class="lnkCopy" recordid="<%#:Eval("AssessmentID") %>"
                                                            visitID="<%#:Eval("VisitID") %>" assessmentType="<%#:Eval("GCAssessmentType") %>" assessmentDate="<%#:Eval("cfAssessmentDatePickerFormat") %>" assessmentTime="<%#:Eval("AssessmentTime") %>" assessmentLayout="<%#:Eval("AssessmentFormLayout") %>" assessmentValue="<%#:Eval("AssessmentFormValue") %>"
                                                            paramedicName="<%#:Eval("ParamedicName") %>" isInitialAssessment="<%#:Eval("IsInitialAssessment") %>" isNeedVerified="<%#:Eval("IsNeedVerified") %>" isVerified="<%#:Eval("IsVerified") %>">Copy</a>
                                                    </div>
                                                </ItemTemplate>
                                            </asp:TemplateField>
                                        </Columns>
