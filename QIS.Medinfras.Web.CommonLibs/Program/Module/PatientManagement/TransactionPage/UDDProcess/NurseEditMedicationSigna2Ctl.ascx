<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="NurseEditMedicationSigna2Ctl.ascx.cs"
    Inherits="QIS.Medinfras.Web.CommonLibs.Program.NurseEditMedicationSigna2Ctl" %>
...
<script type="text/javascript" id="dxss_patiententryctl">
    ...
    $('#lblPhysicianNoteID.lblLink').live('click', function () {
        var filterExpression = "VisitID = " + $('#<%=hdnVisitID.ClientID %>').val() + " AND GCPatientNoteType IN ('X011^010','X011^011')";
    ...
    $('#<%=txtDefaultStartDate.ClientID %>').live('change', function (evt) {
        onRefreshGrid();
    });

    $('.txtFrequency').live('change', function () {
        ...
        Methods.getMedicationSequenceTime(frequency, function (result) {
            if (result != null) {
                ...
                $tr.find('.txtMedicationTime1').val(medicationTimeInfo[0]);
                $tr.find('.txtMedicationTime2').val(medicationTimeInfo[1]);
                $tr.find('.txtMedicationTime3').val(medicationTimeInfo[2]);
                $tr.find('.txtMedicationTime4').val(medicationTimeInfo[3]);
                $tr.find('.txtMedicationTime5').val(medicationTimeInfo[4]);
                $tr.find('.txtMedicationTime6').val(medicationTimeInfo[5]);
            }
            else {
                $tr.find('.txtMedicationTime1').val('-');
    ...
    function getSelectedCheckbox() {
        var tempSelectedID = "";
        var tempSelectedFrequency = "";
        var tempSelectedQuantity = "";
        ...
        $('.grdPrescriptionOrderDt .chkIsProcessItem input:checked').each(function () {
            ...
            var frequency = $tr.find('.txtFrequency').val();
            var quantity = $tr.find('.txtNumberOfDosage').val();
            ...
            if (tempSelectedID != "") {
                ...
                tempSelectedTime5 += ",";
                tempSelectedTime6 += ",";
                tempSelectedFrequency += ",";
                tempSelectedQuantity += ",";
            }
            tempSelectedID += id;
            tempSelectedFrequency += frequency;
            tempSelectedQuantity += quantity;
        ...
        if (tempSelectedID != "") {
            ...
            $('#<%=hdnSelectedFrequency.ClientID %>').val(tempSelectedFrequency);
            $('#<%=hdnSelectedQuantity.ClientID %>').val(tempSelectedQuantity);
    ...
    $('#btnMPEntryProcess').click(function () {
        if (!getSelectedCheckbox()) {
            displayErrorMessageBox("Ubah Frekuensi Pemberian", 'Error Message : ' + "Please select the item to be process !");
        }
        else {
            var message = "Lanjutkan proses perubahan frekuensi pemberian ?";
            displayConfirmationMessageBox("Cara Pemberian", message, function (result) {
    ...
    function onCbpPopupProcesEndCallback(s) {
        ...
        if (param[0] == 'process') {
            if (param[1] == '0') {
                displayErrorMessageBox("Ubah Frekuensi Pemberian", "Error Message : <br/><span style='color:red'>" + param[2] + "</span>");
            }
            else {
                pcRightPanelContent.Hide();
                if (typeof onRefreshList == 'function') {
                    onRefreshList();
                    displayMessageBox("Ubah Frekuensi Pemberian", param[2]);
                }
            }
        }
    }
                    ...
                    <table width="100%">
                        ...
                        <tr>
                            <td class="tdLabel">
                                <label class="lblNormal">
                                    <%=GetLabel("Mulai dari tanggal")%></label>
                            </td>
                            <td>
                        ...
                        <tr style="display: none">
                            <td class="tdLabel" style="vertical-align: top">
                                <label class="lblNormal">
                                    <%=GetLabel("Durasi Pemberian")%></label>
                            </td>
                            <td style="vertical-align: top">
                                <asp:TextBox ID="txtDefaultDuration" runat="server" Width="60px" Text="1" CssClass="number" />
                                &nbsp;
                                <%=GetLabel("hari")%>
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td style="vertical-align: top">
                                <table border="0" cellpadding="0" cellspacing="0" style="width: 100%">
                                    <tr>
                                        <td>
                                            <asp:CheckBox ID="chkIsGenerateCPPT" runat="server" Text=" CPPT" ToolTip="Generate CPPT"
                                                Checked="false" />
                                        </td>
                                        <td>
                                            <asp:CheckBox ID="chkIsGenerateJournal" runat="server" Text=" Catatan Perawat" ToolTip="Generate Catatan"
                                                Checked="true" />
                                        </td>
                                    </tr>
                                </table>
                        ...
                        <div style="position: relative;">
                            <table border="0" cellpadding="1" cellspacing="0" style="width: 100%">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <asp:TextBox ID="txtPatientVisitNoteText" Width="100%" Height="100px" runat="server"
                                    ...
                                    <asp:ListView runat="server" ID="lvwExtendView" OnItemDataBound="lvwExtendView_ItemDataBound">
                                        ...
                                        <LayoutTemplate>
                                            <table id="tblView" runat="server" class="grdPrescriptionOrderDt grdSelected" cellspacing="0"
                                                rules="all">
                                                <tr>
                                                    <th rowspan="2" class="keyField">
                                                        &nbsp;
                                                    </th>
                                                    <th rowspan="2" class="hiddenColumn">
                                                        &nbsp;
                                                    </th>
                                        ...
                                        <ItemTemplate>
                                            <tr>
                                                ...
                                                <td align="right" style="width: 40px;">
                                                    <input type="number" class="txtFrequency number min" min="1" validationgroup="mpDrugEntry"
                                                        value="<%#:Eval("Frequency") %>" style="width: 100%" />
                                                </td>
                                                <td align="left" style="width: 40px; text-align: left">
                                                    <div>
                                                        <%#: Eval("DosingFrequency")%></div>
                                                </td>
                                                <td align="right">
                                                    <input type="number" class="txtNumberOfDosage number" value="<%#:Eval("cfNumberOfDosage") %>"
                                                        style="width: 100%" />
                                                </td>
                                                <td align="left">
                                                    <div>
                                                ...
                                                <td class="tdRoute" style="width: 80px">
                                                    <label class="lblRoute">
                                                        <%#: Eval("Route")%></label>
                                                </td>
                                                <td style="width: 40px;">
                                                    <asp:TextBox ID="txtMedicationTime1" Width="60px" runat="server" CssClass="txtMedicationTime1"
                                                        Text="00:00" Style="text-align: center" />
                                                    <td style="width: 40px;">
                                                        <asp:TextBox ID="txtMedicationTime2" Width="60px" runat="server" CssClass="txtMedicationTime2"
                                                            Text="00:00" Style="text-align: center" />
                                                        <td style="width: 40px;">
                                                            <asp:TextBox ID="txtMedicationTime3" Width="60px" runat="server" CssClass="txtMedicationTime3"
                                                                Text="00:00" Style="text-align: center" />
                                                            <td style="width: 40px;">
                                                                <asp:TextBox ID="txtMedicationTime4" Width="60px" runat="server" CssClass="txtMedicationTime4"
                                                                    Text="00:00" Style="text-align: center" />
                                                                <td style="width: 40px;">
                                                                    <asp:TextBox ID="txtMedicationTime5" Width="60px" runat="server" CssClass="txtMedicationTime5"
                                                                        Text="00:00" Style="text-align: center" />
                                                                    <td style="width: 40px;">
                                                                        <asp:TextBox ID="txtMedicationTime6" Width="60px" runat="server" CssClass="txtMedicationTime6"
                                                                            Text="00:00" Style="text-align: center" />
                                                                        <td style="width: 110px">
                                                                            <asp:TextBox ID="txtStartDate" Width="80px" runat="server" CssClass="txtStartDate datepicker" />
                                                                        </td>
                                                                        <td style="width: 60px">
                                                                            <asp:TextBox ID="txtDuration" Width="60px" runat="server" CssClass="txtDuration number" />
                                                                        </td>
                                            </tr>
                                        </ItemTemplate>
                                    </asp:ListView>
