<%@ Page Language="C#" MasterPageFile="~/libs/MasterPage/PatientPage/MPBasePatientPageList.master"
    AutoEventWireup="true" CodeBehind="MDInitialAssessmentList1.aspx.cs" Inherits="QIS.Medinfras.Web.CommonLibs.Program.MDInitialAssessmentList1" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="plhList" runat="server">
    <script type="text/javascript" id="dxss_IPInitialAssessmentList">
        $(function () {
            $('#<%=grdView.ClientID %> tr:gt(0):not(.trEmpty)').live('click', function () {
                ...
                $('#<%=hdnHealthcareServiceUnitID.ClientID %>').val($(this).find('.healthcareServiceUnitID').html());
            ...
            $('.btnRevision').live('click', function () {
                ...
                if (isAllowRevision == "True") {
                    var message = "Lakukan revisi kajian awal pasien ?";
                    displayConfirmationMessageBox('REVISI KAJIAN AWAL :', message, function (result) {
                        if (result) {
                            var url = ResolveUrl("~/libs/Controls/EMR/Nursing/AssessmentRevisionNoteCtl.ascx");
                            var param = visitID + "|" + id;
                            openUserControlPopup(url, param, 'Revisi Kajian Awal', 400, 300);
                        }
                    });
                }
                else {
                    displayErrorMessageBox('Warning', 'Maaf, Pengkajian hanya bisa direvisi dalam waktu 24 jam.');
                }
            });
            ...
            $('.btnCopy').live('click', function () {
                ...
                displayConfirmationMessageBox('COPY KAJIAN AWAL :', message, function (result) {
                    if (result) {
                        var url = ResolveUrl("~/libs/Controls/EMR/Nursing/CopyMDNurseAssessmentCtl.ascx");
                        var param = visitID + "|" + id;
            ...
            $('.btnComplete').click(function (evt) {
                ...
                if (($('#<%=hdnCurrentParamedicID.ClientID %>').val() == $('#<%=hdnParamedicID.ClientID %>').val()) || $('#<%=hdnCurrentParamedicID.ClientID %>').val() == $('#<%=hdnRevisedParamedicID.ClientID %>').val()) {
                    ...
                    displayErrorMessageBox('Warning', 'Maaf, Pengkajian hanya bisa ditandai selesai oleh PPA yang mengkaji atau merevisi.');
                }
            });
            //#endregion
        ...
        function onBeforeBasePatientPageListEdit() {
            if (($('#<%=hdnCurrentParamedicID.ClientID %>').val() == $('#<%=hdnParamedicID.ClientID %>').val()) || $('#<%=hdnCurrentParamedicID.ClientID %>').val() == $('#<%=hdnRevisedParamedicID.ClientID %>').val()) {
                if ($('#<%=hdnCurrentHealthcareServiceUnitID.ClientID %>').val() == $('#<%=hdnHealthcareServiceUnitID.ClientID %>').val()) {
                    ...
                    displayErrorMessageBox('EDIT', 'Maaf, Kajian awal hanya bisa dilakukan perubahan di Unit Pelayanan yang sama.');
                    return false;
                }
            }
            else {
                displayErrorMessageBox('EDIT', 'Maaf, Kajian awal hanya bisa dilakukan perubahan oleh user yang mengkaji atau yang melakukan revisi.');
                return false;
            }
        }
    ...
    <input type="hidden" runat="server" id="hdnCurrentHealthcareServiceUnitID" value="" />
                    ...
                    <asp:Panel runat="server" ID="pnlView" CssClass="pnlContainerGridPatientPage">
                        <asp:GridView ID="grdView" runat="server" CssClass="grdSelected grdPatientPage" AutoGenerateColumns="false"
                            ...
                            <Columns>
                                ...
                                <asp:BoundField DataField="AssessmentHealthcareServiceUnitID" HeaderStyle-CssClass="hiddenColumn" ItemStyle-CssClass="hiddenColumn healthcareServiceUnitID" />
                                ...
                                <asp:BoundField DataField="AssessmentServiceUnitName" HeaderText="Unit Pelayanan " HeaderStyle-Width="150px" HeaderStyle-HorizontalAlign="Left"
                                ...
                                <asp:BoundField DataField="ParamedicName" HeaderText="Dikaji oleh " HeaderStyle-Width="150px" HeaderStyle-HorizontalAlign="Left"
                                    ItemStyle-HorizontalAlign="Left" ItemStyle-CssClass="paramedicName" />
                                <asp:TemplateField HeaderText="Direvisi Oleh" HeaderStyle-Width="150px" HeaderStyle-HorizontalAlign="Left" ItemStyle-HorizontalAlign="Left">
                                    <ItemTemplate>
                                ...
                                <asp:TemplateField HeaderText="" HeaderStyle-HorizontalAlign="Left" ItemStyle-HorizontalAlign="Left"
                                    ...
                                    <ItemTemplate>
                                        ...
                                        <div id="divView" runat="server" style='margin-top: 5px; text-align: center'>
                                            <input type="button" id="btnView" runat="server" class="btnView w3-btn w3-hover-blue" value="Lihat" style='<%# Eval("cfIsCompleted").ToString() == "False" ? "display:none;": "width: 100px; background-color: Green; color: White;" %>' recordID = '<%#:Eval("ChiefComplaintID") %>' visitID = '<%#:Eval("VisitID") %>'/>
                                            <input type="button" id="btnCopy" runat="server" class="btnCopy w3-btn w3-hover-blue" value="Copy" style='<%# Eval("cfIsCompleted").ToString() == "False" ? "display:none;": "width: 100px; background-color: Red; color: White;" %>' recordID = '<%#:Eval("ChiefComplaintID") %>' visitID = '<%#:Eval("VisitID") %>'/>
                                            <div style="<%# Eval("cfIsAllowRevision").ToString() == "False" ? "display:none": "" %>" >
                                                <input type="button" id="btnRevision" runat="server" class="btnRevision w3-btn w3-hover-blue" value="Revisi"
                            ...
                            <EmptyDataTemplate>
                                <%=GetLabel("Tidak ada informasi pengkajian awal untuk pasien ini") %>
                            </EmptyDataTemplate>
                        </asp:GridView>
                    </asp:Panel>
                </dx:PanelContent>
