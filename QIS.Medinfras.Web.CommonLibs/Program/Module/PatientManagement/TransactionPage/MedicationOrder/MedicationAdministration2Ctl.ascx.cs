...
{
    public partial class MedicationAdministration2Ctl : BaseViewPopupCtl
    {
            ...
            try
            {
                for (int ct = 0; ct < selectedID.Length; ct++)
                {
                    ...
                    if (oSchedule != null)
                    {
                        ...
                        oSchedule.ParamedicID = Convert.ToInt32(cboParamedic.Value);
        ...
        private void SetControlProperties()
        {
            ...
            filterExpression = string.Format("ParentID IN ('{0}','{4}') AND StandardCodeID NOT IN ('{1}','{2}','{3}')", Constant.StandardCode.UDD_MEDICATION_STATUS, Constant.MedicationStatus.OPEN, Constant.MedicationStatus.DIPROSES_FARMASI, Constant.MedicationStatus.DISCONTINUE, Constant.StandardCode.FAMILY_RELATION);
            List<StandardCode> lstStandardCode = BusinessLayer.GetStandardCodeList(filterExpression);
            Methods.SetComboBoxField(cboDefaultMedicationStatus, lstStandardCode.Where(sc => sc.ParentID == Constant.StandardCode.UDD_MEDICATION_STATUS).ToList(), "StandardCodeName", "StandardCodeID");
    ...
    Helper.GetDatePickerValue(txtMedicationDate.Text).ToString(Constant.FormatString.DATE_FORMAT_112), Constant.MedicationStatus.DIPROSES_FARMASI);
