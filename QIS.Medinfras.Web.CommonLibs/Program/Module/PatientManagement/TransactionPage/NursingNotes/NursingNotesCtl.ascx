<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="NursingNotesCtl.ascx.cs" Inherits="QIS.Medinfras.Web.CommonLibs.Program.NursingNotesCtl" %>
...
<script type="text/javascript" id="dxss_chiefcomplaintentryctl">
    $(function () {
        ...
        //#region NoteMode
        var subjectiveText = $('#<%=txtSubjectiveText.ClientID %>').val();
        var objectiveText = $('#<%=txtObjectiveText.ClientID %>').val();
        var assessmentText = $('#<%=txtAssessmentText.ClientID %>').val();
        var planningText = $('#<%=txtPlanningText.ClientID %>').val();
        var instructionText = $('#<%=txtInstructionText.ClientID %>').val();
        var noteMode = $('#<%=rblNoteMode.ClientID %> input').val();
        if (subjectiveText != "" || objectiveText != "" || assessmentText != "" || planningText != "" || instructionText != "") {
            $('#<%=rblNoteMode.ClientID %>').find("input[value='2']").attr("checked", "checked");
            $('#trSOAPINote').removeAttr('style');
            $('#tdCopyButton').removeAttr('style');
            $('#trFreeTextNote').attr('style', 'display:none');
            $('#trIsPanicRangeReporting').attr('style', 'display:none');
        }
        else {
            $('#<%=rblNoteMode.ClientID %>').find("input[value='1']").attr("checked", "checked");
            $('#trSOAPINote').attr('style', 'display:none');
            $('#tdCopyButton').attr('style', 'display:none');
            $('#trFreeTextNote').removeAttr('style');
            $('#trIsPanicRangeReporting').removeAttr('style');
        }
        //#endregion
    });
    ...
    $('#<%=btnCopyNote.ClientID %>').live('click', function () {
        var filterExpression = "VisitID = " + $('#<%=hdnVisitID.ClientID %>').val() + " AND GCPatientNoteType IN ('X011^011','X011^012','X011^016') AND SubjectiveText IS NOT NULL AND GCParamedicMasterType NOT IN ('X019^001')";
    ...
    $('#<%=rblNoteMode.ClientID %> input').change(function () {
        if ($(this).val() == '1') {
            ...
            $('#tdCopyButton').attr('style', 'display:none');
            $('#trFreeTextNote').removeAttr('style');
            $('#trNeedNotification').removeAttr('style');
            $('#trIsPanicRangeReporting').removeAttr('style');
        }
        else {
            $('#trSOAPINote').removeAttr('style');
            $('#tdCopyButton').removeAttr('style');
            $('#trFreeTextNote').attr('style', 'display:none');
            $('#trNeedNotification').attr('style', 'display:none');
            $('#trIsPanicRangeReporting').attr('style', 'display:none');
        }
    });
</script>
        ...
        <tr>
            <td style="padding: 5px; vertical-align: top;">
                <table class="tblEntryContent" style="width: 100%">
                    ...
                    <tr id="trIsPanicRangeReporting">
                        <td>
                           <asp:CheckBox ID="chkIsPanicRangeReporting" runat="server" Checked = "false"/> <span  style="color:Red"><%:GetLabel("Pelaporan Nilai Kritis")%></span>
                        </td>
                        <td colspan="2">
                        </td>
                    </tr>
