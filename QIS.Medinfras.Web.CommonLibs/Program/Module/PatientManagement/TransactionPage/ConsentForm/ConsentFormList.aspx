<%@ Page Title="" Language="C#" MasterPageFile="~/libs/MasterPage/PatientPage/MPBasePatientPageList.master"
    AutoEventWireup="true" CodeBehind="ConsentFormList.aspx.cs" Inherits="QIS.Medinfras.Web.CommonLibs.Program.ConsentFormList" %>
    ...
    <script type="text/javascript" id="dxss_erphysicalexamlist">
        $(function () {
            ...
            $('.btnSignature').live('click', function () {
                ...
                if (paramedicID == $('#<%=hdnCurrentParamedicID.ClientID %>').val()) {
                    var data = (recordID + "|" + signatureName + "|" + recordDate + "|" + recordTime + "|" + "1" + "|" + "03");
                    var url = ResolveUrl("~/Libs/Controls/DigitalSignatureCtl1.ascx");
                    openUserControlPopup(url, data, 'Tanda Tangan Digital', 400, 500);
                }
                else {
                    displayErrorMessageBox("Informed Consent", "Tanda tangan harus dilakukan oleh Dokter yang memberikan informed consent");
                }
            });
            ...
            $('.btnSignature2').live('click', function () {
                ...
                if (paramedicID == $('#<%=hdnCurrentParamedicID.ClientID %>').val()) {
                    var data = (recordID + "|" + signatureName + "|" + recordDate + "|" + recordTime + "|" + "2" + "|" + "03");
                    var url = ResolveUrl("~/Libs/Controls/DigitalSignatureCtl1.ascx");
                    openUserControlPopup(url, data, 'Tanda Tangan Digital', 400, 500);
                }
                else {
                    displayErrorMessageBox("Informed Consent", "Tanda tangan penerima informasi dan saksi harus dilakukan dengan user ID Dokter yang memberikan informed consent");
                }
            });
            ...
            $('.btnSignature3').live('click', function () {
                ...
                if (paramedicID == $('#<%=hdnCurrentParamedicID.ClientID %>').val()) {
                    var data = (recordID + "|" + signatureName + "|" + recordDate + "|" + recordTime + "|" + "3" + "|" + "03");
                    var url = ResolveUrl("~/Libs/Controls/DigitalSignatureCtl1.ascx");
                    openUserControlPopup(url, data, 'Tanda Tangan Digital', 400, 500);
                }
                else {
            ...
            $('.btnSignature4').live('click', function () {
                ...
                if (paramedicID == $('#<%=hdnCurrentParamedicID.ClientID %>').val()) {
                    var data = (recordID + "|" + signatureName + "|" + recordDate + "|" + recordTime + "|" + "4" + "|" + "03");
                    var url = ResolveUrl("~/Libs/Controls/DigitalSignatureCtl1.ascx");
                    openUserControlPopup(url, data, 'Tanda Tangan Digital', 400, 500);
                }
                else {
        ...
        $('.imgEditRecord.imgLink').live('click', function (evt) {
            ...
            if ($('#<%=hdnCurrentParamedicID.ClientID %>').val() == paramedicID) {
                var url = ResolveUrl("~/Libs/Program/Module/PatientManagement/TransactionPage/ConsentForm/ConsentFormCtl1.ascx");
        ...
        $('.imgViewRecord.imgLink').live('click', function (evt) {
            var recordID = $(this).attr('recordID');
            var url = ResolveUrl("~/Libs/Program/Module/PatientManagement/TransactionPage/ConsentForm/ViewConsentFormCtl1.ascx");
            var param = $('#<%=hdnConsentFormGroup.ClientID %>').val() + "|" + recordID;
            openUserControlPopup(url, param, "Informed Consent", 750, 500);
        });
        ...
        function addRecord() {
            ...
            if (allow) {              
                ...
                openUserControlPopup(url, param, "Informed Consent", 700, 500);
            }
        }
        //#endregion
                                    ...
                                    <asp:Panel runat="server" ID="panFormList" CssClass="pnlContainerGridPatientPage">
                                        <asp:GridView ID="grdFormList" runat="server" CssClass="grdSelected grdPatientPage" AutoGenerateColumns="false" ShowHeaderWhenEmpty="true" 
                                            ...
                                            <Columns>
                                                ...
                                                <asp:BoundField DataField="StandardCodeName" HeaderText="Form" HeaderStyle-CssClass="gridColumnText" ItemStyle-CssClass="gridColumnText" />
                                            </Columns>
                                            <EmptyDataTemplate>
                                                <%=GetLabel("Tidak ada jenis informed consent yang bisa digunakan")%>
                                            </EmptyDataTemplate>
                                        </asp:GridView>
                                    </asp:Panel>
                                </dx:PanelContent>
                                    ...
                                    <asp:GridView ID="grdView" runat="server" CssClass="grdSelected grdPatientPage" AutoGenerateColumns="false" ShowHeaderWhenEmpty="true" EmptyDataRowStyle-CssClass="trEmpty"
                                        ...
                                        <Columns>
                                            <asp:TemplateField HeaderStyle-HorizontalAlign="center" HeaderStyle-Width="60px" ItemStyle-Width="20px">
                                                <HeaderTemplate>
                                                    <img class="imgAdd imgLink" title='<%=GetLabel("+ Consent Form")%>' src='<%# ResolveUrl("~/Libs/Images/Button/plus.png")%>'
                                                        alt="" />
                                                </HeaderTemplate>
                                                <ItemTemplate>
                                                        ...
                                                        <tr>
                                                            <td>
                                                                <img class="imgEditRecord imgLink" title='<%=GetLabel("Edit")%>' src='<%# ResolveUrl("~/Libs/Images/Button/edit.png")%>'
                                                                    alt="" recordID = "<%#:Eval("ConsentFormID") %>" paramedicID = '<%#:Eval("ParamedicID") %>' hasSignature2 = '<%#:Eval("cfIsHasSignature2") %>'  />
                                                            </td>
                                                            <td style="width: 1px">
                                                                &nbsp;
                                                            </td>
                                                            <td>
                                                                <img class="imgDeleteRecord imgLink" title='<%=GetLabel("Delete")%>' src='<%# ResolveUrl("~/Libs/Images/Button/delete.png")%>'
                                                                    alt="" recordID = "<%#:Eval("ConsentFormID") %>" formType = "<%#:Eval("ConsentFormType") %>" paramedicID = '<%#:Eval("ParamedicID") %>'   />
                                                            </td>
                                                            <td style="width: 1px">
                                                                &nbsp;
                                                            </td>
                                                            <td>
                                                                <img class="imgViewRecord imgLink" title='<%=GetLabel("Lihat Form Edukasi")%>' src='<%# ResolveUrl("~/Libs/Images/Button/search.png")%>'
                                                                    alt="" recordID = "<%#:Eval("ConsentFormID") %>" paramedicID = '<%#:Eval("ParamedicID") %>'  />
                                                            </td>
                                                        </tr>
                                                    </table>
                                                </ItemTemplate>
                                            </asp:TemplateField>
                                            <asp:BoundField DataField="ConsentFormID" HeaderStyle-CssClass="keyField" ItemStyle-CssClass="keyField" />
                                            <asp:TemplateField HeaderStyle-HorizontalAlign="Left" HeaderStyle-Width="100px">
                                                ...
                                                <ItemTemplate>
                                                    <div>
                                                        <%#:Eval("cfConsentFormDate") %>
                                                        <br />
                                                        <%#:Eval("ConsentFormTime") %>
                                                    </div>
                                                </ItemTemplate>
                                            </asp:TemplateField>
                                            <asp:BoundField DataField="ConsentFormType" HeaderText="Consent Form" HeaderStyle-HorizontalAlign="Left"
                                                HeaderStyle-Width="160px" ItemStyle-HorizontalAlign="Left" />
                                            <asp:TemplateField HeaderText="Pemberi Informasi" HeaderStyle-HorizontalAlign="Left" ItemStyle-HorizontalAlign="Left"
                                                HeaderStyle-Width="150px">
                                                <ItemTemplate>
                                                    <div>
                                                        <b><label id="lblParamedicName" class='<%# Eval("cfIsHasSignature1").ToString() == "True" ? "lblLink lblParamedicName": "lblNormal" %>'
                                                        recordID = '<%#:Eval("ConsentFormID") %>' recordDate = '<%#:Eval("cfConsentFormDate") %>' recordTime = '<%#:Eval("ConsentFormTime") %>' signatureName = '<%#:Eval("ParamedicName") %>' signature1 = '<%#:Eval("SignatureID1Type1") %>'>
                                                            <%#:Eval("ParamedicName") %></label></b>                                            
                                                    </div>
                                                    <div id="divParamedicSignature" runat="server" style='margin-top: 5px; text-align: left'>
                                                        <input type="button" id="btnSignature" runat="server" class="btnSignature" value="Ttd" title="Tanda Tangan" recordID = '<%#:Eval("ConsentFormID") %>' recordDate = '<%#:Eval("cfConsentFormDate") %>' recordTime = '<%#:Eval("ConsentFormTime") %>' paramedicID = '<%#:Eval("ParamedicID") %>' signatureName = '<%#:Eval("ParamedicName") %>'
                                            ...
                                            <asp:TemplateField HeaderText="Penerima Informasi" HeaderStyle-HorizontalAlign="Left" ItemStyle-HorizontalAlign="Left"
                                                ...
                                                <ItemTemplate>
                                                    <div>
                                                        ...
                                                        recordID = '<%#:Eval("ConsentFormID") %>' recordDate = '<%#:Eval("cfConsentFormDate") %>' recordTime = '<%#:Eval("ConsentFormTime") %>' signatureName = '<%#:Eval("SignatureName2") %>' signature2 = '<%#:Eval("SignatureID2Type1") %>'>
                                                            <%#:Eval("SignatureName2") %></label></b>                                            
                                                    </div>
                                                    <div id="divSignature2" runat="server" style='margin-top: 5px; text-align: left'>
                                                        <input type="button" id="btnSignature2" runat="server" class="btnSignature2" value="Ttd" title="Tanda Tangan" recordID = '<%#:Eval("ConsentFormID") %>' recordDate = '<%#:Eval("cfConsentFormDate") %>' recordTime = '<%#:Eval("ConsentFormTime") %>' paramedicID = '<%#:Eval("ParamedicID") %>' signatureName = '<%#:Eval("SignatureName2") %>'
                                                        style='<%# Eval("cfIsHasSignature2").ToString() == "True" ? "display:none;": "height: 25px; width: 60px; background-color: Red; color: White;" %>'  />
                                                    </div>
                                                </ItemTemplate>
                                            </asp:TemplateField>
                                            <asp:TemplateField HeaderText="Saksi-1" HeaderStyle-HorizontalAlign="Left" ItemStyle-HorizontalAlign="Left"
                                                HeaderStyle-Width="150px">
                                                <ItemTemplate>
                                                    <div>
                                                        <b><label id="lblSignatureName3" class='<%# Eval("cfIsHasSignature3").ToString() == "True" ? "lblLink lblSignatureName3": "lblNormal" %>'
                                                        recordID = '<%#:Eval("ConsentFormID") %>' recordDate = '<%#:Eval("cfConsentFormDate") %>' recordTime = '<%#:Eval("ConsentFormTime") %>' signatureName = '<%#:Eval("SignatureName3") %>' signature3 = '<%#:Eval("SignatureID3Type1") %>'>
                                                            <%#:Eval("SignatureName3") %></label></b>                                            
                                                    </div>
                                                    <div id="divSignature3" runat="server" style='margin-top: 5px; text-align: left'>
                                                        <input type="button" id="btnSignature3" runat="server" class="btnSignature3" value="Ttd" title="Tanda Tangan" recordID = '<%#:Eval("ConsentFormID") %>' recordDate = '<%#:Eval("cfConsentFormDate") %>' recordTime = '<%#:Eval("ConsentFormTime") %>' paramedicID = '<%#:Eval("ParamedicID") %>' signatureName = '<%#:Eval("SignatureName3") %>'
                                                        style='<%# Eval("cfIsHasSignature3").ToString() == "True" ? "display:none;": "height: 25px; width: 60px; background-color: Red; color: White;" %>'  />
                                                    </div>
                                                </ItemTemplate>
                                            </asp:TemplateField>
                                            <asp:TemplateField HeaderText="Saksi-2" HeaderStyle-HorizontalAlign="Left" ItemStyle-HorizontalAlign="Left"
                                                HeaderStyle-Width="150px">
                                                <ItemTemplate>
                                                    <div>
                                                        <b><label id="lblSignatureName4" class='<%# Eval("cfIsHasSignature4").ToString() == "True" ? "lblLink lblSignatureName3": "lblNormal" %>'
                                                        recordID = '<%#:Eval("ConsentFormID") %>' recordDate = '<%#:Eval("cfConsentFormDate") %>' recordTime = '<%#:Eval("ConsentFormTime") %>' signatureName = '<%#:Eval("SignatureName4") %>' signature4 = '<%#:Eval("SignatureID4Type1") %>'>
                                                            <%#:Eval("SignatureName4") %></label></b>                                            
                                                    </div>
                                                    <div id="divSignature4" runat="server" style='margin-top: 5px; text-align: left'>
                                                        <input type="button" id="btnSignature4" runat="server" class="btnSignature4" value="Ttd" title="Tanda Tangan" recordID = '<%#:Eval("ConsentFormID") %>' recordDate = '<%#:Eval("cfConsentFormDate") %>' recordTime = '<%#:Eval("ConsentFormTime") %>' paramedicID = '<%#:Eval("ParamedicID") %>' signatureName = '<%#:Eval("SignatureName4") %>'
                                        ...
                                        <EmptyDataTemplate>
                                            <%=GetLabel("Tidak ada dokumentasi pemberian informed consent") %>
                                            <br />
                                            <span class="lblLink" id="lblAddRecord">
                                                <%= GetLabel("+ Informed Consent")%></span>
                                        </EmptyDataTemplate>
                                    </asp:GridView>
                                </asp:Panel>
                            </dx:PanelContent>
