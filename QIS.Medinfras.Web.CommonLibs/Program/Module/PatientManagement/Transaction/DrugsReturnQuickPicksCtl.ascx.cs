...
{
    public partial class DrugsReturnQuickPicksCtl : BaseEntryPopupCtl
    {
        ...
        protected void cboDrugMSReturnLocation_Callback(object sender, DevExpress.Web.ASPxClasses.CallbackEventArgsBase e)
        {
            BindCboLocation();
        }
            ...
            if (Convert.ToInt32(hdnLocationID.Value) > 0)
            {
                ...
                if (loc.IsHeader)
                {
                    ...
                    if (lstLocationByUser.Count > 0)
                    {
                        ...
                        Methods.SetComboBoxField<Location>(cboDrugMSReturnLocation, lstLocationSelected, "LocationName", "LocationID");
                    }
                    else
                    {
                        lstLocation = BusinessLayer.GetLocationList(filterLocation);
                        Methods.SetComboBoxField<Location>(cboDrugMSReturnLocation, lstLocation, "LocationName", "LocationID");
                    }

                    cboDrugMSReturnLocation.SelectedIndex = 0;
                }
                else
                {
        ...
        private string GetFilterExpressionChargeDt()
        {
            ...
            filterExpression += string.Format(" AND LocationID = {0} AND GCItemType IN ('{1}','{2}') AND IsDeleted = 0 AND GCTransactionStatus <> '{3}'", cboDrugMSReturnLocation.Value, Constant.ItemGroupMaster.DRUGS, Constant.ItemGroupMaster.SUPPLIES, Constant.TransactionStatus.VOID);
        ...
        protected override bool OnSaveAddRecord(ref string errMessage, ref string retval)
        {
            ...
            try
            {
                ...
                string[] lstSelectedMember = hdnSelectedMember.Value.Split(','); //ItemID
                ...
                if (isAllowSaveDt)
                {
                    ...
                    foreach (String itemID in lstSelectedMember)
                    {
                        ...
                        decimal grossLineAmount = (patientChargesDt.ChargedQuantity * -1) * price;
                        ...
                        patientChargesDt.LocationID = Convert.ToInt32(cboDrugMSReturnLocation.Value);
