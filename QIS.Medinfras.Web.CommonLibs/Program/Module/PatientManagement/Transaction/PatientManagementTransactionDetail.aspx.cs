...
{
    public partial class PatientManagementTransactionDetail : BasePageTrxPatientManagement
    {
        ...
        protected override void InitializeDataControl()
        {
            if (Page.Request.QueryString.Count > 0)
            {
                ...
                List<SettingParameterDt> lstSettingParameter = BusinessLayer.GetSettingParameterDtList(string.Format("HealthcareID = '{0}' AND ParameterCode IN ('{1}','{2}','{3}','{4}')",
                ...
                string radiotheraphyID = lstSettingParameter.FirstOrDefault(p => p.ParameterCode == Constant.SettingParameter.RT0001).ParameterValue;
                string otID = lstSettingParameter.FirstOrDefault(p => p.ParameterCode == Constant.SettingParameter.MD_SERVICE_UNIT_OPERATING_THEATRE).ParameterValue;

                string filterHSU = string.Empty;
                if (!string.IsNullOrEmpty(otID))
                {
                    filterHSU = string.Format("HealthcareID = '{0}' AND (ServiceUnitID IN ({1},{2}) OR HealthcareServiceUnitID = {3}) AND IsDeleted = 0", AppSession.UserLogin.HealthcareID, laboratoryID, imagingID, otID);
                }
                else {
                    filterHSU = string.Format("HealthcareID = '{0}' AND (ServiceUnitID IN ({1},{2})) AND IsDeleted = 0", AppSession.UserLogin.HealthcareID, laboratoryID, imagingID);

                }
                List<vHealthcareServiceUnitCustom> lstHealthcareServiceUnit = BusinessLayer.GetvHealthcareServiceUnitCustomList(filterHSU);
                ...
                hdnHSUOperatingTheaterID.Value = "0";
                if (!string.IsNullOrEmpty(otID)){
                    hdnHSUOperatingTheaterID.Value = lstHealthcareServiceUnit.FirstOrDefault(p => p.HealthcareServiceUnitID == Convert.ToInt32(otID)).HealthcareServiceUnitID.ToString();
                }
                ...
                if (param[0] == "to")
                {
                    ...
                    if (toHD.HealthcareServiceUnitID == Convert.ToInt32(hdnHSUOperatingTheaterID.Value) && (toHD.ProcedureGroupID != null && toHD.ProcedureGroupID != 0))
                    {
                        hdnIsOperatingRoomOrderProcedure.Value = "1";
                        hdnIsBloodBankOrder.Value = "0";
                    }
                    else {
                        hdnIsBloodBankOrder.Value = toHD.IsBloodBankOrder ? "1" : "0";
                        hdnIsOperatingRoomOrderProcedure.Value = "0";
                    }
                ...
                else
                {
                    ...
                    vHealthcareServiceUnitCustom hsu = BusinessLayer.GetvHealthcareServiceUnitCustomList(string.Format("HealthcareServiceUnitID = {0}", hdnHealthcareServiceUnitID.Value))[0];
                    ...
                    if (entity.PatientBillingID == null && entity.GCTransactionStatus == Constant.TransactionStatus.OPEN)
                    {
                        ...
                        if (entity.TestOrderID != null)
                        {
                            ...
                            if (orderHd.GCTransactionStatus != Constant.TransactionStatus.VOID)
                            {
                                ...
                                if (orderHd.IsBloodBankOrder)
                                {
                                    BloodBankOrderDao bloodBankOrderDao = new BloodBankOrderDao(ctx);
                                    BloodBankOrder bloodBankOrder = BusinessLayer.GetBloodBankOrderList(string.Format("TestOrderID = {0} AND IsDeleted = 0", orderHd.TestOrderID)).FirstOrDefault();
                                    if (bloodBankOrder != null)
                                    {
                                        bloodBankOrder.TestOrderID = Convert.ToInt32(hdnTestOrderID.Value);
                                        bloodBankOrder.GCTransactionStatus = Constant.TransactionStatus.WAIT_FOR_APPROVAL;
                                        bloodBankOrder.GCOrderStatus = Constant.OrderStatus.OPEN;
                                        bloodBankOrder.LastUpdatedBy = AppSession.UserLogin.UserID;
                                        bloodBankOrderDao.Update(bloodBankOrder);
                                    }
                                }
                            }
                        }
