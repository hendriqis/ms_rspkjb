<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="PatientManagementTransactionTestOrderCtl2.ascx.cs"
    Inherits="QIS.Medinfras.Web.CommonLibs.Program.PatientManagementTransactionTestOrderCtl2" %>
...
<input type="hidden" value="false" id="hdnIsPATestLab" runat="server"/>
<input type="hidden" value="0" id="hdnItemTambahanMCUAutoRealisasi" runat="server"/>
            ...
            <dx:PanelContent ID="PanelContent2">
                <fieldset id="fsEntryPopup" style="margin: 0">
                    <asp:Panel runat="server" ID="Panel1" Style="width: 100%; margin-left: auto; margin-right: auto;
                        ...
                        <input type="button" id="btnNewHeader" class="w3-button w3-khaki w3-round-large"
                            value='<%= GetLabel("New")%>' />
                        <input type="button" id="btnSaveHeader" class="w3-button w3-blue w3-round-large"
                            value='<%= GetLabel("Save")%>' <%= IsEditable == true ? "" : "style='display:none'" %> />
                        <input type="button" id="btnProposedHeader" class="w3-button w3-green w3-round-large"
                            value='<%= GetLabel("Proposed")%>' <%= IsEditable == true ? "" : "style='display:none'" %> />
                        <input type="button" id="btnVoidHeader" class="w3-button w3-red w3-round-large" value='<%= GetLabel("Void")%>'
                            <%= IsEditable == true ? "" : "style='display:none'" %> />
                            ...
                            <tr>
                                <td colspan="2">
                                                ...
                                                <tr>
                                                    <td valign="top">
                                                        <table style="width: 100%">
                                                            ...
                                                            <tr id="trExecutorParamedicID" style="display:none;" runat="server">
                                                                 <td class="tdLabel">
                                                                    <label class="lblLink lblMandatory" id="lblExecutorParamedic">
                                                                        <%=GetLabel("Dokter Pelaksana")%></label>
                                                                </td>
                                                                <td colspan="2">
                                                                    <input type="hidden" value="" id="hdnExecutorParamedicID" runat="server" />
                                                                    <table cellpadding="0" cellspacing="0">
                                                                        <colgroup>
                                                                            <col style="width: 120px" />
                                                                            <col style="width: 3px" />
                                                        ...
                                                        <Columns>
                                                            ...
                                                            <asp:TemplateField ItemStyle-Width="80px" ItemStyle-HorizontalAlign="Center">
                                                                <ItemTemplate>
                                                                    ...
                                                                    <input type="hidden" value="<%#:Eval("DetailOrderParamedicID") %>" bindingfield="DetailOrderParamedicID" />
                                    ...
                                    <div style="width: 100%; text-align: center">
                                        <span class="lblLink" id="lblAddData" <%=IsEditable.ToString() == "False" ? "style='display:none'" : "" %> style="margin-right: 200px;">
                                            <%= GetLabel("Add Data")%></span> <span class="lblLink" id="lblQuickPick" <%=IsEditable.ToString() == "False" ? "style='display:none'" : "" %>
...
<script type="text/javascript" id="dxss_patientmanagementtestorderctl">
    ...
    //#region Physician
    function onGetPhysicianExecutorFilterExpression2() {
    ...
    $('#lblExecutorParamedic.lblLink').live('click', function () {
        openSearchDialog('paramedic', onGetPhysicianExecutorFilterExpression2(), function (value) {
    ...
    $('#btnSaveHeader').live('click', function (evt) {
    ...
    $('#btnProposedHeader').live('click', function () {
        if ($('#<%=hdnTestOrderID.ClientID %>').val() != "" && $('#<%=hdnTestOrderID.ClientID %>').val() != "0") {
            ...
            showToast('Warning', 'Pilih nomor order terlebih dahulu.');
        }
    });
    ...
    $('#lblAddData').live('click', function (evt) {
        if (IsValid(evt, 'fsEntryPopup', 'mpEntryPopup')) {
            ...
            $('#<%=txtTestOrderTime.ClientID %>').attr('readonly', 'readonly');
    ...
    $('#lblQuickPick').live('click', function (evt) {
        if (IsValid(evt, 'fsEntryPopup', 'mpEntryPopup')) {
            ...
            var hdnIsPATestLab = $('#<%=hdnIsPATestLab.ClientID %>').val();
            var chkIsMultiVisitScheduleOrder = "False";
            ...
            var id = testOrderID + '|' + paramedicID + '|' + testOrderDate + '|' + testOrderTime + '|' + serviceUnitID + '|' + visitID + '|' + svcUnitID + '|' + realizationDate + '|' + realizationTime + '|' + gcToBePerformed + '|' + notes + '|' + chkChecked + '|' + registrationID + '|' + isLabUnit + '|' + paramedicName + '|' + hdnIsPATestLab + '|' + chkIsMultiVisitScheduleOrder;
    ...
    function onTxtServiceUnitCodeChanged(value) {
        ...
        Methods.getObject('GetvHealthcareServiceUnitList', filterExpression, function (result) {
            if (result != null) {
                ...
                $('#<%=hdnIsLaboratoryUnit.ClientID %>').val(result.IsLaboratoryUnit);
            }
            else {
                ...
                $('#<%=hdnIsLaboratoryUnit.ClientID %>').val('');
            }
        });
    }
    //#endregion
    ...
    $('#<%=grdView.ClientID %> .imgEdit.imgLink').live('click', function () {
        ...
        $('#<%=hdnExecutorParamedicID.ClientID %>').val(entity.DetailOrderParamedicID);
        $('#<%=txtExecutorParamedicCode.ClientID %>').val(entity.DetailOrderParamedicCode);
        $('#<%=txtExecutorParamedicName.ClientID %>').val(entity.DetailOrderParamedicName);
    ...
    function onCbpMainPopupEndCallback(s, e) {
        ...
        var status = $('#<%=txtStatus.ClientID %>').val();
        if (result[0] == 'save') {
            if (result[1] != 'success') {
                if (result[2] != '')
                ...
                $('#containerEntry').hide();
                $('#<%=hdnTestOrderID.ClientID %>').val(result[2]);
                cbpMainPopup.PerformCallback('loadaftersave|qp');
            }
        }
    ...
    function onAfterSaveAddRecordEntryPopup(param) {
        var filterExpression = "TestOrderNo = '" + param + "'";
        Methods.getObject('GetvTestOrderHdList', filterExpression, function (result) {
            if (result != null) {
                $('#<%=hdnTestOrderID.ClientID %>').val(result.TestOrderID);
            }
