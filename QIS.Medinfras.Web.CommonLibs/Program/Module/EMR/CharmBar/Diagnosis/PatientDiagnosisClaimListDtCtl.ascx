<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="PatientDiagnosisClaimListDtCtl.ascx.cs"
    Inherits="QIS.Medinfras.Web.CommonLibs.Program.PatientPage.PatientDiagnosisClaimListDtCtl" %>
...
<script type="text/javascript" id="dxss_PatientDiagnosisClaimListDtCtl">
    $(function () {
    });
    ...
    $(function () {
        setPaging($("#pagingDt"), pageCountDt, function (page) {
            cbpDiagnosisClaimPopupDtView.PerformCallback('changepage|' + page);
        });
    });

    function oncbpDiagnosisClaimPopupDtViewEndCallback(s) {
        ...
        if (param[0] == 'refresh') {
            ...
            if (pageCount > 0)
                $('#<%=grdDiagnosisClaimPopupDtView.ClientID %> tr:eq(1)').click();
        ...
        <tr>
            <td colspan="3">
                <div style="position: relative;">
                    <dxcp:ASPxCallbackPanel ID="cbpDiagnosisClaimPopupDtView" runat="server" Width="99%"
                        ClientInstanceName="cbpDiagnosisClaimPopupDtView" ShowLoadingPanel="false" OnCallback="cbpDiagnosisClaimPopupDtView_Callback">
                        <ClientSideEvents BeginCallback="function(s,e){ $('#containerImgLoadingViewDt').show(); }"
                            EndCallback="function(s,e){ oncbpDiagnosisClaimPopupDtViewEndCallback(s); }" />
                        <PanelCollection>
                            <dx:PanelContent ID="PanelDiagnosisDtViewContent1" runat="server">
                                <asp:Panel runat="server" ID="pnlDtView" CssClass="pnlContainerDtGrid" Style="height: 300px; overflow-y:auto">
                                    <asp:GridView ID="grdDiagnosisClaimPopupDtView" runat="server" CssClass="grdSelected"
                                        ...
                                        <Columns>
                                            ...
                                            <asp:BoundField DataField="cfClaimDiagnosisDate" ItemStyle-HorizontalAlign="Center"
                                                HeaderText="Tgl Diagnosa (Klaim)" HeaderStyle-HorizontalAlign="Center" HeaderStyle-Width="150px" />
                                            <asp:TemplateField HeaderStyle-HorizontalAlign="Left">
                                                ...
                                                <ItemTemplate>
                                                    ...
                                                    <div>
                                                        <b>
                                                            <%#: Eval("DiagnoseTypeClaim")%></b> -
                                                        <%#: Eval("ClaimStatus")%></div>
                                                </ItemTemplate>
                                            </asp:TemplateField>
