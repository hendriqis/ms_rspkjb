<%@ Page Title="" Language="C#" MasterPageFile="~/Libs/MasterPage/MPTrx2.master"
    AutoEventWireup="true" CodeBehind="HcLabLIS.aspx.cs" Inherits="QIS.Medinfras.Web.CommonLibs.Program.HcLabLIS" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="plhEntry" runat="server">
    ...
    <script type="text/javascript">
        ...
        //#region Pharmacogenomic Report
        $('.btnPharmacogenomic').die('click');
        $('.btnPharmacogenomic').live('click', function () {
            var mrn = $(this).attr("mrn");
            var transactionID = $(this).attr("transactionID");
            var sampleID = $(this).attr("sampleID");
            cbpAPICall.PerformCallback(mrn + "|" + transactionID + "|" + sampleID);
        });
        //#endregion
        ...
        $('.imgViewHasil').live('click', function () {
            var transactionID = $(this).closest('tr').find('.keyField').html();
            var id = transactionID;
            var url = ResolveUrl("~/Libs/Program/Module/SystemTool/Bridging/LIS/HcLabLISViewCtl.ascx");
            openUserControlPopup(url, id, 'VIEW HASIL', 550, 500);
        });

        $('.imgSendOrderToLIS').live('click', function () {
            var transactionID = $(this).closest('tr').find('.keyField').html();
            var id = transactionID;
            cbpAPICall.PerformCallback("sendToLIS" + "|" + id);
        });

        $('.lblTransactionNo').live('click', function () {
            var transactionID = $(this).closest('tr').find('.keyField').html();
            var id = transactionID;
            var url = ResolveUrl("~/Libs/Program/Module/SystemTool/Bridging/LIS/HcLabLISDetailCtl.ascx");
        ...
        function onCbpAPICallEndCallback(s) {
            hideLoadingPanel();
            var param = s.cpResult.split('|');
            var paramBridging = s.cpBridging;
            if (paramBridging == "sendToLIS") {
                onRefreshGrid();
            }
            else {
                if (param[0] == 'success') {
                    displayMessageBox('PGx Test Report', 'Proses Penarikan Data Hasil Pharmacogenomic sukses dilakukan (' + param[1] + ')');
                }
                else {
                    displayErrorMessageBox('PGx Test Report', 'Error Message : ' + param[1]);
                }
            }
        }
    </script>
                    ...
                    <asp:Panel runat="server" ID="pnlView" CssClass="pnlContainerGrid">
                        <asp:GridView ID="grdView" runat="server" CssClass="grdSelected" AutoGenerateColumns="false"
                            ...
                            <Columns>
                                ...
                                <asp:BoundField DataField="TransactionID" HeaderText="TrxID" ItemStyle-HorizontalAlign="Center"
                                ...
                                <asp:BoundField DataField="cfSampleID" HeaderText="No. Sample" HeaderStyle-HorizontalAlign="Left"
                                    ItemStyle-HorizontalAlign="Left" HeaderStyle-Width="100px" />
                                <asp:TemplateField HeaderStyle-Width="150px" HeaderText="Sudah Dikirim" ItemStyle-HorizontalAlign="Center">
                                    <ItemTemplate>
                                        <table cellpadding="0" cellspacing="0">
                                            <tr>
                                                <td align='center'>
                                                    <img class="imgHasBeenSent <%# Eval("IsSendToLIS").ToString() == "False" ? "imgDisabled" : "imgLink"%>"
                                                        title='<%=GetLabel("")%>' src='<%# Eval("IsSendToLIS").ToString() == "False" ? ResolveUrl("~/Libs/Images/Button/check_disabled.png") : ResolveUrl("~/Libs/Images/Button/check.png")%>'
                                                        alt="" />
                                                    <br>
                                                    <asp:Label ID="lblStatusKirim" runat="server" />
                                                </td>
                                            </tr>
                                        </table>
                                    </ItemTemplate>
                                ...
                                <asp:TemplateField HeaderStyle-Width="150px" HeaderText="Status Hasil" ItemStyle-HorizontalAlign="Center">
                                    <ItemTemplate>
                                        <img class="imgViewHasil <%# Eval("IsResultExist").ToString() == "False" ? "imgDisabled" : "imgLink"%>"
                                                        title='<%=GetLabel("VIEW MESSAGE FROM LIS")%>' src='<%# Eval("IsResultExist").ToString() == "False" ? ResolveUrl("~/Libs/Images/Button/view_disabled.png") : ResolveUrl("~/Libs/Images/Button/view.png")%>'
                                                        alt="" />
                                        <br>
                                        <asp:Label ID="lblStatusHasil" runat="server" />
                                    </ItemTemplate>
                                </asp:TemplateField>
                                <asp:TemplateField HeaderStyle-Width="50px" HeaderText="Send To LIS" ItemStyle-HorizontalAlign="Center">
                                    <ItemTemplate>
                                        <img class="imgSendOrderToLIS <%# Eval("IsSendToLIS").ToString() == "True" ? "imgDisabled" : "imgLink"%>"
                                            title='<%=GetLabel("")%>' src='<%# Eval("IsSendToLIS").ToString() == "True" ? ResolveUrl("~/Libs/Images/Button/plus_disabled.png") : ResolveUrl("~/Libs/Images/Button/plus.png")%>'
                                            alt="" />
                                        <asp:Label ID="lblBridgingStatus" runat="server" style="display:none"/>
                                    </ItemTemplate>
                                </asp:TemplateField>
                                <asp:TemplateField>
                                    <ItemTemplate>
                                ...
                                <asp:TemplateField>
                                    <ItemTemplate>
                                        <table cellpadding="0" cellspacing="0">
                                            <tr>
                                                <td align='center'>
                                                    <img class="imgViewSudahBayar"
                                                        title='' src='<%# Eval("GCTransactionStatus").ToString() == "X121^005" ? ResolveUrl("~/Libs/Images/Status/coverage_ok.png") : ResolveUrl("")%>'
                                                        alt="" />
                                                    <br>
                                                </td>
                                            </tr>
