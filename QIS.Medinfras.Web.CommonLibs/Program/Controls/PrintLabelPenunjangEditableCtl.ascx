<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="PrintLabelPenunjangEditableCtl.ascx.cs"
    Inherits="QIS.Medinfras.Web.CommonLibs.Program.PrintLabelPenunjangEditableCtl" %>
...
<script type="text/javascript" id="dxss_SuratPenagihan">
    ...
    $('.btnPrint').live('click', function () {
        ...
        if (hdnSelectedID != "") {
            $tr = $(this).closest('tr');
            var itemName = $tr.find('.txtItemName').val();
            $('#<%=hdnItemNameFinal.ClientID %>').val(itemName);
...
<input type="hidden" runat="server" id="hdnItemNameFinal" value="" />
                ...
                <dx:PanelContent ID="PanelContent1" runat="server">
                    <asp:Panel runat="server" ID="pnlDocumentNotesGrdView" Style="width: 100%; margin-left: auto;
                        ...
                        <asp:GridView ID="grdViewLabel" runat="server" CssClass="grdView notAllowSelect"
                            ...
                            <Columns>
                                <asp:TemplateField HeaderStyle-Width="200px" HeaderText="Nama Item" ItemStyle-HorizontalAlign="Left">
                                    <ItemTemplate>
                                        <asp:TextBox ID="txtItemName" class="txtItemName" ReadOnly="true" Style="width: 600px" runat="server" />
                                    </ItemTemplate>
                                </asp:TemplateField>
