<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="LaporanKematianRSSEBCtl.ascx.cs"
    Inherits="QIS.Medinfras.Web.CommonLibs.Program.LaporanKematianRSSEBCtl" %>
...
<script type="text/javascript" id="dxss_medicalsickleave">
    ...
    function getPhysicianBPJSFilterExpression() {
        var filterExpression = "IsRMO = 1 AND IsDeleted = 0";
    ...
    function onReprintPatientPaymentReceiptSuccess() {
        ...
        var registrationID = $('#<%=hdnVisitID.ClientID %>').val();
        var time1 = $('#<%=txtDeathTime1.ClientID %>').val();
        var time2 = $('#<%=txtDeathTime2.ClientID %>').val();
        var diagnoseText = $('#<%=txtDiagnoseText.ClientID %>').val();
        var penyebabKamatian = $('#<%=txtPenyebabKematian.ClientID %>').val();
        var paramedicID = $('#<%=hdnPhysicianID.ClientID %>').val();
        var reportCode = "PM-00709";

        var filterExpression = registrationID + "|" + newDate + "|" + time1 + "|" + time2 + "|" + penyebabKamatian + "|" + diagnoseText + "|" + paramedicID;
        if (reportCode != "") {
        ...
        <table>
            ...
            <tr>
                <td class="tdLabel">
                 ...
                 <td>
                    <asp:TextBox ID="txtPenyebabKematian" Width="300px" CssClass="required" runat="server"
