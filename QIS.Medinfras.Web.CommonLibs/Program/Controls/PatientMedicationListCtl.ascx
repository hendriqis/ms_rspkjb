<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="PatientMedicationListCtl.ascx.cs"
    Inherits="QIS.Medinfras.Web.CommonLibs.Program.PatientMedicationListCtl" %>
...
<script type="text/javascript" id="dxss_medicalsickleave">
    ...
    $('#<%=txtValueDateFrom.ClientID %>').datepicker('option', 'maxDate', '0');
    setDatePicker('<%=txtValueDateFrom.ClientID %>');

    setDatePicker('<%=txtValueDateTo.ClientID %>');
    $('#<%=txtValueDateTo.ClientID %>').datepicker('option', 'maxDate', '0');
    setDatePicker('<%=txtValueDateTo.ClientID %>');
    ...
    function onReprintPatientPaymentReceiptSuccess() {
        ...
        if (cboMedicationRoute.GetValue() != null) {
           var route = cboMedicationRoute.GetValue();
        } else {
           var route = "%%";
        }

        var errMessage = { text: "" };
        var filterExpression = "RegistrationID = " + registrationID + " AND TransactionDate BETWEEN " + "'" + from + "'" + " AND " + "'" + to + "'" + " AND GCRoute LIKE " + "'" + route + "'";
        var reportCode = "IP-00104";
        if (reportCode != "") {
