<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="SuratRencanaInapCtl.ascx.cs"
    Inherits="QIS.Medinfras.Web.CommonLibs.Program.SuratRencanaInapCtl" %>
...
<script type="text/javascript" id="dxss_medicalsickleave">
    ...
    setDatePicker('<%=txtTglSPRI.ClientID %>');
    ...
    $('#<%=txtTglSPRI.ClientID %>').change(function () {
        var date = $(this).val();
    ...
    function onReprintPatientPaymentReceiptSuccess() {
        ...
        var date = $('#<%=txtTglSPRI.ClientID %>').val();
        ...
        var filterExpression = visitID + "|" + newDate;
        var reportCode = "PM-90041";
        if (reportCode != "") {
            ...
            <tr>
                <td class="tdLabel">
                    <label class="lblMandatory">
                        <%=GetLabel("Rencana Inap")%></label>
                </td>
               <td class="tdCustomDate">
                        <asp:TextBox ID="txtTglSPRI" CssClass="txtTglSPRI datepicker" runat="server"
                            Width="125px" />
                        </td>
            </tr>
        </table>
