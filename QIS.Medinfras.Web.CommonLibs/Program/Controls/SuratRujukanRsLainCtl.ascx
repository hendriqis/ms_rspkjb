<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="SuratRujukanRsLainCtl.ascx.cs"
    Inherits="QIS.Medinfras.Web.CommonLibs.Program.SuratRujukanRsLainCtl" %>
...
<script type="text/javascript" id="dxss_medicalconsultation">
    ...
    function onReprintPatientPaymentReceiptSuccess() {
        ...
        var di = $('#<%=txtDiText.ClientID %>').val(); 
        var referralphysicianText = $('#<%=txtReferralPhysicianText.ClientID %>').val();
        var therapyText = $('#<%=txtTheraphyText.ClientID %>').val();
        var pelayananText = $('#<%=txtKebutuhanPelayananText.ClientID %>').val();
        var registrationID = $('#<%=hdnRegistrationID.ClientID %>').val();
        var visitID = $('#<%=hdnVisitID.ClientID %>').val();
        var filterExpression = visitID + "|" + referralphysicianText + "|" + therapyText + "|" + from + "|" + di + "|" + pelayananText;
    ...
    <fieldset id="fsTrxPopup" style="margin: 0">
        ...
        <table>
            ...
            <tr>
                ...
                <td>
                    <asp:TextBox ID="txtDiText" Width="250px" CssClass="required" runat="server" />
                </td>
            </tr>
            <tr>
                <td class="tdLabel">
            ...
            <tr>
                <td class="tdLabel">
                    <label class="lblMandatory">
                        <%=GetLabel("Kebutuhan pelayanan lebih lanjut")%></label>
                </td>
                <td>
                    <asp:TextBox ID="txtKebutuhanPelayananText" Width="250px" CssClass="required" runat="server"
                        TextMode="Multiline" Rows="2" />
                </td>
            </tr>
        </table>
