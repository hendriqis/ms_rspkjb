<%@ Page Title="" Language="C#" MasterPageFile="~/Libs/MasterPage/MPTrx.master" AutoEventWireup="true"
    CodeBehind="ReportPage.aspx.cs" Inherits="QIS.Medinfras.Web.CommonLibs.Program.ReportPage" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="plhCustomButtonToolbar" runat="server">
    ...
    <li id="btnReportPageExport" crudmode="R" runat="server">
        <img src='<%=ResolveUrl("~/Libs/Images/Icon/tbexcel.png")%>' alt="" /><br style="clear: both" />
        <div>
            <%=GetLabel("Export")%></div>
    </li>
    ...
    <li id="btnReportPageRAWExportStaging" crudmode="R" runat="server" style="display:none">
        <img src='<%=ResolveUrl("~/Libs/Images/Icon/tbrawexcel.png")%>' alt="" /><br style="clear: both" />
        <div>
            <%=GetLabel("RAW Excel (Staging)")%></div>
    </li>
</asp:Content>
<asp:Content ID="Content3" ContentPlaceHolderID="plhEntry" runat="server">
        ...
        $(function () {
            $('.treeNode').click(function () {
                ...
                if (!isNaN(id)) {
                    ...
                    var filterExpression = "ReportCode = '" + $('#<%=hdnReportCode.ClientID %>').val() + "'";
                    Methods.getObject('GetReportMasterList', filterExpression, function (result) {
                        if (result != null) {
                            if (result.IsAllowDownloadExcel) {
                                $('#<%=btnReportPageExport.ClientID %>').show();
                            } else {
                                $('#<%=btnReportPageExport.ClientID %>').hide();
                            }

                            if (result.IsAllowDownloadRawExcel) {
                                $('#<%=btnReportPageRAWExport.ClientID %>').show();
                            } else {
                                $('#<%=btnReportPageRAWExport.ClientID %>').hide();
                            }

                            if (result.IsAllowDownloadRawExcelDBStaging) {
                                $('#<%=btnReportPageRAWExportStaging.ClientID %>').show();
                            } else {
                                $('#<%=btnReportPageRAWExportStaging.ClientID %>').hide();
                            }
                        }
                        else {
                            $('#<%=btnReportPageExport.ClientID %>').hide();
            ...
            $('#<%=btnReportPageRAWExport.ClientID %>').click(function (evt) {
                if (IsValid(evt, 'fsMPEntry', 'mpReport')) {
                    __doPostBack('<%=btnRAWExport.UniqueID%>', '');
                }
            });

            $('#<%=btnReportPageRAWExportStaging.ClientID %>').click(function (evt) {
                if (IsValid(evt, 'fsMPEntry', 'mpReport')) {
                    __doPostBack('<%=btnRAWExportStaging.UniqueID%>', '');
                }
            });
        });
                                                        ...
                                                        <tr id="trReportParameter" runat="server">
                                                            ...
                                                            <td>
                                                                <div id="divTxt" runat="server" visible="false">
                                                                    <asp:TextBox ID="txtValue" Width="200px" runat="server" />
                                                                </div>
                                                                    ...
                                                                    <table cellpadding="0" cellspacing="0">
                                                                        <tr>
                                                                            <td style="width: 160px">
                                                                                <dxe:ASPxComboBox ID="cboValue" Width="200px" runat="server">
    ...
    <div style="display: none;">
        ...
        <asp:Button ID="btnRAWExportStaging" Visible="true" runat="server" OnClick="btnRAWExportStaging_Click"
            Text="RAW Export (Staging)" UseSubmitBehavior="false" OnClientClick="return true;" />
    </div>
</asp:Content>
