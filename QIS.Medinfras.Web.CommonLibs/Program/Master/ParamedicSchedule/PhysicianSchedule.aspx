<%@ Page Title="" Language="C#" MasterPageFile="~/libs/MasterPage/MPList.master"
    AutoEventWireup="true" CodeBehind="PhysicianSchedule.aspx.cs" Inherits="QIS.Medinfras.Web.CommonLibs.Program.PhysicianSchedule" %>
...
<asp:content id="Content1" contentplaceholderid="plhList" runat="server">
    <script type="text/javascript">
        ...
        function GetPhysicianFilter() {
            var filterExpression = "<%:GetPhysicianFilter() %>";
        ...
        $('.lnkSchedulePerDate a').live('click', function () {
            ...
            openUserControlPopup(url, id, 'Jadwal Dokter Per Tanggal', 1300, 500);
        });
        ...
        $('.lnkLeaveScheduleByClinic a').live('click', function () {
            var paramedicID = $('#<%=hdnPhysicianID.ClientID %>').val();
            var healthcareServiceUnitID = $(this).closest('tr').find('.keyField').html();
            var id = paramedicID + '|' + healthcareServiceUnitID;
            var url = ResolveUrl("~/Libs/Program/Master/ParamedicSchedule/PSLeaveScheduleByServiceUnitCtl.ascx");
            openUserControlPopup(url, id, 'Jadwal Cuti Per Klinik', 900, 500);
        });

        $('.lnkJKNNotification a').live('click', function () {
            var url = ResolveUrl("~/Libs/Program/Module/SystemTool/Bridging/Centerback/SendJKNNotificationCtl.ascx");
            var messageType = "00";
            var paramedicID = $('#<%:hdnPhysicianID.ClientID %>').val();
            var healthcareServiceUnitID = $(this).closest('tr').find('.keyField').html();
            var referenceID = paramedicID + ";" + healthcareServiceUnitID;
            var param = messageType + "|" + referenceID;
            openUserControlPopup(url, param, 'Notification : JKN : Jadwal Dokter', 600, 400);
        });
    </script>
    <input type="hidden" value="" id="hdnID" runat="server" />
    <input type="hidden" value="" id="hdnDepartmentID" runat="server" />
    <input type="hidden" value="" id="hdnIsBridgingToMobileJKN" runat="server" />
                        ...
                        <asp:gridview id="grdView" runat="server" cssclass="grdSelected" autogeneratecolumns="false"
                            ...
                            <columns>
                                    ...
                                    <asp:HyperLinkField ItemStyle-HorizontalAlign="Center" HeaderText="Jenis Kunjungan" Text="Setting"
                                        ItemStyle-CssClass="lnkVisitType" HeaderStyle-Width="100px" />
                                    <asp:HyperLinkField ItemStyle-HorizontalAlign="Center" HeaderText="Jadwal" Text="Setting"
                                        ItemStyle-CssClass="lnkSchedule" HeaderStyle-Width="100px" />
                                    <asp:HyperLinkField ItemStyle-HorizontalAlign="Center" HeaderText="Jadwal Per Tanggal" Text="Setting"
                                        ItemStyle-CssClass="lnkSchedulePerDate" HeaderStyle-Width="150px" />
                                    <asp:HyperLinkField ItemStyle-HorizontalAlign="Center" HeaderText="Jadwal Cuti" Text="Setting"
                                        ItemStyle-CssClass="lnkLeaveSchedule" HeaderStyle-Width="100px" />
                                    <asp:HyperLinkField ItemStyle-HorizontalAlign="Center" HeaderText="Jadwal Cuti Per Klinik" Text="Setting"
                                        ItemStyle-CssClass="lnkLeaveScheduleByClinic" HeaderStyle-Width="150px" Visible="false" />
                                    <asp:HyperLinkField ItemStyle-HorizontalAlign="Center" HeaderText="JKN" Text="Notification"
                                        ItemStyle-CssClass="lnkJKNNotification" HeaderStyle-Width="100px" visible=false/>
                                </columns>
                            <emptydatatemplate>
