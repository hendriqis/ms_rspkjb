<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="GLRevenueItemGroupServiceUnitCustomerLineCtl.ascx.cs"
    Inherits="QIS.Medinfras.Web.CommonLibs.Program.GLRevenueItemGroupServiceUnitCustomerLineCtl" %>
...
<script type="text/javascript" id="dxss_GLRevenueItemGroupServiceUnitCustomerLineCtl">
    ...
    function getCustomerLineFilterExpression() {
        var filterExpression = "IsDeleted = 0";
        return filterExpression;
    }

    $('#lblCustomerLine.lblLink').live('click', function () {
        openSearchDialog('customerline', getCustomerLineFilterExpression(), function (value) {
            $('#<%=txtCustomerLineCode.ClientID %>').val(value);
            ontxtCustomerLineCodeChanged(value);
        });
    });

    $('#<%=txtCustomerLineCode.ClientID %>').die('change');
    $('#<%=txtCustomerLineCode.ClientID %>').live('change', function () {
        ontxtCustomerLineCodeChanged($(this).val());
    });

    function ontxtCustomerLineCodeChanged(value) {
        var filterExpression = getCustomerLineFilterExpression() + " AND CustomerLineCode = '" + value + "'";
    ...
    $('#btnEntryPopupSave').click(function (evt) {
        if (IsValid(evt, 'fsEntryPopup', 'mpEntryPopup')) {
            var itemGroupID = $('#<%=hdnItemGroupIDCtlCOA.ClientID %>').val();
            var hsuID = $('#<%=hdnHealthcareServiceUnitID.ClientID %>').val();
            var custLineID = $('#<%=hdnCustomerLineID.ClientID %>').val();

            if (itemGroupID != 0 && itemGroupID != "" && itemGroupID != null) {
                if (hsuID != 0 && hsuID != "" && hsuID != null) {
                    if (custLineID != 0 && custLineID != "" && custLineID != null) {
                        cbpEntryPopupView.PerformCallback('save');
                    } else {
                        displayErrorMessageBox('INFORMASI', "Harap isi isian Customer Line terlebih dahulu.");
                    }
                } else {
...
<div style="height: 450px; overflow-y: auto">
    <input type="hidden" id="hdnItemGroupIDCtlCOA" value="" runat="server" />
                    ...
                    <tr>
                        <td class="tdLabel">
                            <label class="lblNormal">
                                <%=GetLabel("Kode Kelompok Item")%></label>
                        </td>
                        <td colspan="2">
                            <asp:TextBox ID="txtItemGroupCodeCtl" ReadOnly="true" Width="100%" runat="server" />
                        </td>
                    </tr>
                    <tr>
                        <td class="tdLabel">
                            <label class="lblNormal">
                                <%=GetLabel("Nama Kelompok Item")%></label>
                        </td>
                        <td colspan="2">
                            <asp:TextBox ID="txtItemGroupNameCtl" ReadOnly="true" Width="100%" runat="server" />
                        </td>
                    </tr>
                </table>
                    ...
                    <fieldset id="fsEntryPopup" style="margin: 0">
                        <table class="tblEntryDetail" style="width: 100%">
                            ...
                            <tr>
                                ...
                                <td colspan="2">
                                    <input type="hidden" value="" id="hdnCustomerLineID" runat="server" />
                                    <table cellpadding="0" cellspacing="0">
                                        <colgroup>
                                            <col style="width: 120px" />
                                            <col style="width: 3px" />
                                        ...
                                        <tr>
                                            <td>
                                                <asp:TextBox ID="txtCustomerLineCode" Width="100%" runat="server" />
                                            </td>
                                            <td>
                                                &nbsp;
                                            </td>
                                            <td>
                                                <asp:TextBox ID="txtCustomerLineName" ReadOnly="true" Width="100%" runat="server" />
                                            </td>
                                        </tr>
                                    </table>
                                </td>
                                ...
                                <asp:GridView ID="grdView" runat="server" CssClass="grdView notAllowSelect" AutoGenerateColumns="false"
                                    ...
                                    <Columns>
                                        ...
                                        <asp:TemplateField>
                                            <HeaderTemplate>
                                                <div style="text-align: center">
                                                    <%=GetLabel("Customer Line")%>
                                                </div>
                                            </HeaderTemplate>
                                            <ItemTemplate>
                                                <label class="lblNormal" style="font-style: italic; font-size: small;">
                                                    <%#: Eval("CustomerLineCode")%></label><br />
                                                <label class="lblNormal" style="font-size: small;">
                                                    <%#: Eval("CustomerLineName")%></label>
                                            </ItemTemplate>
                                        </asp:TemplateField>
