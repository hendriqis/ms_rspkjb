<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="VerifySurgicalSafetyChecklistCtl1.ascx.cs"
    Inherits="QIS.Medinfras.Web.CommonLibs.Program.VerifySurgicalSafetyChecklistCtl1" %>
...
<script type="text/javascript" id="dxss_testorderdetail1">
    ...
    $(".chkIsProcessALL input").click(function () {

        if (this.checked) {

            $(".chkIsProcessItem input").each(function () {
                this.checked = true;
            });
        } else {
            $(".chkIsProcessItem input").each(function () {
    ...
    function getSelectedItem() {
        var tempSelectedID = "";
        var count = 0;
        $('.grdLookupParamedicTeamView .chkIsProcessItem input:checked').each(function () {
    ...
    function onCbpViewEndCallback(s) {
        ...
        if (param[0] == 'refresh') {
            ...
            $('#<%=grdLookupParamedicTeamView.ClientID %> tr:eq(1)').click();
        }
    ...
    function onAfterProcessPopupEntry(param) {
        if (typeof onRefreshParamedicTeamGrid == 'function')
            onRefreshParamedicTeamGrid();
    }
</script>
...
<input type="hidden" id="hdnLookupVisitID" runat="server" value="0" />
            ...
            <PanelCollection>
                <dx:PanelContent ID="PanelContent6" runat="server">
                    <asp:Panel runat="server" ID="Panel5" CssClass="pnlContainerGrid" Style="height: 400px">
                        <asp:GridView ID="grdLookupParamedicTeamView" runat="server" CssClass="grdLookupParamedicTeamView grdSelected grdPatientPage"
                            AutoGenerateColumns="false" ShowHeaderWhenEmpty="true" EmptyDataRowStyle-CssClass="trEmpty">
                            <Columns>
                                <asp:BoundField DataField="ID" HeaderStyle-CssClass="keyField" ItemStyle-CssClass="keyField" />
                                <asp:TemplateField HeaderStyle-HorizontalAlign="Left" ItemStyle-Width="20px">
                                  <HeaderTemplate>
                                        <asp:CheckBox ID="chkIsProcessALL" runat="server" CssClass="chkIsProcessALL" AutoPostBack="false"  />
                                    </HeaderTemplate>
                                    <ItemTemplate>
                                ...
                                <asp:BoundField HeaderText="Peranan dalam Team"  DataField="ParamedicRole" HeaderStyle-HorizontalAlign="Left" HeaderStyle-Width="150px" />
                            </Columns>
                            <EmptyDataTemplate>
                                <%=GetLabel("Belum ada informasi team pelaksana operasi untuk pasien ini") %>
                            </EmptyDataTemplate>
                        </asp:GridView>
