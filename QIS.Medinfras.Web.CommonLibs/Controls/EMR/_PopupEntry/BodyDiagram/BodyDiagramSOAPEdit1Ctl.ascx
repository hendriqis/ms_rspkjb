<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="BodyDiagramSOAPEdit1Ctl.ascx.cs" 
    Inherits="QIS.Medinfras.Web.CommonLibs.Program.BodyDiagramSOAPEdit1Ctl" %>
...
<script type="text/javascript" id="dxss_bodydiagrameditctl">
    ...
    var memNotesText;
    var ctrA = { val: '<%=ctrA%>' }; // Counter Symbol A
    var ctrB = { val: '<%=ctrB%>' }; // Counter Symbol B
    var ctrC = { val: '<%=ctrC%>' }; // Counter Symbol C
    var ctrD = { val: '<%=ctrD%>' }; // Counter Symbol D
    var ctrE = { val: '<%=ctrE%>' }; // Counter Symbol E
    var ctrF = { val: '<%=ctrF%>' }; // Counter Symbol F
    var ctrG = { val: '<%=ctrG%>' }; // Counter Symbol G
    var ctrH = { val: '<%=ctrH%>' }; // Counter Symbol H
    var ctrI = { val: '<%=ctrI%>' }; // Counter Symbol I
    var ctrJ = { val: '<%=ctrJ%>' }; // Counter Symbol J
    var ctrK = { val: '<%=ctrK%>' }; // Counter Symbol K
    var ctrL = { val: '<%=ctrL%>' }; // Counter Symbol L
    var ctrM = { val: '<%=ctrM%>' }; // Counter Symbol M
    var ctrN = { val: '<%=ctrN%>' }; // Counter Symbol N
    var ctrO = { val: '<%=ctrO%>' }; // Counter Symbol O
    var ctrP = { val: '<%=ctrP%>' }; // Counter Symbol P
    var ctrQ = { val: '<%=ctrQ%>' }; // Counter Symbol Q
    var ctrR = { val: '<%=ctrR%>' }; // Counter Symbol R
    var ctrS = { val: '<%=ctrS%>' }; // Counter Symbol S
    var ctrT = { val: '<%=ctrT%>' }; // Counter Symbol T
    var ctrU = { val: '<%=ctrU%>' }; // Counter Symbol U
    var ctrV = { val: '<%=ctrV%>' }; // Counter Symbol V
    var ctrW = { val: '<%=ctrW%>' }; // Counter Symbol W
    var ctrX = { val: '<%=ctrX%>' }; // Counter Symbol X
    var ctrY = { val: '<%=ctrY%>' }; // Counter Symbol Y
    var ctrZ = { val: '<%=ctrW%>' }; // Counter Symbol Z
    ...
    function openPcNotes(symbolCode, text) {
        ...
        pcNotes.SetHeaderText('Body Diagram - Remarks');
        pcNotes.Show();
...
<dx:ASPxPopupControl id="pcNotes" runat="server" clientinstancename="pcNotes"
    height="150px" HeaderText="Body Diagram - Remarks" CloseAction="None" width="350px" Modal="True" PopupAction="None" 
                    ...
                    <dx:PanelContent ID="PanelContent1" runat="server">
                        <div style="text-align:center;width:100%;">
                            ...
                            <table style="margin-left:auto;margin-right:auto;margin-top:5px;">
                                <tr>
                                    <td><input type="button" id="btnNotesOk" style="width:80px; height:25px" value='<%= GetLabel("OK")%>' /></td>
                                    <td><input type="button" id="btnNotesCancel" style="width:80px; height:25px" value='<%= GetLabel("CANCEL")%>' /></td>
                                </tr>
                            </table> 
                        </div>
                    </dx:PanelContent>
