<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="GridPatientMedicalRecordHaveDiagCtl.ascx.cs"
    Inherits="QIS.Medinfras.Web.CommonLibs.Controls.GridPatientMedicalRecordHaveDiagCtl" %>
<%@ Register Assembly="DevExpress.Web.v11.1, Version=11.1.5.0, Culture=neutral, PublicKeyToken=b88d1754d700e49a"
    Namespace="DevExpress.Web.ASPxCallbackPanel" TagPrefix="dxcp" %>
<%@ Register Assembly="DevExpress.Web.v11.1, Version=11.1.5.0, Culture=neutral, PublicKeyToken=b88d1754d700e49a"
    Namespace="DevExpress.Web.ASPxPanel" TagPrefix="dx" %>
<script type="text/javascript" id="dxss_GridPatientMedicalRecordHaveDiagCtl">
    $('.lvwViewHaveDiag > tbody > tr:gt(0):not(.trEmpty)').live('click', function () {
        if (!isHoverTdExpand) {
            showLoadingPanel();
    ...
    $('.lvwViewHaveDiag tr:gt(0) td.tdExpand').live({
        mouseenter: function () { isHoverTdExpand = true; },
        mouseleave: function () { isHoverTdExpand = false; }
    });

    $('.lvwViewHaveDiag tr:gt(0) td.tdExpand').live('click', function () {
        $tr = $(this).parent().next();
        if (!$tr.is(":visible")) {
    ...
    $(function () {
        ...
        setPaging($("#paging"), pageCountRegOrder, function (page) {
            cbpViewDiag.PerformCallback('changepage|' + page);
        });

        var pageNo = $('#<%=hdnLastPagging2.ClientID %>').val();


        $('#containerDiagnosa').hide();
    });

    function oncbpViewDiagEndCallback(s) {
        hideLoadingPanel();
        var gender = $('.hdnPatientGender').val();
        Methods.checkImageError('imgPatientImage', 'patient', gender);
    ...
    function refreshGrdRegisteredPatientHaveDiagnose() {
        cbpViewDiag.PerformCallback('refresh');
    }

    function changePagePaggingNo2(idx) {
        var idx = idx - 1;
        $('.div2').find('li:eq(' + idx + ')').click();
    }
...
<input type="hidden" runat="server" id="hdnLastPagging2" value="" />
<input type="hidden" runat="server" id="hdnDiagnosisNotOnlyMainDiagnosis" value="" />
<dxcp:ASPxCallbackPanel ID="cbpViewDiag" runat="server" Width="100%" ClientInstanceName="cbpViewDiag"
    ShowLoadingPanel="false" OnCallback="cbpViewDiag_Callback">
    <ClientSideEvents BeginCallback="function(s,e){ showLoadingPanel(); }" EndCallback="function(s,e){ oncbpViewDiagEndCallback(s); }" />
    <PanelCollection>
        <dx:PanelContent ID="PanelContent1" runat="server">
            <asp:Panel runat="server" ID="pnlGridView" Style="width: 100%; margin-left: auto;
                ...
                <asp:ListView runat="server" ID="lvwViewHaveDiag" OnItemDataBound="lvwViewHaveDiag_ItemDataBound">
                    <EmptyDataTemplate>
                        <table id="tblView" runat="server" class="grdCollapsible lvwViewHaveDiag" cellspacing="0"
                            rules="all">
                            <tr>
                                <th style="width: 15px">
                                </th>
