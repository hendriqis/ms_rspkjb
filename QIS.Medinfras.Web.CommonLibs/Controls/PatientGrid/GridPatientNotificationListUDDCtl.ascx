<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="GridPatientNotificationListUDDCtl.ascx.cs" 
    Inherits="QIS.Medinfras.Web.CommonLibs.Controls.GridPatientNotificationListUDDCtl" %>
...
<script type="text/javascript" id="dxss_gridreigsteredpatientctl">
    ...
    $('.lvwView2 tr:gt(0) td.tdExpand').live({
        mouseenter: function () { isHoverTdExpand = true; },
        mouseleave: function () { isHoverTdExpand = false; }
    });

    $('.lvwView2 tr:gt(0) td.tdExpand').live('click', function () {
        $tr = $(this).parent().next();
        if (!$tr.is(":visible")) {
    ...
    //#region Paging
    var pageCountListNotification = parseInt('<%=PageCountListNotification %>');
    $(function () {
        var gender = $('.hdnPatientGender').val();
        Methods.checkImageError('imgPatientImage', 'patient', gender);
        setPaging($("#pagingListNotificationCtl"), pageCountListNotification, function (page) {
    ...
    function onCbpView2EndCallback(s) {
        ...
        if (param[0] == 'refresh') {
            ...
            setPaging($("#pagingListNotificationCtl"), pageCount, function (page) {
                cbpView2.PerformCallback('changepage|' + page);
            });
        }
    }
...
<dxcp:ASPxCallbackPanel ID="cbpView2" runat="server" Width="100%" ClientInstanceName="cbpView2"
    ...
    <PanelCollection>
        <dx:PanelContent ID="PanelContent1" runat="server">
            <asp:Panel runat="server" ID="pnlGridView2" Style="width: 100%; margin-left: auto; margin-right: auto; position: relative;font-size:0.95em;height:365px;overflow-y:scroll;">
                <asp:ListView runat="server" ID="lvwView2" OnItemDataBound="lvwView2_ItemDataBound">
                    <EmptyDataTemplate>
                        <table id="tblView" runat="server" class="grdCollapsible lvwView" cellspacing="0" rules="all" >
                            <tr>
                                ...
                                <th style="width: 100px; text-align: left;"><%=GetLabel("NO. BED")%></th>
                                <th style="width:400px" align="left"><%=GetLabel("DATA PASIEN")%></th>
                                <th style="width:250px" align="left"><%=GetLabel("DATA KUNJUNGAN")%></th>
                                <th style="width:350px" align="left"><%=GetLabel("ALAMAT DAN KONTAK PASIEN")%></th>
                    ...
                    <LayoutTemplate>
                        <table id="tblView" runat="server" class="grdCollapsible lvwView2" cellspacing="0" rules="all" >
                            <tr>
                                <th style="width:15px"></th>
                                <th style="width: 100px; text-align: left;"><%=GetLabel("NO. BED")%></th>
                                <th style="width:400px" align="left"><%=GetLabel("DATA PASIEN")%></th>
                    ...
                    <ItemTemplate>
                        <tr>
                            ...
                            <td class='tdBedStatus<%#:Eval("GenderCodeSuffix")%>' >
                                <div style="font-size:13pt; font-weight:bold"><%#: Eval("BedCode") %></div>
                            </td>
                            <td>
                                ...
                                <div>
                                    <%#: Eval("MedicalNo") %>, <%#: Eval("DateOfBirthInString") %>, <%#: Eval("Sex") %>, <%#: Eval("Religion") %>
                                </div>
                                <div style="font-style:italic"><%#: Eval("BusinessPartner")%></div>
                                <div>
                                    <font color="red"><%#: Eval("cfTextDischargeDateOrPlanDischargeDateWithNotes") %></font>
                                </div>
                            </td>
                            <td>
                                <div><%#: Eval("RegistrationNo") %>
                                <div><%#: Eval("ParamedicName")%></div>
                                <div><%#: Eval("ServiceUnitName")%>, <%#: Eval("ClassName") %></div>
...
<div class="containerPaging">
    <div class="wrapperPaging">
        <div id="pagingListNotificationCtl"></div>
    </div>
</div> 