<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="GridInpatientRegistrationCtl.ascx.cs"
    Inherits="QIS.Medinfras.Web.CommonLibs.Controls.GridInpatientRegistrationCtl" %>
...
<script type="text/javascript" id="dxss_gridreigsteredpatientctl">
    ...
    function getFilterReport() {
        return $('#<%=hdnFilterExpressionReport.ClientID %>').val();
    }
    ...
    <PanelCollection>
        <dx:PanelContent ID="PanelContent1" runat="server">
            <asp:Panel runat="server" ID="pnlGridView" Style="width: 100%; margin-left: auto;
                ...
                <input type="hidden" value="" id="hdnFilterExpressionReport" runat="server" />
                <asp:ListView runat="server" ID="lvwView">
                    ...
                    <ItemTemplate>
                        <tr>
                            ...
                            <td>
                                <div>
                                    ...
                                    <label style="font-weight: bold;">
                                        (<%#: Eval("cfInpatientLOS") %>)</label></div>
                                <div class="blink-alert" style='<%# Eval("IsVisitorRestriction").ToString() == "True" ? "display:block": "display:none" %>'>
                                    TIDAK MAU DIKUNJUNGI (RAHASIA)
                                </div>
                            </td>
                            <td>
                                <div>
                                    <span style="font-weight: bold">
                                        <%#: Eval("PatientName") %></span> (<%#: Eval("DateOfBirthInString") %>,
                                    <%#: Eval("Sex") %>,
                                    <%#: Eval("Religion") %>)</div>
                            </td>
                            <td>
                            ...
                            <td align="center">
                                <img class="imgLock <%# Eval("IsHasRegistrationNotes").ToString() == "1" ? "imgDisabled" : "imgLock"%>"
                                 style='<%# Eval("IsHasRegistrationNotes").ToString() == "True" ? "width:25px": "width:25px;display:none" %>'
                                 src='<%= ResolveUrl("~/Libs/Images/Toolbar/registration_notes.png")%>'
