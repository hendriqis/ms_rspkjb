<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="GridPatientSatuSehatSudahIntegrasiListCtl.ascx.cs"
    Inherits="QIS.Medinfras.Web.CommonLibs.Controls.GridPatientSatuSehatSudahIntegrasiListCtl" %>
...
//        }
    ...
    $(function () {
        ...
        setPaging($("#paging2"), pageCountRegOrder2, function (page) {
            cbpViewSudahIntegrasi.PerformCallback('changepage|' + page);
        });
    ...
    $('.lblEncounterID.lblLink').live('click', function () {
        $tr = $(this).closest('tr');
        var registrationID = $tr.find('.hdnKeyField').val();
//        var encounterID = $tr.find('.hdnEncounterID').val();

        var url = ResolveUrl("~/Program/Process/IHS/_Popup/RegistrationSatuSehatBridgingDetailCtl.ascx");
        var id = registrationID;
        openUserControlPopup(url, id, 'Informasi Data Satu Sehat', 1200, 500);
    });

    $('.chkIsSelectedSudahIntegrasi input').change(function () {
        $('.chkSelectAllSudahIntegrasi input').prop('checked', false);
    });

    $('#chkSelectAllSudahIntegrasi').die('change');
    $('#chkSelectAllSudahIntegrasi').live('change', function () {
        var isChecked = $(this).is(":checked");
        $('.chkIsSelectedSudahIntegrasi input').each(function () {
            $(this).prop('checked', isChecked);
            $(this).change();
        });
    });

    function oncbpViewSudahIntegrasiEndCallback(s) {
        hideLoadingPanel();
        var gender = $('.hdnPatientGender').val();
        Methods.checkImageError('imgPatientImage', 'patient', gender);
    ...
    function refreshGrdSudahIntegrasi() {
        cbpViewSudahIntegrasi.PerformCallback('refresh');
    }
    ...
    function getListCheckedSudahIntegrasi() {
        var result = "";
        if ($('.chkIsSelectedSudahIntegrasi input:checked').length < 1) {
            result = "0|Harap pilih registras yang akan diproses";
        }
        else {
            var param = '';
            var lstBillID = "";
            $('.chkIsSelectedSudahIntegrasi input:checked').each(function () {
    ...
    function onChkSelectedChangeSudahIntegrasi() {
        $('#chkSelectAllSudahIntegrasi').prop('checked', false);
    }
</script>
...
<dxcp:ASPxCallbackPanel ID="cbpViewSudahIntegrasi" runat="server" Width="100%" ClientInstanceName="cbpViewSudahIntegrasi"
    ShowLoadingPanel="false" OnCallback="cbpViewSudahIntegrasi_Callback">
    <ClientSideEvents BeginCallback="function(s,e){ showLoadingPanel(); }" EndCallback="function(s,e){ oncbpViewSudahIntegrasiEndCallback(s); }" />
    <PanelCollection>
                ...
                <asp:ListView runat="server" ID="lvwView" OnItemDataBound="lvwView_ItemDataBound">
                    ...
                    <LayoutTemplate>
                        <table id="tblView" runat="server" class="grdCollapsible lvwView" cellspacing="0"
                            ...
                            <tr>
                                <th style="width: 15px">
                                    <input id="chkSelectAllSudahIntegrasi" type="checkbox" />
                                </th>
                               <th style="width: 400px" align="left">
                                    <%=GetLabel("INFORMASI PASIEN")%>
                                </th>
                    ...
                    <ItemTemplate>
                        <tr>
                            <td align="center">
                                <div style="padding: 3px">
                                    <asp:CheckBox ID="chkIsSelectedSudahIntegrasi" CssClass="chkIsSelectedSudahIntegrasi" runat="server" onclick="onChkSelectedChangeSudahIntegrasi();"/>
                                    <input type="hidden" class="hdnKeyField" value="<%#: Eval("RegistrationID")%>" />
                                    <input type="hidden" class="hdnVisitID" value='<%#: Eval("VisitID") %>' />
                                    <input type="hidden" class="hdnEncounterID" value='<%#: Eval("EncounterID") %>' />
                                </div>
                            </td>
                            <td>
                            ...
                            <td>
                                <label class="lblLink lblEncounterID" id="lblEncounterID">
                                        <%#: Eval("EncounterID")%></label>
                            </td>
                            <td align="center">
