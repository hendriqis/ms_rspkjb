<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="GridServiceOrderCtl.ascx.cs"
    Inherits="QIS.Medinfras.Web.CommonLibs.Controls.GridServiceOrderCtl" %>
...
<script type="text/javascript" id="dxss_gridreigsteredpatientctl">
    $('.lvwViewOrder tr:gt(0):not(.trEmpty)').live('click', function () {
        if (!isHoverTdExpandOrder) {
            ...
            $('#<%=hdnServiceOrderNo.ClientID %>').val($(this).find('.hdnServiceOrderID').val());
...
<input type="hidden" runat="server" id="hdnServiceOrderNo" value="" />
                    ...
                    <ItemTemplate>
                        <tr>
                            ...
                            <td>
                                <div>
                                    <%#: Eval("ServiceOrderNo") %>
                                    <input type="hidden" class="hdnServiceOrderID" value='<%#: Eval("ServiceOrderID") %>' />
                                    <br />
                                    <span style="color: Red">
                                        <%#: Eval("cfTransactionNoDisplay") %>
                                    </span>
                            ...
                            <td>
                                <div>
                                    <%#: Eval("OrderServiceUnitName") %></span>
                                </div>
                            </td>
                            <td>
                                <div>
                            ...
                            <td align="center">
                                <img class="imgPayment" title='<%=GetLabel("Sudah Ada Pembayaran")%>' src=' <%# ResolveUrl("~/Libs/Images/Status/coverage_ok.png") %>'
                                    style='<%# Eval("ChargesGCTransactionStatus").ToString() == "X121^005" ? "width:25px": "width:25px; display:none" %>'
                                    alt="" />
                            </td>
                        </tr>
                        <tr style="display: none" class="trDetail">
                            ...
                            <td>
                                <div>
                                    <div>
                                        <%#: Eval("ServiceOrderNo") %></span></div>
                                    <input type="hidden" class="hdnServiceOrderID" value='<%#: Eval("ServiceOrderID") %>' />
                                    <div style="float: left">
                                        <%#: Eval("ServiceOrderDateInString")%></div>
                                    <div style="margin-left: 100px">
                                        <%#: Eval("ServiceOrderTime")%></div>
                                    <div>
                                    ...
                                    <table cellpadding="0" cellspacing="0">
                                        ...
                                        <tr>
                                            <td style="text-align: right; font-size: 0.9em; font-style: italic">
                                                <%=GetLabel("Preferred Name")%>
                                            </td>
                                            <td>
                                                &nbsp;
                                            </td>
                                            <td>
                                                <%#: HttpUtility.HtmlEncode(("PreferredName"))%>
                                            </td>
                                            <td style="text-align: right; font-size: 0.9em; font-style: italic">
                                                <%=GetLabel("MRN")%>
                                            </td>
