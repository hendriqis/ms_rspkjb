<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="APInvoiceSupplierProcessCtl.ascx.cs"
    Inherits="QIS.Medinfras.Web.Finance.Program.APInvoiceSupplierProcessCtl" %>
    ...
    function getCheckedPurchaseReceive() {
        ...
        $('.chkPurchaseReceive input').each(function () {
            if ($(this).is(':checked')) {
                ...
                if (param == '') {
                    param = '$setData|' + key + '|' + referenceNo + '|' + referenceDate + '|' + taxInvoiceNo + '|' + taxInvoiceDate + '|' + netAmount;
                }
                else {
                    param += '$setData|' + key + '|' + referenceNo + '|' + referenceDate + '|' + taxInvoiceNo + '|' + taxInvoiceDate + '|' + netAmount;
                }
                ...
                $('#<%=hdnSelectedReferenceNo.ClientID %>').val();
            }
        });

        $('#<%=hdnDataSave.ClientID %>').val(param);
        $('#<%=txtTotalAmount.ClientID %>').val(totalNetAmount).trigger('changeValue');
    }

    $('.chkPurchaseReceive input').live('click', function () {
        $('.chkSelectAllPR input').prop('checked', false);
        var totalNetAmount = 0;
        $('.chkPurchaseReceive input').each(function () {
            if ($(this).is(':checked')) {
                var $tr = $(this).closest('tr');
                var netAmount = parseFloat($tr.find('.txtNetAmount').val());
                ...
                <table>
                    ...
                    <tr>
                        <td colspan="2" style="text-align: left">
                            <asp:CheckBox ID="chkFilter" Checked="true" runat="server" />&nbsp;<b><%=GetLabel("Filter POR yang Jatuh Tempo :")%></b>
                        </td>
                    </tr>
                    <tr>
                ...
                <table>
                    <tr>
                        <td class="tdLabel" style="font-style: oblique">
                            <label>
                                <%=GetLabel("Total Selected") %></label>
                        </td>
                        <td align="right">
                            <asp:TextBox ID="txtTotalAmount" ReadOnly="true" Width="90%" CssClass="txtCurrency"
                                runat="server" />
                        </td>
                    </tr>
                </table>
                                    ...
                                    <ItemTemplate>
                                        <tr>
                                            <td align="center">
                                                ...
                                                <input type="hidden" id="hdnChargesAmount" runat="server" value='<%#: Eval("ChargesAmount")%>' />
                                                <input type="hidden" id="hdnStampAmount" runat="server" value='<%#: Eval("StampAmount")%>' />
                                                <input type="hidden" id="hdnDPAmount" runat="server" value='<%#: Eval("DownPaymentAmount")%>' />
                                                <input type="hidden" id="hdnReferenceNo" runat="server" value='<%#: Eval("ReferenceNo")%>' />
                                                <input type="hidden" id="hdnReferenceDate" runat="server" value='<%#: Eval("ReferenceDate")%>' />
                                                <input type="hidden" id="hdnVATAmount" runat="server" value='<%#: Eval("VATAmount")%>' />
                                            </td>
                                            <td>
                                                <div>
                                                    <%#: Eval("PurchaseReceiveNo")%></div>
                                                <div style="font-size:12px; font-weight:550">
                                                    <i>
                                                        <%#: Eval("LocationName")%></i></div>
                                            </td>
                                            <td>
                                            ...
                                            <td align="right">
                                                <%#: Eval("NetTransactionAmount", "{0:N2}")%>
                                            </td>
                                            <td>
                                                <asp:TextBox runat="server" ID="txtNetAmount" CssClass="txtNetAmount number" Width="95%" />
                                            </td>
                                            <td align="center">
                                                <img class="imgWarning <%# Eval("cfIsHasPurchaseReturnCustom").ToString() == "1" ? "imgDisabled" : "imgLink"%>"
                                                    title='<%=GetLabel("Has Return")%>' src=' <%# ResolveUrl("~/Libs/Images/warning.png") %>'
                                                    style='<%# Eval("cfIsHasPurchaseReturnCustom").ToString() == "True" ? "width:25px": "width:25px;display:none" %>'
                                                    alt="" />
                                            </td>
                                        </tr>
                                    </ItemTemplate>
