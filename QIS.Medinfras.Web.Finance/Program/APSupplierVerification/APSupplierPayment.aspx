<%@ Page Language="C#" MasterPageFile="~/libs/MasterPage/MPTrx2.master" AutoEventWireup="true"
    CodeBehind="APSupplierPayment.aspx.cs" Inherits="QIS.Medinfras.Web.Finance.Program.APSupplierPayment" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="plhEntry" runat="server">
    <script type="text/javascript">
        ...
        function onLoad() {
            ...
            var useApprVerif = $('#<%=hdnUsingApprovalVerification.ClientID %>').val();
            if (useApprVerif == "1") {
                $('.infoCtl1').removeAttr("style");
                $('.infoCtl2').removeAttr("style");
                $('.infoCtl3').removeAttr("style");
                $('.imgInfo').removeAttr("style");
            }
            else {
                $('.infoCtl1').attr('style', 'display:none');
                $('.infoCtl2').attr('style', 'display:none');
                $('.infoCtl3').attr('style', 'display:none');
                $('.imgInfo').attr('style', 'display:none');
            }

            $('#<%=btnApprove.ClientID %>').click(function (evt) {
                getCheckedMember();
                if ($('#<%=hdnSelectedMember.ClientID %>').val() == '') {
                    ...
                    if (IsValid(evt, 'fsMPEntry', 'mpEntry')) {
                        ...
                        showToast('Approve Success', "Proses Approve pada No. Pembayaran <b>" + suppPaymentNo + "</b> Sudah Berhasil Dilakukan");
                    }
                }
            });
        ...
        function getCheckedMember() {
            var lstSelectedMember = $('#<%=hdnSelectedMember.ClientID %>').val().split(',');
            var lstSelectedPaymentDate = $('#<%=hdnSelectedMemberPaymentDate.ClientID %>').val().split(',');
            ...
            $('.lvwView .chkIsSelected input').each(function () {
                if ($(this).is(':checked')) {
                    ...
                    var paymentDate = $tr.find('.txtPaymentDate').val();
                    ...
                    if (idx > -1) {
                        ...
                        lstSelectedPaymentDate.splice(idx, 1);
            ...
            $('#<%=hdnSelectedMemberPaymentDate.ClientID %>').val(lstSelectedPaymentDate.join(','));
        ...
        $td = null;
        $('.lblPaymentMethod.lblLink').live('click', function () {
            $td = $(this).parent();
            var filterExpression = "ParentID = 'X178' AND IsDeleted = 0 AND IsActive = 1";
            openSearchDialog('standardcode', filterExpression, function (value) {
                onPaymentMethodChanged(value);
            });
        });

        function onPaymentMethodChanged(value) {
            var $tr = $td.closest('tr');
            var filterExpression = "ParentID = 'X178' AND IsDeleted = 0 AND IsActive = 1 AND StandardCodeID = '" + value + "'";
            Methods.getObject('GetStandardCodeList', filterExpression, function (result) {
                if (result != null) {
                    $td.find('.hdnPaymentMethod').val(result.StandardCodeID);
                    $td.find('.lblPaymentMethod').html(result.StandardCodeName);

                    if (result.StandardCodeID == 'X178^001') {
                        $tr.find('.hdnBankID').val('00');
                        $tr.find('.lblBank').html('');
                        //                        $tr.find('.lblBank').attr('class', 'lblDisabled');
                        $tr.find('.txtBankReferenceNo').attr('readonly', 'readonly');
                        $tr.find('.txtBankReferenceNo').val('');
                    } else {
                        var filterExpression = "IsDeleted = 0 AND (GCBankType = '" + Constant.BankType.BANK_HUTANG + "' OR GCBankType IS NULL)";
                        Methods.getObject('GetBankList', filterExpression, function (result) {
                            if (result != null) {
                                $tr.find('.hdnBankID').val(result.BankID);
                                $tr.find('.lblBank').html(result.BankName);
                            }
                            else {
                                $tr.find('.hdnBankID').val('0');
                                $tr.find('.lblBank').html('');
                            }
                        });
                    }
                    ...
                    $td.find('.hdnPaymentMethod').val('0');
                    $td.find('.lblPaymentMethod').html('');
                }
            });
        }
        //#endregion
        ...
        function onBankChanged(value) {
            var $tr = $(this).closest('tr');
            var filterExpression = "IsDeleted = 0 AND BankID = '" + value + "'";
            Methods.getObject('GetBankList', filterExpression, function (result) {
                if (result != null) {
                    $td.find('.hdnBankID').val(result.BankID);
                    $td.find('.lblBank').html(result.BankName);
                }
                else {
                    $td.find('.hdnBankID').val('0');
                    $td.find('.lblBank').html('');
                }
            });
        }
        //#endregion
    ...
    <input type="hidden" id="hdnSelectedMemberPaymentDate" runat="server" value="" />
                                    ...
                                    <asp:ListView runat="server" ID="lvwView" OnItemDataBound="lvwView_ItemDataBound">
                                        <EmptyDataTemplate>
                                            <table id="tblView" runat="server" class="grdView notAllowSelect" cellspacing="0"
                                                ...
                                                <tr>
                                                    ...
                                                    <th class="infoCtl1" align="center">
                                                        <%=GetLabel("")%>
                                                    </th>
                                                </tr>
                                                <tr class="trEmpty">
                                        ...
                                        <LayoutTemplate>
                                            <table id="tblView" runat="server" class="lvwView grdView notAllowSelect" cellspacing="0"
                                                ...
                                                <tr>
                                                    ...
                                                    <th class="infoCtl2" align="center">
                                                        <%=GetLabel("")%>
                                                    </th>
                                                </tr>
                                        ...
                                        <ItemTemplate>
                                            <tr>
                                                ...
                                                <td align="center">
                                                    <asp:TextBox runat="server" ID="txtPaymentDate" CssClass="txtPaymentDate datepicker"
                                                        Width="80px" />
                                                </td>
                                                <td align="center">
                                                    <input type="hidden" class="hdnPaymentMethod" id="hdnPaymentMethod" runat="server"
                                                        value='0' />
                                                    <label runat="server" id="lblPaymentMethod" class="lblLink lblPaymentMethod">
                                                        <%#: Eval("PaymentMethod") %></label>
                                                </td>
                                                <td align="center">
                                                    <input type="hidden" class="hdnBankID" id="hdnBankID" runat="server" value='0' />
                                                    <label runat="server" id="lblBank" class="lblLink lblBank">
                                                        <%#: Eval("BankName") %></label>
                                                </td>
                                                <td align="center">
                                                ...
                                                <td class="infoCtl3" align="center">
                                                    <img class="imgInfo <%#"imgAPSupplierPaymentInformation"%>" title='<%#: Eval("cfAPSupplierPaymentInfoTitle") %>'
                                                        src='<%# ResolveUrl("~/Libs/Images/Button/info.png")%>' alt="" />
                                                </td>
                                            </tr>
                                        </ItemTemplate>
                                    </asp:ListView>
