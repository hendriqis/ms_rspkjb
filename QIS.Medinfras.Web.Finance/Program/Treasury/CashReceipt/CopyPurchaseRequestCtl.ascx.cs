...
{
    public partial class CopyPurchaseRequestCtl : BaseEntryPopupCtl
    {
        ...
        public override void InitializeDataControl(string param)
        {
            ...
            hdnCOAPermintaanSPKCtl.Value = paramList[5];
        ...
        {
            string filterExpression = string.Format("GCTransactionStatus IN ('{0}','{1}','{2}') AND GLTransactionIDRequest IS NULL AND GLTransactionDtIDRequest IS NULL",
                                                        Constant.TransactionStatus.APPROVED, Constant.TransactionStatus.PROCESSED, Constant.TransactionStatus.CLOSED);

            filterExpression += string.Format(" AND PurchaseDate BETWEEN '{0}' AND '{1}'", Helper.GetDatePickerValue(txtPeriodFrom.Text).ToString(Constant.FormatString.DATE_FORMAT_112), Helper.GetDatePickerValue(txtPeriodTo.Text).ToString(Constant.FormatString.DATE_FORMAT_112));

            List<vPurchaseRequestHd> lstEntity = BusinessLayer.GetvPurchaseRequestHdList(filterExpression, int.MaxValue, 1, "TransactionDate, PurchaseRequestNo");
        ...
        private void ControlToEntity(IDbContext ctx, List<GLTransactionDt> lstEntity)
        {
            ...
            int count = 0;
            List<String> lstSelectedPurchaseRequestID = hdnSelectedPurchaseRequestID.Value.Split(',').ToList();
            ...
            lstSelectedPurchaseRequestID.RemoveAt(0);
            lstSelectedDebitAmount.RemoveAt(0);
            lstSelectedCreditAmount.RemoveAt(0);
            lstSelectedRemarks.RemoveAt(0);

            string filterExpression = string.Format("PurchaseRequestID IN ({0})", hdnSelectedPurchaseRequestID.Value.Substring(1));
            List<vPurchaseRequestHd> lstHd = BusinessLayer.GetvPurchaseRequestHdList(filterExpression, ctx);

            GLTransactionHd treasuryHd = glTransactionHdDao.Get(Convert.ToInt32(hdnGLTransactionIDctl.Value));

            foreach (vPurchaseRequestHd entity in lstHd)
            {
                PurchaseRequestHd purchaseRequestHd = purchaseRequestHdDao.Get(entity.PurchaseRequestID);

                //BusinessPartners bp = businessPartnersDao.Get(0);

                GLTransactionDt entityDt = new GLTransactionDt();
                entityDt.ReferenceNo = purchaseRequestHd.PurchaseRequestNo;
                entityDt.HealthcareID = AppSession.UserLogin.HealthcareID;

                //if (purchaseRequestHd != null && purchaseRequestHd.BusinessPartnerID != 0)
                //{
                //    entityDt.BusinessPartnerID = purchaseRequestHd.BusinessPartnerID;
                //}
                //else
                //{
                ...
                entityDt.GLAccount = hdnCOAPermintaanSPKCtl.Value != null && hdnCOAPermintaanSPKCtl.Value != "" ? Convert.ToInt32(hdnCOAPermintaanSPKCtl.Value) : 0;
                ...
                //                                    purchaseRequestHd.PurchaseRequestNo,
                //                                    entity.BusinessPartnerName,
                //                                    purchaseRequestHd.PurchaseDate.ToString(Constant.FormatString.DATE_FORMAT),
                //                                    remarks);
                ...
                purchaseRequestHd.GLTransactionIDRequest = treasuryHd.GLTransactionID;
                purchaseRequestHd.GLTransactionDtIDRequest = glDtID;
