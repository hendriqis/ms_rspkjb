<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="CopyPurchaseRequestCtl.ascx.cs"
    Inherits="QIS.Medinfras.Web.Finance.Program.CopyPurchaseRequestCtl" %>
...
<script type="text/javascript" id="dxss_CopyPurchaseRequestCtl">
    ...
    $('#chkSelectAllPurchaseRequest').die('change');
    $('#chkSelectAllPurchaseRequest').live('change', function () {
        var isChecked = $(this).is(":checked");
        $('.chkSelectPurchaseRequest').each(function () {
            $chk = $(this).find('input');
            if (!$chk.is(":disabled")) {
    ...
    function getCheckedPurchaseRequest() {
        var lstSelectedPurchaseRequest = $('#<%=hdnSelectedPurchaseRequestID.ClientID %>').val().split(',');
        ...
        $('.chkSelectPurchaseRequest input').each(function () {
            if ($(this).is(':checked')) {
                ...
                var idx = lstSelectedPurchaseRequest.indexOf(key);
                if (idx < 0) {
                    lstSelectedPurchaseRequest.push(key);
                ...
                var idx = lstSelectedPurchaseRequest.indexOf(key);
                if (idx > -1) {
                    lstSelectedPurchaseRequest.splice(idx, 1);
        ...
        $('#<%=hdnSelectedPurchaseRequestID.ClientID %>').val(lstSelectedPurchaseRequest.join(','));
    ...
    function onBeforeSaveRecord(errMessage) {
        ...
        if (accountdt != "" && accountdt != "0") {
            ...
            if ($('#<%=hdnSelectedPurchaseRequestID.ClientID %>').val() == '') {
...
<div style="height: 500px; overflow-y: auto; overflow-x: hidden" id="containerPopup">
    ...
    <input type="hidden" id="hdnCOAPermintaanSPKCtl" runat="server" value="" />
        ...
        <tr>
            <td>
                <label class="lblNormal">
                    <%=GetLabel("Periode Pembelian") %></label>
            </td>
            <td>
                <table width="50%">
                    <colgroup>
                                ...
                                <asp:Panel runat="server" ID="pnlEntryPopupGrdView" Style="width: 100%; margin-left: auto;
                                    ...
                                    <asp:ListView runat="server" ID="lvwView" OnItemDataBound="lvwView_ItemDataBound">
                                            ...
                                            <table id="tblView" runat="server" class="grdView notAllowSelect" cellspacing="0"
                                                ...
                                                <tr>
                                                    <th style="width: 30px" align="center">
                                                        <input id="chkSelectAllPurchaseRequest" type="checkbox" />
                                                    </th>
                                        ...
                                        <ItemTemplate>
                                            <tr>
                                                <td align="center">
                                                    <asp:CheckBox ID="chkSelectPurchaseRequest" runat="server" CssClass="chkSelectPurchaseRequest" />
                                                    <input type="hidden" class="keyField" id="keyField" runat="server" value='<%#: Eval("PurchaseRequestID")%>' />
                                                    <input type="hidden" class="PurchaseRequestNo" id="PurchaseRequestNo" runat="server"
                                                        value='<%#: Eval("PurchaseRequestNo")%>' />
                                                    <input type="hidden" class="TotalDtAmount" id="TotalDtAmount" runat="server" value='<%#: Eval("TotalDtAmount")%>' />
                                                </td>
                                                <td>
