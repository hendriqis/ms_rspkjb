...
{
    public partial class CopyPurchaseReceiveCtl : BaseEntryPopupCtl
    {
        ...
        public override void InitializeDataControl(string param)
        {
            ...
            hdnCOARealisasiSPKCtl.Value = paramList[5];
        ...
        {
            string filterExpression = string.Format("GCTransactionStatus IN ('{0}','{1}','{2}') AND GLTransactionID IS NULL AND GLTransactionDtID IS NULL",
                                                        Constant.TransactionStatus.APPROVED, Constant.TransactionStatus.PROCESSED, Constant.TransactionStatus.CLOSED);

            filterExpression += string.Format(" AND TransactionDate BETWEEN '{0}' AND '{1}'", Helper.GetDatePickerValue(txtPeriodFrom.Text).ToString(Constant.FormatString.DATE_FORMAT_112), Helper.GetDatePickerValue(txtPeriodTo.Text).ToString(Constant.FormatString.DATE_FORMAT_112));

            List<vPurchaseReceiveHd> lstEntity = BusinessLayer.GetvPurchaseReceiveHdList(filterExpression, int.MaxValue, 1, "TransactionDate, PurchaseReceiveNo");
        ...
        private void ControlToEntity(IDbContext ctx, List<GLTransactionDt> lstEntity)
        {
            ...
            int count = 0;
            List<String> lstSelectedPurchaseReceiveID = hdnSelectedPurchaseReceiveID.Value.Split(',').ToList();
            ...
            lstSelectedPurchaseReceiveID.RemoveAt(0);
            lstSelectedDebitAmount.RemoveAt(0);
            lstSelectedCreditAmount.RemoveAt(0);
            lstSelectedRemarks.RemoveAt(0);

            string filterExpression = string.Format("PurchaseReceiveID IN ({0})", hdnSelectedPurchaseReceiveID.Value.Substring(1));
            List<vPurchaseReceiveHd> lstHd = BusinessLayer.GetvPurchaseReceiveHdList(filterExpression, ctx);

            GLTransactionHd treasuryHd = glTransactionHdDao.Get(Convert.ToInt32(hdnGLTransactionIDctl.Value));

            foreach (vPurchaseReceiveHd entity in lstHd)
            {
                PurchaseReceiveHd purchaseReceiveHd = purchaseReceiveHdDao.Get(entity.PurchaseReceiveID);

                BusinessPartners bp = businessPartnersDao.Get(purchaseReceiveHd.BusinessPartnerID);

                GLTransactionDt entityDt = new GLTransactionDt();
                entityDt.ReferenceNo = purchaseReceiveHd.PurchaseReceiveNo;
                entityDt.HealthcareID = AppSession.UserLogin.HealthcareID;

                if (purchaseReceiveHd != null && purchaseReceiveHd.BusinessPartnerID != 0)
                {
                    entityDt.BusinessPartnerID = purchaseReceiveHd.BusinessPartnerID;
                }
                else
                {
                ...
                entityDt.GLAccount = hdnCOARealisasiSPKCtl.Value != null && hdnCOARealisasiSPKCtl.Value != "" ? Convert.ToInt32(hdnCOARealisasiSPKCtl.Value) : 0;
                ...
                entityDt.Remarks = string.Format("{0}|{1}|{2}|{3}",
                                                    purchaseReceiveHd.PurchaseReceiveNo,
                                                    entity.SupplierName,
                                                    purchaseReceiveHd.ReceivedDate.ToString(Constant.FormatString.DATE_FORMAT),
                                                    remarks);
                ...
                purchaseReceiveHd.GLTransactionID = treasuryHd.GLTransactionID;
                purchaseReceiveHd.GLTransactionDtID = glDtID;
