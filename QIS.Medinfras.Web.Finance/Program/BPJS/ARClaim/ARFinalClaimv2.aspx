<%@ Page Language="C#" MasterPageFile="~/libs/MasterPage/MPTrx2.master" AutoEventWireup="true"
    CodeBehind="ARFinalClaimv2.aspx.cs" Inherits="QIS.Medinfras.Web.Finance.Program.ARFinalClaimv2" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="plhEntry" runat="server">
    <script type="text/javascript">
        ...
        $('.chkIsSelected input').live('change', function () {
            ...
            if ($(this).is(':checked')) {
                $tr.find('.txtGrouperCodeFinal').removeAttr('readonly');
                $tr.find('.txtGrouperAmountFinal').removeAttr('readonly');

                cboFeedbackStatus.SetEnabled(true);
            }
            else {
                $tr.find('.txtGrouperCodeFinal').attr('readonly', 'readonly');
                $tr.find('.txtGrouperAmountFinal').attr('readonly', 'readonly');
        ...
        $('#<%=BPJSDocumentUpload.ClientID %>').live('change', function () {
            $('#<%=hdnBPJSUploadedFile.ClientID %>').val('');
            readURL(this);
        });
