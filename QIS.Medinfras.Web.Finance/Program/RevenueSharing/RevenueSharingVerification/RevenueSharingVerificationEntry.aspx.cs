...
{
    public partial class RevenueSharingVerificationEntry : BasePageTrx
    {
        protected int PageCount = 1;

        public override string OnGetMenuCode()
        {
            return Constant.MenuCode.Finance.REVENUE_SHARING_VERIFICATION;
        }
        ...
        protected override void InitializeDataControl()
        {
            ...
            txtVerificationDate.Text = DateTime.Now.ToString(Constant.FormatString.DATE_PICKER_FORMAT);
        ...
        protected override bool OnCustomButtonClick(string type, ref string errMessage, ref string retval)
        {
            ...
            TransRevenueSharingHdDao entityDao = new TransRevenueSharingHdDao(ctx);
            bool result = true;
            try
            {
                string lstTransactionID = hdnSelectedMember.Value.Substring(1);
                string filterExpression = string.Format("RSTransactionID IN ({0})", lstTransactionID);

                List<TransRevenueSharingHd> lstTransRevenue = BusinessLayer.GetTransRevenueSharingHdList(filterExpression, ctx);
                foreach (TransRevenueSharingHd obj in lstTransRevenue)
                {
                    obj.GCTransactionStatus = Constant.TransactionStatus.APPROVED;
                    obj.ApprovedBy = AppSession.UserLogin.UserID;
