<%@ Page Language="C#" MasterPageFile="~/Libs/MasterPage/MPMain.master" AutoEventWireup="true"
    CodeBehind="PatientDischargeList.aspx.cs" Inherits="QIS.Medinfras.Web.EmergencyCare.Program.PatientDischargeList" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="plhMPMain" runat="server">
    <script type="text/javascript">
        $(function () {
            ...
            $('#<%=txtVisitDate.ClientID %>').datepicker('option', 'maxDate', '0');
        ...
        $('.lvwView tr:gt(0):not(.trEmpty)').live('click', function () {
            if (!isHoverTdExpand) {
                showLoadingPanel();
                if ($('#<%=hdnTransactionNo.ClientID %>').val($(this).find('.hdnRegistrationID').val()) != null) {
                    var filterExpression = "RegistrationID = " + $('#<%=hdnTransactionNo.ClientID %>').val();
                    Methods.getObject('GetvConsultVisitList', filterExpression, function (result) {
                        if (result != null) {
                            if (result.BedID == 0) {
                                showToast('Informasi', 'Pasien tidak dapat dipulangkan karena Tempat Tidur Belum Dipilih');
                            }
                            else {
                                if ($('#<%=hdnIsPatientDischarge.ClientID %>').val() == '1') {
                                    ...
                                    openUserControlPopup(url, id, 'Pasien Pulang', 650, 450);
                                }
                            }
                        }
                    });
                ...
                <td style="padding: 5px; vertical-align: top">
                    <fieldset id="fsPatientList">
                        <table class="tblEntryContent" style="width: 60%;">
                            ...
                            <tr>
                                <td class="tdLabel"><label class="lblNormal"><%=GetLabel("Tanggal Kunjungan")%></label></td>
                                <td><asp:TextBox ID="txtVisitDate" Width="120px" runat="server" CssClass="datepicker" /></td>
                            </tr>
                            <tr>
                                <td class="tdLabel">
