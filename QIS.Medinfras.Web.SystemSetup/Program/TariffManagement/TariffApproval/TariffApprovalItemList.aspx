<%@ Page Title="" Language="C#" MasterPageFile="~/Libs/MasterPage/MPListEntry.master" AutoEventWireup="true" 
    CodeBehind="TariffApprovalItemList.aspx.cs" Inherits="QIS.Medinfras.Web.SystemSetup.Program.TariffApprovalItemList" %>
            ...
            if (entity != null) {
                ...
                Methods.getListObject('GetTariffBookDtCostList', 'BookID = ' + bookID + ' AND ItemID = ' + entity.ItemID, function (result) {
                    ...
                    if (entity.IsApproved == 'False') {
                        ...
                        var lstApprovedTariff = entity.ProposedTariff.split(';');
                        $('#ulClassApproved li').each(function () {
                            var classID = $(this).find('.hdnClassID').val();
                            var idx = lstClassID.indexOf(classID);
                            $(this).find('.txtClass').val(lstApprovedTariff[idx]).trigger('changeValue');
                            for (var i = 0; i < result.length; ++i) {
                                if (parseInt(result[i].ClassID) == parseInt(classID)) {
                                    var itemCost = result[i];
                                    $(this).find('.hdnPrevBurden').val(itemCost.PreviousBurden);
                    ...
                    var diff = parseInt($('#<%=txtApprovedBaseTariff.ClientID %>').val()) - parseInt($('#<%=txtBaseTariff.ClientID %>').val());
                    var diffPercentage = diff * 100 / parseInt($('#<%=txtBaseTariff.ClientID %>').val());
        ...
        <tr>
            <td valign="top">
                <table>
                    ...
                    <tr>
                        <td colspan="2"><div class="lblComponent" style="text-align:left; margin-top: 10px;"><%=GetLabel("Proposed Tariff")%></div></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td>
                            <asp:Repeater id="rptClassCareProposed" runat="server">
                                <HeaderTemplate>
                                ...
                                <ItemTemplate>
                                    <li>
                                        ...
                                        <div><asp:TextBox ID="txtClass" runat="server" Width="99%" CssClass="txtCurrency txtClass" /></div>
                    ...
                    <tr>
                        <td colspan="2"><div class="lblComponent" style="text-align:left; margin-top: 10px;"><%=GetLabel("Approved Tariff")%></div></td>
                    </tr>
                    <tr>
                        <td class="tdLabel"><label class="lblNormal"><%=GetLabel("Approved")%></label></td>
                        <td><asp:CheckBox ID="chkIsApproved" runat="server" /> </td>
                    </tr>
                    <tr>
                        <td class="tdLabel"><label class="lblNormal"><%=GetLabel("Base Tariff")%></label></td>
                        <td><asp:TextBox ID="txtApprovedBaseTariff" CssClass="txtCurrency" Width="150px" runat="server" /> </td>
                    </tr>
                    <tr>
                        <td class="tdLabel"><label class="lblNormal"><%=GetLabel("Diff")%> (%)</label></td>
                        <td><asp:TextBox ID="txtDiff" CssClass="number" Width="50px" runat="server" /> </td>
                    </tr>
                    <tr>
                        <td class="tdLabel" valign="top"><label class="lblNormal"><%=GetLabel("Class")%></label></td>
                        <td>
                            <asp:Repeater id="rptClassCareApproved" runat="server">
                                <HeaderTemplate>
