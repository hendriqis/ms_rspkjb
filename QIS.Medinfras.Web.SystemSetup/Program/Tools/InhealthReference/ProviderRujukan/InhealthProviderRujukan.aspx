<%@ Page Title="" Language="C#" MasterPageFile="~/Libs/MasterPage/MPList.master"
    AutoEventWireup="true" CodeBehind="InhealthProviderRujukan.aspx.cs" Inherits="QIS.Medinfras.Web.SystemSetup.Tools.InhealthProviderRujukan" %>
...
<asp:Content ID="Content3" ContentPlaceHolderID="plhCustomButtonToolbarLeft" runat="server">
    <li id="btnImportProviderRujukan" runat="server" crudmode="R">
...
<asp:Content ID="Content1" ContentPlaceHolderID="plhList" runat="server">
    ...
    <script type="text/javascript">
        ...
        //#region Search Provider Rujukan
        $('#<%=btnImportProviderRujukan.ClientID %>').live('click', function (evt) {
//            var isBridgingInhealth = $('#<%=hdnIsBridgingToInhealth.ClientID %>').val();
//            if (isBridgingInhealth == "1") {
//                var keyword = $('#<%=txtParamProvider.ClientID %>').val();
//                SearchInhealthProviderRujukan(keyword);
//            } else {
...
//            }
            ...
            if (isBridgingToInhealth == "1") {
                InhealthService.providerrujukan_API(keyword, function (result) {
                    if (result != null) {
        ...
        function SearchInhealthProviderRujukan(keyword) {
            var token = $('#<%=hdnInhealthToken.ClientID %>').val();
            var kodeprovider = $('#<%=hdnInhealthKodeProvider.ClientID %>').val();
            InhealthService.providerrujukan(token, kodeprovider, keyword, function (result) {
                if (result != null) {
                    showToast('INFORMATION', "Ambil data provider rujukan berhasil.");
                }
            });
            onRefreshControl("");
        }
    ...
    <table class="tblContentArea">
        ...
        <tr>
            <td class="tdLabel">
                <label>
                    <%=GetLabel("Kode / Nama Provider Rujukan")%></label>
            </td>
            <td>
                <asp:TextBox runat="server" ID="txtParamProvider" Width="460px" />
            </td>
        </tr>
        <tr>
            <td colspan="2">
                                ...
                                <asp:Panel runat="server" ID="pnlView" CssClass="pnlContainerGrid">
                                    <asp:GridView ID="grdView" runat="server" CssClass="grdSelected" AutoGenerateColumns="false"
                                        ...
                                        <Columns>
                                            ...
                                            <asp:BoundField DataField="ObjectCode" HeaderText="Kode Provider Rujukan" HeaderStyle-Width="50px" HeaderStyle-HorizontalAlign="Left" />
                                            <asp:BoundField DataField="ObjectName" HeaderText="Nama Provider Rujukan" HeaderStyle-Width="150px" HeaderStyle-HorizontalAlign="Left" />
                                            <asp:BoundField DataField="Others" HeaderText="Lokasi" HeaderStyle-Width="25px" HeaderStyle-HorizontalAlign="Left" />
