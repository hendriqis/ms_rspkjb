<%@ Page Title="" Language="C#" MasterPageFile="~/Libs/MasterPage/MPList.master"
    AutoEventWireup="true" CodeBehind="KabupatenList.aspx.cs" Inherits="QIS.Medinfras.Web.SystemSetup.Tools.KabupatenList" %>
...
<asp:Content ID="Content3" ContentPlaceHolderID="plhCustomButtonToolbarLeft" runat="server">
    <li id="btnImportKabupaten" runat="server" crudmode="R">
...
<asp:Content ID="Content1" ContentPlaceHolderID="plhList" runat="server">
    ...
    <script type="text/javascript">
        ...
        //#region Search Kabupaten
        $('#<%=btnImportKabupaten.ClientID %>').live('click', function (evt) {
            var IsBridgingBPJS = $('#<%=hdnIsBridgingBPJSReference.ClientID %>').val();
            if (IsBridgingBPJS == "1") {
                var keyword = $('#<%=txtParamKodeKabupaten.ClientID %>').val();
               
                SearchBPJSReferenceKabupaten(keyword);
            } else {
                showToast('INFORMATION', 'Status bridging dengan BPJS sedang nonaktif.');
            }
        });

        function SearchBPJSReferenceKabupaten(keyword) {
            if ($('#<%=hdnIsBridgingBPJSVClaimVersion.ClientID %>').val() == Constant.VersionBridgingBPJSVClaim.v1_0) {
                BPJSService.reinsertBPJSReferenceKabupaten(keyword, function (result) {
                    if (result != null) {
                ...
                BPJSService.reinsertBPJSReferenceKabupatenMedinfrasAPI(keyword, function (result) {
                    if (result != null) {
                        showToast('INFORMATION', result);
                        //
                        onCbpViewEndCallback(s);
    ...
    <table class="tblContentArea">
        ...
        <tr>
            ...
            <td>
                <asp:TextBox runat="server" ID="txtParamKodeKabupaten" Width="460px" />
            </td>
        </tr>
        <tr>
            <td colspan="2">
