<%@ Page Title="" Language="C#" MasterPageFile="~/Libs/MasterPage/MPList.master"
    AutoEventWireup="true" CodeBehind="PropinsiList.aspx.cs" Inherits="QIS.Medinfras.Web.SystemSetup.Tools.PropinsiList" %>
...
<asp:Content ID="Content3" ContentPlaceHolderID="plhCustomButtonToolbarLeft" runat="server">
    <li id="btnImportPropinsi" runat="server" crudmode="R">
...
<asp:Content ID="Content1" ContentPlaceHolderID="plhList" runat="server">
    ...
    <script type="text/javascript">
        ...
        //#region Search Propinsi
        $('#<%=btnImportPropinsi.ClientID %>').live('click', function (evt) {
            var IsBridgingBPJS = $('#<%=hdnIsBridgingBPJSReference.ClientID %>').val();
            var keyword = '';
            if (IsBridgingBPJS == "1") {
                SearchBPJSReferencePropinsi(keyword);
            } else {
                showToast('INFORMATION', 'Status bridging dengan BPJS sedang nonaktif.');
            }
        });

        function SearchBPJSReferencePropinsi(keyword) {
            if ($('#<%=hdnIsBridgingBPJSVClaimVersion.ClientID %>').val() == Constant.VersionBridgingBPJSVClaim.v1_0) {

                BPJSService.reinsertBPJSReferencePropinsi(keyword, function (result) {
                    if (result != null) {
                ...
                BPJSService.reinsertBPJSReferencePropinsiMedinfrasAPI(keyword, function (result) {
                    if (result != null) {
                        showToast('INFORMATION', result);
                        onCbpViewEndCallback(s);
                    }
