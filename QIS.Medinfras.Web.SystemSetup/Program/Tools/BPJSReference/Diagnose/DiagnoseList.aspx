<%@ Page Title="" Language="C#" MasterPageFile="~/Libs/MasterPage/MPList.master"
    AutoEventWireup="true" CodeBehind="DiagnoseList.aspx.cs" Inherits="QIS.Medinfras.Web.SystemSetup.Tools.DiagnoseList" %>
...
<asp:Content ID="Content3" ContentPlaceHolderID="plhCustomButtonToolbarLeft" runat="server">
    <li id="btnImportDiagnose" runat="server" crudmode="R">
...
<asp:Content ID="Content1" ContentPlaceHolderID="plhList" runat="server">
    ...
    <script type="text/javascript">
        ...
        //#region Search Diagnose
        $('#<%=btnImportDiagnose.ClientID %>').live('click', function (evt) {
            var IsBridgingBPJS = $('#<%=hdnIsBridgingBPJSReference.ClientID %>').val();
            if (IsBridgingBPJS == "1") {
                var diagnoseSearch = $('#<%=txtParamDiagnose.ClientID %>').val();
                SearchBPJSReferenceDiagnose(diagnoseSearch);
            } else {
                showToast('INFORMATION', 'Status bridging dengan BPJS sedang nonaktif.');
            }
        });

        function SearchBPJSReferenceDiagnose(diagnoseSearch) {
            if ($('#<%=hdnIsBridgingBPJSVClaimVersion.ClientID %>').val() == Constant.VersionBridgingBPJSVClaim.v1_0) {
                BPJSService.reinsertBPJSReferenceDiagnose(diagnoseSearch, function (result) {
                    if (result != null) {
                ...
                BPJSService.reinsertBPJSReferenceDiagnoseMedinfrasAPI(diagnoseSearch, function (result) {
                    if (result != null) {
                        showToast('INFORMATION', result);
                        onCbpViewEndCallback(s);
                    }
    ...
    <table class="tblContentArea">
        ...
        <tr>
            <td class="tdLabel">
                <label>
                    <%=GetLabel("Kode Diagnosa / Nama Diagnosa")%></label>
            </td>
            <td>
                <asp:TextBox runat="server" ID="txtParamDiagnose" Width="460px" />
            </td>
        </tr>
        <tr>
            <td colspan="2">
