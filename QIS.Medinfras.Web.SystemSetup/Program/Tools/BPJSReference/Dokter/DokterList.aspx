<%@ Page Title="" Language="C#" MasterPageFile="~/Libs/MasterPage/MPList.master" AutoEventWireup="true" 
    CodeBehind="DokterList.aspx.cs" Inherits="QIS.Medinfras.Web.SystemSetup.Tools.DokterList" %>
...
<asp:Content ID="Content3" ContentPlaceHolderID="plhCustomButtonToolbarLeft" runat="server">
    <li id="btnImportDokter" runat="server" crudmode="R">
...
<asp:Content ID="Content1" ContentPlaceHolderID="plhList" runat="server">
    ...
    <script type="text/javascript">
        ...
        //#region Search Dokter
        $('#<%=btnImportDokter.ClientID %>').live('click', function (evt) {
            var IsBridgingToBPJS = $('#<%=hdnIsBridgingBPJSReference.ClientID %>').val();
            if (IsBridgingToBPJS == "1") {
                var dokterSearch = $('#<%=txtParamParamedic.ClientID %>').val();
                SearchBPJSReferenceParamedic(dokterSearch);
            } else {
                showToast('INFORMATION', 'Status bridging dengan bpjs sedang nonaktif.');
            }
        });
        function SearchBPJSReferenceParamedic(dokterSearch) {
            if ($('#<%=hdnIsBridgingBPJSVClaimVersion.ClientID %>').val() == Constant.VersionBridgingBPJSVClaim.v1_0) {
                BPJSService.reinsertBPJSReferenceParamedic(dokterSearch, function (result) {
                    if (result != null) {
                ...
                BPJSService.reinsertBPJSReferenceParamedicMedinfrasAPI(dokterSearch, function (result) {
                    if (result != null) {
                        showToast('INFORMATION', result);
                    }
                });
        ...
        //#endregion
            ...
            <table class="tblContentArea">
                ...
                <tr>
                    <td class="tdLabel">
                        <label>
                            <%=GetLabel("Kode Dokter / Nama Dokter")%></label>
             </td>
                    <td>
                        <asp:TextBox runat="server" ID="txtParamParamedic" Width="460px" />
                    </td>
                </tr>
             <tr>
                    <td colspan="2">
