<%@ Page Language="C#" MasterPageFile="~/MasterPage/MPPatientDataPageList.master"
    AutoEventWireup="true" CodeBehind="PatientAppointmentHistoryList.aspx.cs" Inherits="QIS.Medinfras.Web.MedicalRecord.Program.PatientAppointmentHistoryList" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="plhList" runat="server">
    ...
    <script type="text/javascript">
        ...
        function onBeforeRightPanelPrint(code, filterExpression, errMessage) {
            var appointmentID = $('#<%=hdnID.ClientID %>').val();
            if (code == 'PM-00505') {
                    ...
                    <asp:Panel runat="server" ID="pnlView" CssClass="pnlContainerGrid" Style="overflow-y: scroll;">
                        <asp:GridView ID="grdView" runat="server" CssClass="grdSelected" AutoGenerateColumns="false"
                            ...
                            <Columns>
                                ...
                                <asp:BoundField DataField="AppointmentNo" HeaderText="No Reservasi" HeaderStyle-Width="150px"
                                    HeaderStyle-HorizontalAlign="Left" />
                                <asp:TemplateField HeaderStyle-Width="450px" HeaderStyle-HorizontalAlign="Left">
                                    <HeaderTemplate>
                                        <%=GetLabel("Informasi Reservasi")%>
                                    </HeaderTemplate>
                                    <ItemTemplate>
                                        <div style="color: blue">
                                            <%=GetLabel("Masuk : ")%><%#: Eval("StartDateTimeInString")%>,
                                            <%#: Eval("StartTime")%></div>
                                        <div style="color: red">
                                            <%=GetLabel("Pulang : ")%><%#: Eval("StartDateTimeInString")%>
                                            <%#: Eval("EndTime")%></div>
                                        <div>
                            ...
                            <EmptyDataTemplate>
                                <%=GetLabel("No Patient Appointment History To Display")%>
                            </EmptyDataTemplate>
                        </asp:GridView>
                    </asp:Panel>
                </dx:PanelContent>
