<%@ Page Title="" Language="C#" MasterPageFile="~/libs/MasterPage/MPMain.master"
    AutoEventWireup="true" CodeBehind="EncounterList1.aspx.cs" Inherits="QIS.Medinfras.Web.MedicalRecord.Program.EncounterList1" %>
    
<%@ Register Src="~/Libs/Controls/PatientGrid/GridPatientSatuSehatBelumIntegrasiListCtl.ascx" TagName="ctlGrdRegisteredPatient2"
    TagPrefix="uc1" %>
<%@ Register Src="~/Libs/Controls/PatientGrid/GridPatientSatuSehatDalamIntegrasiListCtl.ascx"
    TagName="ctlGrdRegisteredPatient" TagPrefix="uc1" %>
<%@ Register Src="~/Libs/Controls/PatientGrid/GridPatientSatuSehatSudahIntegrasiListCtl.ascx"
...
<asp:Content ID="Content1" ContentPlaceHolderID="plhMPMain" runat="server">
    <script type="text/javascript">
        ...
        $(function () {
            if ($('#<%=hdnLastContentID.ClientID %>').val() == '') {
                $('#containerBelumIntegrasi').show();
                $('#containerDalamIntegrasi').hide();
                $('#containerSudahIntegrasi').hide();
            }
            else {
                var lastPage = $('#<%=hdnLastPage.ClientID %>').val();
                if ($('#<%=hdnLastContentID.ClientID %>').val() == 'containerDalamIntegrasi') {
                    $('#containerBelumIntegrasi').hide();
                    $('#containerDalamIntegrasi').show();
                    $('#containerSudahIntegrasi').hide();
                ...
                else if ($('#<%=hdnLastContentID.ClientID %>').val() == 'containerBelumIntegrasi') {
                    $('#containerBelumIntegrasi').show();
                    $('#containerDalamIntegrasi').hide();
                    $('#containerSudahIntegrasi').hide();
                }
            ...
            $('#btnRefreshDalamIntegrasi').click(function (evt) {
                onRefreshGridDalamIntegrasi();
            });

            $('#btnRefreshBelumIntegrasi').click(function (evt) {
                onRefreshGridBelumIntegrasi();
            });

            $('#btnRefreshSudahIntegrasi').click(function (evt) {
                onRefreshGridSudahIntegrasi();
            });

            //#region Update Status
            $('#btnUpdateStatusDalamIntegrasi').click(function (evt) {
                var lstRegID = getListCheckedDalamIntegrasi();
                if (lstRegID.split('|')[0] == "true") {
                    cbpView.PerformCallback('ready|' + lstRegID.split('|')[1]);
                }
            });

            $('#btnUpdateStatusBelumIntegrasi').click(function (evt) {
                var lstRegID = getListCheckedBelumIntegrasi();
                if (lstRegID.split('|')[0] == "true") {
                    cbpView.PerformCallback('ready|' + lstRegID.split('|')[1]);
                }
            });

            $('#btnUpdateStatusSudahIntegrasi').click(function (evt) {
                var lstRegID = getListCheckedSudahIntegrasi();
                if (lstRegID.split('|')[0] == "true") {
                    cbpView.PerformCallback('ready|' + lstRegID.split('|')[1]);
                }
            });
        ...
        function onCbpViewEndCallback(s) {
            ...
            if (param[0] == 'ready') {
                if (param[1] == 'fail')
                    ...
                    if ($('#<%=hdnLastContentID.ClientID %>').val() == "containerBelumIntegrasi") {
                        onRefreshGridBelumIntegrasi();
                    }
                    else if ($('#<%=hdnLastContentID.ClientID %>').val() == "containerDalamIntegrasi") {
                        onRefreshGridDalamIntegrasi();
                    }
                    else if ($('#<%=hdnLastContentID.ClientID %>').val() == "containerSudahIntegrasi") {
                        onRefreshGridSudahIntegrasi();
                    }
                }
            }
        ...
        function onRefreshGridDalamIntegrasi() {
            if (IsValid(null, 'fsPatientList', 'mpPatientList'))
                refreshGrdDalamIntegrasi();
        }

        function onRefreshGridBelumIntegrasi() {
            if (IsValid(null, 'fsPatientList', 'mpPatientList'))
                refreshGrdBelumIntegrasi();
        }

        function onRefreshGridSudahIntegrasi() {
            if (IsValid(null, 'fsPatientList', 'mpPatientList'))
                refreshGrdSudahIntegrasi();
        }
    ...
    <div style="padding: 15px">
        <div class="containerUlTabPage">
            <ul class="ulTabPage" id="ulTab">
                <li class="selected" contentid="containerBelumIntegrasi">
                    <%=GetLabel("Belum Integrasi")%></li>
                <li contentid="containerDalamIntegrasi">
                    <%=GetLabel("Dalam Integrasi")%></li>
                <li contentid="containerSudahIntegrasi">
                    <%=GetLabel("Sudah Integrasi")%></li>
            </ul>
        </div>
        <div style="padding: 2px; display; none" id="containerDalamIntegrasi" class="containerDalamIntegrasi">
            <div class="pageTitle">
                <%=GetLabel("Registrasi Pasien yang Dalam Integrasi dengan Satu Sehat")%></div>
                ...
                <tr>
                    <td style="padding: 5px; vertical-align: top">
                        <table class="tblEntryContent" style="width: 80%">
                            ...
                            <tr>
                                <td colspan="2">
                                    <table border="0" cellpadding="0" cellspacing="0" style="width: 1000px">
                                        <tr>
                                            <td> <%--chkOnlyAgreeDalamIntegrasi--%>
                                                <asp:CheckBox ID="chkOnlyAgreeDalamIntegrasi" runat="server" Checked="false" /><%:GetLabel("Tampilkan hanya yang bersedia dikirimkan data kunjungan ke Satu Sehat")%>
                                            </td>
                                        </tr>
                                    </table>
                                </td>
                            ...
                            <tr>
                                <td>
                                    <input type="button" id="btnUpdateStatusDalamIntegrasi" value="Kirim Ulang ke Satu Sehat" class="btnUpdateStatusDalamIntegrasi w3-button w3-teal w3-border w3-border-blue w3-round-large" />
                                </td>
                                <td>
                                    <table cellpadding="0" cellspacing="0">
                                        <colgroup>
                                        ...
                                        <tr>
                                            <td>
                                                <input type="button" id="btnRefreshDalamIntegrasi" value="R e f r e s h" class="btnRefreshDalamIntegrasi w3-button w3-blue w3-border w3-border-blue w3-round-large" />
                                            </td>
                                        </tr>
                                    </table>
                                </td>
        ...
        <div style="padding: 2px;" id="containerBelumIntegrasi" class="containerBelumIntegrasi">
            <div class="pageTitle">
                <%=GetLabel("Registrasi Pasien yang Belum Integrasi dengan Satu Sehat")%></div>
            <table class="tblContentArea">
                <colgroup>
                    <col style="width: 60%" />
                    <col style="width: 40%" />
                ...
                <tr>
                    <td style="padding: 5px; vertical-align: top">
                        <table class="tblEntryContent" style="width: 80%">
                            ...
                            <tr>
                                <td colspan="2">
                                    <table border="0" cellpadding="0" cellspacing="0" style="width: 1000px">
                                        <tr>
                                            <td> <%--chkOnlyAgreeDalamIntegrasi--%>
                                                <asp:CheckBox ID="chkOnlyAgreeBelumIntegrasi" runat="server" Checked="false" /><%:GetLabel("Tampilkan hanya yang bersedia dikirimkan data kunjungan ke Satu Sehat")%>
                                            </td>
                                        </tr>
                                    </table>
                                </td>
                            ...
                            <tr>
                                <td>
                                    <input type="button" id="btnUpdateStatusBelumIntegrasi" value="Kirim ke Satu Sehat" class="btnUpdateStatusBelumIntegrasi w3-button w3-teal w3-border w3-border-blue w3-round-large" />
                                </td>
                                <td>
                                    <table cellpadding="0" cellspacing="0">
                                        <colgroup>
                                        ...
                                        <tr>
                                            <td>
                                                <input type="button" id="btnRefreshBelumIntegrasi" value="R e f r e s h" class="btnRefreshBelumIntegrasi w3-button w3-blue w3-border w3-border-blue w3-round-large" />
                                            </td>
                                        </tr>
                                    </table>
                                </td>
        ...
        <div style="padding: 2px; display; none" id="containerSudahIntegrasi" class="containerSudahIntegrasi">
            <div class="pageTitle">
                <%=GetLabel("Registrasi Pasien yang Sudah Integrasi dengan Satu Sehat")%></div>
            <table class="tblContentArea">
                <colgroup>
                    <col style="width: 60%" />
                    <col style="width: 40%" />
                ...
                <tr>
                    <td style="padding: 5px; vertical-align: top">
                        <table class="tblEntryContent" style="width: 80%">
                            ...
                            <tr>
                                <td colspan="2">
                                    <table border="0" cellpadding="0" cellspacing="0" style="width: 1000px">
                                        <tr>
                                            <td> <%--chkOnlyAgreeDalamIntegrasi--%>
                                                <asp:CheckBox ID="chkOnlyAgreeSudahIntegrasi" runat="server" Checked="false" /><%:GetLabel("Tampilkan hanya yang bersedia dikirimkan data kunjungan ke Satu Sehat")%>
                                            </td>
                                        </tr>
                                    </table>
                                </td>
                            ...
                            <tr>
                                <td>
                                    <input type="button" id="btnUpdateStatusSudahIntegrasi" value="Kirim Ulang ke Satu Sehat" class="btnUpdateStatusSudahIntegrasi w3-button w3-teal w3-border w3-border-blue w3-round-large" />
                                </td>
                                <td>
                                    <table cellpadding="0" cellspacing="0">
                                        <colgroup>
                                        ...
                                        <tr>
                                            <td>
                                                <input type="button" id="btnRefreshSudahIntegrasi" value="R e f r e s h" class="btnRefreshSudahIntegrasi w3-button w3-blue w3-border w3-border-blue w3-round-large" />
                                            </td>
                                        </tr>
                                    </table>
                                </td>
