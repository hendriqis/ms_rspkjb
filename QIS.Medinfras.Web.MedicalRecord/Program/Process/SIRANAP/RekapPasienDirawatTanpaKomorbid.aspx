<%@ Page Language="C#" MasterPageFile="~/libs/MasterPage/MPTrx.master" AutoEventWireup="true"
    CodeBehind="RekapPasienDirawatTanpaKomorbid.aspx.cs" Inherits="QIS.Medinfras.Web.MedicalRecord.Program.RekapPasienDirawatTanpaKomorbid" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="plhEntry" runat="server">
    <script type="text/javascript">
        ...
        $('#<%=btnImport.ClientID %>').live('click', function (evt) {
            KemenKesService.getPasienDirawattanpaKomorbid(function (result) {
                if (result != null) {
                    console.log(result);
                    ...
                    if (resp[0] == "1") {
                        ...
                        var respData = jData["RekapPasienDirawatTanpaKomorbid"];
                        ...
                        for (var i = 0; i < respData.length; i++) {
                            if (respData[i]["tanggal"] == tanggal) {
                                ...
                                $('#<%=txtIsolasiTekananNegatifConfirmL.ClientID %>').val(respData[i]["isolasi_tekanan_negatif_confirm_l"]);
                                $('#<%=txtIsolasitanpaTekananNegatifSuspectL.ClientID %>').val(respData[i]["isolasi_tanpa_tekanan_negatif_suspect_l"]);
                                $('#<%=txtIsolasitanpaTekananNegatifSuspectP.ClientID %>').val(respData[i]["isolasi_tanpa_tekanan_negatif_suspect_p"]);
                                $('#<%=txtIsolasitanpaTekananNegatifConfirmL.ClientID %>').val(respData[i]["isolasi_tanpa_tekanan_negatif_confirm_l"]);
                                $('#<%=txtIsolasitanpaTekananNegatifConfirmP.ClientID %>').val(respData[i]["isolasi_tanpa_tekanan_negatif_confirm_p"]);
                                $('#<%=txtNICUKhususCovidSuspectL.ClientID %>').val(respData[i]["nicu_khusus_covid_suspect_l"]);
                                $('#<%=txtNICUKhususCovidSuspectP.ClientID %>').val(respData[i]["nicu_khusus_covid_suspect_p"]);
                                $('#<%=txtNICUKhususCovidConfirmL.ClientID %>').val(respData[i]["nicu_khusus_covid_confirm_l"]);
                                $('#<%=txtNICUKhususCovidConfirmP.ClientID %>').val(respData[i]["nicu_khusus_covid_confirm_p"]);
                                $('#<%=txtPICUKhususCovidSuspectL.ClientID %>').val(respData[i]["picu_khusus_covid_suspect_l"]);
                                $('#<%=txtPICUKhususCovidSuspectP.ClientID %>').val(respData[i]["picu_khusus_covid_suspect_p"]);
        ...
        $('#<%=btnSend.ClientID%>').live('click', function (evt) {
            ...
            KemenKesService.pasienDirawattanpaKomorbid(tanggal, icu_dengan_ventilator_suspect_l, icu_dengan_ventilator_suspect_p, icu_dengan_ventilator_confirm_l, icu_dengan_ventilator_confirm_p,
  ...
  <div class="w3-card w3-card w3-white">
     <header class="w3-container w3-blue">
        <h2>Form Rekap Pasien Dirawat Tanpa Komorbid</h2> 
     </header>
    <table class="tblInfoTT w3-table w3-bordered w3-card-4">
        <colgroup>
            <col />
