...
{
    public abstract class BasePagePatientPageEntryCtl : BaseContentPopupCtl
    {
        ...
        {
            //MasterControl = (BaseMPPopupEntry)LoadControl("~/Controls/MPPatientPageEntry.ascx");
            MasterControl = (BaseMPPopupEntry)LoadControl("~/Libs/Controls/PatientPage/MPPatientPageEntry.ascx");
        ...
        public void ReInitControl()
        {
            ...
            foreach (DictionaryEntry entry in ControlEntryList)
            {
                ...
                {
                    ControlEntrySetting setting = (ControlEntrySetting)entry.Value;
                    ((ASPxEdit)ctrl).Value = setting.DefaultValue;
                }
                else if (ctrl is WebControl || ctrl is HtmlInputHidden)
                {
                    ControlEntrySetting setting = (ControlEntrySetting)entry.Value;
