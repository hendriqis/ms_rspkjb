<%@ Page Language="C#" MasterPageFile="~/Libs/MasterPage/MPMain.master" AutoEventWireup="true"
    CodeBehind="ControlOrderList.aspx.cs" Inherits="QIS.Medinfras.Web.MedicalCheckup.Program.ControlOrderList" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="plhMPMain" runat="server">
    <script type="text/javascript">
        ...
        $('#<%=grdView.ClientID %> > tbody > tr:gt(0):not(.trEmpty)').live('click', function () {
            ...
            $('#<%=hdnRegistrationID.ClientID %>').val($(this).find('.hdnRegistrationID').val());
        ...
        $('.imgOutstanding.imgLink1').live('click', function () {
            ...
            var id = visitID + '|' + regNo + '|' + patientName + '|' + itemID + '|' + itemCode + '|' + itemName + '|' + healthcareServiceUnitID;
        ...
        $('.imgPrint.imgPrintSuratKetSehat').die('click');
        $('.imgPrint.imgPrintSuratKetSehat').live('click', function () {
            $tr = $(this).closest('tr');
            var reportCode = "PM-00142";
            var regID = $tr.find('.hdnRegistrationID').val();
            var url = ResolveUrl("~/Libs/Program/Controls/HealthyInformationReasonCtl.ascx");
            openUserControlPopup(url, regID, 'Keterangan Sehat', 500, 150);
        });

        $('.imgPrint.imgPrintBuktiTransaksi').die('click');
        $('.imgPrint.imgPrintBuktiTransaksi').live('click', function () {
            $tr = $(this).closest('tr');
            var reportCode = "PM-00166";
            var hdnHealthcareInitial = $('#<%:hdnHealthcareInitial.ClientID %>').val();
            if (hdnHealthcareInitial == "RSRT") {
                reportCode = "MC-00016";
            }
        ...
        function onBeforeRightPanelPrint(code, filterExpression, errMessage) {
            ...
            if (code == 'PM-00190' || code == 'PM-00687') {
                                    ...
                                    <asp:GridView ID="grdView" runat="server" CssClass="grdSelected" AutoGenerateColumns="false"
                                        ...
                                        <Columns>
                                            ...
                                            <asp:TemplateField HeaderStyle-Width="50px" HeaderStyle-HorizontalAlign="Center">
                                                <ItemTemplate>
                                                    <div style="text-align: center">
                                                        <img class="imgPrint <%#"imgPrintSuratKetSehat"%>" title='<%=GetLabel("Surat Keterangan Sehat")%>'
                                                            src='<%# ResolveUrl("~/Libs/Images/Button/print.png")%>' alt="" style="margin-right: 2px" />
                                                    </div>
                                                </ItemTemplate>
                                            </asp:TemplateField>
                                            <asp:TemplateField HeaderStyle-Width="50px" HeaderStyle-HorizontalAlign="Center">
                                                <ItemTemplate>
                                                    <div style="text-align: center">
                                                        <img class="imgPrint <%#"imgPrintBuktiTransaksi"%>" title='<%=GetLabel("Bukti Transaksi")%>'
                                                            src='<%# ResolveUrl("~/Libs/Images/Button/print.png")%>' alt="" style="margin-right: 2px" />
                                                    </div>
                                                </ItemTemplate>
                                            </asp:TemplateField>
