<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ItemMCUDtEntryCtl.ascx.cs"
    Inherits="QIS.Medinfras.Web.MedicalCheckup.Program.ItemMCUDtEntryCtl" %>
...
<script type="text/javascript" id="dxss_ItemMCUDtEntryCtl">
    $('#lblEntryPopupAddData').live('click', function () {
        ...
        $('#<%=chkIsAllowChanged.ClientID%>').prop("checked", false);
        $('#<%=chkIsAllowEntryFromDesktop.ClientID%>').prop("checked", false);
    ...
    $('.imgEdit.imgLink').live('click', function () {
        ...
        $('#<%=chkIsAllowChanged.ClientID %>').prop("checked", entity.IsAllowChanged == "True");
        $('#<%=chkIsAllowEntryFromDesktop.ClientID %>').prop("checked", entity.IsAllowEntryFromDesktop == "True");
    ...
    function onCbpEntryPopupViewEndCallback(s) {
        ...
        if (param[0] == 'save') {
            ...
            var hdntxtPackagePrice = $('#<%=hdntxtPackagePrice.ClientID %>').val();
            var hdntxtTotalPrice = $('#<%=hdntxtTotalPrice.ClientID %>').val();
            var hdntxtGapPrice = $('#<%=hdntxtGapPrice.ClientID %>').val();

            $('#<%=txtPackagePrice.ClientID %>').val(hdntxtPackagePrice);
            $('#<%=txtTotalPrice.ClientID %>').val(hdntxtTotalPrice);
            $('#<%=txtGapPrice.ClientID %>').val(hdntxtGapPrice);
        }
        else if (param[0] == 'refresh') {

            var pageCountRefresh = parseInt(param[1]);
            setPagingDetailItem(pageCountRefresh);
            var hdnIsValueExist = $('#<%=hdnIsValueExist.ClientID %>').val();
            if (hdnIsValueExist === "1") {
                cboTariffScheme.SetEnabled(false);
            }
    ...
    function onCboTariffSchemeValueChanged() {
        ...
        cbpEntryPopupView.PerformCallback('refreshScheme');
    }

    //#region HealthcareServiceUnit
    function onGetHealthcareServiceUnitFilterExpression() {
        var filterExpression = "IsUsingRegistration = 1 AND IsDeleted = 0";

        var deptID = $('#<%=hdnDepartmentID.ClientID %>').val();
        if (deptID != null && deptID != "") {
            filterExpression += " AND DepartmentID IN ('" + deptID + "')";
        } else {
            filterExpression += " AND DepartmentID IN ('" + Constant.Facility.DIAGNOSTIC + "','" + Constant.Facility.OUTPATIENT + "')";
        }

        return filterExpression;
    }

    $('#lblHealthcareServiceUnit.lblLink').live('click', function () {
    ...
    function ontxtServiceUnitCodeChanged(value) {
        ...
        Methods.getObject('GetvHealthcareServiceUnitList', filterExpression, function (result) {
            if (result != null) {
                ...
                $('#<%=hdnDepartmentIDDetail.ClientID %>').val(result.DepartmentID);
                $('#<%=hdnServiceUnitID.ClientID %>').val(result.ServiceUnitID);
                $('#<%=txtServiceUnitName.ClientID %>').val(result.ServiceUnitName);
            }
            else {
                $('#<%=hdnHealthcareServiceUnitID.ClientID %>').val('');
                $('#<%=hdnDepartmentIDDetail.ClientID %>').val('');
                $('#<%=hdnServiceUnitID.ClientID %>').val('');
                $('#<%=txtServiceUnitCode.ClientID %>').val('');
                $('#<%=txtServiceUnitName.ClientID %>').val('');
            }
    ...
    function onGetItemServiceDtFilterExpression() {
        ...
        var filterExpression = "ItemID NOT IN (SELECT DetailItemID FROM ItemServiceDt WITH(NOLOCK) WHERE ServiceUnitID = " + serviceUnit + " AND ItemID = " + itemID + " AND IsDeleted = 0) AND IsDeleted = 0" + " AND GCItemType NOT IN (" + itemType + ")";

        filterExpression += " AND ItemID IN (SELECT ItemID FROM vServiceUnitItem WITH(NOLOCK) WHERE HealthcareServiceUnitID = " + $('#<%=hdnHealthcareServiceUnitID.ClientID %>').val() + ")";

        filterExpression += " AND IsDeleted = 0 AND GCItemStatus != '" + Constant.ItemStatus.ITEM_STATUS_IN_ACTIVE + "'";
    ...
    $('#lblParamedic.lblLink').live('click', function () {
        ...
        if (serviceUnitID != '0')
            filterExpression = "ParamedicID IN (SELECT ParamedicID FROM ServiceUnitParamedic WHERE HealthcareServiceUnitID = " + serviceUnitID + ") AND GCParamedicMasterType = 'X019^001' AND IsDeleted = 0";
    ...
    function onTxtParamedicCodeChanged(value) {
        var filterExpression = "GCParamedicMasterType = 'X019^001' AND isDeleted = 0 AND ParamedicCode = '" + value + "'";
...
<div style="height: 400px; overflow-y: auto; overflow-x: hidden;">
    ...
    <input type="hidden" id="hdnMCUClassID" value="" runat="server" />
    <input type="hidden" id="hdnDepartmentIDDetail" runat="server" value="" />
                ...
                <table class="tblEntryContent" style="width: 70%">
                    ...
                    <tr>
                        ...
                        <td colspan="3">
                            ...
                            <asp:TextBox ID="txtTariffScheme" ReadOnly="true" Width="100%" runat="server" />
                        </td>
                    </tr>
                    <tr>
                        <td class="tdLabel">
                        ...
                        <table class="tblEntryDetail" style="width: 100%">
                            ...
                            <tr>
                                <td style="width: 120px">
                                    <label class="lblMandatory lblLink" id="lblHealthcareServiceUnit">
                                ...
                                <td>
                                    <table>
                                        <tr>
                                            ...
                                            <td>
                                                <asp:CheckBox ID="chkIsAllowChanged" runat="server" /><%=GetLabel("Boleh diganti")%>
                                                <div style="display: none;">
                                                    <asp:CheckBox ID="chkIsAllowEntryFromDesktop" runat="server" /><%=GetLabel("Input Hasil Dari MCU Desktop")%></div>
                                            </td>
                                        </tr>
                                    </table>
                                </td>
                ...
                <dxcp:ASPxCallbackPanel ID="cbpEntryPopupView" runat="server" Width="100%" ClientInstanceName="cbpEntryPopupView"
                    ...
                    <PanelCollection>
                        <dx:PanelContent ID="PanelContent1" runat="server">
                            <input type="hidden" value="0" id="hdntxtPackagePrice" runat="server" />
                            <input type="hidden" value="0" id="hdntxtTotalPrice" runat="server" />
                            <input type="hidden" value="0" id="hdntxtGapPrice" runat="server" />
                            <input type="hidden" value="0" id="hdnIsValueExist" runat="server" />
                                    ...
                                    <Columns>
                                        <asp:TemplateField HeaderStyle-Width="70px" ItemStyle-HorizontalAlign="Center">
                                            <ItemTemplate>
                                                ...
                                                <input type="hidden" bindingfield="IsAllowEntryFromDesktop" value="<%#: Eval("IsAllowEntryFromDesktop")%>" />
                                        ...
                                        <asp:BoundField DataField="MCUPrice" ItemStyle-HorizontalAlign="Right" HeaderStyle-HorizontalAlign="Right"
                                            HeaderText="Harga MCU" HeaderStyle-Width="100px" />
                                    </Columns>
                                    <EmptyDataTemplate>
