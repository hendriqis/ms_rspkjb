<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage/MPPatientPageList.master"
    AutoEventWireup="true" CodeBehind="MedicationList1.aspx.cs" Inherits="QIS.Medinfras.Web.EMR.Program.MedicationList1" %>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $('#<%=btnVerification.ClientID %>').click(function () {
                if ($('#<%=hdnPhysicianID.ClientID %>').val() == $('#<%=hdnUserParamedicID.ClientID %>').val()) {
                    var serviceUnitID = $('#<%=hdnServiceUnitID.ClientID %>').val();
                    var url = ResolveUrl("~/Program/PatientPage/_PopupEntry/Medication/Administration/VerifyMedicationAdministrationCtl1.ascx");
                    var param = serviceUnitID + '|' + $('#<%=hdnMRN.ClientID %>').val();
                    openUserControlPopup(url, param, 'Verifikasi Pemberian Obat', 800, 450);
                }
                else {
                    displayErrorMessageBox('Verifikasi Pemberian Obat', 'Verifikasi Pemberian Obat hanya bisa dilakukan oleh DPJP.');
                }
            });

            $('#<%=btnDiscontinue.ClientID %>').click(function () {
                if (getSelectedCheckbox()) {
                    var visitID = $('#<%=hdnVisitID.ClientID %>').val();
                    var selectedItems = $('#<%=hdnSelectedID.ClientID %>').val();
                    var url = ResolveUrl("~/Program/PatientPage/_PopupEntry/Instruction/Medication/DiscontinueMedicationCtl.ascx");
                    var param = visitID + '|' + selectedItems;
                    openUserControlPopup(url, param, 'Discontinue/Stop Pengobatan', 800, 450);
                }
                else {
                    displayErrorMessageBox('Instruksi Stop Pengobatan', 'Obat yang akan distop belum dipilih.');
                }
            });

            $('#<%=btnExtend.ClientID %>').click(function () {
                if (getSelectedCheckbox()) {
                    var visitID = $('#<%=hdnVisitID.ClientID %>').val();
                    var selectedItems = $('#<%=hdnSelectedID.ClientID %>').val();
                    var url = ResolveUrl("~/Program/PatientPage/_PopupEntry/Instruction/Medication/ExtendMedicationCtl.ascx");
                    var param = visitID + '|' + selectedItems;
                    openUserControlPopup(url, param, 'Instruksi Tambah Durasi Pengobatan', 800, 450);
                }
                else {
                    displayErrorMessageBox('Instruksi Tambah Durasi Pengobatan', 'Obat yang akan diextend belum dipilih.');
                }
            });

            $('#<%=btnSigna.ClientID %>').click(function () {
                if (getSelectedCheckbox()) {
                    var visitID = $('#<%=hdnVisitID.ClientID %>').val();
                    var selectedItems = $('#<%=hdnSelectedID.ClientID %>').val();
                    var url = ResolveUrl("~/Program/PatientPage/_PopupEntry/Instruction/Medication/ChangeMedicationSignaCtl.ascx");
                    var param = visitID + '|' + selectedItems;
                    openUserControlPopup(url, param, 'Instruksi Ubah Aturan Pemberian', 800, 450);
                }
                else {
                    displayErrorMessageBox('Instruksi Ubah Aturan Pemberian', 'Obat yang akan diubah aturan pemberian belum dipilih.');
                }
            });
