<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="TestOrderEntryQuickPicksCtl.ascx.cs" 
    Inherits="QIS.Medinfras.Web.EMR.Program.TestOrderEntryQuickPicksCtl" %>
...
<script type="text/javascript" id="dxss_serviceunititementryctl">
    ...
    function onBeforeSaveRecordEntryPopup() {
        var selectedItem = getCheckedSelected();
        if (selectedItem != '' && $('#<%=txtRemarks.ClientID %>').val() != '')
            return true;
        else {
    ...
    function getCheckedAvailable() {
        var result = '';
        $('#<%=grdAvailable.ClientID %> input:checked').each(function () {
            if (result != '')
                result += ';';
    ...
    function getCheckedSelected() {
        var result = '';
        $('#<%=grdSelected.ClientID %> input:checked').each(function () {
            if (result != '')
                result += ';';
            result += $(this).closest('tr').find('.keyField').html();
        });
    ...
    function onAfterSaveRecordPatientPageEntry(value) {
        $('#hdnTestOrderEntryQuickPicksTestOrderID').val(value);
    }
</script>
...
<input type="hidden" value="" runat="server" id="hdnTestOrderID" />
<input type="hidden" value="" id="hdnTestOrderEntryQuickPicksTestOrderID" />
<input type="hidden" value="" id="hdnPopupHealthcareServiceUnitID" runat="server" />
...
<table>
    ...
    <tr>
        ...
        <td colspan="3"><dxe:ASPxComboBox runat="server" ID="cboDiagnose" Width="100%" /></td>
    </tr>
    <tr>
                ...
                <dxcp:ASPxCallbackPanel ID="cbpMatrixAvailable" runat="server" Width="100%" ClientInstanceName="cbpMatrixAvailable"
                    ...
                    <PanelCollection>
                        <dx:PanelContent ID="PanelContent1" runat="server">
                            <asp:Panel runat="server" ID="pnlPatientVisitTransHdGrdView" Style="width: 100%; margin-left: auto; margin-right: auto; position: relative;font-size:0.95em;height: 272px;overflow-y:auto; overflow-x:hidden; ">
                                <asp:GridView ID="grdAvailable" runat="server" CssClass="grdView notAllowSelect" AutoGenerateColumns="false" ShowHeaderWhenEmpty="true" EmptyDataRowStyle-CssClass="trEmpty">
                                    <Columns>
                                        <asp:TemplateField HeaderStyle-Width="70px" HeaderStyle-HorizontalAlign="Center" ItemStyle-HorizontalAlign="Center">
                                            <HeaderTemplate>
                                                <input id="chkSelectAvailable" type="checkbox" />
                                            </HeaderTemplate>
                                            <ItemTemplate>
                                                <asp:CheckBox ID="CheckBox1" Checked='<%# (bool)(DataBinder.Eval(Container.DataItem, "IsChecked") == null) ? true : false %>' runat="server" CssClass="chkAvailable" />
                                            </ItemTemplate>
                                        </asp:TemplateField>
                ...
                <dxcp:ASPxCallbackPanel ID="cbpMatrixSelected" runat="server" Width="100%" ClientInstanceName="cbpMatrixSelected"
                    ...
                    <PanelCollection>
                        <dx:PanelContent ID="PanelContent2" runat="server">
                            <asp:Panel runat="server" ID="Panel1" Style="width: 100%; margin-left: auto; margin-right: auto; position: relative;font-size:0.95em;height: 272px;overflow-y:auto; overflow-x:hidden;">
                                <asp:GridView ID="grdSelected" runat="server" CssClass="grdView notAllowSelect" AutoGenerateColumns="false" ShowHeaderWhenEmpty="true" EmptyDataRowStyle-CssClass="trEmpty">
                                    <Columns>
                                        <asp:TemplateField HeaderStyle-Width="70px" HeaderStyle-HorizontalAlign="Center" ItemStyle-HorizontalAlign="Center">
                                            ...
                                            <ItemTemplate>
                                                <asp:CheckBox ID="CheckBox1" Checked='<%# (bool)(DataBinder.Eval(Container.DataItem, "IsChecked") == null) ? true : false %>' runat="server" CssClass="chkSelected" />
                                            </ItemTemplate>
                                        </asp:TemplateField>
                                        <asp:BoundField DataField="ID" ItemStyle-CssClass="keyField" HeaderStyle-CssClass="keyField"  />
                                        <asp:BoundField DataField="Name" HeaderText="Name" />
