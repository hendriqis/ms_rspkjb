<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="PrescriptionCompoundEntryCtl.ascx.cs" 
    Inherits="QIS.Medinfras.Web.EMR.Program.PrescriptionCompoundEntryCtl" %>
...
<script type="text/javascript" id="dxss_prescriptioncompoundentryctl">
    ...
    function txtSignaLabelCompoundChanged(value) {
        ...
        Methods.getObject('GetvSignaList', filterExpression, function (result) {
            if (result != null) {
                ...
                cboCoenamRuleCompoundCtl.SetValue(result.GCCoenamRule);
                cboFrequencyTimelineCompoundCtl.SetValue(result.GCDosingFrequency);
                $('#<%=txtFrequencyNumber.ClientID %>').val(result.Frequency);
                $('#<%=txtFrequencyNumber.ClientID %>').change();
                cboDosingUnitCompoundCtl.SetValue(result.GCDoseUnit);
    ...
    function onTemplateCodeChanged(value) {
        ...
        Methods.getListObject("GetvCompoundTemplateDtList", filterExpression, function (result) {
            ...
            cboFrequencyTimelineCompoundCtl.SetValue(result[0].GCDosingFrequency);
    ...
    window.init = function () {
        ...
        if (prescriptionDetailID > -1) {
            var filterExpression = "PrescriptionOrderDetailID = " + prescriptionDetailID + " OR ParentID = " + prescriptionDetailID + " AND IsDeleted = 0 ORDER BY PrescriptionOrderDetailID";
            Methods.getListObject("GetvPrescriptionOrderDt2List", filterExpression, function (result) {
                for (var i = 0; i < result.length; ++i) {
                    var entity = result[i];
                    grdCompound.addRow();
                    $row = grdCompound.getRow(i);
    ...
    function fillTableData($row, entity) {
        if (entity != null && entity != "") {
            ...
            grdCompound.setKeyValue($row, 0);

            cboCompoundUnit.ClearItems();

            grdCompound.setCellHiddenValue($row, 'hdnGCItemUnit', "");
            grdCompound.setCellHiddenValue($row, 'hdnItemUnit', "");
            grdCompound.setCellHiddenValue($row, 'hdnDose', "");

            grdCompound.setSearchTextBoxProperties($row, 'ledProduct', { "value": 0 });
            grdCompound.setTextBoxProperties($row, 'txtStrengthAmount', { "value": "" });
            grdCompound.setComboBoxProperties($row, 'cboCompoundStrengthUnit', { "value": "" });
            grdCompound.setTextBoxProperties($row, 'txtCompoundQty', { "value": "" });
            grdCompound.setComboBoxProperties($row, 'cboCompoundUnit', { "value": "" });
            grdCompound.setCellHiddenValue($row, 'hdnDrugName', "");

            grdCompound.setTextBoxProperties($row, 'txtConversionFactor', { "value": "" });
        }
    }
    ...
    function onBeforeSaveRecordEntryPopup(errMessage) {
        ...
        var paramHd = $('#<%=hdnQueryString.ClientID %>').val().split('|');
        var paramDt = $('#<%=hdnInlineEditingData.ClientID %>').val().split('|');
        var locationID = $('#<%=hdnLocationIDCtlCompound.ClientID %>').val();
        var lstItemNoStock = '';

        var isValidate = $('#<%=hdnValidationEmptyStockCtlCompound.ClientID %>').val();
        if (isValidate == '0') {
            for (var i = 0; i < paramDt.length;i++ ) {
                var dataPerRow = paramDt[i].split(';');
                var itemID = dataPerRow[3];
                var itemName = dataPerRow[9];
                var qty = parseFloat(dataPerRow[6]);
                if (itemID != '' && qty > 0) {
                    ...
                    Methods.getObject("GetItemBalanceList", filterBalance, function (resultBalance) {
                        if (resultBalance != null) {
                            if (resultBalance.QuantityEND <= 0) {
                                if (lstItemNoStock == '') {
                                    lstItemNoStock = itemName;
                                }
                                else {
                                    lstItemNoStock += '<br>' + itemName;
                                }
                            }
                        }
                        else {
...
<input type="hidden" id="hdnValidationEmptyStockCtlCompound" value="" runat="server" />
<input type="hidden" id="hdnLocationIDCtlCompound" value="" runat="server" />
        ...
        <td valign="top">
            <table style="width:100%" cellpadding="1" cellspacing="1">
                ...
                <tr>
                    ...
                    <td><dxe:ASPxComboBox ID="cboFrequencyTimelineCompoundCtl" ClientInstanceName="cboFrequencyTimelineCompoundCtl" runat="server" Width="100%">
                    ...
                    <td><dxe:ASPxComboBox ID="cboDosingUnitCompoundCtl" ClientInstanceName="cboDosingUnitCompoundCtl" runat="server" Width="100%">
...
<div id="divSaveAsNewTemplate" runat="server">
    <table>
        <tr>
            <td><asp:CheckBox runat="server" ID="chkSaveAsNewTemplate" /><%=GetLabel("Save as My New Template")%></td>
        </tr>
    </table>
</div>
