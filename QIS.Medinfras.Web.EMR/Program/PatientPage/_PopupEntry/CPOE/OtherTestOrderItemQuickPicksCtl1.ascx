<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="OtherTestOrderItemQuickPicksCtl1.ascx.cs"
    Inherits="QIS.Medinfras.Web.EMR.Program.OtherTestOrderItemQuickPicksCtl1" %>
...
<script type="text/javascript" id="dxss_OtherTestOrderItemQuickPicksCtl1">
    ...
    function onBeforeSaveRecordEntryPopup() {
        ...
        if ($('#<%=hdnSelectedMember.ClientID %>').val() != '')
            ...
            displayMessageBox("Order Pemeriksaan", "Belum ada item pemeriksaan yang dipilih");
            return false;
        }
        //        }
        return false;
    ...
    function onAfterSaveRecordPatientPageEntry(value) {
        if (typeof onRefreshDiagnosticGrid == 'function')
            onRefreshDiagnosticGrid();
        if (value != "") {
            var param = value.split("|");
            if (typeof onAfterAddDiagnosticTestOrder == 'function')
                onAfterAddDiagnosticTestOrder(param[1]);
    ...
    function onCboServiceUnitValueChanged() {
        ...
        if ($('#<%=hdnIsUsingMultiVisitScheduleOrder.ClientID %>').val() == "1") {
            Methods.getObject("GetvHealthcareServiceUnitCustomList", "HealthcareServiceUnitID = " + cboServiceUnit.GetValue(), function (result) {
                if (result != null) {
                    if (result.IsAllowMultiVisitSchedule) {
                        $('#<%:trMultiVisitScheduleOrder.ClientID %>').removeAttr('style');
                        if ($('#<%:hdnCheckedIsUsingMultiVisitScheduleOrder.ClientID %>').val() == "True") {
                            $('#<%=chkIsMultiVisitScheduleOrder.ClientID %>').prop("checked", true);
                        }
                        $('#<%:hdnServiceUnitUsingMultiVisit.ClientID %>').val('1');
                    }
                    else {
                        $('#<%:trMultiVisitScheduleOrder.ClientID %>').attr('style', 'display:none');
                        $('#<%=chkIsMultiVisitScheduleOrder.ClientID %>').prop("checked", false);
                        $('#<%:hdnServiceUnitUsingMultiVisit.ClientID %>').val('0');
                    }
                }
                else {
                    $('#<%:trMultiVisitScheduleOrder.ClientID %>').attr('style', 'display:none');
        ...
        $('#tblSelectedItem tr.trSelectedItem').remove();
        cbpPopup.PerformCallback('refresh');
    }
    ...
    $('#lblTestPanel.lblLink').click(function () {
        var filterExpression = "GCItemType = '" + $('#<%=hdnGCItemType.ClientID %>').val() + "' AND HealthcareServiceUnitID = " + cboServiceUnit.GetValue() + " AND IsDeleted = 0";
...
<div style="padding: 10px;">
    ...
    <input type="hidden" id="hdnIsDIsabledHSUOption" value="0" runat="server" />
    <input type="hidden" value="" id="hdnIsUsingMultiVisitScheduleOrder" runat="server" />
    <input type="hidden" value="" id="hdnCheckedIsUsingMultiVisitScheduleOrder" runat="server" />
    <input type="hidden" value="" id="hdnServiceUnitUsingMultiVisit" runat="server" />
    <input type="hidden" value="" id="hdnTestTemplateID" runat="server" />
    <input type="hidden" id="hdnIsLimitedCPOEItemForBPJSOth" runat="server" value="" />
        ...
        <tr>
            <td style="padding: 5px; vertical-align: top">
                ...
                <table cellspacing="0" width="100%" style="padding-bottom: 5px">
                    ...
                    <tr>
                        <td class="tdLabel"><label class="lblLink" id="lblTestPanel"><%=GetLabel("Panel")%></label></td>
                        <td><asp:TextBox ID="txtTemplateCode" Width="100%" runat="server" /></td>
                        <td><asp:TextBox ID="txtTemplateName" Width="100%" runat="server" ReadOnly="true"/></td>
                    </tr>
