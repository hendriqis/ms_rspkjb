<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="BodyDiagramSOAPAdd1Ctl.ascx.cs"
    Inherits="QIS.Medinfras.Web.EMR.Program.PatientPage.BodyDiagramSOAPAdd1Ctl" %>
...
<%@ Register Src="~/Program/PatientPage/_PopupEntry/BodyDiagram/BodyDiagramSOAPTakePhoto1Ctl.ascx"
...
<%--<script type="text/javascript" id="dxis_bodydiagramctl9" src='<%= ResolveUrl("~/Libs/Scripts/jquery/jquery-1.7.min.js")%>'></script>--%>
<script id="dxis_Webcamctl1" src='<%= ResolveUrl("~/Libs/Scripts/JqueryWebcam/webcam.js")%>'
...
<script type="text/javascript" id="dxss_dragctl1">
    ...
    function saveFreeDrawImage() {
        var image = canvas.toDataURL();
        var url = '<%= ResolveUrl("~/Libs/Service/UploadService.asmx/UploadFreeDrawImage")%>';
        ...
        $.ajax({
            ...
            dataType: 'json',
            error: function (msg) {
                displayErrorMessageBox('Free Drawing', "Terjadi kesalahan ketika proses upload gambar");
                success = false;
                ctrProcess++;
            },
    ...
    $('#<%=btnAcceptDrawing.ClientID %>').click(function () {
        saveFreeDrawImage();
    });

    $('#<%=btnClearDrawing.ClientID %>').click(function () {
        clearCanvas(canvas, ctx);
    });
    //#endregion
    ...
    var ctrA = { val: 0 }; // Counter Symbol A
    var ctrB = { val: 0 }; // Counter Symbol B
    var ctrC = { val: 0 }; // Counter Symbol C
    var ctrD = { val: 0 }; // Counter Symbol D
    var ctrE = { val: 0 }; // Counter Symbol E
    var ctrF = { val: 0 }; // Counter Symbol F
    var ctrG = { val: 0 }; // Counter Symbol G
    var ctrH = { val: 0 }; // Counter Symbol H
    var ctrI = { val: 0 }; // Counter Symbol I
    var ctrJ = { val: 0 }; // Counter Symbol J
    var ctrK = { val: 0 }; // Counter Symbol K
    var ctrL = { val: 0 }; // Counter Symbol L
    var ctrM = { val: 0 }; // Counter Symbol M
    var ctrN = { val: 0 }; // Counter Symbol N
    var ctrO = { val: 0 }; // Counter Symbol O
    var ctrP = { val: 0 }; // Counter Symbol P
    var ctrQ = { val: 0 }; // Counter Symbol Q
    var ctrR = { val: 0 }; // Counter Symbol R
    var ctrS = { val: 0 }; // Counter Symbol S
    var ctrT = { val: 0 }; // Counter Symbol T
    var ctrU = { val: 0 }; // Counter Symbol U
    var ctrV = { val: 0 }; // Counter Symbol V
    var ctrW = { val: 0 }; // Counter Symbol W
    var ctrX = { val: 0 }; // Counter Symbol X
    var ctrY = { val: 0 }; // Counter Symbol Y
    var ctrZ = { val: 0 }; // Counter Symbol Z
    ...
    $('.ad-image-wrapper').droppable({
        drop: function (event, ui) {
            ...
            if (!$clone.is('.insideDropZone')) {
                ...
                var ctr;
                switch (symbolGroup) {
                    case 'A': ctr = ctrA; break;
                    case 'B': ctr = ctrB; break;
                    case 'C': ctr = ctrC; break;
                    case 'D': ctr = ctrD; break;
                    case 'E': ctr = ctrE; break;
                    case 'F': ctr = ctrF; break;
                    case 'G': ctr = ctrG; break;
                    case 'H': ctr = ctrH; break;
                    case 'I': ctr = ctrI; break;
                    case 'J': ctr = ctrJ; break;
                    case 'K': ctr = ctrK; break;
                    case 'L': ctr = ctrL; break;
                    case 'M': ctr = ctrM; break;
                    case 'N': ctr = ctrN; break;
                    case 'O': ctr = ctrO; break;
                    case 'P': ctr = ctrP; break;
                    case 'Q': ctr = ctrQ; break;
                    case 'R': ctr = ctrR; break;
                    case 'S': ctr = ctrS; break;
                    case 'T': ctr = ctrT; break;
                    case 'U': ctr = ctrU; break;
                    case 'V': ctr = ctrV; break;
                    case 'W': ctr = ctrW; break;
                    case 'X': ctr = ctrX; break;
                    case 'Y': ctr = ctrY; break;
                    case 'Z': ctr = ctrZ; break;
                }
                ctr.val++;
    ...
    function OnClickAppliedImage(e) {
        ...
        for (var i = 1; i < param.length; ++i) {
            ...
            switch (arr[3].substr(0, 1)) {
                ...
                case 'V': ctr = ctrV; break;
                case 'W': ctr = ctrW; break;
                case 'X': ctr = ctrF; break;
                case 'Y': ctr = ctrC; break;
                case 'Z': ctr = ctrD; break;
            }
    ...
    $("#btnFreeAnnotation").click(OnApplyClickForFreeAnnonation);
    $("#btnFreeDrawing").click(OnClickFreeDrawing);
    $("#btnApply").click(OnApplyClick);
    $("#btnReset").click(OnResetClick);
    $("#btnSave").click(OnSaveClick);
    $("#btnCancel").click(OnCancelClick);

    function OnApplyClickForFreeAnnonation() {
        var $imgPreview = $('.ad-image-wrapper').find('img');
        if ($imgPreview.attr('src') == '') {
            showToast('Warning', 'Please select an image!');
            return;
        }
        $("#<%=hdnIsApplied.ClientID %>").val('1');
        var bgImg = new Image;
        $temp = $('.ad-thumb-list').find('img[src="' + $imgPreview.attr('src') + '"]');
        if ($temp.length > 0) {
            bgImg.src = $temp.attr('urlbase64');
            bgImg.onload = function () {
                onApplyClickCreateImageForFreeAnnonation(bgImg, $imgPreview);
            }
        }
        else {
    ...
    function onApplyClickCreateImageForFreeAnnonation(bgImg, $imgPreview) {
        ...
        imgToBind = canvas.toDataURL("image/png");
        pcFreeDrawing.Show();
    }

    function OnClickFreeDrawing() {
        imgToBind = '';
        pcFreeDrawing.Show();    
    }
    ...
    function OnSaveClick() {
        if ($("#<%=hdnIsApplied.ClientID %>").val() == '0') {
            displayMessageBox('Body Diagram', "Please press <b>Apply</b> before save or press <b>Cancel</b> to dismiss all the changes");
            return;
        }
        if ($("#<%=txtObservationTime.ClientID %>").valid()) {
            ...
            $("#containerApplied .content").each(function () {
                ...
                customSrc = customSrc.replace('data:image/bmp;base64,', '');

                customSrc = customSrc.replace('', '');
                var params = '{ "observationDate" : "' + observationDate + '", "observationTime" : "' + observationTime + '", "imageData" : "' + image + '", "idx" : "' + idxImage++ + '", "listSymbol" : "' + $(this).find('.listImageSymbol').val() + '", "customSrc" : "' + customSrc + '", "paramedicID" : "' + paramedicID + '", "testOrderID" : "' + testOrderID + '"}';
                ...
                $.ajax({
                    ...
                    error: function (xhr) {
                        ...
                        displayErrorMessageBox('Body Diagram', errorMessage);
                        success = false;
                    ...
                    success: function (msg) {
                        if (ctrProcess == totalImage - 1) {
                            ...
                            hideLoadingPanel();
                            if (!success)
                                displayErrorMessageBox('Body Diagram', "Terjadi kesalahan ketika proses upload data gambar");
                            else {
                                if (typeof onRefreshBodyDiagram != 'undefined')
                                    onRefreshBodyDiagram('');
    ...
    $(function () {
        ...
        var DefaultThumbGroup = $("#<%=hdnDefaultThumbGroup.ClientID %>").val();
        filterImageThumbs(DefaultThumbGroup);
        //        setTimeout(function () {
        //            filterImageThumbs(null);
        //        }, 500);
    });

    function onDefaultBodyDiagram(val) {
        filterImageThumbs(val);
    }
    ...
    function filterImageThumbs(groupID) {
        ...
        $('.ad-thumb-list li').each(function () {
            ...
            if (groupID == $(this).find('.imageGroupValue').val()) {
                $(this).show();
    ...
    function initBodyDiagramTakePhoto() {
        Webcam.set({
            width: 320,
            height: 240,
            image_format: 'jpeg',
    ...
    function onPcFreeDrawHide() {
        Methods.getSessionValue("freeDrawImage", function (result) {
            var $imgPreview = $('.ad-image-wrapper').find('img').first();
            $imgPreview.attr('src', "data:image/png;base64," + result);
            $parent = $imgPreview.parent();
            $parent.removeAttr('style');
...
<style type="text/css">
    ...
    {
        float: right;
        padding: 10px 10px 0px 5px; /*padding-top:10px;
        text-align:center;*/
        width: 30px;
        border: 1px solid #9C9898;
        height: 450px;
...
<input type="hidden" runat="server" id="hdnIsApplied" value="0" />
<input type="hidden" runat="server" id="hdnDefaultThumbGroup" value="" />
<div id="toolbarArea">
        ...
        <tr>
                        ...
                        <td>
                            <table cellpadding="0" cellspacing="5">
                                <tr>
                                    ...
                                    <td>
                                        <dxe:ASPxComboBox ID="cboParamedicID" ClientInstanceName="cboParamedicID" Width="200px"
                                            runat="server" />
                                    </td>
                                </tr>
                            </table>
            ...
            <td style="width: 300px; text-align: right" valign="top">
                <table id="Table1" runat="server" cellpadding="0">
                    <tr>
                        <td class="tdLabel">
                            <%=GetLabel("Body Diagram Group")%>
                        </td>
                        <td>
                            <dxe:ASPxComboBox ID="cboImageGroup" Width="100%" runat="server">
                                <ClientSideEvents Init="function(s) { onCboImageGroupChanged(s); }" ValueChanged="function(s) { onCboImageGroupChanged(s); }" />
                            </dxe:ASPxComboBox>
                        </td>
                    </tr>
                </table>
...
<div class="container" style="height: 530px">
    ...
    <div class="contentRight">
        <table id="tblContainerSymbol" runat="server">
        </table>
    </div>
    <div class="contentMiddle">
        <div id="gallery" class="ad-gallery">
            ...
            <div class="containerNotes">
                <div style="font-size: large; font-weight: bolder; margin: 10px; text-align: left">
                    Remarks :</div>
    ...
    <div class="contentFooter">
        <table cellpadding="0" cellspacing="10" style="margin-left: auto; margin-right: auto;
            ...
            <tr>
                <td>
                    <input type="button" class="w3-btn w3-hover-blue" value='<%= GetLabel("FREE ANNOTATION")%>'
                        id="btnFreeAnnotation" style="width: 125px;" />
                </td>
                <td>
                    <input type="button" class="w3-btn w3-hover-blue" value='<%= GetLabel("FREE DRAWING")%>'
                        id="btnFreeDrawing" style="width: 120px;" />
                </td>
                <td>
                    <input type="button" class="w3-btn w3-hover-blue" value='<%= GetLabel("PHOTO")%>'
                        onclick="pcTakePhoto.Show();" style="width: 80px;" />
                </td>
                <td>
                    <input type="button" class="w3-btn w3-hover-blue" value='<%= GetLabel("RESET")%>'
                        id="btnReset" style="width: 80px;" />
                </td>
                <td>
                    <input type="button" class="w3-btn w3-hover-blue" value='<%= GetLabel("APPLY")%>'
                        id="btnApply" style="width: 80px;" />
                </td>
                <td>
                    <input type="button" class="w3-btn w3-hover-blue" value='<%= GetLabel("SAVE")%>'
                        id="btnSave" style="width: 80px;" />
                </td>
                <td>
                    <input type="button" class="w3-btn w3-hover-blue" value='<%= GetLabel("CANCEL")%>'
                        id="btnCancel" style="width: 80px;" />
                </td>
            </tr>
        </table>
    </div>
...
<dx:ASPxPopupControl ID="pcNotes" runat="server" ClientInstanceName="pcNotes" Height="150px"
    HeaderText="Keterangan Anotasi Gambar" CloseAction="None" Width="350px" Modal="True"
                    ...
                    <dx:PanelContent ID="PanelContent1" runat="server">
                        <div style="text-align: center; width: 100%;">
                            ...
                            <table style="margin-left: auto; margin-right: auto; margin-top: 5px;">
                                <tr>
                                    <td>
                                        <input type="button" id="btnNotesOk" class="w3-btn w3-hover-blue" style="width: 80px"
                                            value='<%= GetLabel("OK")%>' />
                                    </td>
                                    <td>
                                        <input type="button" id="btnNotesCancel" class="w3-btn w3-hover-blue" style="width: 80px"
...
<dx:ASPxPopupControl ID="pcTakePhoto" runat="server" ClientInstanceName="pcTakePhoto"
    EnableHierarchyRecreation="True" FooterText="" HeaderText="MEDINFRAS : PHOTO CAPTURE"
    HeaderStyle-HorizontalAlign="Left" Height="400px" Modal="True" PopupHorizontalAlign="WindowCenter"
    PopupVerticalAlign="WindowCenter" Width="300px" CloseAction="CloseButton" AllowDragging="true">
    <ClientSideEvents Shown="function (s,e) { initBodyDiagramTakePhoto(); }" Closing="function(s,e){ onPcTakePhotoHide(); }" />
    <ContentCollection>
...
<dx:ASPxPopupControl ID="pcFreeDrawing" runat="server" ClientInstanceName="pcFreeDrawing"
    EnableHierarchyRecreation="True" FooterText="" HeaderText="MEDINFRAS : Free Drawing"
    HeaderStyle-HorizontalAlign="Left" Height="450px" Modal="True" PopupHorizontalAlign="WindowCenter"
    PopupVerticalAlign="WindowCenter" Width="300px" CloseAction="CloseButton" AllowDragging="true">
    <ClientSideEvents Shown="function (s,e) { initCanvas(); }" Closing="function(s,e){ onPcFreeDrawHide(); }" />
    <ContentCollection>
        <dx:PopupControlContentControl ID="pcFreeDrawingContainer" runat="server">
            <div style="text-align: center; width: 100%;">
                <dxcp:ASPxCallbackPanel ID="cbpFreeDrawing" runat="server" Width="100%" ClientInstanceName="cbpFreeDrawing"
                    ShowLoadingPanelImage="true">
                    <PanelCollection>
                        <dx:PanelContent ID="PanelContentDraw" runat="server">
                            <asp:Panel ID="panFreeDraw" Style="width: 100%;" runat="server" Height="350px">
                                ...
                                <center>
                                    <table>
                                        <tr>
                                            ...
                                            <td>
                                                <input type="color" id="favcolor" name="favcolor" value="#0e25f0">
                                            </td>
                                        </tr>
                                    </table>
                                    <table id="navcontainer" style="width: 180px;">
                                        <tr>
                                            <td class="divBtnImage">
                                                <input type="button" id="btnAcceptDrawing" runat="server" class="btnAcceptDrawing w3-btn w3-hover-blue"
                                                    value="ACCEPT" style="width: 100px; background-color: Red; color: White;" />
                                            </td>
                                            <td class="divBtnImage">
                                                <input type="button" id="btnClearDrawing" runat="server" class="btnClearDrawing w3-btn w3-hover-blue"
                                                    value="RESET" style="width: 100px; background-color: Red; color: White;" />
                                            </td>
                                        </tr>
                                    </table>
                                </center>
