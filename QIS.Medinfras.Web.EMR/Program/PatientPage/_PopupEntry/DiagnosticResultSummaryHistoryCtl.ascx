<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="DiagnosticResultSummaryHistoryCtl.ascx.cs"
    Inherits="QIS.Medinfras.Web.EMR.Program.DiagnosticResultSummaryHistoryCtl" %>
...
<script type="text/javascript" id="dxss_testorderdetail1">
    ...
    function getSelectedItem() {
        ...
        if (count == 0) {
            var messageBody = "Belum ada catatan hasil pemeriksaan penunjang.";
    ...
    function onAfterProcessPopupEntry(param) {
        if (typeof onAfterDiagnosticResultSummary == 'function') {
            onAfterDiagnosticResultSummary(param);
        }
    }
</script>
                    ...
                    <asp:Panel runat="server" ID="Panel5" CssClass="pnlContainerGrid" Style="height: 400px">
                        <asp:GridView ID="grdView" runat="server"  CssClass="grdView"
                            ...
                            <Columns>
                                ...
                                <asp:TemplateField HeaderStyle-HorizontalAlign="Left" ItemStyle-Width="10px">
                                    <ItemTemplate>
                                        <asp:CheckBox ID="chkIsProcessItem" runat="server" CssClass="chkIsProcessItem" AutoPostBack="false"  />
                                    </ItemTemplate>
                                </asp:TemplateField>
                                <asp:BoundField DataField="ObservationDateInString" HeaderText="Tanggal" HeaderStyle-Width="60px"/>
                                <asp:BoundField DataField="DiagnosticResultSummary" HeaderText="Catatan Hasil Pemeriksaan" HeaderStyle-Width="60px"/>
                               <asp:BoundField DataField="ParamedicName" HeaderText="Dokter/Tenaga Medis" HeaderStyle-Width="60px"/>
                            </Columns>
                            <EmptyDataTemplate>
                                <%=GetLabel("Tidak ada Riwayat Catatan Hasil Pemeriksaan Penunjang yang tersedia") %>
                            </EmptyDataTemplate>
                        </asp:GridView>
