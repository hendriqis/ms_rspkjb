...
{
    public partial class VerifyMedicationAdministrationCtl1 : BaseViewPopupCtl
    {
            ...
            try
            {
                for (int ct = 0; ct < selectedID.Length; ct++)
                {
                    ...
                    if (oSchedule != null)
                    {
                        ...
                        oSchedule.IsPhysicianVerified = true;
                        oSchedule.VerifiedPhysicianDateTime = DateTime.Now;
                        oSchedule.VerifiedPhysicianID = AppSession.UserLogin.ParamedicID;
                ...
                string message = string.Format("Medication Administration was verified successfully");
        ...
        private void SetControlProperties()
        {
            ...
            List<vParamedicMaster> lstParamedic = BusinessLayer.GetvParamedicMasterList(string.Format("GCParamedicMasterType IN ('{0}')", Constant.ParamedicType.Physician));
        ...
        {
            string filterExpression = string.Format("VisitID = {0} AND MedicationDate = '{1}' AND GCMedicationStatus IN ('{2}','{3}','{4}','{5}','{6}') AND IsPhysicianVerified = 0 AND IsDeleted = 0", AppSession.RegisteredPatient.VisitID,
                                                        Helper.GetDatePickerValue(txtMedicationDate.Text).ToString(Constant.FormatString.DATE_FORMAT_112),
                                                        Constant.MedicationStatus.TELAH_DIBERIKAN, Constant.MedicationStatus.DI_TUNDA, Constant.MedicationStatus.DISCONTINUE, Constant.MedicationStatus.PASIEN_ABSEN, Constant.MedicationStatus.PASIEN_MENOLAK, Constant.MedicationStatus.PASIEN_PUASA);
