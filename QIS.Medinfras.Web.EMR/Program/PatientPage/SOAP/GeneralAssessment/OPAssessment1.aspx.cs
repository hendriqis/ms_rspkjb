...
{
    public partial class OPAssessment1 : BasePagePatientPageList
    {
            ...
            try
            {
                ...
                if (isAllowSave)
                {
                    if (type == "save")
                    {
                        ...
                        if (!string.IsNullOrEmpty(txtMedicationHistory.Text))
                        {
                            sbNotes.AppendLine(string.Format("RPO  : "));
                            sbNotes.AppendLine(string.Format("{0}", txtMedicationHistory.Text));
                        }
                        ...
                        //if (visitNoteID != "0")
                        //{
                        vReviewOfSystemHd entityReviewOfSystem = BusinessLayer.GetvReviewOfSystemHdList(string.Format("VisitID = {0} AND ParamedicID = {1} AND ObservationDate = '{2}' AND IsDeleted = 0", hdnVisitID.Value, AppSession.UserLogin.ParamedicID, Helper.GetDatePickerValue(txtServiceDate).ToString(Constant.FormatString.DATE_FORMAT_112))).FirstOrDefault();
                        if (entityReviewOfSystem != null)
                        {
                            List<vReviewOfSystemDt> lstROS = BusinessLayer.GetvReviewOfSystemDtList(string.Format("ID = {0} AND IsDeleted = 0 ORDER BY GCRoSystem", entityReviewOfSystem.ID, Helper.GetDatePickerValue(txtServiceDate).ToString(Constant.FormatString.DATE_FORMAT_112)));
                            if (lstROS.Count > 0)
        ...
        private void CopyFromPreviousVisit(IDbContext ctx)
        {
            ...
            if (chkIsCopyROS.Checked)
            {
                ...
                StandardCodeDao entitystdDao = new StandardCodeDao(ctx);
                ...
                foreach (vReviewOfSystemHd ros in lstReviewOfSystemHd)
                {
                    ...
                    ctx.CommandType = CommandType.Text;
                    ctx.Command.Parameters.Clear();
                    List<vReviewOfSystemDt> lstEntityDt = BusinessLayer.GetvReviewOfSystemDtList(string.Format("ID IN ({0})", ros.ID), ctx);
                    foreach (vReviewOfSystemDt entityROSDtBefore in lstEntityDt)
                    {
                        ReviewOfSystemDt entityROSDt = new ReviewOfSystemDt();
                        entityROSDt.ID = oROS_id;
                        entityROSDt.GCROSystem = entityROSDtBefore.GCROSystem;
                        entityROSDt.IsNotExamined = entityROSDtBefore.IsNotExamined;
                        entityROSDt.IsNormal = entityROSDtBefore.IsNormal;
                        entityROSDt.Remarks = entityROSDtBefore.Remarks;
                        entityROSDt.IsOther = entityROSDtBefore.IsOther;

                        StandardCode stdCheck = entitystdDao.Get(entityROSDtBefore.GCROSystem);
                        if (stdCheck != null)
                        {
                            //ctx.CommandType = CommandType.Text;
                            //ctx.Command.Parameters.Clear();
                            entityROSDtDao.Insert(entityROSDt);
                            objectiveText.AppendLine(string.Format(" {0}: {1}", entityROSDtBefore.ROSystem, entityROSDtBefore.cfRemarks));
                        }
                    }
                }
            ...
            if (chkIsCopyVitalSign.Checked)
            {
                ...
                VitalSignTypeDao entityVSTypeDao = new VitalSignTypeDao(ctx);
                ...
                if (oVitalSignHd != null)
                {
                    ctx.CommandType = CommandType.Text;
                    ctx.Command.Parameters.Clear();
                    List<vVitalSignDt> lstEntityVSDt = BusinessLayer.GetvVitalSignDtList(string.Format("ID = {0}", oVitalSignHd.ID));
                    ...
                    foreach (vVitalSignDt entityDt in lstEntityVSDt)
                    {
                        VitalSignDt entityVSDt = new VitalSignDt();
                        entityVSDt.ID = entityVS.ID;
                        entityVSDt.VitalSignID = entityDt.VitalSignID;
                        entityVSDt.VitalSignValue = entityDt.VitalSignValue;
                        entityVSDt.IsAutoGenerated = entityDt.IsAutoGenerated;
                        entityVSDt.IHSReferenceID = entityDt.IHSReferenceID;

                        VitalSignType vitalSignType = entityVSTypeDao.Get(entityVSDt.VitalSignID);
                        if (vitalSignType != null)
                        {
                            entityVSDtDao.Insert(entityVSDt);
                            objectiveText.AppendLine(string.Format(" {0} {1} {2}", entityDt.VitalSignLabel, entityDt.VitalSignValue, entityDt.ValueUnit));
                        }
                    }
                }
            ...
            StringBuilder diagnoseText = new StringBuilder();
            if (chkIsCopyDiagnosis.Checked)
            {
                ...
                if (lstDiagnosis.Count > 0)
                {
                    foreach (vPatientDiagnosis item in lstDiagnosis)
                    {
                        ...
                        if (string.IsNullOrEmpty(item.DiagnoseID))
                            diagnoseText.AppendLine(string.Format("- {0}", item.cfDiagnosisText));
                        else
                            diagnoseText.AppendLine(string.Format("- {0} ({1})", item.cfDiagnosisText, item.DiagnoseID));
            ...
            PatientVisitNote entityCPPT = null;
            bool isPatientVisitNote = true;

            if (hdnPatientVisitNoteID.Value != "" && hdnPatientVisitNoteID.Value != "0")
            {
                entityCPPT = patientVisitNoteDao.Get(Convert.ToInt32(hdnPatientVisitNoteID.Value));
                //if (entityCPPT.CreatedBy != AppSession.UserLogin.UserID)
                //{
                //    ctx.CommandType = CommandType.Text;
                //    ctx.Command.Parameters.Clear();
                //    string filterexp = string.Format("VisitID = {0} AND CreatedBy = {1} AND ChiefComplaintID IS NOT NULL AND IsDeleted = 0", hdnVisitIDMainPageOPAssesment1.Value, AppSession.UserLogin.UserID);
                //    entityCPPT = BusinessLayer.GetPatientVisitNoteList(filterexp, ctx).FirstOrDefault();
                //    if (entityCPPT != null)
                //    {
                //        isPatientVisitNote = false;
                //        entityCPPT.LastUpdatedBy = AppSession.UserLogin.UserID;
                //    }
                //    else
                //    {
                //        entityCPPT = new PatientVisitNote();
                //        entityCPPT.VisitID = Convert.ToInt32(hdnVisitIDMainPageOPAssesment1.Value);
                //        entityCPPT.HealthcareServiceUnitID = Convert.ToInt32(hdnHealthcareServiceUnitID.Value);
                //        entityCPPT.ParamedicID = AppSession.UserLogin.ParamedicID;
                //        entityCPPT.CreatedBy = AppSession.UserLogin.UserID;

                //        entityCPPT.NoteDate = Helper.GetDatePickerValue(txtServiceDate);
                //        entityCPPT.NoteTime = txtServiceTime.Text;
                //        entityCPPT.GCPatientNoteType = Constant.PatientVisitNotes.SOAP_SUMMARY_NOTES;
                //    }
            ...
            {
                entityCPPT = new PatientVisitNote();
                entityCPPT.VisitID = Convert.ToInt32(hdnVisitID.Value);
                entityCPPT.HealthcareServiceUnitID = Convert.ToInt32(hdnHealthcareServiceUnitID.Value);
                entityCPPT.ParamedicID = AppSession.UserLogin.ParamedicID;
                entityCPPT.CreatedBy = AppSession.UserLogin.UserID;
            ...
            StringBuilder sbNotes = new StringBuilder();
            sbNotes.AppendLine(string.Format("{0}   ", entityCPPT.SubjectiveText));
            if (chkIsCopyPastMedicalHistory.Checked)
            {
                sbNotes.AppendLine(string.Format("Riwayat Penyakit Dahulu  : "));
                sbNotes.AppendLine(string.Format(" {0}   ", entity.PastMedicalHistory));
            }

            if (chkIsCopyPastMedicationHistory.Checked)
            {
                sbNotes.AppendLine(string.Format("Riwayat Pengobatan : "));
                sbNotes.AppendLine(string.Format(" {0}   ", entity.PastMedicationHistory));
            }

            string subjectiveText = sbNotes.ToString();
            entityCPPT.SubjectiveText = subjectiveText;

            entityCPPT.ObjectiveText = objectiveText.ToString();

            string assessmentText = diagnoseText.ToString();
            entityCPPT.AssessmentText = assessmentText;

            if (chkIsCopyInstruction.Checked)
            {
                //string filterExpression = string.Format("VisitID = {0} AND IsDeleted = 0", visitID);
                //PatientVisitNote oPatientVisitNote = BusinessLayer.GetPatientVisitNoteList(filterExpression, ctx).LastOrDefault();

                //if (oPatientVisitNote != null)
                //{
                //    string instructionText = oPatientVisitNote.InstructionText;

                //    entityCPPT.InstructionText = string.Format("{0}", instructionText);
                //}

                string intruction = "";
                foreach (RepeaterItem itemEquipment in rptPatientInstruction.Items)
                {
                    var hiddenField = itemEquipment.FindControl("hdnDescription") as HiddenField;
                    intruction += hiddenField.Value;
                }

                if (!String.IsNullOrEmpty(intruction))
                {
                    entityCPPT.InstructionText = string.Format("{0}", intruction);
                }
            }

            entityCPPT.NoteText = string.Format(@"S:{0}{1}{2}O:{3}{4}{5}A:{6}{7}{8}",
                Environment.NewLine, subjectiveText, Environment.NewLine,
                Environment.NewLine, objectiveText, Environment.NewLine,
                Environment.NewLine, assessmentText, Environment.NewLine);
            ...
            {
                hdnPatientVisitNoteID.Value = patientVisitNoteDao.InsertReturnPrimaryKeyID(entityCPPT).ToString();
            }
            else
            {
                patientVisitNoteDao.Update(entityCPPT);
            }
