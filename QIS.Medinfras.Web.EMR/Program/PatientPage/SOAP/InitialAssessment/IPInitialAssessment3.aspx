<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage/MPPatientPageList.master"
    AutoEventWireup="true" CodeBehind="IPInitialAssessment3.aspx.cs" Inherits="QIS.Medinfras.Web.EMR.Program.IPInitialAssessment3" %>
    ...
    <script type="text/javascript" id="dxss_erpatientstatus1">
        $(function () {
            ...
            $('#<%=btnPatientStatusSave.ClientID %>').click(function (evt) {
                if (UserLoginParamedicIsRegisteredPhysician()) {
                    if (OldRegisteredPhysicianHasMadeChiefComplaint()) {
        ...
        function UserLoginParamedicIsRegisteredPhysician() {
            var isValid = true;
            var userParamedicID = <%=HttpUtility.HtmlEncode(GetUserParamedicID())%>;
            var registeredPhysician = <%=HttpUtility.HtmlEncode(GetRegisteredPhysician())%>;
            if (userParamedicID != registeredPhysician) {
                isValid = false;
            }
            if (!isValid) {
                ShowSnackbarError("Tidak bisa melanjutkan proses. Anda bukan Dokter DPJP Utama untuk registrasi ini.");
            }
            return isValid;
        }

        function OldRegisteredPhysicianHasMadeChiefComplaint() {
            var isValid = true;
            var userParamedicID = <%=HttpUtility.HtmlEncode(GetUserParamedicID())%>;
            var registeredPhysician = <%=HttpUtility.HtmlEncode(GetRegisteredPhysician())%>;
            var chiefComplaintID = $('#<%=hdnChiefComplaintID.ClientID %>').val();
            var paramedicChiefComplaintID = $('#<%=hdnChiefComplaintParamedicID.ClientID %>').val();
            if (chiefComplaintID != "" && chiefComplaintID != "0") {
                if (paramedicChiefComplaintID != registeredPhysician) {
                    isValid = false;
                }
            }
            if (!isValid) {
                ShowSnackbarError("Tidak bisa melanjutkan proses. Dokter DPJP Utama sebelumnya sudah mengisi kajian awal.");
            }
            return isValid;
        }
        ...
        $('#lblChiefComplaint').live('click', function (evt) {
            if (UserLoginParamedicIsRegisteredPhysician()) {
                if (OldRegisteredPhysicianHasMadeChiefComplaint()) {
                    ...
                    openUserControlPopup("~/libs/Controls/EMR/Lookup/SOAPTemplateLookupCtl1.ascx", param, "Template Text : Chief Complaint", 700, 500);
                }
            }
        });
        ...
        function onTxtChiefComplaintChanged(value) {
            if (value.length <= 6 && value.slice(-1) == ";") {
                ...
                Methods.getObject('GetvPhysicianTextList', filterExpression, function (obj) {
                    if (obj != null)
                        if ($('#<%=txtChiefComplaint.ClientID %>').val() != '') {
                            var message1 = "Ganti catatan di keluhan utama dengan teks dari template ?";
        ...
        $('#lblHPI').live('click', function (evt) {
            if (UserLoginParamedicIsRegisteredPhysician()) {
                if (OldRegisteredPhysicianHasMadeChiefComplaint()) {
                    var visitNoteID = 0;
                    var param = "X058^02|" + visitNoteID + "|1|1";
                    openUserControlPopup("~/libs/Controls/EMR/Lookup/SOAPTemplateLookupCtl1.ascx", param, "Template Text : HPI", 700, 500);
                }
            }
        });
        ...
        function onTxtHPIChanged(value) {
            if (value.length <= 6 && value.slice(-1) == ";") {
                ...
                Methods.getObject('GetvPhysicianTextList', filterExpression, function (obj) {
                    if (obj != null)
                        if ($('#<%=txtHPISummary.ClientID %>').val() != '') {
                            var message1 = "Ganti catatan di keluhan yang menyertai dengan teks dari template ?";
                            var message2 = "<i>"+obj.TemplateText+"</i>";
                            displayConfirmationMessageBox('TEMPLATE TEXT', message1+"<br/><br/>"+message2, function (result) {
                                if (result) {
                                    $('#<%=txtHPISummary.ClientID %>').val(obj.TemplateText);
                                    $('#<%=txtHPISummary.ClientID %>').focus();
                                }
                            });
                        }
                });
            ...
            if (UserLoginParamedicIsRegisteredPhysician()) {
                if (OldRegisteredPhysicianHasMadeChiefComplaint()) {
                    if ($('#<%=hdnPatientVisitNoteID.ClientID %>').val() != "0") {
                        ...
                        displayMessageBox("Kajian Awal Rawat Inap","Mohon simpan data terlebih dahulu, sebelum melakukan proses ini.");
                    }
                }
            }
        });
        ...
        $('.imgEditVitalSign.imgLink').live('click', function () {
            if (UserLoginParamedicIsRegisteredPhysician()) {
                if (OldRegisteredPhysicianHasMadeChiefComplaint()) {
                    ...
                    $('#<%=hdnVitalSignRecordID.ClientID %>').val($tr.find('.keyField').html());
                    var visitNoteID = <%=HttpUtility.HtmlEncode(GetVisitNoteID())%>;
            ...
            if (UserLoginParamedicIsRegisteredPhysician()) {
                if (OldRegisteredPhysicianHasMadeChiefComplaint()) {
                    if ($('#<%=hdnPatientVisitNoteID.ClientID %>').val() != "0") {
                        ...
                        openUserControlPopup("~/libs/Controls/EMR/Lookup/ROSLookupCtl1.ascx", param, "Review of System", 700, 500);            
                    }
                    else {
                        displayMessageBox("Kajian Awal Rawat Inap","Mohon simpan data terlebih dahulu, sebelum melakukan proses ini.");
                    }
        ...
        $('.imgEditROS.imgLink').live('click', function () {
            if (UserLoginParamedicIsRegisteredPhysician()) {
                if (OldRegisteredPhysicianHasMadeChiefComplaint()) {
                    ...
                    var visitNoteID = <%=HttpUtility.HtmlEncode(GetVisitNoteID())%>;
                    var param = $('#<%=hdnReviewOfSystemID.ClientID %>').val() + "|" + visitNoteID + "|1|1";       
                    openUserControlPopup("~/Program/PatientPage/_PopupEntry/ROSEntry1Ctl.ascx", param, "Pemeriksaan Fisik", 700, 500);
                }
            }
        });
        ...
        function onCbpDiagnosisViewEndCallback(s) {
            ...
            $('#<%=hdnAssessmentText.ClientID %>').val(diagnosis);
        }
            ...
            <table class="tblContentArea">
                ...
                <tr>
                    ...
                    <td style="vertical-align: top">
                        ...
                        <h4 class="h4collapsed" id="mstTitle" runat="server">
                            <%=GetLabel("MST")%></h4>
                        <div class="containerTblEntryContent" id="mstContent" runat="server">
                            <table border="0" cellpadding="1" cellspacing="0" width="100%" style="margin-top: 5px">
                                <colgroup>
                                    <col width="210px" />
                                    <col />
                                ...
                                <tr>
                                    <td>
                                        <asp:RadioButtonList ID="rblIsFoodIntakeChanged" runat="server" RepeatDirection="Horizontal">
                                            <asp:ListItem Text="Tidak" Selected Value="0" />
                        ...
                        <div class="containerTblEntryContent">
                            <table border="0" cellpadding="1" cellspacing="0" width="100%" style="margin-top: 5px">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <asp:TextBox ID="txtEstimatedLOS" runat="server" Width="60px" CssClass="number" />
                                    </td>
                                    <td>
                                        <asp:RadioButtonList ID="rblEstimatedLOSUnit" runat="server" RepeatDirection="Horizontal">
                                            <asp:ListItem Text=" hari" Selected Value="1" />
                                ...
                                <tr>
                                    ...
                                    <td colspan="2">
                                        <asp:RadioButtonList ID="rblIsNeedDischargePlan" runat="server" RepeatDirection="Horizontal">
                                            <asp:ListItem Text="Ya" Selected Value="1" />
                                            <asp:ListItem Text="Tidak" Value="0" />
                                        </asp:RadioButtonList>
                                    </td>
                                </tr>
