<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage/MPPatientPageList.master"
    AutoEventWireup="true" CodeBehind="DentistForm1.aspx.cs" Inherits="QIS.Medinfras.Web.EMR.Program.DentistForm1" %>
...
//////            location.reload(true);
        ...
        //#region Dental Chart Button
        $('.imgAddTreatment.imgLink').die('click');
        $('.imgAddTreatment.imgLink').live('click', function (evt) {
            addToothTreatment();
        });

        $('.imgEditTreatment.imgLink').die('click');
        $('.imgEditTreatment.imgLink').live('click', function (evt) {
            var recordID = $(this).attr('recordID');
            var paramedicID = $(this).attr('paramedicID');

            if ($('#<%=hdnCurrentParamedicID.ClientID %>').val() == paramedicID) {
                var url = ResolveUrl("~/Program/PatientPage/_PopupEntry/DentalChart/DentalChartEntryCtl1.ascx");
                var param = "edit" + "|" + recordID;
                openUserControlPopup(url, param, "Treatment Gigi", 700, 500);
            }
            else {
                displayErrorMessageBox('Dental Chart', 'Maaf, tidak diijinkan mengedit pengkajian user lain.');
                return false;
            }
        });

        $('.imgDeleteTreatment.imgLink').die('click');
        $('.imgDeleteTreatment.imgLink').live('click', function () {
            var recordID = $(this).attr('recordID');
            var paramedicID = $(this).attr('paramedicID');

            if ($('#<%=hdnCurrentParamedicID.ClientID %>').val() == paramedicID) {
                var message = "Hapus catatan perawatan gigi untuk pasien ini ?";
                displayConfirmationMessageBox("Dental Chart", message, function (result) {
                    if (result) {
                        cbpDeleteDentalTreatment.PerformCallback(recordID);
                    }
                });
            }
            else {
                displayErrorMessageBox('Dental Chart', 'Maaf, tidak diijinkan menghapus pengkajian user lain.');
                return false;
            }
        });


        function addToothTreatment() {
            var allow = true;
            if (allow) {
                var url = ResolveUrl("~/Program/PatientPage/_PopupEntry/DentalChart/DentalChartEntryCtl1.ascx");
                var param = "add" + "|" + $('#<%=hdnToothID.ClientID %>').val();
                openUserControlPopup(url, param, "Treatment Gigi", 700, 500);
            }
        }

        function onCbpDeleteDentalTreatmentEndCallback(s) {
            var param = s.cpResult.split('|');
            if (param[0] == 'success') {
                cbpListTooth.PerformCallback();
            }
            else {
                displayErrorMessageBox("Dental Chart", param[1]);
            }
        }
        //#endregion   
    ...
    <div>
                                ...
                                <tr>
                                    <td>
                                        <table style="width:100%">
                                            ...
                                            <tr>
                                                <td>
                                                    <img src='<%=ResolveUrl("~/Libs/Images/warning.png")%>' alt="" height=35px" width="35px" />
                                                </td>
                                                <td style="vertical-align:top;">
                                                    <h4 style="background-color:transparent;color:red;font-weight:bold"><%=GetLabel("Mohon simpan dulu perubahan pengisian asesmen, jika belum disimpan")%></h4>
                                                    Sebelum melakukan proses pemindahan/klik gambar gigi
                                                </td>
                                            </tr>
                                        </table>
                                    </td>
                                        ...
                                        <dxcp:ASPxCallbackPanel ID="cbpTreatmentDetail" runat="server" Width="100%" ClientInstanceName="cbpTreatmentDetail"
                                            ...
                                            <PanelCollection>
                                                <dx:PanelContent ID="PanelContent12" runat="server">
                                                    <asp:Panel runat="server" ID="Panel12" CssClass="pnlContainerGrid" Style="height:300px">
                                                        <asp:GridView ID="grdTreatmentDetail" runat="server" CssClass="grdSelected grdPatientPage" AutoGenerateColumns="false" ShowHeaderWhenEmpty="true" EmptyDataRowStyle-CssClass="trEmpty">
                                                            <Columns>
                                                                ...
                                                                <asp:TemplateField HeaderStyle-HorizontalAlign="Center" HeaderStyle-Width="50px">
                                                                    <HeaderTemplate>
                                                                       <img class="imgAddTreatment imgLink" title='<%=GetLabel("+ Treatment")%>' src='<%# ResolveUrl("~/Libs/Images/Button/plus.png")%>' alt="" />
                                                                    </HeaderTemplate>
                                                                    <ItemTemplate>
                                                                        <table cellpadding="0" cellspacing="0">
                                                                            <tr>
                                                                                <td>
                                                                                    <img class="imgEditTreatment imgLink" title='<%=GetLabel("Edit")%>' src='<%# ResolveUrl("~/Libs/Images/Button/edit.png")%>'
                                                                                ...
                                                                                <td>
                                                                                    <img class="imgDeleteTreatment imgLink" title='<%=GetLabel("Delete")%>' src='<%# ResolveUrl("~/Libs/Images/Button/delete.png")%>'
                                                                                        alt="" recordID = "<%#:Eval("ID") %>" paramedicID = "<%#:Eval("ParamedicID") %>"  />
                                                                                </td>
                                                                            </tr>
                                                                        </table>
                                                                ...
                                                                <asp:TemplateField HeaderStyle-HorizontalAlign="Left">
                                                                    <HeaderTemplate>
                                                                        <div class="thColumnCaption"><%=GetLabel("Dental Treatment") %></div>                                                         
                                                                    </HeaderTemplate>
                                                                    <ItemTemplate>
                                                                        <div><%#: Eval("ToothStatus")%></div>
                                                                        <div><%#: Eval("ToothProblem")%></div>
                                                            ...
                                                            <EmptyDataTemplate>
                                                                <%=GetLabel("Belum ada riwayat pencatatan perawatan gigi") %>
                                                            </EmptyDataTemplate>
                                                        </asp:GridView>
                                                    </asp:Panel>
                                                </dx:PanelContent>
                                        ...
                                        <td>
                                            <table border="0" cellpadding="1" cellspacing="0">
                                                ...
                                                <tr id="trWriteInstructionCPPT" runat="server">
                                                    <td colspan="2">
                                                        <input type="button" id="btnWriteInstructionCPPT" runat="server" class="btnWriteInstructionCPPT w3-btn w3-hover-blue" value="Salin ke CPPT"
                                                            style='margin-top:5px; background-color: Red; color: White;' />
                                                    </td>
                                                </tr>
                                            </table>
                                        </td>
        ...
        <div style="display: none">
            <dxcp:ASPxCallbackPanel ID="cbpDeleteDentalTreatment" runat="server" Width="100%" ClientInstanceName="cbpDeleteDentalTreatment"
                ShowLoadingPanel="false" OnCallback="cbpDeleteDentalTreatment_Callback">
                <ClientSideEvents EndCallback="function(s,e){ onCbpDeleteDentalTreatmentEndCallback(s); }" />
            </dxcp:ASPxCallbackPanel>
        </div>
    </div>
                    ...
                    <dxcp:ASPxCallbackPanel ID="cbpCopyProcess" runat="server" Width="100%" ClientInstanceName="cbpCopyProcess"
                        ...
                        <PanelCollection>
                            <dx:PanelContent ID="PanelContent111" runat="server">
                                <asp:Panel ID="Panel1" Style="width: 100%;" runat="server" Height="250px">
                                    <table class="tblContentArea">
                                        <tr>
                                            <td>
