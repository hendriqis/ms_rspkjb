...
{
    public partial class OncologistOPAssessment1 : BasePagePatientPageList
    {
        ...
        protected int gridCTSimulatorViewPageCount = 1;
        protected int gridRTProgramViewPageCount = 1;
        ...
        {
            return Constant.MenuCode.EMR.OUTPATIENT_SOAP_ONCOLOGIST_ASSESSMENT;
        }
        ...
        protected override void InitializeDataControl()
        {
            ...
            {
                Helper.SetControlEntrySetting(txtServiceDate, new ControlEntrySetting(true, false, true), "mpPatientStatus");
                Helper.SetControlEntrySetting(txtServiceTime, new ControlEntrySetting(true, false, true), "mpPatientStatus");
                txtServiceDate.Attributes.Add("readonly", "readonly");
                txtServiceTime.Attributes.Add("readonly", "readonly");
            }
            else
            {
                Helper.SetControlEntrySetting(txtServiceDate, new ControlEntrySetting(true, true, true), "mpPatientStatus");
                Helper.SetControlEntrySetting(txtServiceTime, new ControlEntrySetting(true, true, true), "mpPatientStatus");
            }
            ...
            if (entityVisit.MRN == AppSession.RegisteredPatient.MRN)
            {
                ...
                BindGridViewCTSimulatorView(1, true, ref gridCTSimulatorViewPageCount);
                BindGridViewRTProgramView(1, true, ref gridRTProgramViewPageCount);
        ...
        private void SetSettingParameter()
        {
            ...
            string FilterExpressionDt = string.Format("HealthcareID = '{0}' AND ParameterCode IN ('{1}','{2}','{3}','{4}','{5}','{6}','{7}','{8}')",
                                                        ...
                                                        Constant.SettingParameter.EM0089
                                                    );
            ...
            hdnDisableServiceDateTime.Value = lstSetvarDt.Where(p => p.ParameterCode == Constant.SettingParameter.EM0089).FirstOrDefault().ParameterValue;
        ...
        private void SetEntityToControl(vConsultVisit1 entityVisit)
        {
            ...
            hdnPageMRN.Value = entityVisit.MRN.ToString();
            hdnPageMedicalNo.Value = entityVisit.MedicalNo;
            hdnPagePatientName.Value = entityVisit.cfPatientNameInLabel;
            hdnPageVisitID.Value = entityVisit.VisitID.ToString();
            ...
            hdnPatientInformation.Value = string.Format("{0} (MRN = {1}, REG = {2}, LOC = {3}, DOB = {4})", entityVisit.cfPatientNameInLabel, entityVisit.MedicalNo, entityVisit.RegistrationNo, entityVisit.ServiceUnitName, entityVisit.DateOfBirth.ToString(Constant.FormatString.DATE_FORMAT));
            ...
            vChiefComplaint obj = BusinessLayer.GetvChiefComplaintList(string.Format("VisitID = {0} AND IsDeleted = 0", hdnPageVisitID.Value)).FirstOrDefault();
            if (obj != null)
            {
                if (obj.ID == 0)
                {
                ...
                else
                {
                    ...
                    cboCancerStaging.Value = obj.GCCancerStaging;
                    txtTValue.Text = obj.TValue;
                    txtNValue.Text = obj.NValue;
                    txtMValue.Text = obj.MValue;
            ...
            vNurseChiefComplaint entity = BusinessLayer.GetvNurseChiefComplaintList(string.Format("VisitID = {0}", hdnPageVisitID.Value)).FirstOrDefault();
            ...
            #region Appointment
            Appointment oAppointment = BusinessLayer.GetAppointmentList(string.Format("FromVisitID  = {0}", hdnPageVisitID.Value)).FirstOrDefault();
            if (oAppointment != null)
            {
                ...
                vAppointment entityAppointment = BusinessLayer.GetvAppointmentList(string.Format("FromVisitID = {0}", hdnPageVisitID.Value)).FirstOrDefault();
            ...
            {
                LoadPreviousVisitInformation(Convert.ToInt32(hdnPageMRN.Value), true);
            }
            else
            {
            ...
            if (entityVisit.IsReferralVisit)
            {
                ...
                LoadSourceReferralVisitInfo(Convert.ToInt32(hdnPageVisitID.Value));
            }
            else
            {
        ...
        private bool OnSaveAppointment(ref string errMessage, ref int retval)
        {
            ...
            try
            {
                ...
                int visitID = Convert.ToInt32(hdnPageVisitID.Value);
        ...
        {
            string filterExpression = string.Format("MRN = {0} AND DepartmentID = '{1}' AND ParamedicID = {2} AND VisitID != {3} AND VisitID < {3} AND GCVisitStatus != '{4}' ORDER BY VisitID DESC", mrn, hdnDepartmentID.Value, AppSession.UserLogin.ParamedicID, hdnPageVisitID.Value, Constant.VisitStatus.CANCELLED);
        ...
        {
            string filterExpression = string.Format("VisitID = {0} AND IsDeleted = 0", hdnPageVisitID.Value);
        ...
        protected override void SetControlProperties()
        {
            string filterExpression = string.Format("ParentID IN ('{0}','{1}','{2}','{3}','{4}','{5}','{6}') AND IsActive = 1 AND IsDeleted = 0", 
                ...
                Constant.StandardCode.PATIENT_INSTRUCTION_GROUP,
                Constant.StandardCode.CANCER_STADIUM);
            ...
            Methods.SetComboBoxField(cboCancerStaging, lstSc.Where(sc => sc.ParentID == Constant.StandardCode.CANCER_STADIUM).ToList(), "StandardCodeName", "StandardCodeID");
        ...
        private string GenerateSOAPText()
        {
            ...
            sbNotes.AppendLine("-".PadRight(20, '-'));
            vVitalSignHd entityVitalSignHd = BusinessLayer.GetvVitalSignHdList(string.Format("VisitID = {0} AND PatientVisitNoteID = {1} AND IsInitialAssessment = 1 AND IsDeleted = 0", hdnPageVisitID.Value, visitNoteID)).FirstOrDefault();
            ...
            vReviewOfSystemHd entityReviewOfSystem = BusinessLayer.GetvReviewOfSystemHdList(string.Format("VisitID = {0} AND PatientVisitNoteID = {1} AND IsDeleted = 0", hdnPageVisitID.Value, visitNoteID)).FirstOrDefault();
            ...
            vPatientDiagnosis entityDiagnose = BusinessLayer.GetvPatientDiagnosisList(string.Format("VisitID = {0} AND IsDeleted = 0 AND IsNutritionDiagnosis = 0", hdnPageVisitID.Value)).FirstOrDefault();
            if (entityDiagnose != null)
            {
                string filterExpression = string.Format("VisitID = {0} AND IsDeleted = 0 AND IsNutritionDiagnosis = 0 ORDER BY GCDiagnoseType", hdnPageVisitID.Value);
        ...
        private string GenerateSOAPSummaryText(ref string subjectiveText, ref string objectiveText, ref string assessmentText, ref string planningText)
        {
            ...
            {
                vChiefComplaint oChiefComplaint = BusinessLayer.GetvChiefComplaintList(string.Format("VisitID = {0}", hdnPageVisitID.Value)).FirstOrDefault();
                if (oChiefComplaint != null)
                {
            ...
            List<vVitalSignDt> lstVitalSignDt = BusinessLayer.GetvVitalSignDtList(string.Format("VisitID = {0} AND ObservationDate = '{1}' AND IsDeleted = 0 ORDER BY DisplayOrder", hdnPageVisitID.Value, Helper.GetDatePickerValue(txtServiceDate).ToString(Constant.FormatString.DATE_FORMAT_112)));
            if (lstVitalSignDt.Count > 0)
            {
                foreach (vVitalSignDt vitalSign in lstVitalSignDt)
                {
            ...
            sbNotes = new StringBuilder();
            List<vReviewOfSystemDt> lstROS = BusinessLayer.GetvReviewOfSystemDtList(string.Format("ID IN (SELECT ID FROM ReviewOfSystemHd WHERE VisitID = {0} AND  ObservationDate = '{1}') AND IsDeleted = 0 ORDER BY GCRoSystem", hdnPageVisitID.Value, Helper.GetDatePickerValue(txtServiceDate).ToString(Constant.FormatString.DATE_FORMAT_112)));
            if (lstROS.Count > 0)
            {
                foreach (vReviewOfSystemDt item in lstROS)
                {
        ...
        private void UpdateConsultVisitRegistration(IDbContext ctx, ref string errMessage)
        {
            ...
            ConsultVisit entityConsultVisit = consultVisitDao.Get(Convert.ToInt32(hdnPageVisitID.Value));
            ...
            else
            {
                ...
                entity.VisitID = Convert.ToInt32(hdnPageVisitID.Value);
            ...
            if (cboCancerStaging.Value != null)
            {
                entity.GCCancerStaging = cboCancerStaging.Value.ToString();
                entity.TValue = txtTValue.Text;
                entity.NValue = txtNValue.Text;
                entity.MValue = txtMValue.Text;
            }
        ...
        protected override bool OnCustomButtonClick(string type, ref string errMessage)
        {
            ...
            try
            {
                ...
                if (isAllowSave)
                {
                      if (type == "save")
                    {
                        ChiefComplaint obj = BusinessLayer.GetChiefComplaintList(string.Format("VisitID = {0}", hdnPageVisitID.Value), ctx).FirstOrDefault();
                        if (obj != null)
                            hdnChiefComplaintID.Value = obj.ID.ToString();
                        else
                            hdnChiefComplaintID.Value = "0";
                        ...
                        sbNotes = new StringBuilder();
                        vVitalSignHd entityVitalSignHd = BusinessLayer.GetvVitalSignHdList(string.Format("VisitID = {0} AND IsDeleted = 0", hdnPageVisitID.Value)).FirstOrDefault();
                        if (entityVitalSignHd != null)
                        {
                            List<vVitalSignDt> lstVitalSignDt = BusinessLayer.GetvVitalSignDtList(string.Format("ID = {0} AND ObservationDate = '{1}' AND IsDeleted = 0 ORDER BY DisplayOrder", entityVitalSignHd.ID, Helper.GetDatePickerValue(txtServiceDate).ToString(Constant.FormatString.DATE_FORMAT_112)));
                            if (lstVitalSignDt.Count > 0)
                        ...
                        vReviewOfSystemHd entityReviewOfSystem = BusinessLayer.GetvReviewOfSystemHdList(string.Format("VisitID = {0} AND IsDeleted = 0", hdnPageVisitID.Value)).FirstOrDefault();
                        if (entityReviewOfSystem != null)
                        {
                            List<vReviewOfSystemDt> lstROS = BusinessLayer.GetvReviewOfSystemDtList(string.Format("ID = {0} AND IsDeleted = 0 ORDER BY GCRoSystem", entityReviewOfSystem.ID, Helper.GetDatePickerValue(txtServiceDate).ToString(Constant.FormatString.DATE_FORMAT_112)));
                            if (lstROS.Count > 0)
                        ...
                        if (!string.IsNullOrEmpty(hdnCTSimulatorSummaryText.Value))
                        {
                            sbNotes.AppendLine("Permintaan CT Simulator :");
                            sbNotes.AppendLine(hdnCTSimulatorSummaryText.Value);
                        }
                        if (!string.IsNullOrEmpty(hdnRTProgramSummaryText.Value))
                        {
                            sbNotes.AppendLine("Program Radiasi :");
                            sbNotes.AppendLine(hdnRTProgramSummaryText.Value);
                        }
                        ...
                        PatientVisitNote entityVisitNote = BusinessLayer.GetPatientVisitNoteList(string.Format("VisitID = {0} AND GCPatientNoteType = '{1}' AND CreatedBy = {2}", hdnPageVisitID.Value, Constant.PatientVisitNotes.SOAP_SUMMARY_NOTES, AppSession.UserLogin.UserID), ctx).FirstOrDefault();
                        ...
                        entityVisitNote.VisitID = Convert.ToInt32(hdnPageVisitID.Value);
                        ...
                        if (!string.IsNullOrEmpty(txtInstructionText.Text))
                        {
                            ...
                            if (oInstruction == null)
                            {
                                ...
                                oInstruction.VisitID = Convert.ToInt32(hdnPageVisitID.Value);
        ...
        private bool ProcessDischargePatient(IDbContext ctx, ref string errMessage)
        {
            if (IsValidToDischarge())
            {
                ...
                ConsultVisit entity = lstConsultVisit.Where(t => t.VisitID == Convert.ToInt32(hdnPageVisitID.Value)).FirstOrDefault();
        ...
        private bool IsValidToDischarge()
        {
            ...
            ChiefComplaint oChiefComplaint = BusinessLayer.GetChiefComplaintList(string.Format("VisitID = {0} AND IsDeleted = 0", hdnPageVisitID.Value)).FirstOrDefault();
            bool isChiefComplaintExist = oChiefComplaint != null;

            PatientDiagnosis oDiagnosis = BusinessLayer.GetPatientDiagnosisList(string.Format("VisitID = {0} AND IsDeleted = 0", hdnPageVisitID.Value)).FirstOrDefault();
        ...
        {
            string filterExpression = string.Format("MRN = {0} AND IsDeleted = 0", hdnPageMRN.Value);
        ...
        private void BindGridViewVitalSign(int pageIndex, bool isCountPageCount, ref int pageCount)
        {
            ...
            filterExpression += string.Format("VisitID = {0} AND IsDischargeVitalSign = 0 AND IsDeleted = 0", hdnPageVisitID.Value);
            ...
            lstVitalSignDt = BusinessLayer.GetvVitalSignDtList(string.Format("VisitID = {0} ORDER BY DisplayOrder", hdnPageVisitID.Value));
            ...
            try
            {
                if (e.Parameter != null && e.Parameter != "")
                {
                    ...
                    if (param[0] == "add")
                    {
                        ...
                        oAllergy.MRN = Convert.ToInt32(hdnPageMRN.Value);
                        ...
                        Patient oPatient = patientDao.Get(Convert.ToInt32(hdnPageMRN.Value));
                    ...
                    else
                    {
                        ...
                        if (oAllergy != null)
                        {
                            ...
                            List<PatientAllergy> lstAllergy = BusinessLayer.GetPatientAllergyList(string.Format("MRN = {0} AND IsDeleted = 0", hdnPageMRN.Value), ctx);
        ...
        private void BindGridViewROS(int pageIndex, bool isCountPageCount, ref int pageCount)
        {
            ...
            lstReviewOfSystemDt = BusinessLayer.GetvReviewOfSystemDtList(string.Format("VisitID = {0} ORDER BY TagProperty", hdnPageVisitID.Value));
        ...
        {
            string filterExpression = string.Format("VisitID = {0} AND IsDeleted = 0 AND IsNutritionDiagnosis = 0", hdnPageVisitID.Value);
        ...
        {
            string filterExpression = string.Format("VisitID = {0} AND TransactionCode  = {1} AND GCTransactionStatus != '{2}'", hdnPageVisitID.Value, Constant.TransactionCode.LABORATORY_TEST_ORDER, Constant.TransactionStatus.VOID);
        ...
        {
            string filterExpression = string.Format("VisitID = {0} AND HealthcareServiceUnitID = {1} AND GCTransactionStatus != '{2}'", hdnPageVisitID.Value, hdnImagingServiceUnitID.Value, Constant.TransactionStatus.VOID);
        ...
        {
            string filterExpression = string.Format("VisitID = {0} AND HealthcareServiceUnitID NOT IN ({1},{2}) AND GCTransactionStatus != '{3}'", hdnPageVisitID.Value, hdnLaboratoryServiceUnitID.Value, hdnImagingServiceUnitID.Value, Constant.TransactionStatus.VOID);
        ...
        #region CT Simulator
        protected void cbpCTSimulatorView_Callback(object sender, DevExpress.Web.ASPxClasses.CallbackEventArgsBase e)
        {
            int pageCount = 1;
            string result = "";
            if (e.Parameter != null && e.Parameter != "")
            {
                ...
                if (param[0] == "changepage")
                {
                    BindGridViewCTSimulatorView(Convert.ToInt32(param[1]), false, ref pageCount);
                    result = "changepage";
                }
                else // refresh
                {

                    BindGridViewCTSimulatorView(1, true, ref pageCount);
                    result = "refresh|" + pageCount + '|' + hdnCTSimulatorSummaryText.Value;
                }
            }

            ASPxCallbackPanel panel = sender as ASPxCallbackPanel;
            panel.JSProperties["cpResult"] = result;
            panel.JSProperties["cpSummary"] = hdnCTSimulatorSummaryText.Value;
        }

        private void BindGridViewCTSimulatorView(int pageIndex, bool isCountPageCount, ref int pageCount)
        {
            string filterExpression = string.Format("VisitID = {0} AND IsDeleted = 0", hdnPageVisitID.Value);
            ...
            List<vCTSimulationRequest> lstEntity = BusinessLayer.GetvCTSimulationRequestList(filterExpression);
            StringBuilder sbText = new StringBuilder();
            foreach (vCTSimulationRequest item in lstEntity)
            {
                sbText.AppendLine(string.Format("- {0}, Scan Area : {1}", item.SimulationRequestType, item.cfScanArea));
            }
            hdnCTSimulatorSummaryText.Value = sbText.ToString();

            grdCTSimulatorView.DataSource = lstEntity;
            grdCTSimulatorView.DataBind();
        }

        protected void cbpCTSimulation_Callback(object sender, DevExpress.Web.ASPxClasses.CallbackEventArgsBase e)
        {
            string result = "1|";
            ...
            if (e.Parameter != null && e.Parameter != "")
            {
                ...
                string mode = paramInfo[0];
                string recordID = paramInfo[1];

                CTSimulationRequest entity;

                switch (mode)
                {
                    case "delete":
                        entity = BusinessLayer.GetCTSimulationRequest(Convert.ToInt32(recordID));
                        entity.IsDeleted = true;
                        entity.LastUpdatedBy = AppSession.UserLogin.UserID;
                        BusinessLayer.UpdateCTSimulationRequest(entity);
                        result = string.Format("1|{0}", e.Parameter);
                        break;
                    case "send":
                        entity = BusinessLayer.GetCTSimulationRequest(Convert.ToInt32(recordID));
                        entity.GCRequestStatus = Constant.TransactionStatus.APPROVED;
                        entity.LastUpdatedBy = AppSession.UserLogin.UserID;
                        BusinessLayer.UpdateCTSimulationRequest(entity);
                        result = string.Format("1|{0}", e.Parameter);
                        break;
        ...
        #region RT Program
        protected void cbpRTProgramView_Callback(object sender, DevExpress.Web.ASPxClasses.CallbackEventArgsBase e)
        {
            int pageCount = 1;
            string result = "";
            if (e.Parameter != null && e.Parameter != "")
            {
                ...
                if (param[0] == "changepage")
                {
                    BindGridViewRTProgramView(Convert.ToInt32(param[1]), false, ref pageCount);
                    result = "changepage";
                }
                else // refresh
                {

                    BindGridViewRTProgramView(1, true, ref pageCount);
                    result = "refresh|" + pageCount + '|' + hdnCTSimulatorSummaryText.Value;
                }
            }
            ...
            ASPxCallbackPanel panel = sender as ASPxCallbackPanel;
            panel.JSProperties["cpResult"] = result;
            panel.JSProperties["cpSummary"] = hdnRTProgramSummaryText.Value;
        }

        private void BindGridViewRTProgramView(int pageIndex, bool isCountPageCount, ref int pageCount)
        {
            string filterExpression = string.Format("VisitID = {0} AND IsDeleted = 0", hdnPageVisitID.Value);
            ...
            List<vRadiotheraphyProgram> lstEntity = BusinessLayer.GetvRadiotheraphyProgramList(filterExpression);
            StringBuilder sbText = new StringBuilder();
            foreach (vRadiotheraphyProgram item in lstEntity)
            {
                sbText.AppendLine(string.Format("- {0}", item.TherapyType));
            }
            hdnRTProgramSummaryText.Value = sbText.ToString();

            grdRTProgramView.DataSource = lstEntity;
            grdRTProgramView.DataBind();
        }

        protected void cbpRTProgram_Callback(object sender, DevExpress.Web.ASPxClasses.CallbackEventArgsBase e)
        {
            string result = "1|";
            ...
            if (e.Parameter != null && e.Parameter != "")
            {
                ...
                RadiotherapyProgram entity;

                switch (mode)
                {
                    case "delete":
                        string filterExpressionLog = string.Format("ProgramID = {0} AND IsDeleted = 0", recordID);
                        List<RadiotheraphyProgramLog> lstLog = BusinessLayer.GetRadiotheraphyProgramLogList(filterExpressionLog);
                        if (lstLog.Count == 0)
                        {
                            entity = BusinessLayer.GetRadiotherapyProgram(Convert.ToInt32(recordID));
                            entity.IsDeleted = true;
                            entity.LastUpdatedBy = AppSession.UserLogin.UserID;
                        ...
                        {
                            result = string.Format("0|{0}", "Program tidak bisa dihapus karena sudah memiliki catatan log penyinaran.");
                        }
                        break;
                    case "send":
                        entity = BusinessLayer.GetRadiotherapyProgram(Convert.ToInt32(recordID));
                        entity.GCProgramStatus = Constant.TransactionStatus.APPROVED;
                        entity.LastUpdatedBy = AppSession.UserLogin.UserID;
                        BusinessLayer.UpdateRadiotherapyProgram(entity);
                        result = string.Format("1|{0}", e.Parameter);
                        break;
        ...
        private void CopyFromPreviousVisit(IDbContext ctx)
        {
            ...
            if (chkIsCopyROS.Checked)
            {
                ...
                foreach (vReviewOfSystemHd ros in lstReviewOfSystemHd)
                {
                    ...
                    entityROS.VisitID = Convert.ToInt32(hdnPageVisitID.Value);
            ...
            if (chkIsCopyVitalSign.Checked)
            {
                ...
                if (oVitalSignHd != null)
                {
                    ...
                    entityVS.VisitID = Convert.ToInt32(hdnPageVisitID.Value);
            ...
            if (chkIsCopyDiagnosis.Checked)
            {
                ...
                if (lstDiagnosis.Count > 0)
                {
                    foreach (vPatientDiagnosis item in lstDiagnosis)
                    {
                        ...
                        oDiagnosis.VisitID = Convert.ToInt32(hdnPageVisitID.Value);
            ...
            if (chkIsCopyInstruction.Checked)
            {
                ...
                else
                {
                    ...
                    entityInstruction.VisitID = Convert.ToInt32(hdnPageVisitID.Value);
        ...
        protected void cbpIntegrationNote_Callback(object sender, DevExpress.Web.ASPxClasses.CallbackEventArgsBase e)
        {
            ...
            else
            {
                ...
                try
                {
                    PatientVisitNote entityOutpatientCaseNote = BusinessLayer.GetPatientVisitNoteList(string.Format("VisitID = {0} AND GCPatientNoteType = '{1}'", hdnPageVisitID.Value, Constant.PatientVisitNotes.SOAP_SUMMARY_NOTES), ctx).FirstOrDefault();
                    ...
                    string filterExpression = string.Format("VisitID IN ({0}) AND PatientVisitNoteID = {1} AND IsDeleted = 0 ORDER BY PatientInstructionID", hdnPageVisitID.Value, _visitNoteID);
                    ...
                    {
                        entityOutpatientCaseNote.VisitID = Convert.ToInt32(hdnPageVisitID.Value);
