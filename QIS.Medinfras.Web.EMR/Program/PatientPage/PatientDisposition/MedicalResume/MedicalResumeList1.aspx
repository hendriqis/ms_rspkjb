<%@ Page Language="C#" MasterPageFile="~/MasterPage/MPPatientPageList.master" AutoEventWireup="true"
    CodeBehind="MedicalResumeList1.aspx.cs" Inherits="QIS.Medinfras.Web.EMR.Program.MedicalResumeList1" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="plhList" runat="server">
    <script id="dxis_IPInitialAssessmentListctl1" src='<%= ResolveUrl("~/Libs/Scripts/jquery/ui/jquery-ui.js")%>'
    ...
    <script type="text/javascript" id="dxss_IPInitialAssessmentList">
        $(function () {
            ...
            $('.btnRevision').live('click', function () {
                ...
                var message = "Lakukan koreksi resume medis pasien ?";
                displayConfirmationMessageBox('KOREKSI RESUME MEDIS :', message, function (result) {
                    if (result) {
                        var url = ResolveUrl("~/libs/Controls/EMR/MedicalResume/MedicalResumeRevisionNoteCtl1.ascx");
                        var param = visitID + "|" + id;
                        openUserControlPopup(url, param, 'KOREKSI RESUME MEDIS', 500, 350);
                    }
                });
            });
            ...
            $('.btnView').live('click', function () {
                ...
                if ($('#<%=hdnDepartmentID.ClientID %>').val() != Constant.Facility.INPATIENT) {
                    url = ResolveUrl("~/libs/Controls/EMR/MedicalResume/OPMedicalResumeCtl1.ascx");
                }

                var param = visitID + '|' + id;
                openUserControlPopup(url, param, 'Resume Medis', 1300, 600);
            });
            ...
            $('.btnComplete').click(function (evt) {
                ...
                if ($('#<%=hdnID.ClientID %>').val() != '') {
                    if (($('#<%=hdnCurrentParamedicID.ClientID %>').val() == $('#<%=hdnParamedicID.ClientID %>').val())
                        || ($('#<%=hdnCurrentParamedicID.ClientID %>').val() == $('#<%=hdnRevisedParamedicID.ClientID %>').val())) {
                        var message = "Proses resume medis menjadi selesai (completed) ?";
                        displayConfirmationMessageBox('STATUS RESUME MEDIS :', message, function (result) {
                            if (result) {
                        ...
                        var casemixAllowChange = $('#<%=hdnIsCasemixAllowChange.ClientID %>').val();
                        if (casemixAllowChange == "1") {
                            var message = "Proses resume medis menjadi selesai (completed) ?";
                            displayConfirmationMessageBox('STATUS RESUME MEDIS :', message, function (result) {
                                if (result) {
                                    cbpCompleted.PerformCallback($('#<%=hdnID.ClientID %>').val());
                            ...
                            displayErrorMessageBox('Warning', 'Maaf, Resume Medis hanya bisa ditandai selesai oleh PPA yang mengkaji atau mengkoreksi.');
                        }
                    }
                } else {
                    displayErrorMessageBox('Warning', 'Maaf, Resume Medis tidak ditemukan.');
                }
            });
            //#endregion
        ...
        $('.imgView.imgLink').die('click');
        $('.imgView.imgLink').live('click', function () {
        ...
        function onBeforeBasePatientPageListEdit() {
            if ($('#<%=hdnID.ClientID %>').val() == 0 || $('#<%=hdnID.ClientID %>').val() == "") {
                displayErrorMessageBox('Warning', 'Maaf, Tidak ada Resume Medis yang dapat diedit.');
                return false;
            }
            else {
        ...
        function onBeforeRightPanelPrint(code, filterExpression, errMessage) {
            ...
            if (hdnID == '' || hdnID == '0') {
                ...
                else if (code == 'MR000013' || code == 'MR000017' || code == 'MR000009' || code == 'MR000050' || code == 'MR000052' || code == 'MR000042' || code == 'MR000055' || code == 'MR000058') {
            ...
            else if (code == 'MR000013' || code == 'MR000014' || code == 'MR000017' || code == 'MR000019' || code == 'PM-90008' ||
                     code == 'PM-90009' || code == 'PM-90010' || code == 'PM-90011' || code == 'PM-90012' || code == 'PM-90013' ||
                     code == 'PM-90014' || code == 'PM-90015' || code == 'PM-90019' || code == 'PM-00575' || code == 'PM-90020' || code == 'PM-900201' ||
                     code == 'PM-90022' || code == 'PM-90023' || code == 'PM-90035' || code == 'PM-90040' || code == 'PM-90017' ||
                     code == 'PM-90063' || code == 'MR000009' || code == "PM-90056" || code == 'MR000050' || code == 'MR000052' || code == 'MR000042' ||
                     code == 'MR000055' || code == 'MR000058' || code == 'PM-90063' || code == 'PM-90065' || code == 'PM-90085' || code == 'PM-90114' ||
                    ...
                    <asp:Panel runat="server" ID="pnlView" CssClass="pnlContainerGridPatientPage">
                        <asp:GridView ID="grdView" runat="server" CssClass="grdSelected grdPatientPage" AutoGenerateColumns="false"
                            ...
                            <Columns>
                                ...
                                <asp:TemplateField HeaderStyle-HorizontalAlign="Left" ItemStyle-Width="20px">
                                    <ItemTemplate>
                                        <table cellpadding="0" cellspacing="0">
                                            <tr>
                                                <td>
                                                    <img class="imgView imgLink" title='<%=GetLabel("View Resume Medis")%>' src='<%# ResolveUrl("~/Libs/Images/Button/search.png")%>'
                                ...
                                <asp:BoundField DataField="RevisedByParamedicID" HeaderStyle-CssClass="hiddenColumn"
                                    ItemStyle-CssClass="hiddenColumn revisedByParamedicID" />
                                ...
                                <asp:TemplateField HeaderText="Resume Medis" HeaderStyle-HorizontalAlign="Left" ItemStyle-HorizontalAlign="Left">
                                    <ItemTemplate>
                                        <div <%# Eval("IsRevised").ToString() == "True" ? "Style='color:red;font-style:italic; text-decoration: line-through'":"style='height: 130px; overflow-y: auto;'" %>>
                                ...
                                <asp:TemplateField HeaderText="" HeaderStyle-HorizontalAlign="Left" ItemStyle-HorizontalAlign="Left"
                                    ...
                                    <ItemTemplate>
                                        <div id="divCompleted" runat="server" style='margin-top: 5px; text-align: center;'>
                                            <input type="button" id="btnComplete" runat="server" class="btnComplete w3-btn w3-hover-blue"
                                                value="Complete" style='<%# Eval("cfIsCompleted").ToString() == "True" || Eval("IsRevised").ToString() == "True" ? "display:none;": "width: 100px; background-color: Red; color: White;" %>' />
                                        </div>
                                        <div id="divView" runat="server" style='margin-top: 5px; text-align: center'>
                                            <input type="button" id="btnView" runat="server" class="btnView w3-btn w3-hover-blue"
                                                value="Lihat" style='<%# Eval("cfIsCompleted").ToString() == "False" ? "display:none;": "width: 100px; background-color: Green; color: White;" %>' />
                                            <div style="<%# Eval("cfIsAllowRevision").ToString() == "False" ? "display:none": "" %>">
                                                <input type="button" id="btnRevision" runat="server" class="btnRevision w3-btn w3-hover-blue"
                                                    value="Koreksi" style='<%# Eval("cfIsCompleted").ToString() == "False" ? "display:none;": "margin-top:5px; width: 100px; background-color: Red; color: White;" %>' />
                                            </div>
                                        </div>
                                    </ItemTemplate>
                                </asp:TemplateField>
                            ...
                            <EmptyDataTemplate>
                                <%=GetLabel("Tidak ada data Resume Medis untuk pasien ini") %>
                            </EmptyDataTemplate>
                        </asp:GridView>
                    </asp:Panel>
                </dx:PanelContent>
