<%@ Page Title="" Language="C#" MasterPageFile="~/libs/MasterPage/MPEntry.master"
    AutoEventWireup="true" CodeBehind="GLAPPaymentEntry.aspx.cs" Inherits="QIS.Medinfras.Web.Accounting.Program.GLAPPaymentEntry" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="plhEntry" runat="server">
    <script type="text/javascript">
        function onLoad() {
            //#region Kolom Bank mandatory / tidak
            var suppPaymentMethod = cboSupplierPaymentMethod.GetValue();
            $('#<%=hdnSupplierPaymentMethod.ClientID %>').val(suppPaymentMethod);
            if (cboSupplierPaymentMethod.GetValue() == '<%=OnGetSupplierPaymentMethodCash() %>') {
                $('#<%=trBank.ClientID %>').attr('style', 'display:none');
            }
            else {
                $('#<%=trBank.ClientID %>').removeAttr('style');
                if (cboSupplierPaymentMethod.GetValue() == Constant.SupplierPaymentMethod.TRANSFER || cboSupplierPaymentMethod.GetValue() == Constant.SupplierPaymentMethod.GIRO || cboSupplierPaymentMethod.GetValue() == Constant.SupplierPaymentMethod.CHEQUE) {
                    $('#<%:lblBank.ClientID %>').attr('class', 'lblMandatory');
                }
                else {
                    $('#<%:lblBank.ClientID %>').attr('class', 'lblNormal');
                }
            }
        ...
        function onCboSupplierPaymentMethodValueChanged() {
            var suppPaymentMethod = cboSupplierPaymentMethod.GetValue();
            $('#<%=hdnSupplierPaymentMethod.ClientID %>').val(suppPaymentMethod);
            if (cboSupplierPaymentMethod.GetValue() == '<%=OnGetSupplierPaymentMethodCash() %>') {
                $('#<%=trBank.ClientID %>').attr('style', 'display:none');
    ...
    <input type="hidden" id="hdnSupplierPaymentMethod" runat="server" />
                    ...
                    <tr>
                        <td class="tdLabel">
                            <label class="lblMandatory">
                                <%=GetLabel("Cara Bayar Hutang")%></label>
                        </td>
                        <td>
                            <dxe:ASPxComboBox ID="cboSupplierPaymentMethod" ClientInstanceName="cboSupplierPaymentMethod"
                                runat="server" Width="100%">
                                <ClientSideEvents ValueChanged="function(s,e){ onCboSupplierPaymentMethodValueChanged(); }" />
                            </dxe:ASPxComboBox>
                        </td>
                    </tr>
                    <tr id="trBank" runat="server">
                        <td class="tdLabel">
                            <label id="lblBank" class="lblMandatory" runat="server">
                                <%=GetLabel("Bank")%></label>
                        </td>
                        <td>
                            <dxe:ASPxComboBox ID="cboBank" runat="server" Width="100%" />
                        </td>
                    </tr>
                    <tr>
