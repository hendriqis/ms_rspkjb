<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="JournalEntryVoidCtl.ascx.cs"
    Inherits="QIS.Medinfras.Web.Accounting.Program.JournalEntryVoidCtl" %>
...
<script type="text/javascript" id="dxss_journalvoidctl">
    $(function () {
        hideLoadingPanel();
    });

    $('#<%=btnVoidJournal.ClientID %>').click(function (evt) {
        if (IsValid(evt, 'fsTrxPopup', 'mpTrxPopup')) {
            cbpJournalVoid.PerformCallback('void');
        }
    });

    function onVoidJournalSuccess() {
...
<div class="toolbarArea">
    <ul id="ulMPListToolbar" runat="server">
        <li id="btnVoidJournal" runat="server">
...
<div style="padding: 10px;">
    <fieldset id="fsTrxPopup" style="margin: 0">
        ...
        <dxcp:ASPxCallbackPanel ID="cbpJournalVoid" runat="server" Width="100%" ClientInstanceName="cbpJournalVoid"
            ShowLoadingPanel="false" OnCallback="cbpJournalVoid_Callback">
            <ClientSideEvents BeginCallback="function(s,e){ showLoadingPanel();}" EndCallback="function(s,e){
                    var result = s.cpResult.split('|');
                    if(result[0] == 'fail')
                        showToast('Save Failed', result[1]);
                    else
                        onVoidJournalSuccess();
                    hideLoadingPanel();
                }" />
