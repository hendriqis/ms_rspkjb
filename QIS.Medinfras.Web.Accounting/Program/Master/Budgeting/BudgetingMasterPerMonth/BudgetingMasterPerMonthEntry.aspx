<%@ Page Title="" Language="C#" MasterPageFile="~/libs/MasterPage/MPTrx2.master"
    AutoEventWireup="true" CodeBehind="BudgetingMasterPerMonthEntry.aspx.cs" Inherits="QIS.Medinfras.Web.Accounting.Program.BudgetingMasterPerMonthEntry" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="plhEntry" runat="server">
    <script type="text/javascript">
        function onLoad() {
            if ($('#<%=hdnIsEditable.ClientID %>').val() == '1') {
                $('#lblAddData').show();
                $('#lblSalinTemplate').show();
                $('#<%=btnReOpen.ClientID %>').hide();
            }
            else {
                $('#lblAddData').hide();
                $('#lblSalinTemplate').hide();
            }
        ...
        $('#<%=btnReOpen.ClientID %>').click(function (evt) {
            if (IsValid(evt, 'fsTrxPopup', 'mpTrxPopup')) {
                showToastConfirmation("Apakah yakin akan RE-OPEN Anggaran ini ?", function (result) {
                    if (result) {
                        cbpProcess.PerformCallback('reopen');
                    }
                });
            }
        });
        ...
        //#region Add, Edit, Delete Tambah Data
        $('#lblAddData').live('click', function (evt) {
            if (IsValid(evt, 'fsMPEntry', 'mpEntry')) {
                $('#<%=hdnBudgetTransactionDtID.ClientID %>').val('');
        ...
        $('#<%=grdView.ClientID %> .imgEdit.imgLink').live('click', function () {
            ...
            $('#<%=hdnBudgetTransactionDtID.ClientID %>').val(entity.ID);
        ...
        $('#lblSelectBudget.lblLink').live('click', function () {
            var filterExpression = "BudgetTransactionID IS NOT NULL";
            openSearchDialog('budgettransactionhd', filterExpression, function (value) {
                $('#<%=hdnBudgetHdID.ClientID %>').val(value);
                onHdnBudgetHdIDChanged(value);
            });
        });

        function onHdnBudgetHdIDChanged(value) {
            onLoadObject(value);
        }
        //#endregion

        $('#lblSalinTemplate').live('click', function () {
            if (IsValid(null, 'fsMPEntry', 'mpEntry')) {
                showLoadingPanel();
                var param = ""
                var url = ResolveUrl('~/Program/Master/Budgeting/BudgetingMasterPerMonth/BudgetMasterPerMonthTemplateCtl.ascx');
                openUserControlPopup(url, param, 'Pilih Template', 1000, 500);
            }
        ...
        function onAfterSaveRecordDtSuccess(BudgetHdID) {
            $('#<%=hdnBudgetHdID.ClientID %>').val(BudgetHdID);
            $('#<%=hdnBudgetTransactionDtID.ClientID %>').val('');
        ...
        function onCbpProcesEndCallback(s) {
            ...
            if (param[0] == 'save') {
                ...
                if (param[1] == 'fail')
                    ...
                    isAfterAdd = false;
                    var BudgetHdID = s.cpBudgetID;
                    onHdnBudgetHdIDChanged(BudgetHdID);
                }
            }
        }
        ...
        function onAfterSaveAddRecordEntryPopup(param) {
            $('#<%=hdnBudgetHdID.ClientID %>').val(param);
            cbpView.PerformCallback('refresh');
        }

        function onAfterPopupControlClosing() {
            var BudgetHdID = $('#<%=hdnBudgetHdID.ClientID %>').val();
            onHdnBudgetHdIDChanged(BudgetHdID);
        }
        ...
        $('.lnkDetailRCC a').live('click', function () {
            ...
            if (isEditable == "1") {
                ...
                var url = ResolveUrl("~/Program/Master/Budgeting/BudgetingMasterPerMonth/BudgetingMasterPerMonthRCCEntryCtl.ascx");
                openUserControlPopup(url, id, 'Budgeting Per Month Detail - Revenue Cost Center', 900, 500);
            } else {
    ...
    <input type="hidden" id="hdnBudgetHdID" runat="server" value="" />
        ...
        <tr>
            <td style="padding: 5px; vertical-align: top">
                <table class="tblEntryContent" style="width: 100%">
                    ...
                    <tr>
                        <td class="tdLabel">
                            <label class="lblLink" id="lblSelectBudget">
                                <%=GetLabel("Pilih Data Anggaran") %></label>
                        </td>
                    </tr>
                    <tr>
                        <td class="tdLabel">
                    ...
                    <tr>
                        ...
                        <td>
                            <dxe:ASPxComboBox ID="cboMonth" ClientInstanceName="cboMonth" Width="250px" runat="server" />
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td>
                            <label class="lblNormal">
                                <%=GetLabel("Total Anggaran")%></label>
                        </td>
                        <td>
                            <asp:TextBox ID="txtTotalBudgetAmount" runat="server" CssClass="txtCurrency" Width="100%"
                                ReadOnly="true" />
                        </td>
                    </tr>
                </table>
            ...
            <td colspan="2">
                <div id="containerEntry" style="margin-top: 4px; display: none;">
                    ...
                    <fieldset id="fsTrxPopup" style="margin: 0">
                        <input type="hidden" value="" id="hdnBudgetTransactionDtID" runat="server" />
                ...
                <div style="width: 100%; text-align: center">
                    <div style="width: 100%; text-align: center">
                        <span class="lblLink" id="lblAddData" style="margin-right: 100px;">
                            <%= GetLabel("Tambah Data")%></span><span class="lblLink" id="lblSalinTemplate">
                                <%= GetLabel("Salin Template")%></span>
                    </div>
                </div>
                <div>
                    <table width="100%">
