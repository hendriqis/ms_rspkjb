...
{
    public partial class LPendapatanPerUnit : BaseDailyPortraitRpt
    {
        public LPendapatanPerUnit()
        {
            InitializeComponent();
        }
        ...
        private void Calculate_SummaryRowChanged(object sender, EventArgs e)
        {
            ...
            decimal patientAmount = Convert.ToDecimal(GetCurrentColumnValue("TotalPatientAmount"));
            decimal payerAmount = Convert.ToDecimal(GetCurrentColumnValue("TotalPayerAmount"));
            if (reg != regisID)
            {
                regisID = reg;
