...
{
    public partial class BPiutangInstansiDetail2 : BaseDailyPortraitRpt
    {
        Boolean isControlCoverageLimit;
        PatientBill entityBill = null;
        String custType;

        public BPiutangInstansiDetail2()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            lblBillingNo.Text = entityPayment.BillingNo;
            lblPaymentDate.Text = string.Format("{0}", entityPayment.PaymentDateInString);

            entityBill = BusinessLayer.GetPatientBillList(string.Format("{0}", param[0]))[0];
            lblTotalAmount.Text = entityBill.TotalPayerAmount.ToString("N2");
