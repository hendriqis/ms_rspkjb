...
{
    public partial class BaseCustomPharmacyNoloopHeaderA6Rpt : BaseRpt
    {
        ...
        public BaseCustomPharmacyNoloopHeaderA6Rpt()
        {
            InitializeComponent();
        }
        ...
        {
            string filterSetVarDt = string.Format("HealthcareID = '{0}' AND ParameterCode IN ('{1}','{2}','{3}')",
                                                        appSession.HealthcareID,
                                                        Constant.SettingParameter.SA_REPORT_FOOTER_PRINTEDBY_USERNAME_FULLNAME,
                                                        Constant.SettingParameter.PHARMACIST,
                                                        Constant.SettingParameter.PHARMACIST_LICENSE_NO
                                                    );
            List<SettingParameterDt> lstSerVarDt = BusinessLayer.GetSettingParameterDtList(filterSetVarDt);

            cApoteker.Text = lstSerVarDt.Where(a => a.ParameterCode == Constant.SettingParameter.PHARMACIST).FirstOrDefault().ParameterValue;
            cSIPA.Text = lstSerVarDt.Where(a => a.ParameterCode == Constant.SettingParameter.PHARMACIST_LICENSE_NO).FirstOrDefault().ParameterValue;
            ...
            {
                lblReportProperties.Text = string.Format("MEDINFRAS-{0}, Print Date/Time:{1}, User:{2}",
