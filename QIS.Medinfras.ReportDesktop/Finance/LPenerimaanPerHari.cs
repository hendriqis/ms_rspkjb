...
{
    public partial class LPenerimaanPerHari : BaseCustomDailyLandscapeA3Rpt
    {
        public LPenerimaanPerHari()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            xrLabel4.Text = string.Format("Jam : {0} s/d {1}", (temp2[0]).ToString(), (temp2[1]).ToString());

            if (param[3].ToString() == "1")
            {
                xrLabel5.Text = "Status Transaksi : NON VOID";
            }
            else
            {
                xrLabel5.Text = "Status Transaksi : VOID";
            }
