...
{
    public partial class BTandaTerimaBerkasPenagihanKolektifRSSBB : BaseRpt
    {
        private string HealthcareName = "";
        private string Address = "";
        private string City = "";

        public BTandaTerimaBerkasPenagihanKolektifRSSBB()
        {
            InitializeComponent();
        }
        ...
        {
            string arReceiptDate = Convert.ToDateTime(GetCurrentColumnValue("ARReceiptDate")).ToString(Constant.FormatString.DATE_FORMAT);
            string createdDate = Convert.ToDateTime(GetCurrentColumnValue("CreatedDate")).ToString(Constant.FormatString.DATE_FORMAT);

            if (arReceiptDate != "01-Jan-1900")
            {
                lblTanggal.Text = string.Format("{0}, {1}", City, arReceiptDate);
            }
            else
            {
