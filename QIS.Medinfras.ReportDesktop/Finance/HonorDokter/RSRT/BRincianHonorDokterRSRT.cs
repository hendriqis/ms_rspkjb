...
{
    public partial class BRincianHonorDokterRSRT : BaseCustomDailyPotraitRpt
    {
        public BRincianHonorDokterRSRT()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            // Detail List
            List<GetTRSSummaryDtTransRegistrationDetailRSRT> lstDetail = BusinessLayer.GetTRSSummaryDtTransRegistrationDetailRSRTList(entity.RSSummaryID);
            srTransRegistrationDetailRSRT.CanGrow = true;
            SubTransRegistrationDetailRSRT.InitializeReport(lstDetail);
            ...
            srTransSummaryAdjustmentDetailPlusRSRT.CanGrow = true;
            SubTransSummaryAdjustmentDetailPlusRSRT.InitializeReport(lstAdj.Where(a => a.GCRSAdjustmentGroup == Constant.RevenueSharingAdjustmentGroup.PENAMBAHAN && a.GCRSAdjustmentType != Constant.RevenueSharingAdjustmentType.PENYESUAIAN_PAJAK_PENAMBAHAN).ToList());

            srTransSummaryAdjustmentDetailMinusRSRT.CanGrow = true;
            SubTransSummaryAdjustmentDetailMinusRSRT.InitializeReport(lstAdj.Where(a => a.GCRSAdjustmentGroup == Constant.RevenueSharingAdjustmentGroup.PENGURANGAN && a.GCRSAdjustmentType != Constant.RevenueSharingAdjustmentType.PENYESUAIAN_PAJAK_PENGURANGAN).ToList());
            ...
            {
                srTransSummaryAdjustmentDetailPlusRSRT2.Visible = true;
                srTransSummaryAdjustmentDetailPlusRSRT2.CanGrow = true;
                subTransRevenueAdjustmentDetailPlusRSRT2.InitializeReport(lstAdjPlus2);
            }
            else
            {
                srTransSummaryAdjustmentDetailPlusRSRT2.Visible = false;
            }
            ...
            {
                srTransSummaryAdjustmentDetailMinusRSRT2.Visible = true;
                srTransSummaryAdjustmentDetailMinusRSRT2.CanGrow = true;
                subTransRevenueAdjustmentDetailMinusRSRT2.InitializeReport(lstAdjMin2);
            }
            else
            {
                srTransSummaryAdjustmentDetailMinusRSRT2.Visible = false;
            }
            ...
            srParamedicTaxBalanceRSRT.CanGrow = true;
            subParamedicTaxBalanceRSRT.InitializeReport(lstTaxBalance);
            ...
            ///SubTransGroupTotalRSRT
            ///

            Decimal pphAmount = 0;
            if (lstTaxBalance.Count > 0) {
                pphAmount = lstTaxBalance.Sum(p=>p.TaxAmount) + lstTaxBalance.Sum(p=>p.ExtraTaxAmount);
            }
                //lstAdj.Where(a => a.GCRSAdjustmentGroup == Constant.RevenueSharingAdjustmentGroup.PENGURANGAN && a.GCRSAdjustmentType != Constant.RevenueSharingAdjustmentType.PENYESUAIAN_PAJAK_PENGURANGAN).Sum(p=> p.AdjustmentAmount);
            Decimal TotalHonorSetelahPajak = revenueSharingAmount;
            srTransGroupTotal.CanGrow = true;
            subTransGroupTotal.InitializeReport(lstDetail, pphAmount, TotalHonorSetelahPajak, 
                totalOthersAdjustmentPlus, totalOthersAdjustmentMinus, honorBersih);
