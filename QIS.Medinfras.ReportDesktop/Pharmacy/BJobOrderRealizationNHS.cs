...
{
    public partial class BJobOrderRealizationNHS : BaseCustomPharmacyA5Rpt
    {
        public BJobOrderRealizationNHS()
        {
            InitializeComponent();
        }
...
//                lblOrderNotes.Text = entity.Remarks;
            ...
            private void cItemName_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
            {
                ...
                String DrugName = Convert.ToString(GetCurrentColumnValue("DrugName"));
                ...
                if (CompoundQtyInString == "")
                {
                    ...
                    {
                        cItemName.Text = string.Format("{0} {1} {2} ({3} {4})", DrugName, ChargeQty, ResultQty, CoenamRule, SignaLabel);
                    }
                    else
                    {
                        cItemName.Text = string.Format("{0} {1} {2}", DrugName, ChargeQty, ResultQty);
                    }
                }
                else
                {
                    if (CoenamRule != "" && SignaLabel != "")
                    {
                        cItemName.Text = string.Format("{0} ({1} {2}) {3} {4} ({5} {6})",
                            DrugName, ChargeQty, CompoundQtyInString, CompoundUnit, ResultQty, CoenamRule, SignaLabel);
                    }
                    else
                    {
                        cItemName.Text = string.Format("{0} ({1} {2}) {3} {4}",
                            DrugName, ChargeQty, CompoundQtyInString, CompoundUnit, ResultQty);
                    }
                }
            }
