...
{
    public partial class BJobOrderRealizationUKRIDA : BaseCustomPharmacyA5Rpt1
    {
        public BJobOrderRealizationUKRIDA()
        {
            InitializeComponent();
        }
            ...
            {
                GetPrescriptionOrderDtCustom4 entity = BusinessLayer.GetPrescriptionOrderDtCustom4List(Convert.ToInt32(param[0])).FirstOrDefault();
...
//                lblOrderNotes.Text = entity.Remarks;
            ...
            private void cItemName_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
            {
                ...
                String ChargeQty = Convert.ToString(GetCurrentColumnValue("ChargeQty"));
                ...
                String CompoundQty = Convert.ToDecimal(GetCurrentColumnValue("CompoundQty")).ToString("G29");

                if (CompoundQtyInString == "")
                {
                    if (CoenamRule != "" && SignaLabel != "")
                    {
                        cItemName.Text = string.Format("{0} {1} ({2} {3})", ItemName, CompoundQty, CoenamRule, SignaLabel);
                    }
                    else
                    {
                        cItemName.Text = string.Format("{0} {1}", ItemName, CompoundQty);
                    }
                }
                else
                {
                    if (CoenamRule != "" && SignaLabel != "")
                    {
                        cItemName.Text = string.Format("{0} ({1} {2}) {3} {4} ({5})",
                            ItemName, CompoundQtyInString, CompoundUnit, CompoundQty, CoenamRule, SignaLabel);
                    }
                    else
                    {
                        cItemName.Text = string.Format("{0} ({1} {2}) {3}",
                            ItemName, CompoundQtyInString, CompoundUnit, CompoundQty);
                    }
                }
            }
