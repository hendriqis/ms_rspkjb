...
{
    public partial class BNewPembelianTunai_RSDOSOBA : BaseDailyPortraitRpt
    {
        public BNewPembelianTunai_RSDOSOBA()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            SettingParameterDt setvarDTL = BusinessLayer.GetSettingParameterDt(
                AppSession.UserLogin.HealthcareID, Constant.SettingParameter.IM_DEFAULT_LOCATION_PURCHASEORDER_LOGISTIC);
            SettingParameterDt setvarDTP = BusinessLayer.GetSettingParameterDt(
                AppSession.UserLogin.HealthcareID, Constant.SettingParameter.IM_DEFAULT_LOCATION_PURCHASEORDER_PHARMACY);

            if (entity.LocationCode == setvarDTL.ParameterValue)
            {
                SettingParameterDt lstParam = BusinessLayer.GetSettingParameterDt(AppSession.UserLogin.HealthcareID, Constant.SettingParameter.KEPALA_LOGISTIK_UMUM);
            ...
            {
                SettingParameterDt lstParam = BusinessLayer.GetSettingParameterDt(AppSession.UserLogin.HealthcareID, Constant.SettingParameter.KEPALA_LOGISTIK_OBAT);
                if (lstParam.ParameterValue != "" && lstParam.ParameterValue != null)
                {
                    lblApprovedByName.Text = lstParam.ParameterValue;
                }
