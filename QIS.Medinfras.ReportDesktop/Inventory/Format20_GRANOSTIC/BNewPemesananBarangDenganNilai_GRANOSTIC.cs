...
{
    public partial class BNewPemesananBarangDenganNilai_GRANOSTIC : BaseDailyPortraitRpt
    {
        public BNewPemesananBarangDenganNilai_GRANOSTIC()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            if (entity.GCPurchaseOrderType != Constant.PurchaseOrderType.LOGISTIC && entity.GCPurchaseOrderType != Constant.PurchaseOrderType.ASSET && entity.LocationID.ToString() == oLocation)
            {
                ...
                string filterExpressionManager = string.Format("ParameterCode IN ('{0}')", Constant.SettingParameter.MANAGER_KEUANGAN);
                SettingParameter managerKeuangan = BusinessLayer.GetSettingParameterList(filterExpressionManager).FirstOrDefault();

                lblManagerKeuangan.Text = managerKeuangan.ParameterValue;
                lblJabatan3.Text = managerKeuangan.ParameterName;
