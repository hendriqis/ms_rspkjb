...
{
    public partial class BNewPemesananBarangDenganNilai_RSPW : BaseDailyPortraitRpt
    {
        public BNewPemesananBarangDenganNilai_RSPW()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            lblNoIzin.Text = string.Format("SIO : {0}", noIzinRS.ParameterValue);
            lblNPWP.Text = string.Format("NPWP : 01.236.100.2-651.000");
            #endregion
            ...
            if (entity.Remarks == "")
                ...
                lblRemarks.Text = String.Format("Catatan : {0}", entity.PaymentRemarks);
            ...
            || entity.GCPurchaseOrderType == Constant.PurchaseOrderType.OBAT_RANAP || entity.GCPurchaseOrderType == Constant.PurchaseOrderType.OBAT_RAJAL) && entity.LocationID.ToString() == location)
            {
                ...
                string filterExpressionNoSIPA = string.Format("ParameterCode IN ('{0}')", Constant.SettingParameter.MANAGER_PHARMACIST_LICENSE_NO);
                SettingParameterDt noSIPAManagerFarmasi = BusinessLayer.GetSettingParameterDtList(filterExpressionNoSIPA).FirstOrDefault();
                if (!String.IsNullOrEmpty(noSIPAManagerFarmasi.ParameterValue))
                {
                    lblNoSIPAManagerFarmasi.Text = noSIPAManagerFarmasi.ParameterValue;
                }
                else
                {
                    lblNoSIPAManagerFarmasi.Text = string.Empty;
                }

                string filterExpressionPharmacyOrder = string.Format("ParameterCode IN ('{0}')", Constant.SettingParameter.IM_DEFAULT_USER_PURCHASE_PHARMACY);
                SettingParameter userFarmasi = BusinessLayer.GetSettingParameterList(filterExpressionPharmacyOrder).FirstOrDefault();
                if (!String.IsNullOrEmpty(userFarmasi.ParameterValue))
                {
                    lblUser.Text = userFarmasi.ParameterValue;
                    lblNoSIPAUser.Text = userFarmasi.Notes;
                }
                else
                {
                    lblUser.Text = AppSession.UserLogin.UserFullName;
                    lblNoSIPAUser.Text = string.Empty;
                }
            }
            else
            {
