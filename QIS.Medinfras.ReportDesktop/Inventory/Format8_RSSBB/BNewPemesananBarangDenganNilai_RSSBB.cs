...
{
    public partial class BNewPemesananBarangDenganNilai_RSSBB : BaseDailyPortraitRpt
    {
        public BNewPemesananBarangDenganNilai_RSSBB()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            lblPrintDate2.Text = string.Format("{0}, {1}", oHealthcare.City, DateTime.Now.ToString(Constant.FormatString.DATE_FORMAT));
            ...
            if (entity.GCPurchaseOrderType != Constant.PurchaseOrderType.LOGISTIC && entity.GCPurchaseOrderType != Constant.PurchaseOrderType.ASSET && entity.LocationID.ToString() == oLocation)
            {
                ...
                string filterExpressionPharmLicNo = string.Format("ParameterCode IN ('{0}')", Constant.SettingParameter.PHARMACIST_LICENSE_NO);
                SettingParameterDt licNo = BusinessLayer.GetSettingParameterDtList(filterExpressionPharmLicNo).FirstOrDefault();
                ...
                lblPrintDate2.Visible = true;

                xrLabel35.Visible = false;
                xrLabel37.Visible = true;

                lblJabatan.Visible = false;
                lblJabatan2.Visible = true;
                lblJabatan2.Text = "Kepala Instalasi Farmasi,";
                lblNoSIPA.Visible = true;
                lblNoSIPA.Text = string.Format("SIPA : {0}", licNo.ParameterValue);
            }
            else
            {
                ...
                lblPrintDate2.Visible = false;

                xrLabel35.Visible = true;
                xrLabel37.Visible = false;

                lblJabatan.Visible = true;
                lblJabatan2.Visible = false;
                lblNoSIPA.Visible = false;
            }
