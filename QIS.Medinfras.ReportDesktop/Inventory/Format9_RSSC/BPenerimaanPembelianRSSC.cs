...
{
    public partial class BPenerimaanPembelianRSSC : BaseDailyPortraitRpt
    {
        public BPenerimaanPembelianRSSC()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            string log = string.Format("ParameterCode IN ('{0}','{1}','{2}','{3}')", Constant.SettingParameter.IM_DEFAULT_LOCATION_PURCHASEORDER_LOGISTIC, Constant.SettingParameter.IM_DEFAULT_KODE_LOKASI_MEDIK, Constant.SettingParameter.IM_DEFAULT_LOCATION_PURCHASEORDER_PHARMACY, Constant.SettingParameter.IM_DEFAULT_LOCATION_PURCHASEORDER_NUTRITION);
            ...
            SettingParameterDt nutrition = lstlogistic.Where(t => t.ParameterCode == Constant.SettingParameter.IM_DEFAULT_LOCATION_PURCHASEORDER_NUTRITION).FirstOrDefault();

            List<SettingParameterDt> lstSetvarDt = BusinessLayer.GetSettingParameterDtList(string.Format("ParameterCode IN ('{0}','{1}','{2}','{3}','{4}','{5}','{6}')", 
                ...
                Web.Common.Constant.SettingParameter.IM_APPROVER_PURCHASE_RECEIVE_NUTRITION, Constant.SettingParameter.KEPALA_LOGISTIK_UMUM,
                Constant.SettingParameter.KEPALA_LOGISTIK_OBAT, Constant.SettingParameter.KEPALA_LOGISTIK_MEDIK, Constant.SettingParameter.IM_APPROVER_PURCHASE_RECEIVE_MEDIK));

            List<SettingParameter> lstSetvar = BusinessLayer.GetSettingParameterList(string.Format("ParameterCode IN ('{0}','{1}')",
                Web.Common.Constant.SettingParameter.IM_APPROVER_PURCHASE_RECEIVE_LOGISTIC, Constant.SettingParameter.KEPALA_LOGISTIK_UMUM));
            ...
            vLocation locNutrition = new vLocation();
            if (nutrition.ParameterValue != null && nutrition.ParameterValue != "" && nutrition.ParameterValue != "0")
            {
                locNutrition = BusinessLayer.GetvLocationList(string.Format("LocationID = '{0}'", nutrition.ParameterValue)).FirstOrDefault();
            }
            ...
            else if (entityHd.LocationID == locNutrition.LocationID)
            {
                SettingParameterDt lstParam1 = lstSetvarDt.Where(t => t.ParameterCode == Web.Common.Constant.SettingParameter.IM_APPROVER_PURCHASE_RECEIVE_NUTRITION).FirstOrDefault();
                SettingParameterDt lstParam2 = lstSetvarDt.Where(t => t.ParameterCode == Web.Common.Constant.SettingParameter.IM_APPROVER_PURCHASE_RECEIVE_NUTRITION).FirstOrDefault();
                SettingParameterDt lstParam3 = lstSetvarDt.Where(t => t.ParameterCode == Web.Common.Constant.SettingParameter.IM_APPROVER_PURCHASE_RECEIVE_NUTRITION).FirstOrDefault();
                lblKALogistik.Text = lstParam1.ParameterValue;
                lblNamaPJMedik.Text = lstParam2.ParameterValue;
                lblCreatedByName.Text = lstParam3.ParameterValue;
                xrLabel40.Text = "KA. Instalasi Gizi";
            ...
            else if (entityHd.LocationID == locLogistic.LocationID)
            {
                ...
                SettingParameter lstParam3 = lstSetvar.Where(t => t.ParameterCode == Constant.SettingParameter.KEPALA_LOGISTIK_UMUM).FirstOrDefault();
                SettingParameter lstParam4 = lstSetvar.Where(t => t.ParameterCode == Web.Common.Constant.SettingParameter.IM_APPROVER_PURCHASE_RECEIVE_LOGISTIC).FirstOrDefault();
                lblKALogistik.Text = lstParam1.ParameterValue;
                lblNamaPJMedik.Text = lstParam2.ParameterValue;
                xrLabel39.Text = lstParam3.Notes;
                xrLabel40.Text = lstParam4.Notes;
                xrLabel36.Text = "Petugas";
            }
            else
            {
