...
{
    public partial class BNewPatientBillSummaryPerBillingRSDOSKAPayer : BaseCustomDailyPotraitRpt
    {
        public BNewPatientBillSummaryPerBillingRSDOSKAPayer()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            List<GetPatientChargesHdDtPatientBill5> lstHDDT = BusinessLayer.GetPatientChargesHdDtPatientBill5List(entityReg.RegistrationID, lstBill.FirstOrDefault().PatientBillingID);

            List<vPatientPaymentHd> lstPayment = BusinessLayer.GetvPatientPaymentHdList(string.Format("RegistrationID IN ({0}) AND GCTransactionStatus != '{1}' AND BillingNo LIKE '%{2}%' ORDER BY PaymentDate",
                entityReg.RegistrationID, Constant.TransactionStatus.VOID, lstBill.FirstOrDefault().PatientBillingNo));
            List<vPatientPaymentHdDt> lstPaymentHdDt = BusinessLayer.GetvPatientPaymentHdDtList(string.Format("RegistrationID IN ({0}) AND GCTransactionStatus != '{1}' AND BillingNo LIKE '%{2}%'",
                entityReg.RegistrationID, Constant.TransactionStatus.VOID, lstBill.FirstOrDefault().PatientBillingNo));
            ...
            billPaymentSummaryTransactionPerBillingRSDOSKAPayer.InitializeReport(lstHDDT.Where(a => a.PayerAmount != 0).ToList());
            #endregion
            ...
            decimal coverP = lstBill.Sum(p => p.TotalPatientAmount);
            ...
            billPaymentDetailPaymentAllDM.InitializeReport(lstPaymentHdDt.Where(a => a.GCPaymentType == Constant.PaymentType.AR_PAYER).ToList());
            #endregion
