...
{
    public partial class BSuratKeteranganIstirahatRSSA : BaseDailyPortrait2Rpt
    {
        public BSuratKeteranganIstirahatRSSA()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            {
                PatientDiagnosis entityDS = BusinessLayer.GetPatientDiagnosisList(string.Format("VisitID = {0} AND GCDiagnoseType = '{1}' ", entityCV.VisitID, Constant.DiagnoseType.MAIN_DIAGNOSIS)).FirstOrDefault();
                if(entityDS != null)
                {
                    lblDiagnose.Text = string.Format("{0}", entityDS.DiagnosisText);
                }
                else
                {
                    lblDiagnose.Text = " ";
