...
{
    public partial class BNewPatientBillDetailRSPM2 : BaseCustomDailyPotraitRpt
    {
        public BNewPatientBillDetailRSPM2()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            List<GetPatientChargesHdDtAll13> lstHDDT = BusinessLayer.GetPatientChargesHdDtAll13List(entityReg.RegistrationID);
            ...
            List<GetPatientChargesHdDtAll13> lstDetail = lstHDDT.OrderBy(lst => lst.ItemCreatedDate).OrderBy(lst => lst.ItemCreatedTime).ToList();
            billPaymentDetailTransactionRSPM2.InitializeReport(lstDetail);
            #endregion
            ...
            cTotalBillP.Text = (transP + adminP + serviceP).ToString("N2");
            cTotalBillC.Text = (transC + adminC + serviceC).ToString("N2");

            cTotalBillAll.Text = (transP + adminP + serviceP + transC + adminC + serviceC).ToString("N2");
            cCoverageLimit.Text = coverC.ToString("N2");
            cSubtotal.Text = (transP + adminP + serviceP + transC + adminC + serviceC - coverC).ToString("N2");
            ...
            decimal total = (transP + adminP + serviceP + transC + adminC + serviceC - coverC);
            cTotalPersonal.Text = (total + roundingP + roundingC).ToString("N2");
            #endregion
