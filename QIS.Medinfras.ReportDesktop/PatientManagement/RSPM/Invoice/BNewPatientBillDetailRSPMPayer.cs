...
{
    public partial class BNewPatientBillDetailRSPMPayer : BaseCustomDailyPotraitRpt
    {
        public BNewPatientBillDetailRSPMPayer()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            if (entityReg.BusinessPartnerID != 1)
            {
                billPaymentDetailTransactionRSPMPayer.InitializeReport(lstHDDT.Where(a => a.PayerAmount < 0 || a.PayerAmount > 0 || (a.PatientAmount + a.PayerAmount) == 0).ToList());
            }
            else
            {
                billPaymentDetailTransactionRSPMPayer.InitializeReport(lstHDDT.Where(a => a.PayerAmount != 0).ToList());
            }
            #endregion
