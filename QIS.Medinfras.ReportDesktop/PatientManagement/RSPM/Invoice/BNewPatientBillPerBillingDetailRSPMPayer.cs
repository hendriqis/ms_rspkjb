...
{
    public partial class BNewPatientBillPerBillingDetailRSPMPayer : BaseCustomDailyPotraitRpt
    {
        public BNewPatientBillPerBillingDetailRSPMPayer()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            List<GetPatientChargesHdDtPerBillingRSPM> lstHDDT = BusinessLayer.GetPatientChargesHdDtPerBillingRSPMList(entityReg.RegistrationID, entityBill.PatientBillingID);
            ...
            if (entityReg.BusinessPartnerID != 1)
            {
                billPaymentPerBillingDetailTransactionRSPMPayer.InitializeReport(lstHDDT.Where(a => a.PayerAmount < 0 || a.PayerAmount > 0 || (a.PatientAmount + a.PayerAmount) == 0).ToList());
            }
            else
            {
                billPaymentPerBillingDetailTransactionRSPMPayer.InitializeReport(lstHDDT.Where(a => a.PayerAmount != 0).ToList());
            }
            #endregion
