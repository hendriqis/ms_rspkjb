...
{
    public partial class BPatientItemPerBillingGroupDetail_RSSBB : BaseCustomDailyPotraitRpt
    {
        public BPatientItemPerBillingGroupDetail_RSSBB()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            Registration entityRegLinked = BusinessLayer.GetRegistrationList(string.Format("RegistrationID = {0}", entityReg.LinkedRegistrationID)).FirstOrDefault();
            Healthcare entityHealthcare = BusinessLayer.GetHealthcare(appSession.HealthcareID);

            List<GetPatientChargesDtPerBillingGroup> lstHDDT = BusinessLayer.GetPatientChargesDtPerBillingGroupList(entityReg.RegistrationID, entityReg.LinkedRegistrationID);
            ...
            billPaymentPerBillingGroupDetailRSSBB1.InitializeReport(lstHDDT);
            #endregion

            #region Discount
            subDiscount.CanGrow = true;
            billPaymentDetailDiscountAllRSSBB1.InitializeReport(entityReg.RegistrationID);
            #endregion

            #region Payment All
            subPaymentAll.CanGrow = true;
            billPaymentDetailPaymentAllRSSBB1.InitializeReport(lstPaymentHdDt);
            #endregion
            ...
            decimal gtP = transP + amountP - discP;
