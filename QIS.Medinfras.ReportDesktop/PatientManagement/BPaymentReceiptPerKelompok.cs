...
{
    public partial class BPaymentReceiptPerKelompok : BaseDailyPortraitRpt
    {
        Boolean isControlCoverageLimit;
        PatientBill entityBill = null;
        String custType;

        public BPaymentReceiptPerKelompok()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            PaymentReceipt entityPaymentReceipt = BusinessLayer.GetPaymentReceiptList(param[0])[0];
            lblPaymentReceiptNo.Text = entityPaymentReceipt.PaymentReceiptNo;
            lblPrintAsName.Text = entityPaymentReceipt.PrintAsName;
            lblReceiptAmount.Text = entityPaymentReceipt.ReceiptAmount.ToString("N2");
            lblLastUpdatedBy.Text = appSession.UserFullName;
            lblLastUpdatedDate.Text = string.Format("{0}, {1}", entityHealthcare.City, entityPaymentReceipt.PaymentReceiptDateTime.ToString(Constant.FormatString.DATE_FORMAT));

            vRegistration entityReg = BusinessLayer.GetvRegistrationList(string.Format("RegistrationID = {0}", entityPaymentReceipt.RegistrationID))[0];
            custType = entityReg.GCCustomerType;
            lblRegistrationNo.Text = entityReg.RegistrationNo;
            lblRegistrationDate.Text = entityReg.RegistrationDateInString;
