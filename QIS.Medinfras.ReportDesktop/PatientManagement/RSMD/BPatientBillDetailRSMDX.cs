...
{
    public partial class BPatientBillDetailRSMDX : BaseDailyPortraitRpt
    {
        List<PatientBill> lstPatientBill = null;
        vRegistration entity = null;
        string customer = "";

        public BPatientBillDetailRSMDX()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            lblPeriod.Text = string.Format("Periode Perawatan : {0} s/d {1}",entity.ActualVisitDate.ToString("dd-MMM-yyyy"), DateTime.Now.ToString("dd-MMM-yyyy"));
            ...
            lblTanggalMasuk.Text = entity.ActualVisitDate.ToString("dd-MMM-yyyy hh:mm");
            if (entity.cfDischargeDateInString != "")
            {
                lblTanggalKeluar.Text = string.Format("{0} {1}", entity.cfDischargeDateInString, entity.DischargeTime);
            }
            else 
            {
                lblTanggalKeluar.Text = "-";
            }
            lblRuang.Text = entity.RoomName;
            lblClassBed.Text = string.Format("{0} / {1}", entity.ClassName, entity.BedCode);
        ...
        private void lblRuangPerawatan_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            String healthcareServiceUnitID = GetCurrentColumnValue("HealthcareServiceUnitID").ToString();
            vHealthcareServiceUnit HSU = BusinessLayer.GetvHealthcareServiceUnitList(string.Format("HealthcareServiceUnitID = {0}", healthcareServiceUnitID))[0];

            lblRuangPerawatan.Text = HSU.ServiceUnitName;
        }
