...
{
    public partial class BNewPatientBillSummaryBeforeGenerate3 : BaseCustomDailyPotraitRpt
    {
        public BNewPatientBillSummaryBeforeGenerate3()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            List<GetPatientChargesHdDtAllBeforeBill> lstHDDT = BusinessLayer.GetPatientChargesHdDtAllBeforeBillList(entityReg.RegistrationID);
            ...
            {
                cDischargeDate.Text = DateTime.Now.ToString(Constant.FormatString.DATE_FORMAT);
            }
            ...
            billPaymentSummaryTransactionBeforeGenerateLineAmount21.InitializeReport(lstHDDT);
            #endregion
            ...
            totalDiscount = lstHDDT.Sum(t => t.DiscountAmount);
            totalBruto = lstHDDT.Sum(t => t.TransactionAmount);
            cTotalTransactionBruto.Text = totalBruto.ToString("N2");
            cTotalDiscountTransaction.Text = totalDiscount.ToString("N2");

            #endregion

            #region Summary Right OLD
            ...
            decimal transP = lstHDDT.Sum(p => p.PatientAmount);
            decimal transC = lstHDDT.Sum(p => p.PayerAmount);
            decimal trans = transP + transC;
            cTotalTransaction.Text = trans.ToString(Constant.FormatString.NUMERIC_2);

            decimal adminP = lstBill.Sum(p => p.PatientAdminFeeAmount);
            decimal adminC = lstBill.Sum(p => p.AdministrationFeeAmount);
            decimal admin = adminP + adminC;
            cTotalAdmin.Text = admin.ToString(Constant.FormatString.NUMERIC_2);

            decimal serviceP = lstBill.Sum(p => p.PatientServiceFeeAmount);
            decimal serviceC = lstBill.Sum(p => p.ServiceFeeAmount);
            decimal service = serviceP + serviceC;
            cServiceAmount.Text = service.ToString(Constant.FormatString.NUMERIC_2);

            decimal discP = lstBill.Sum(p => p.PatientDiscountAmount);
            decimal discC = lstBill.Sum(p => p.PayerDiscountAmount);
            decimal disc = discP + discC;
            cTotalDiscount.Text = disc.ToString(Constant.FormatString.NUMERIC_2);

            decimal gtTrans = trans + admin + service - disc;
            cGTTransaction.Text = gtTrans.ToString(Constant.FormatString.NUMERIC_2);

            decimal coverC = lstBill.Sum(p => p.CoverageAmount) - discC;
            cPayerAmount.Text = coverC.ToString(Constant.FormatString.NUMERIC_2);

            decimal coverP = gtTrans - coverC;
            cPatientAmount.Text = coverP.ToString(Constant.FormatString.NUMERIC_2);

            decimal uangMuka = lstPayment.Sum(a => a.DownPaymentOut);
            decimal payment = lstPayment.Where(p => p.GCPaymentType == Constant.PaymentType.SETTLEMENT || p.GCPaymentType == Constant.PaymentType.CUSTOM || p.GCPaymentType == Constant.PaymentType.DOWN_PAYMENT).Sum(p => p.NotInDownPayment)
                                - uangMuka;
            cPatientPaymentAmount.Text = payment.ToString(Constant.FormatString.NUMERIC_2);

            decimal remaining = coverP - payment;
            cRemainigAmount.Text = remaining.ToString(Constant.FormatString.NUMERIC_2);
