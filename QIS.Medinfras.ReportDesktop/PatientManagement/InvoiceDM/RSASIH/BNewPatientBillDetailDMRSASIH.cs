...
{
    public partial class BNewPatientBillDetailDMRSASIH : BaseCustomDailyPotraitCustomeRpt
    {
        public BNewPatientBillDetailDMRSASIH()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            List<GetPatientChargesHdDtALL3> lstHDDT = BusinessLayer.GetPatientChargesHdDtALL3List(entityReg.RegistrationID);
            ...
            vPatientBill entityPatBill = BusinessLayer.GetvPatientBillList(String.Format("RegistrationID = {0} AND GCTransactionStatus != '{1}'",
                entityReg.RegistrationID, Constant.TransactionStatus.VOID)).FirstOrDefault();
            ...
            if (entityReg.CorporateAccountNo != null && entityReg.CorporateAccountNo != "")
            {
                cCorporateNo.Text = entityReg.CorporateAccountNo;
            }
            else
            {
                cCorporateNo.Text = " -";
           ...
           // cDischargeDate.Text = entityReg.cfDischargeDateInString;
            ...
            new4BillPaymentDetailTransactionDM.InitializeReport(lstHDDT);
            #endregion
            ...
            decimal roundingP = lstBill.Sum(p => p.PatientRoundingAmount);
            decimal roundingC = lstBill.Sum(p => p.PayerRoundingAmount);
            ...
            cServiceP.Text = serviceP.ToString("N2");
            cServiceC.Text = serviceC.ToString("N2");
            ...
            if (entityPatBill != null)
            {
                cTTDPrintedBy.Text = entityPatBill.CreatedByFullName;
            }
            else
            {
            ...
            {
                SettingParameterDt oSetpar = BusinessLayer.GetSettingParameterDtList(string.Format("ParameterCode IN('{0}')", Constant.SettingParameter.VAT_PERCENTAGE)).FirstOrDefault();
                if (oSetpar != null)
                {
                    lblRemarks.Text = string.Format("*) Harga obat sudah termasuk PPN {0}%", oSetpar.ParameterValue);
                }

                #region Nomor Antrian
                ctTitleAntrian.Visible = false;
                List <vPatientChargesHd> lstChargesPharmacy = BusinessLayer.GetvPatientChargesHdList(string.Format("RegistrationID={0} AND PatientBillingID IS NOT NULL AND PrescriptionOrderID IS NOT NULL AND GCTransactionStatus <> '{1}'", entityReg.RegistrationID, Constant.TransactionStatus.VOID));
                if (lstChargesPharmacy.Count > 0) {
                    
                    string nomor = ""; 
                    foreach (vPatientChargesHd row in lstChargesPharmacy) 
                    {
                        PrescriptionOrderHd odata = BusinessLayer.GetPrescriptionOrderHdList(string.Format("PrescriptionOrderID='{0}'", row.PrescriptionOrderID)).FirstOrDefault();
                        if (odata != null) {
                            if (!string.IsNullOrEmpty(odata.ReferenceNo))
                            {
                                string[] data = odata.ReferenceNo.Split('|');
                                if (data.Length >= 2) {
                                    nomor += string.Format("{0} ", data[1]);
                                }
                            }
                            
                        }

                        ctTitleAntrian.Visible = true;
                        ctTitleNoAntrian.Text = string.Format("{0}", nomor);
                    }
                }
            ...
            if (entityReg.BusinessPartnerID.ToString() != "1")
            {
                if (reportPrinterBy == "1")
                {
                    lblReportProperties.Text = string.Format("MEDINFRAS-{0}|{1}",
                                                        reportMaster.ReportCode, reportMaster.ReportTitle1);
                }
                else
                {
                    lblReportProperties.Text = string.Format("MEDINFRAS-{0}|{1}",
            ...
            else
            {
                ...
                {
                    lblReportProperties.Text = string.Format("MEDINFRAS-{0}|{1}, Print Date/Time:{2}, User:{3}",
                                                        reportMaster.ReportCode, reportMaster.ReportTitle1, DateTime.Now.ToString("dd-MMM-yyyy/HH:mm:ss"), appSession.UserFullName);
                }
            }
        }
    }
