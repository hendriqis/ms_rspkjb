...
{
    public partial class BillPaymentDetailPaymentAllDMRSP : DevExpress.XtraReports.UI.XtraReport
    {
        public BillPaymentDetailPaymentAllDMRSP()
        {
            InitializeComponent();
        }
        ...
        public void InitializeReport(List<vPatientPaymentHdDtRSP> lst)
        {
            ...
            List<vPatientPaymentHdDtRSP> lst2 = lst.OrderBy(a => a.PaymentID).ToList();
            List<vPatientPaymentHdDtRSP> lstNonDeposit = lst2.OrderBy(a => a.GCPaymentType != Constant.PaymentType.DEPOSIT_IN || a.GCPaymentType != Constant.PaymentType.DEPOSIT_OUT).ToList();

            foreach (vPatientPaymentHdDtRSP dt in lstNonDeposit)
            {
