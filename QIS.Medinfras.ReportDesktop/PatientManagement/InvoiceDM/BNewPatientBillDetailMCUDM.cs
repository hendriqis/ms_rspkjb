...
{
    public partial class BNewPatientBillDetailMCUDM : BaseCustomDailyPotraitRpt
    {
        public BNewPatientBillDetailMCUDM()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            List<GetPatientChargesHdDtAllMCU> lstHDDT = BusinessLayer.GetPatientChargesHdDtAllMCUList(entityReg.RegistrationID);
            ...
            billPaymentDetailTransactionMCUDM.InitializeReport(lstHDDT);
            #endregion
            ...
            List<GetPatientChargesHdDtAllMCU> lstTemp = (from a in lstHDDT orderby a.ItemName1 select a).GroupBy(y => y.ItemName1).Select(x => x.FirstOrDefault()).ToList();
            
            decimal transP = lstTemp.Sum(p => p.MainTariffPatient);
            decimal transC = lstTemp.Sum(p => p.MainTariffPayer);
            ...
            decimal paymentP = lstPayment.Where(p => p.GCPaymentType == Constant.PaymentType.SETTLEMENT || p.GCPaymentType == Constant.PaymentType.DOWN_PAYMENT).Sum(p => p.NotInDownPayment)
                                - uangMuka;
            decimal paymentC = 0;
