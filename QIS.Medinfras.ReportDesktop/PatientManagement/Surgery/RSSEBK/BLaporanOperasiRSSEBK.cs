...
{
    public partial class BLaporanOperasiRSSEBK : BaseRpt
    {
        public BLaporanOperasiRSSEBK()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            vTestOrderDtParamedicTeam ovTestOrderDtParamedicTeam = BusinessLayer.GetvTestOrderDtParamedicTeamList(string.Format("VisitID = '{0}' AND TestOrderID = {1} AND GCParamedicRole = '{2}'  AND IsDeleted = 0", entity.VisitID, entity.TestOrderID, Constant.SurgeryTeamRole.OPERATOR)).FirstOrDefault();
            ...
            if (ovTestOrderDtParamedicTeam != null)
            {
                cDPJP.Text = ovTestOrderDtParamedicTeam.ParamedicName;
            }
            else
            {
            ...
            #region Footer
            string defaultParamedicCode = entityCV.ParamedicCode;
            string defaultParamedicName = entityCV.ParamedicName;
            if (entity != null)
            {
                ...
                if (entityPs != null)
                {
                    lblTTDTanggal.Text = string.Format("{0} {1}", entityPs.cfStartDateInString, entityPs.StartTime);
                }
                else
                {
                    lblTTDTanggal.Text = "";
                ...
                if (ovTestOrderDtParamedicTeam != null)
                {
                    defaultParamedicCode = ovTestOrderDtParamedicTeam.ParamedicCode;
                    defaultParamedicName = ovTestOrderDtParamedicTeam.ParamedicName; 
                }
            }

            ttdDokter.ImageUrl = string.Format("{0}{1}/Signature/{2}.png", AppConfigManager.QISPhysicalDirectory, AppConfigManager.QISParamedicImagePath, defaultParamedicCode);
            ttdDokter.Visible = true;

            lblSignParamedicName.Text = defaultParamedicName;
