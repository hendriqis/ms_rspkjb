...
{
    public partial class BLaporanOperasiRSSES : BaseRpt
    {
        public BLaporanOperasiRSSES()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            if (lstSetvarDt.Where(t => t.ParameterCode == Constant.SettingParameter.RM0072).FirstOrDefault().ParameterValue == "1")
            {
                ...
                if (entityPs != null)
                {
                    contenPlain = string.Format(@"{0}|{1}|{2}|{3}|{4}", reportMaster.ReportCode, entityCV.VisitID, DateTime.Now.ToString("dd-MMM-yyyy/HH:mm:ss"), entityPs.ParamedicCode, entityPs.ParamedicName);
                }
