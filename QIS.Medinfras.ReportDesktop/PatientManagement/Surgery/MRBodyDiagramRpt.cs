...
{
    public partial class MRBodyDiagramRpt : DevExpress.XtraReports.UI.XtraReport
    {
        public MRBodyDiagramRpt()
        {
            InitializeComponent();
        }
        ...
        private void sbrBodyDiagramDt_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            if (GetCurrentColumnValue("ID") != null)
            {
                ...
                ((MRBodyDiagramDtRpt)subreport.ReportSource).InitializeReport(lstDt.Where(p => p.ID == ID).ToList());
                //subreport.In
            }
        }
