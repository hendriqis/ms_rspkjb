...
{
    public partial class BLaporanOperasiRSPKSB : BaseRpt
    {
        public BLaporanOperasiRSPKSB()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            if (entity != null)
            {
                ...
                if (lstSurgery.Count() > 0)
                {
                    ...
                    mrSurgeryReportRSPKSB.InitializeReport(entityCV.VisitID, Convert.ToInt32(param[1]));
                }
                else
                {
            ...
            if (entity != null)
            {
                List<vPatientSurgery> lstPatientSurgery = BusinessLayer.GetvPatientSurgeryList(string.Format("PatientSurgeryID = {0} AND IsDeleted = 0", entity.PatientSurgeryID));

                if (lstPatientSurgery.Count() > 0)
                {
                    subProcedureRSSK.CanGrow = true;
                    mrProcedureRSSK.InitializeReport(entity.VisitID, Convert.ToInt32(param[1]));
                }
                else
                {
                    subProcedureRSSK.Visible = false;
                }
            }
            else
            {
            ...
            if (entity != null)
            {
                lblTTDTanggal.Text = entity.cfReportDate;
