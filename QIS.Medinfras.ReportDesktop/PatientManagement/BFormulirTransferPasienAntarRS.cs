...
{
    public partial class BFormulirTransferPasienAntarRS : BaseRpt
    {

        public BFormulirTransferPasienAntarRS()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            ChiefComplaint entityComplaint = BusinessLayer.GetChiefComplaintList(string.Format("VisitID = {0} AND IsDeleted = 0", param[0])).FirstOrDefault();
            ...
            vDataTransferPasien obj = BusinessLayer.GetvDataTransferPasienList(string.Format("RegistrationID = {0}", entityCV.RegistrationID)).FirstOrDefault();
            ...
            cBirthDay.Text = entityPatient.cfDOBInDatePicker;
            cMedicalNo.Text = entityPatient.MedicalNo;
            cGender.Text = entityCV.Gender;

            if (obj.ChiefComplaintText == null)
            {
                lblChieftComplaint.Text = "";
            }
            else
            {
                lblChieftComplaint.Text = obj.ChiefComplaintText;
            }
            if (obj.RiwayatPenyakit == null)
            {
                lblMedicalSummary.Text = "";
            }
            else
            {
                lblMedicalSummary.Text = obj.RiwayatPenyakit;
            }
            if (obj.VitalSign == null)
            {
                lblVitalSign.Text = "";
            }
            else
            {
                lblVitalSign.Text = obj.VitalSign;
            }    
           ...
           // xrTableCell20.Text = string.Format("I hereby authorize any physician of {0}, who has been giving me health service due to what ever the causes are, to give my medical record /information including my past medical history to company/insurance as written above. And I will pay the exceeded a mount in accordance with the company procedure as written in my policy insurance. I declare that all this statement was written truthfully without force from any one. Disciosure, copying, altering or communication of this message if you are not the addresses is prohibit.", oHealthcare.HealthcareName);
            ...
            #region Alergi
            List<vPatientAllergy> lstEntityAlergi = BusinessLayer.GetvPatientAllergyList(string.Format("MRN = {0}", entityCV.MRN));

            if (lstEntityAlergi.Count() > 0)
            {
                subAlergi.CanGrow = true;
                DetailSummaryAllergyRptRSPBT.InitializeReport(entityCV.MRN);
            }
            else
            {
                subAlergi.Visible = false;
            }
            #endregion

            #region Imaging Order
            List<vTestOrderHd> lstTestOrder = BusinessLayer.GetvTestOrderHdList(string.Format("VisitID = {0} AND GCTransactionStatus != '{1}'", entityCV.VisitID, Constant.TransactionStatus.VOID));

            if (lstTestOrder.Count() > 0)
            {
                subOrderPenunjang.CanGrow = true;
                DetailSummaryTestOrderRptRSPBT.InitializeReport(entityCV.VisitID);
            }
            else
            {
                subOrderPenunjang.Visible = false;
            }
            #endregion
            ...
            if (lstentityDiagnose.Count() > 0)
            {
                ...
                DetailSummaryDiagnosisRptRSPBT1.InitializeReport(entityCV.VisitID);
            }
            else
            {
            ...
            if (lstVitalSignHd.Count() > 0)
            {
                ...
                DetailSummaryVitalSignRpt.InitializeReport(entityCV.VisitID, 1);
            }
            else
            {
            ...
            //    DetailSummaryReviewOfSystemRptRSPBT.InitializeReport(entityCV.VisitID, 1);
            //}
            //else
            //{
            ...
            #region Review of System
            List<ReviewOfSystemHd> oROSHd = BusinessLayer.GetReviewOfSystemHdList(string.Format("VisitID = {0} AND IsDeleted = 0", entityCV.VisitID));
            //if (oLabResultHd != null)
            if (oROSHd.Count != 0)
            {
                List<vReviewOfSystemDt> lstROS = BusinessLayer.GetvReviewOfSystemDtList(string.Format("ID = {0} AND IsNormal = 0 AND IsDeleted = 0", oROSHd.FirstOrDefault().ID));
                //if (lstLabResultDt != null)
                if (lstROS.Count != 0)
                {
                    subReviewOfSystem.CanGrow = true;
                    DetailSummaryReviewOfSystemRptRSPBT.InitializeReport(lstROS);
                }
            }
            //List<vReviewOfSystemHd> lstReviewOfSystemHd = BusinessLayer.GetvReviewOfSystemHdList(string.Format("VisitID = {0} AND IsDeleted = 0", entityCV.VisitID));
            //if (lstReviewOfSystemHd.Count() > 0)
            //{
            //    subReviewOfSystem.CanGrow = true;
            //    DetailSummaryReviewOfSystemRptRSPBT.InitializeReport(entityCV.VisitID);
            //}
            //else
            //{
            //    xrLabel13.Visible = false;
            //    subReviewOfSystem.Visible = false;
            //}
            //List<vReviewOfSystemDt> lstHdReviewOfSystem = BusinessLayer.GetvReviewOfSystemDtList(string.Format("VisitID = {0} AND IsDeleted = 0", entityCV.VisitID));

            //subReviewOfSystem.CanGrow = true;
            //DetailSummaryReviewOfSystemRptRSPBT.InitializeReport(lstHdReviewOfSystem);
            //if (lstHdReviewOfSystem.Count() > 0)
            //{
            //    subReviewOfSystem.CanGrow = true;
            //    DetailSummaryReviewOfSystemRptRSPBT.InitializeReport(entityCV.VisitID,1);
