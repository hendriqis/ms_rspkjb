...
{
    public partial class BPatientItemPerBillingGroupForPatient_RSAJ : BaseCustomDailyPotraitRpt
    {
        ...
        public BPatientItemPerBillingGroupForPatient_RSAJ()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            cINACBGs.Text = string.Format("Nilai INACBGs {0}", entity.ChargeClassName);
            ...
            decimal allRS = lstPatientBill.Sum(a => a.TotalPatientAmount + a.TotalPayerAmount);
            decimal bpjs = lstPatientBill.Sum(a => a.TotalPayerAmount);
            decimal selisih = allRS - bpjs;

            lblBiayaRS.Text = allRS.ToString("N2");
            lblBatasTanggugan.Text = bpjs.ToString("N2");
            lblSelisih.Text = selisih.ToString("N2");
        ...
        private void cPatientAmount_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            vPatientChargesDtPerBillingGroup current = (GetCurrentRow() as vPatientChargesDtPerBillingGroup);
            if (current != null)
            {
                if (current.PatientAmount == 0)
                {
