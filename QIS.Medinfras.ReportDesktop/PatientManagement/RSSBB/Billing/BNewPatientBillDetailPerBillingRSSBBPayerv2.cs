...
{
    public partial class BNewPatientBillDetailPerBillingRSSBBPayerv2 : BaseCustomDailyPotraitRpt
    {
        public BNewPatientBillDetailPerBillingRSSBBPayerv2()
        {
            InitializeComponent();
        }
        ...
        {
            List<vPatientBill> lstBill = BusinessLayer.GetvPatientBillList(string.Format("{0} AND GCTransactionStatus != '{1}'",
                param[0], Constant.TransactionStatus.VOID));

            vRegistration entityReg = BusinessLayer.GetvRegistrationList(string.Format("RegistrationID = {0}", lstBill.FirstOrDefault().RegistrationID)).FirstOrDefault();
            ...
            List<GetPatientChargesHdDtPatientBill2> lstHDDT = BusinessLayer.GetPatientChargesHdDtPatientBill2List(entityReg.RegistrationID, lstBill.FirstOrDefault().PatientBillingID);

            List<vPatientPaymentHd> lstPayment = BusinessLayer.GetvPatientPaymentHdList(string.Format("RegistrationID IN ({0}) AND GCTransactionStatus != '{1}' AND BillingNo LIKE '%{2}%'",
                entityReg.RegistrationID, Constant.TransactionStatus.VOID, lstBill.FirstOrDefault().PatientBillingNo));
            List<vPatientPaymentHdDt> lstPaymentHdDt = BusinessLayer.GetvPatientPaymentHdDtList(string.Format("RegistrationID IN ({0}) AND GCTransactionStatus != '{1}' AND BillingNo LIKE '%{2}%' AND GCPaymentType = '{3}' ORDER BY PaymentDate",
                entityReg.RegistrationID, Constant.TransactionStatus.VOID, lstBill.FirstOrDefault().PatientBillingNo, Constant.PaymentType.AR_PAYER));
            ...
            lblPatientBillNo.Text = string.Format("Billing No : {0}", lstBill.FirstOrDefault().PatientBillingNo);
            ...
            if (entityReg.BusinessPartnerID != 1)
            {
                billPaymentDetailTransactionPerBillingRSSBBPayer.InitializeReport(lstHDDT.Where(a => a.PayerAmount < 0 || a.PayerAmount > 0 || (a.PatientAmount + a.PayerAmount) == 0).ToList());
            }
            else
            {
                billPaymentDetailTransactionPerBillingRSSBBPayer.InitializeReport(lstHDDT.Where(a => a.PayerAmount != 0).ToList());
            }
            #endregion

            #region Discount
            subDiscount.CanGrow = true;
            billPaymentDetailDiscountAllDMPayer.InitializeReport(entityReg.RegistrationID);
            #endregion
            ...
            decimal roundingP = lstBill.Sum(a => a.PatientRoundingAmount);
            decimal roundingC = lstBill.Sum(a => a.PayerRoundingAmount);
            ...
            decimal balanceP = transP - discP - paymentP;
            decimal balanceC = billC;
            ...
            if (lstHDDT.Where(a => a.PayerAmount != 0 && a.TariffComp2 != 0).ToList().Count() > 0)
            {
                subTariffComp2.CanGrow = true;
                billPaymentDetailTransactionPerBillingComp2RSSBB.InitializeReport(lstHDDT.Where(a => a.PayerAmount != 0 && a.TariffComp2 != 0).ToList());
            }
            else
            {
                xrLabel3.Visible = false;
                lblDoctorFeeInfo.Visible = false;
