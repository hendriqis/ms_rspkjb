...
{
    public partial class BBuktiOrderReturResepRSSBB : BaseDailyPortraitRpt
    {
        ...
        public BBuktiOrderReturResepRSSBB()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            vPrescriptionReturnOrderDt entityDt = BusinessLayer.GetvPrescriptionReturnOrderDtList(param[0]).FirstOrDefault();
            vPrescriptionReturnOrderHd entityHd = BusinessLayer.GetvPrescriptionReturnOrderHdList(param[0]).FirstOrDefault();
            vRegistration entityReg = BusinessLayer.GetvRegistrationList(string.Format("VisitID = {0}", entityHd.VisitID)).FirstOrDefault();
            //UserAttribute entityUA = BusinessLayer.GetUserAttributeList(string.Format("UserID = {0}", entityHd.LockDownBy)).FirstOrDefault();
            ...
            cTglOrder.Text = entityHd.OrderDateInString;
            ...
            cTransaction.Text = string.Format("{0} | {1}", entityHd.PrescriptionReturnOrderNo, entityHd.LastUpdatedDate);
            cServiceUnitName.Text = entityHd.LocationName;
          ...
          //  cHeaderPatient.Text = string.Format("{0}", entityReg.MedicalNo);
           // cHeaderRegTrans.Text = string.Format("{0} | {1}", entityReg.RegistrationNo, entityHd.TestOrderNo);
            ...
            if (entityHd.LastUpdatedBy != 0 && entityHd.LastUpdatedBy != null)
            {
                lblLastUpdatedBy.Text = BusinessLayer.GetUserAttribute(entityHd.LastUpdatedBy).FullName;
            }
            else
            {
                lblLastUpdatedBy.Text = BusinessLayer.GetUserAttribute(entityHd.CreatedBy).FullName;
            }
            //lblLastUpdatedBy.Text = string.Format("{0}", entityHd.LastUpdatedBy);
