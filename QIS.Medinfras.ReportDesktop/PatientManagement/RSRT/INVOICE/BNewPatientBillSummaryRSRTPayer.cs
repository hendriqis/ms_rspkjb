...
{
    public partial class BNewPatientBillSummaryRSRTPayer : BaseCustomDailyPotraitRSRTRpt
    {
        public BNewPatientBillSummaryRSRTPayer()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            if (entityReg.BusinessPartnerID != 1)
            {
                billPaymentSummaryTransactionRSRTPayer.InitializeReport(lstHDDT.Where(a => a.PayerAmount < 0 || a.PayerAmount > 0 || (a.PatientAmount + a.PayerAmount) == 0).ToList());
            }
            else
            {
                billPaymentSummaryTransactionRSRTPayer.InitializeReport(lstHDDT.Where(a => a.PayerAmount != 0).ToList());
            }
            #endregion

            #region Discount
            subDiscount.CanGrow = true;
            billPaymentDetailDiscountAllDMPayerRSRT.InitializeReport(entityReg.RegistrationID);
            #endregion
            ...
            billPaymentDetailDiscComp2TransactionRSSBB.InitializeReport(lstHDDT.Where(a => a.cfDiscountAmountComp2 != 0).ToList());
            #endregion
