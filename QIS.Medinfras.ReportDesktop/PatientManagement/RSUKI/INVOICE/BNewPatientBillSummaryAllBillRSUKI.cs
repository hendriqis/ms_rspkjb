...
{
    public partial class BNewPatientBillSummaryAllBillRSUKI : BaseCustomDailyPotraitRpt
    {
        public BNewPatientBillSummaryAllBillRSUKI()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            vRegistrationBPJS entityRegBPJS = BusinessLayer.GetvRegistrationBPJSList(param[0]).FirstOrDefault();
            ...
            List<GetPatientChargesHdDtAllBill> lstHDDT = BusinessLayer.GetPatientChargesHdDtAllBillList(entityReg.RegistrationID);
            ...
            cDOB.Text = string.Format("{0} / {1}y / {2}", entityReg.DateOfBirthInString, entityReg.PatientAgeInYear, entityReg.cfGenderInitial);
            cRegisteredPhysician.Text = entityReg.ParamedicName;
            if (entityRegBPJS != null)
            {
                cNoPeserta.Text = entityRegBPJS.NoPeserta;
            }
            else
            {
                cNoPeserta.Text = "";
            ...
            string physicianDischarge = entityReg.PhysicianDischargeOrderDate.ToString(Constant.FormatString.DATE_FORMAT);
            string registrationDate = entityReg.RegistrationDate.ToString(Constant.FormatString.DATE_FORMAT);
            string dischargeDateInfo = "-";

            if (entityRegLinked != null)
            {
                cRegistrationDate.Text = string.Format("{0} {1}", entityRegLinked.RegistrationDate.ToString(Constant.FormatString.DATE_FORMAT), entityRegLinked.RegistrationTime);
            }
            else
            {
                cRegistrationDate.Text = string.Format("{0} {1}", registrationDate, entityReg.RegistrationTime);
            }

            if (entityReg.PhysicianDischargeOrderDate.Date.ToString(Constant.FormatString.DATE_FORMAT_112) != "19000101")
            {
                dischargeDateInfo = string.Format("{0} {1}", physicianDischarge, entityReg.PhysicianDischargeOrderTime);
            }
            else
            {
                dischargeDateInfo = "-";
            ...
            billPaymentSummaryTransactionAllBill.InitializeReport(lstHDDT);
            #endregion
            ...
            #region Footer
            if (dischargeDateInfo != "-" || dischargeDateInfo != "")
            {
                lblTanggal.Text = string.Format("{0}, {1} {2}", entityHealthcare.City, dischargeDateInfo, entityReg.PhysicianDischargeOrderTime);
            }
            else
            {
                lblTanggal.Text = string.Format("{0}, {1} {2}", entityHealthcare.City, DateTime.Now.ToString(Constant.FormatString.DATE_FORMAT), DateTime.Now.ToString(Constant.FormatString.TIME_FORMAT));
            }
            ...
            billPaymentDetailDiscComp2TransactionAllBillRSSBB.InitializeReport(lstHDDT.Where(a => a.cfDiscountAmountComp2 != 0).ToList());
            #endregion
            ...
            ////    billPaymentSummaryTransactionAllBillComp2.InitializeReport(lstHDDTComp2);
            ////}
            ////else
            ////{
