...
{
    public partial class BSuratKeteranganKelahiranRSSK : BaseRpt
    {
        public BSuratKeteranganKelahiranRSSK()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            #region Detail
            lblSklNo.Text = string.Format("NO : {0}", entity.ReferenceNo);
            cellTTD.Text = entityVisitMom.ParamedicName;
            string gender = "";
            if (entity.Gender == "Perempuan") {
                gender = "FEMALE";
            }
            else if (entity.Gender == "Laki-Laki") {
                gender = "MALE";
            }
            ...
            cellHari.Text = string.Format("{0} / {1}", dobData[0].ToUpper(), dobData[1].ToUpper());
            cellTanggalLahir.Text = GetMonthFormat(entity.DateOfBirth) ; 
          
            cellPanjang.Text = string.Format("{0} Cm", entity.Length.ToString(Constant.FormatString.NUMERIC_2));
            ...
            cellBerat.Text = string.Format("{0} Gr", weightGram.ToString(Constant.FormatString.NUMERIC_2));
            cellJam.Text = string.Format("{0} WIB", entity.TimeOfBirth);

            cellJenisPersalinan.Text = entity.BirthMethod.ToString().ToUpper();
            cellKembar.Text = entity.TwinSingle.ToString().ToUpper();
            cellNamaIbu.Text = entity.MotherFullName.ToString().ToUpper();
            cellNamaAyah.Text = entity.FatherFullName.ToString().ToUpper();
            ...
            lblTtdName.Text = string.Format("({0})", entityVisitMom.ParamedicName);
            lblttd.Text = string.Format("{0} {1}", oHealthcare.City, GetMonthFormat(DateTime.Now));
            #endregion
