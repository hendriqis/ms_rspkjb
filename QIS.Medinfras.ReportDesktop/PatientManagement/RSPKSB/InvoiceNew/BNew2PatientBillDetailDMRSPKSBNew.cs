...
{
    public partial class BNew2PatientBillDetailDMRSPKSBNew : BaseCustomDailyPotraitRpt
    {
        public BNew2PatientBillDetailDMRSPKSBNew()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            List<GetPatientChargesHdDtALLRSPKSB> lstHDDT = BusinessLayer.GetPatientChargesHdDtALLRSPKSBList(entityReg.RegistrationID);
            ...
            if (entityReg.BusinessPartnerID != 1)
            {
                ...
                foreach (vRegistrationPayer rp in lstRegPayer)
                {
                    ...
                    {
                        if (rp.BillToID != null && rp.BillToID != 0)
                        {
                            corporate = lstRegPayer.Where(a => a.GCCustomerType == Constant.CustomerType.BPJS).FirstOrDefault().BillToName;
                        }
                        else
                        {
                            corporate = lstRegPayer.Where(a => a.GCCustomerType == Constant.CustomerType.BPJS).FirstOrDefault().BusinessPartnerName;
                        }
                    }
                    else
                    {
            ...
            lblNPWP.Text = string.Format("PT Permata Husada Utama, NPWP : {0}", entityHealthcare.TaxRegistrantNo);

            cCorporate.Text = corporate;
            if (entityEC != null)
            {
                cEmergencyContact.Text = entityEC.ContactName;
            }
            else
            {
                cEmergencyContact.Text = "";
            }
            ...
            //string filterRegPayer = string.Format("RegistrationID = {0} AND IsDeleted = 0", entityReg.RegistrationID);
            //List<vRegistrationPayer> lstRp = BusinessLayer.GetvRegistrationPayerList(filterRegPayer);
            //if (lstRp.Count() > 0)
            //{
            //    string bpNameList = "";
            //    foreach (vRegistrationPayer rp in lstRp)
            //    {
            //        if (bpNameList != "")
            //        {
            //            bpNameList += ", ";
            //        }
            //        bpNameList += rp.BusinessPartnerName;
            //    }

            //    cCorporateName.Text = bpNameList;
            //}
            //else
            //{
            //    cCorporateName.Text = entityReg.BusinessPartnerName;
            //}
            ...
            new2BillPaymentDetailTransactionDMRSPKSBNew.InitializeReport(lstHDDT);
            #endregion

            #region Discount
            subDiscount.CanGrow = true;
            billPaymentDetailDiscountAllDMRSPKSBNew.InitializeReport(entityReg.RegistrationID);
            #endregion

            #region Payment All
            subPaymentAll.CanGrow = true;
            billPaymentDetailPaymentAllDMRSPKSBNew.InitializeReport(lstPaymentHdDt);
            #endregion
