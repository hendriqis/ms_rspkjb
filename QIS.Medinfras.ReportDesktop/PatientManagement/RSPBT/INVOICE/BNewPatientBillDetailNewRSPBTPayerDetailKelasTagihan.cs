...
{
    public partial class BNewPatientBillDetailNewRSPBTPayerDetailKelasTagihan : BaseCustomDailyPotraitRpt
    {
        public BNewPatientBillDetailNewRSPBTPayerDetailKelasTagihan()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            if (entityReg.BusinessPartnerID != 1)
            {
                billPaymentDetailTransactionRSPBTPayer.InitializeReport(lstHDDT.Where(a => a.PayerAmount < 0 || a.PayerAmount > 0 || (a.PatientAmount + a.PayerAmount) == 0).ToList());
            }
            else
            {
                billPaymentDetailTransactionRSPBTPayer.InitializeReport(lstHDDT.Where(a => a.PayerAmount != 0).ToList());
            }
            #endregion
