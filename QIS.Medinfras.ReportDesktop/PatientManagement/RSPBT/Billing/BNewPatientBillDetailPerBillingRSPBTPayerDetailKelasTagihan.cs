...
{
    public partial class BNewPatientBillDetailPerBillingRSPBTPayerDetailKelasTagihan : BaseCustomDailyPotraitRpt
    {
        public BNewPatientBillDetailPerBillingRSPBTPayerDetailKelasTagihan()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            //List<GetPatientChargesHdDtAll5> lstHDDT = BusinessLayer.GetPatientChargesHdDtAll5List(entityReg.RegistrationID);
            List<GetPatientChargesHdDtALLChargeClass> lstHDDT = BusinessLayer.GetPatientChargesHdDtALLChargeClassList(entityReg.RegistrationID, lstBill.FirstOrDefault().PatientBillingID);
            ...
            List<vPatientPaymentHdDt> lstPaymentHdDt = BusinessLayer.GetvPatientPaymentHdDtList(string.Format("RegistrationID IN ({0}) AND GCTransactionStatus != '{1}' AND BillingNo LIKE '%{2}%' ORDER BY PaymentDate",
                entityReg.RegistrationID, Constant.TransactionStatus.VOID, lstBill.FirstOrDefault().PatientBillingNo));
            ...
            cRegistrationDate.Text = string.Format("{0} {1}", entityReg.RegistrationDateInString, entityReg.RegistrationTime);
            ...
            cServiceUnitChargeClass.Text = string.Format("{0} | {1}", entityReg.ServiceUnitName, entityReg.ChargeClassName);
            ...
            if (entityReg.BusinessPartnerID != 1)
            {
                billPaymentDetailTransactionRSPBTPayerKelasTagihan.InitializeReport(lstHDDT.Where(a => a.PayerAmount < 0 || a.PayerAmount > 0 || (a.PatientAmount + a.PayerAmount) == 0).ToList());
            }
            else
            {
                billPaymentDetailTransactionRSPBTPayerKelasTagihan.InitializeReport(lstHDDT.Where(a => a.PayerAmount != 0).ToList());
            }
            #endregion
            ...
            decimal paymentP = lstPayment.Where(p => p.GCPaymentType == Constant.PaymentType.SETTLEMENT || p.GCPaymentType == Constant.PaymentType.CUSTOM || 
                    p.GCPaymentType == Constant.PaymentType.DOWN_PAYMENT).Sum(p => p.NotInDownPayment);
            decimal paymentC = 0;
            ...
            billPaymentDetailPaymentAllDMRSPBT.InitializeReport(lstPaymentHdDt);
            #endregion
