...
{
    public partial class BNewPatientBillPaymentSummaryMCURSSES : BaseCustomDailyPotraitRpt
    {
        public BNewPatientBillPaymentSummaryMCURSSES()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            List<GetPatientChargesHdDtAllPerBillingMCURSSES> lstHDDT = BusinessLayer.GetPatientChargesHdDtAllPerBillingMCURSSESList(entityReg.RegistrationID, entityPayment.PaymentID);
            ...
            if (entityPayment.GCPaymentType != Constant.PaymentType.DOWN_PAYMENT)
            {
                ...
                billPaymentSummaryTransactionByPaymentMCURSSES.InitializeReport(lstHDDT);
            }
            else
            {
            ...
            //List<GetPatientChargesHdDtAllPerBillingMCURSSES> lstTemp = (from a in lstHDDT orderby a.ItemName1 select a).GroupBy(y => y.ItemName1).Select(x => x.FirstOrDefault()).ToList();

            decimal transP = lstHDDT.Sum(p => p.DetailPatientAmount);
            decimal transC = lstHDDT.Sum(p => p.DetailPayerAmount);
