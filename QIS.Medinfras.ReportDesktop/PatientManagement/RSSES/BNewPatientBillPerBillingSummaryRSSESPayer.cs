...
{
    public partial class BNewPatientBillPerBillingSummaryRSSESPayer : BaseCustomDailyPotraitRpt
    {
        public BNewPatientBillPerBillingSummaryRSSESPayer()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            if (entityReg.BusinessPartnerID != 1)
            {
                billPaymentPerBillingSummaryTransactionRSSESPayer.InitializeReport(lstHDDT.Where(a => a.PayerAmount < 0 || a.PayerAmount > 0 || (a.PatientAmount + a.PayerAmount) == 0).ToList());
            }
            else
            {
                billPaymentPerBillingSummaryTransactionRSSESPayer.InitializeReport(lstHDDT.Where(a => a.PayerAmount != 0).ToList());
            }
            #endregion
