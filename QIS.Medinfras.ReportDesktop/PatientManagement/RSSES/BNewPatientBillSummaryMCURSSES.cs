...
{
    public partial class BNewPatientBillSummaryMCURSSES : BaseCustomDailyPotraitRpt
    {
        public BNewPatientBillSummaryMCURSSES()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            List<GetPatientChargesHdDtAllMCURSSES> lstHDDT = BusinessLayer.GetPatientChargesHdDtAllMCURSSESList(entityReg.RegistrationID);
            ...
            billPaymentSummaryTransactionMCURSSES.InitializeReport(lstHDDT);
            #endregion

            #region Discount
            subDiscount.CanGrow = true;
            billPaymentDetailDiscountAllWithoutRemarksDM.InitializeReport(entityReg.RegistrationID);
            #endregion
            ...
            List<GetPatientChargesHdDtAllMCURSSES> lstTemp = (from a in lstHDDT orderby a.ItemName1 select a).GroupBy(y => y.ItemName1).Select(x => x.FirstOrDefault()).ToList();

            decimal transP = lstTemp.Sum(p => p.DetailPatientAmount);
            decimal transC = lstTemp.Sum(p => p.DetailPayerAmount);
