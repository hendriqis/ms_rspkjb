...
{
    public partial class BNewPatientBillSummaryRSPPayer : BaseCustomDailyPotraitRpt
    {
        public BNewPatientBillSummaryRSPPayer()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            List<GetPatientChargesHdDtAll2> lstHDDT = BusinessLayer.GetPatientChargesHdDtAll2List(entityReg.RegistrationID);
            ...
            if (entityReg.BusinessPartnerID != 1)
            {
                billPaymentSummaryTransactionRSPPayer.InitializeReport(lstHDDT.Where(a => a.PayerAmount < 0 || a.PayerAmount > 0 || (a.PatientAmount + a.PayerAmount) == 0).ToList());
            }
            else
            {
                billPaymentSummaryTransactionRSPPayer.InitializeReport(lstHDDT.Where(a => a.PayerAmount != 0).ToList());
            }
            #endregion

            #region Discount
            subDiscount.CanGrow = true;
            billPaymentDetailDiscountAllDMPayerRSP.InitializeReport(entityReg.RegistrationID);
            #endregion
            ...
            cCash.Text = 0.ToString("N2");
            cDebitCard.Text = 0.ToString("N2");
            cCreditCard.Text = 0.ToString("N2");
            cTransferBank.Text = 0.ToString("N2");
            cVoucher.Text = 0.ToString("N2");
            cDownPayment.Text = 0.ToString("N2");
            cRefund.Text = 0.ToString("N2");
            ...
            #region Payment Deposit All

            List<vPatientPaymentHdDtRSP> lstDeposit = BusinessLayer.GetvPatientPaymentHdDtRSPList(string.Format("RegistrationID IN ({0}) AND GCTransactionStatus != '{1}' AND (  DownPaymentOut > 0 OR NotInDownPayment > 0 ) AND GCPaymentType IN ('{2}', '{3}')",
                entityReg.RegistrationID,
                Constant.TransactionStatus.VOID,
                Constant.PaymentType.DEPOSIT_IN,
                Constant.PaymentType.DEPOSIT_OUT
                ));
            if (lstDeposit.Count > 0)
            {
                subPaymentAllDMDeposit.CanGrow = true;
                billPaymentDetailPaymentAllDMRSPDeposite.InitializeReport(lstDeposit);
            }
            else
            {
                subPaymentAllDMDeposit.Visible = false;
                //// billPaymentDetailPaymentAllDMRSPDeposite.Visible = false;
            }

            #endregion

            #region Payment All
            string filter = string.Format("RegistrationID IN ({0}) AND GCTransactionStatus != '{1}'",
            entityReg.RegistrationID, Constant.TransactionStatus.VOID);
            List<vPatientPaymentHdDtRSP> lstPaymentHdDt = BusinessLayer.GetvPatientPaymentHdDtRSPList(string.Format("RegistrationID IN ({0}) AND GCTransactionStatus != '{1}' AND (  DownPaymentOut > 0 OR NotInDownPayment > 0 )  AND GCPaymentType NOT IN ('{2}', '{3}') ",
                entityReg.RegistrationID,
                Constant.TransactionStatus.VOID,
                Constant.PaymentType.DEPOSIT_IN,
                Constant.PaymentType.DEPOSIT_OUT
            ...
            billPaymentDetailPaymentAllDMRSP.InitializeReport(lstPaymentHdDt.Where(a => a.GCPaymentType == Constant.PaymentType.AR_PAYER).ToList());
            #endregion
            ...
            if (lstBill.Count > 0)
            {
                billingdate = lstBill.FirstOrDefault().CreatedDate.ToString(Constant.FormatString.DAY_DATE_TIME_FORMAT);
                int? userid = lstBill.FirstOrDefault().CreatedBy;
                if (userid > 0 && userid != null)
                {
                    vUser oUser = BusinessLayer.GetvUserList(string.Format("UserID='{0}'", userid)).FirstOrDefault();
                    createdby = oUser.FullName;
                }

            }
            lblTanggal.Text = string.Format("{0}, {1}", entityHealthcare.City, billingdate);
            lblTandaTangan.Text = createdby;

            //if (entityReg.DepartmentID == Constant.Facility.INPATIENT)
            //{
            //    lblCatatan.Text = string.Format("●     PPN dibebaskan sesuai PP No. 49 tahun 2022 (jasa pelayanan kesehatan)");
            //    lblCatatan2.Visible = false;
            //}
            //else
            //{
            //    lblCatatan.Text = string.Format("●     Obat Rawat Jalan sudah termasuk PPN");
            //    lblCatatan2.Text = string.Format("●     PPN dibebaskan sesuai PP No. 49 tahun 2022 (jasa pelayanan kesehatan)");
            //}
            #endregion

            #region Discount Comp2 Custom RSSBB
            subDiscountComp2.CanGrow = true;
            billPaymentDetailDiscComp2TransactionRSP.InitializeReport(lstHDDT.Where(a => a.cfDiscountAmountComp2 != 0).ToList());
            #endregion
            ...
            //    billPaymentSummaryTransactionComp2RSSBB.InitializeReport(lstHDDTComp2);
            //}
            //else
            //{
