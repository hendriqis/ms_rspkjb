...
{
    public partial class BPaymentReceiptOutpatientRSP : BaseCustomDailyPotraitRpt
    {
        public BPaymentReceiptOutpatientRSP()
        {
            InitializeComponent();
        }
        ...
        {
            vRegistration entityReg = BusinessLayer.GetvRegistrationList(string.Format("{0}", param[0])).FirstOrDefault();
            PaymentReceipt oPayementRecive = BusinessLayer.GetPaymentReceiptList(string.Format("{0}", param[1])).FirstOrDefault();
            lblPaymentReceiptNo.Text = oPayementRecive.PaymentReceiptNo;
            string ReceiptAmountStr = Function.NumberWithPointInWordsInIndonesian(Convert.ToDouble(oPayementRecive.ReceiptAmount), true);
            lblTerbilang.Text = string.Format("Terbilang : #{0}#", ReceiptAmountStr);
            lblPrintTo.Text = string.Format("Cetakan ke- {0}",oPayementRecive.PrintNumber);
            ...
            int PaymentReceiptID = oPayementRecive.PaymentReceiptID;
            List<GetPatientChargesHdDtAllPerBillingByPaymentReceiptID> lstHDDT = BusinessLayer.GetPatientChargesHdDtAllPerBillingByPaymentReceiptIDList(entityReg.RegistrationID, PaymentReceiptID);

            string paymentID = "";
            List<PatientPaymentHd> lstPatientPaymentHd = BusinessLayer.GetPatientPaymentHdList(string.Format("GCTransactionStatus  != '{0}' AND PaymentReceiptID={1} ",  Constant.TransactionStatus.VOID, PaymentReceiptID));
            foreach(PatientPaymentHd row in lstPatientPaymentHd){
                paymentID += string.Format("'{0}',", row.PaymentID);
                //if (row.GCPaymentType != Constant.PaymentType.DOWN_PAYMENT)
                //{
                //    paymentID += string.Format("'{0}',", row.PaymentID);
                //}
            }
            if (lstPatientPaymentHd.Count > 0)
            {
                paymentID = paymentID.Remove(paymentID.Length - 1, 1);
            }
            else {
                paymentID = "0";
            }
            ...
            List<PatientBillPayment> lstEntityBillPayment = BusinessLayer.GetPatientBillPaymentList(string.Format("PaymentID IN ({0})", paymentID));

            List<vPatientPaymentHd> lstPayment = BusinessLayer.GetvPatientPaymentHdList(string.Format("PaymentID IN ({0}) AND GCTransactionStatus != '{1}'",
                paymentID, Constant.TransactionStatus.VOID));
            List<vPatientPaymentHdDt> lstPaymentHdDt = BusinessLayer.GetvPatientPaymentHdDtList(string.Format("PaymentID IN ({0}) AND GCTransactionStatus != '{1}'",
                paymentID, Constant.TransactionStatus.VOID));
            ...
            cCorporate.Text = string.Format("{0} | {1}", entityReg.RoomName, entityReg.BedCode);
            ...
            billPaymentDetailTransactionByPayment.InitializeReport(lstHDDT);
            ...
            billPaymentDetailDiscountByBillingRSP.InitializeReport(entityReg.RegistrationID, billID);
            #endregion

            #region Payment
            subPayment.CanGrow = true;
            billPaymentDetailPayment.InitializeReport(lstPaymentHdDt);
            #endregion
            ...
            //decimal balanceP = transP - discP + roundingP - paymentP;
            //decimal balanceC = transC - discC + roundingC - paymentC;
            ...
            vUser ouser = BusinessLayer.GetvUserList(string.Format("userid='{0}'", oPayementRecive.CreatedBy)).FirstOrDefault();
            string PrintedBy = ""; 
            if (ouser != null) {
                PrintedBy = ouser.FullName;
            }
            cTTDPrintedBy.Text = PrintedBy;
            #endregion
