...
{
    public partial class BNewPaymentReceipt34DtRekap : BaseRpt
    {
        public BNewPaymentReceipt34DtRekap()
        {
            InitializeComponent();
        }

        public void InitializeReport(List<vPatientBill> lstBill, Int32 ReceiptID)
        {
            List<vPatientPaymentHd> lstPayment = BusinessLayer.GetvPatientPaymentHdList(string.Format("RegistrationID IN ({0}) AND GCTransactionStatus != '{1}' AND PaymentReceiptID = '{2}'",
                AppSession.RegisteredPatient.RegistrationID, Constant.TransactionStatus.VOID, ReceiptID));
