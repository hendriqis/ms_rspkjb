...
{
    public partial class BNewPaymentReceipt4_1_DtRekap : BaseRpt
    {
        public BNewPaymentReceipt4_1_DtRekap()
        {
            InitializeComponent();
        }

        public void InitializeReport(List<vPatientBill> lstBill, int RegistrationID)
        {
            List<vPatientPaymentHd> lstPayment = new List<vPatientPaymentHd>();

            if (lstBill.Count() > 0)
            {
                lstPayment = BusinessLayer.GetvPatientPaymentHdList(string.Format("RegistrationID IN ({0}) AND GCTransactionStatus != '{1}'",
                lstBill.FirstOrDefault().RegistrationID, Constant.TransactionStatus.VOID));
            }
            else
            {
                lstPayment = BusinessLayer.GetvPatientPaymentHdList(string.Format("RegistrationID IN ({0}) AND GCTransactionStatus != '{1}'",
