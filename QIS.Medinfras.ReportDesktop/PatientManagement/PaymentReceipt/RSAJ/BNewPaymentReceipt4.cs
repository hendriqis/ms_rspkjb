...
{
    public partial class BNewPaymentReceipt4 : BaseRpt
    {
        private string city = "";

        public BNewPaymentReceipt4()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            RegistrationBPJS regBPJS = BusinessLayer.GetRegistrationBPJS(lstEntity.FirstOrDefault().RegistrationID);
            if (regBPJS != null)
            {
                cSEPInfo.Text = String.Format("SEP No : {0}" ,regBPJS.NoSEP);
            }
            else 
            {
                cSEPInfo.Visible = false;        
            }
