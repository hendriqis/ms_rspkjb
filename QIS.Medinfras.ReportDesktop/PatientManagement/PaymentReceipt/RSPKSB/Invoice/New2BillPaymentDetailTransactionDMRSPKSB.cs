...
{
    public partial class New2BillPaymentDetailTransactionDMRSPKSB : DevExpress.XtraReports.UI.XtraReport
    {
        public New2BillPaymentDetailTransactionDMRSPKSB()
        {
            InitializeComponent();
        }
        ...
        {
            Decimal TariffComp2 = Convert.ToDecimal(GetCurrentColumnValue("TariffComp2"));

            string test = Detail.HeightF.ToString();

            if (TariffComp2 != 0)
            {
                xrTariffComp1.Visible = true;
                Detail.HeightF = 60;
            }
            else
            {
                xrTariffComp1.Visible = false;
                Detail.HeightF = 15;
            }
        }
    }
