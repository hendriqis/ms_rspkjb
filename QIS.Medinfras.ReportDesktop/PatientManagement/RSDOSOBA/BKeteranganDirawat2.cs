...
{
    public partial class BKeteranganDirawat2 : BaseCustomDailyPotraitRpt
    {
        public BKeteranganDirawat2()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            lblDiagnosa.Text = string.Format("{0}", param[5]);
            string[] tempDate = param[1].Split(';');
            String startDate = param[3];
            String endDate = param[4];
            if (endDate == null || endDate == "")
            {
                lblTgl.Text = string.Format("{0} mulai tanggal {1} dan masih dalam masa perawatan.", entityH2.HealthcareName, startDate);
            }
            else
            {
                lblTgl.Text = string.Format("{0} mulai tanggal {1} s/d tanggal {2}.", entityH2.HealthcareName, startDate, endDate);
            }
