...
{
    public partial class BNewPatientBillSummaryEngRSDOSOBAv2 : BaseCustomDailyPotraitRpt
    {
        public BNewPatientBillSummaryEngRSDOSOBAv2()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            List<vPatientPaymentHdDt> lstPaymentHdDt = BusinessLayer.GetvPatientPaymentHdDtList(string.Format("RegistrationID IN ({0}) AND GCTransactionStatus != '{1}'",
                entityReg.RegistrationID, Constant.TransactionStatus.VOID));
            List<vPatientChargesDt> lstChargesDt = BusinessLayer.GetvPatientChargesDtList(string.Format("RegistrationID = {0} AND GCTransactionDetailStatus != '{1}' AND IsDeleted = 0",
                entityReg.RegistrationID, Constant.TransactionStatus.VOID));
            ...
            billPaymentSummaryTransactionEngRSDOSOBAv2.InitializeReport(lstHDDT);
            #endregion
            ...
            decimal transP = lstHDDT.Sum(p => p.PatientAmount + p.DiscountPatient);
            decimal transC = lstHDDT.Sum(p => p.PayerAmount + p.DiscountPayer);
            ...
            decimal discBillP = lstBill.Sum(p => p.PatientDiscountAmount);
            decimal discBillC = lstBill.Sum(p => p.PayerDiscountAmount);

            decimal discChargesP = lstHDDT.Sum(p => p.DiscountPatient);
            decimal discChargesC = lstHDDT.Sum(p => p.DiscountPayer);

            decimal discP = discBillP + discChargesP;
            decimal discC = discBillC + discChargesC;

            decimal billFixP = billP + discChargesP;
            decimal billFixC = billC + discChargesC;
            ...
            decimal gtP = billP - discBillP;
            decimal gtC = amountC - discBillC;
            ...
            if (balanceP != 0)
                balanceTotalInWord = Function.NumberInWordsInEnglish(Convert.ToInt32(balanceP), true);
            else
                balanceTotalInWord = Function.NumberInWordsInEnglish(Convert.ToInt32(balanceC), true);
