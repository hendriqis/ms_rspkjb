...
{
    public partial class BNewPatientBillDetailRSRACv2 : BaseCustomDailyPotraitRpt
    {
        public BNewPatientBillDetailRSRACv2()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            ConsultVisit entityLinkedCV = BusinessLayer.GetConsultVisitList(string.Format("RegistrationID = {0}", entityReg.LinkedRegistrationID)).FirstOrDefault();
            ...
            List<GetPatientChargesHdDtAll9> lstHDDT = BusinessLayer.GetPatientChargesHdDtAll9List(entityReg.RegistrationID);
            ...
            //}
            //else if (entityLinkedCV != null)
            //{
            //    if (entityLinkedCV.VisitID != null && entityLinkedCV.VisitID != 0)
            //    {
            //        ParamedicMaster entityPMR = BusinessLayer.GetParamedicMasterList(string.Format("ParamedicID = {0}", entityLinkedCV.ParamedicID)).FirstOrDefault();
            //        cReferrerPhysician.Text = entityPMR.FullName;
            //    }
            //    else
            //    {
            ...
            //            cReferrerPhysician.Visible = false;
            //        }
            //        else
            //        {
            ...
            cUnitRoomBed.Text = string.Format("{0} | {1} | {2}", entityReg.ServiceUnitName, entityReg.RoomName, entityReg.BedCode);

            cCorporateClass.Text = corporate + " | " + entityReg.ChargeClassName;

            string physicianOrderDischarge = entityReg.cfPhysicianDischargedDateOrderInString;
            string physicianDischarge = entityReg.cfPhysicianDischargedDateTimeInString;
            string discharge = entityReg.DischargeDate.ToString(Constant.FormatString.DATE_FORMAT);
            string dischargeDateInfo = "-";

            if (entityReg.PhysicianDischargeOrderDate != null && physicianOrderDischarge != "01-Jan-1900")
            {
                dischargeDateInfo = physicianOrderDischarge + " " + entityReg.PhysicianDischargeOrderTime;
            }
            else if (entityReg.PhysicianDischargedDate != null && physicianDischarge != "01-Jan-1900")
            {
                dischargeDateInfo = physicianDischarge;
            }
            else
            {
                dischargeDateInfo = "-";
            ...
            cDischargedDate.Text = dischargeDateInfo;
            ...
            billPaymentDetailTransactionRSRAC.InitializeReport(lstHDDT);
            #endregion
            ...
            decimal balanceP = billP - paymentP - discP;
            decimal balanceC = billC - paymentC - discC;

            decimal chargeP = transP + adminP;
            decimal chargeC = transC + adminC;
            ...
            cChargeP.Text = chargeP.ToString("N2");
            cChargeC.Text = chargeC.ToString("N2");
            ...
            String balanceTotalInWord;
            if (balanceP != 0)
                balanceTotalInWord = Function.NumberInWords(Convert.ToInt32(balanceP), true);
            else
                balanceTotalInWord = Function.NumberInWords(Convert.ToInt32(balanceC), true);
            cBalanceTotalInWord.Text = balanceTotalInWord;
            ...
            #region Footer
            lblTanggal.Text = string.Format("{0}, {1}", entityHealthcare.City, dischargeDateInfo);
            ...
            {
                lblPPN.Text = string.Format("*Harga sudah termasuk PPN 11%");
            }
            #endregion
