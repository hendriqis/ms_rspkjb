...
{
    public partial class BillPaymentDetailTransactionMCURSRA : DevExpress.XtraReports.UI.XtraReport
    {
        public BillPaymentDetailTransactionMCURSRA()
        {
            InitializeComponent();
        }

        public void InitializeReport(List<GetPatientChargesHdDtAllMCU2> lst)
        {
            decimal mainTarifPatient = 0, mainTarifPayer = 0;
            List<GetPatientChargesHdDtAllMCU2> lstTemp = (from a in lst orderby a.ItemName1 select a).GroupBy(y => y.ItemName1).Select(x => x.FirstOrDefault()).ToList();
        ...
        private void xrLabel1_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            String ParamedicID = Convert.ToString(GetCurrentColumnValue("DetailItemParamedicName2"));

            if (ParamedicID != "0" && ParamedicID != "" && ParamedicID != null)
            {
                if (ItemGroupID == "0")
                {
                    xrLabel1.Text = string.Format("{0} | {1}", Convert.ToString(GetCurrentColumnValue("DetailItemType")), Convert.ToString(GetCurrentColumnValue("DetailItemParamedicName2")));
                }
                else
                {
                    xrLabel1.Text = string.Format("{0} | {1} | {2}", Convert.ToString(GetCurrentColumnValue("DetailItemType")), Convert.ToString(GetCurrentColumnValue("DetailItemGroupName1")), Convert.ToString(GetCurrentColumnValue("DetailItemParamedicName2")));
                }
            }
            else
            {
