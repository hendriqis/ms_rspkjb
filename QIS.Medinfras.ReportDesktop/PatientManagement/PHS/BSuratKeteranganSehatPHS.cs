...
{
    public partial class BSuratKeteranganSehatPHS : BaseDailyPortrait2Rpt
    {
        public BSuratKeteranganSehatPHS()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            vVitalSignDt VitalSignNadi = BusinessLayer.GetvVitalSignDtList(string.Format("RegistrationID = {0} AND VitalSignLabel = 'NADI' AND IsDeleted = 0 ORDER BY ID", entityVisit.RegistrationID)).FirstOrDefault();
            ...
            if (VitalSignNadi == null)
            {
                lblNadi.Text = string.Format("-");
            }
            else {
                lblNadi.Text = string.Format("{0} bpm", VitalSignNadi.VitalSignValue);
            }
            ...
            lblUntuk.Text = param[1];
            lblStatement.Text = string.Format("{0}", dateNow.ToString(Constant.FormatString.DATE_FORMAT), param[1]);
