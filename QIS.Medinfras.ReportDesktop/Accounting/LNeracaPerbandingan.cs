...
{
    public partial class LNeracaPerbandingan : BaseDailyLandscapeRpt
    {
        List<GetGLBalancePerLevelCompare> lst = new List<GetGLBalancePerLevelCompare>();

        public LNeracaPerbandingan()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            if (month == 13)
            {
                date1 = new DateTime(year, 12, 1).AddMonths(1).AddDays(-1);
            }
            else
            {
                date1 = new DateTime(year, month, 1).AddMonths(1).AddDays(-1);
            }
            DateTime date2;
            if (month2 == 13)
            {
                date2 = new DateTime(year2, 12, 1).AddMonths(1).AddDays(-1);
            }
            else
            {
                date2 = new DateTime(year2, month2, 1).AddMonths(1).AddDays(-1);
            }
        ...
        private void xrLabel1_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            xrLabel1.Text = lst.Where(t => t.GLAccountType == GLAccountType && t.AccountLevel == 1).FirstOrDefault().BalanceEND.ToString("N2");
        }

        private void xrLabel2_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            String GLAccountType = GetCurrentColumnValue("GLAccountType").ToString();
            xrLabel2.Text = lst.Where(t => t.GLAccountType == GLAccountType && t.AccountLevel == 1).FirstOrDefault().BalanceEND2.ToString("N2");
        }        
    }
}
