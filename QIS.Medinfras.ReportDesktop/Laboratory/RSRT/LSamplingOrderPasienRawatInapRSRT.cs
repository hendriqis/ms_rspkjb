...
{
    public partial class LSamplingOrderPasienRawatInapRSRT : BaseCustomDailyLandscapeA5Rpt
    {
        public LSamplingOrderPasienRawatInapRSRT()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            lblHeader.Text = string.Format("Periode : {0} s/d {1}", Helper.YYYYMMDDToDate(temp[0]).ToString(Constant.FormatString.DATE_FORMAT), Helper.YYYYMMDDToDate(temp[1]).ToString(Constant.FormatString.DATE_FORMAT));
            lblHeaderTime.Text = string.Format("Jam : {0} s/d {1}", temp1[0].ToString(), temp1[1].ToString());
