...
{
    public partial class LaboratoryResultRptRSFMv2 : BaseDailyPortrait2Rpt
    {
        public LaboratoryResultRptRSFMv2()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            ttdDokter.ImageUrl = string.Format("{0}{1}/Signature/{2}.png", AppConfigManager.QISPhysicalDirectory, AppConfigManager.QISParamedicImagePath, entityParamedic.ParamedicCode);
            ttdDokter.Visible = true;

            lblPenanggungJawab.Text = string.Format("{0}", entityParamedic.FullName);
            ...
            lblPetugas.Text = string.Format("{0}", AppSession.UserLogin.UserFullName);
            ...
            foreach (vLaboratoryResultDt resultDt in entity)
            {
                if (resultDt.IsUsingFooterNote)
                {
                    ...
                    string[] footerNote = sbFooterNote.ToString().Split('|');

                    if (resultDt.FooterNote != "")
                    {
                        if (!resultDt.IsNormal)
                        {
                            rtFooterNote.Text = string.Format("{0}", footerNote[0]);
                        }
                        else
                        {
                            rtFooterNote.Text = string.Format("{0}", footerNote[1]);
                        }
                    }
                }
                else
                {
                    rtFooterNote.Visible = false;
