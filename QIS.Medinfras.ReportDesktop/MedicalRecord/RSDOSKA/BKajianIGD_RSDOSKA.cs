...
{
    public partial class BKajianIGD_RSDOSKA : BaseRpt
    {

        public BKajianIGD_RSDOSKA()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            ConsultVisit entityCV1 = BusinessLayer.GetConsultVisitList(string.Format("VisitID = {0}", param[0])).FirstOrDefault();
            //Registration entityReg = BusinessLayer.GetRegistrationList(string.Format("RegistrationID = {0}", entityCV.RegistrationID))[0];
            vPatient entityPatient = BusinessLayer.GetvPatientList(string.Format("MRN = {0}", entityCV.MRN))[0];
            ...
            cPatientName.Text = entityPatient.PatientName;
            ...
            cAgama.Text = entityPatient.Religion;
            xrTableCell15.Text = entityPatient.Occupation;
            xrTableCell109.Text = entityPatient.MaritalStatus;
            xrTableCell234.Text = entityPatient.Education;
            ...
            cHeaderPatient.Text = string.Format("{0} | {1} | {2} yr {3} mnth {4} day",
                entityPatient.PatientName, entityCV.Gender, entityCV.AgeInYear, entityCV.AgeInMonth, entityCV.AgeInDay);
            cHeaderRegistration.Text = entityCV.RegistrationNo;
            cHeaderMedicalNo.Text = entityPatient.MedicalNo;

            PatientVisitNote entityPatientVisitNote = BusinessLayer.GetPatientVisitNoteList(string.Format("VisitID = {0} ORDER BY ID DESC", entityCV.VisitID))[0];
            cProsedurDiagnostikObjective.Text = entityPatientVisitNote.ObjectiveText;
            #endregion
            ...
            if (entityCV1.DischargeDate != null)
            {
                StandardCode sc = BusinessLayer.GetStandardCodeList(string.Format("StandardCodeID = '{0}'", entityCV1.GCRoomType)).FirstOrDefault();
                xrCheckBox1.Text = string.Format("RAWAT {0}", sc.StandardCodeName);
                xrCheckBox1.Checked = true;
            }
            else
            {
                xrCheckBox1.Text = string.Format("RAWAT SU-IC/ICU/NICU-PICU/LVL 2/ISILASI/HND/RUANGAN");
