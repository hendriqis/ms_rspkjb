...
{
    public partial class MRVitalSignNewMROutRSRA : DevExpress.XtraReports.UI.XtraReport
    {
        public MRVitalSignNewMROutRSRA()
        {
            InitializeComponent();
        }
        ...
        public void InitializeReport(int VisitID, int MedicalResumeID)
        {
            ...
            if (entityHd != null)
            {
                ...
                if (entityVisit.PhysicianDischargedDate.ToString(Constant.FormatString.DATE_PICKER_FORMAT) == Constant.ConstantDate.DEFAULT_NULL)
                {
                    ...
                    {
                        lblVitalSignOut.Text = string.Format("{0} / {1} ({2})", entityVisit.cfDischargeDate, entityVisit.DischargeTime, entityHd.ParamedicName);
                    }
                    else
                    {
