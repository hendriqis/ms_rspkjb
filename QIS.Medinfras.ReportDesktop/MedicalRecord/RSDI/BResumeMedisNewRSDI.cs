...
{
    public partial class BResumeMedisNewRSDI : BaseRpt
    {
        public BResumeMedisNewRSDI()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            PatientEducationForm entityEF = BusinessLayer.GetPatientEducationFormList(string.Format("VisitID = {0} AND IsDeleted = 0 AND GCEducationFormGroup = 'X088^99' AND GCEducationFormType = 'X515^019'", entityCV.VisitID)).FirstOrDefault();
            ...
            if (entityCV.PhysicianDischargedDate.ToString(Constant.FormatString.DATE_PICKER_FORMAT) == Constant.ConstantDate.DEFAULT_NULL)
            {
                ...
                {
                    cDischargeDateTime.Text = string.Format("Tgl & Jam Pulang : {0}", entityCV.cfDischargeDateTimeInString);
                }
                else
                {
            ...
            else
            {
                if (entityCV.DischargeDate.ToString(Constant.FormatString.DATE_PICKER_FORMAT) == Constant.ConstantDate.DEFAULT_NULL)
                {
                    cDischargeDateTime.Text = string.Format("Tgl & Jam Pulang : {0}", entityCV.cfPhysicianDischargedDateTimeOrderInString);
                }
                else
                {
                    cDischargeDateTime.Text = string.Format("Tgl & Jam Pulang : {0}", entityCV.cfDischargeDateTimeInString);
            ...
            if (entity != null)
            {
                ...
                if (lstHdReviewOfSystem.Count() > 0)
                {
                    ...
                    mrReviewOfSystemNewOutRSDI.InitializeReport(entityCV.VisitID, entity.ID);
                }
                else
                {
            ...
            if (entity != null)
            {
                ...
                if (lstHdVitalSign.Count() > 0)
                {
                    ...
                    mrVitalSignNewMROutRSDI.InitializeReport(entityCV.VisitID, entity.ID);
                }
                else
                {
            ...
            if (entityEF != null)
            {
                lblOtorisasi.Visible = true;
            }
            else
            {
                lblOtorisasi.Visible = false;
            }
