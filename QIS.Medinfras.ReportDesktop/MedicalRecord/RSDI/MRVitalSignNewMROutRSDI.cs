...
{
    public partial class MRVitalSignNewMROutRSDI : DevExpress.XtraReports.UI.XtraReport
    {
        public MRVitalSignNewMROutRSDI()
        {
            InitializeComponent();
        }
        ...
        public void InitializeReport(int VisitID, int MedicalResumeID)
        {
            ...
            vVitalSignRSSEB entityDt = BusinessLayer.GetvVitalSignRSSEBList(string.Format(filterExpression)).FirstOrDefault();

            if (entityHd != null)
            {
                List<vVitalSignHd> lstHd = BusinessLayer.GetvVitalSignHdList(string.Format("ID = {0} AND IsDeleted = 0", entityHd.ID));
                List<vVitalSignRSSEB> lstDt = BusinessLayer.GetvVitalSignRSSEBList(string.Format("ID = {0} AND IsDeleted = 0", entityHd.ID));
