...
{
    public partial class BResumeMedisNewRSRTH : BaseRpt
    {
        public BResumeMedisNewRSRTH()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            vRegistration entityLinkReg = BusinessLayer.GetvRegistrationList(string.Format("LinkedToRegistrationID = {0}", entityCV.RegistrationID)).FirstOrDefault();
            ...
            if (entityPTR != null)
            {
                cUnitVisit.Text = string.Format("Kamar/Kelas     : {0}/{1}", entityPTR.FromRoomName, entityPTR.FromClassName);
            }
            else
            {
                cUnitVisit.Text = string.Format("Kamar/Kelas     : {0}/{1}", entityCV.RoomName, entityCV.ClassName);
            }
            ...
            {
                cUnitDischarge.Text = string.Format("Kamar/Kelas     : {0}/{1}", entityCV.RoomName, entityCV.ClassName);
            }
            else
            {
                cUnitDischarge.Text = "Kamar/Kelas   :";
            }
            ...
            #region Triage
            if (entityLinkReg != null)
            {
                subTriage.CanGrow = true;
                mrTriage.InitializeReport(entityCV.RegistrationID);
            }
            else
            {
                subTriage.Visible = false;
            ...
            //string PerawatID = AppSession.UserLogin.ParamedicID.ToString();
            //ParamedicMaster entityPM2 = BusinessLayer.GetParamedicMasterList(string.Format("ParamedicID = {0}", PerawatID)).FirstOrDefault();

            //if (entityPM2 != null)
            //{
            //    if (entityPM2.GCParamedicMasterType == Constant.ParamedicType.Nurse)
            //    {
            //        lblSignNurseName.Text = AppSession.UserLogin.UserFullName;
            //        lblPerawatBidan.Text = "Perawat";
            //    }
            //    else if (entityPM2.GCParamedicMasterType == Constant.ParamedicType.Bidan)
            //    {
            //        lblSignNurseName.Text = AppSession.UserLogin.UserFullName;
            //        lblPerawatBidan.Text = "Bidan";
            //    }
            //    else
            //    {
            //        lblSignNurseName.Text = "";
            //        lblPerawatBidan.Text = "Bidan / Perawat";
            //    }
            //}
            //else
            //{
