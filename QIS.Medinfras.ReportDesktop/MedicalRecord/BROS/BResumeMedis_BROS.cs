...
{
    public partial class BResumeMedis_BROS : BaseRpt
    {
        public BResumeMedis_BROS()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            {
                cDischargeDate.Text = string.Format("{0}", entityCV.cfPhysicianDischargedDateTimeOrderInString);
            }
            #endregion
            ...
            if (entity != null)
            {
                ...
                if (lstPatientDiagnosis.Count() > 0)
                {
                    ...
                    mrDiagnoseInformationBROS.InitializeReport(entityCV.VisitID, entity.ID);
                }
                else
                {
            ...
            #region Prosedur dan Tindakan
            List<vPatientProcedure> lstPatientProcedure = BusinessLayer.GetvPatientProcedureList(string.Format("VisitID = {0} AND IsDeleted = 0", entityCV.VisitID));

            if (lstPatientProcedure.Count() > 0)
            {
                subProcedure.CanGrow = true;
                mrProcedure.InitializeReport(entityCV.VisitID);
            }
            else
            {
