...
{
    public partial class EpisodeSummaryWithIntegratedRSSC : BaseRpt
    {

        public EpisodeSummaryWithIntegratedRSSC()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            vConsultVisit entityLinkedRegistrationID = BusinessLayer.GetvConsultVisitList(string.Format(
                    "VisitID = {0}", entityCV.VisitID)).FirstOrDefault();
            int cvLinked = 0;
            if (entityLinkedRegistration.LinkedRegistrationID != null)
            {
                cvLinked = entityLinkedRegistration.LinkedRegistrationID;
            }
            string filterExpression = string.Format(
                "VisitID IN ({0},{1}) AND IsDeleted = 0",
               entityCV.VisitID, cvLinkedID);
            ...
            if (lstPatientInstruction.Count() > 0)
            {
                ...
                episodeSummaryInstruction.InitializeReport(lstPatientInstruction);
            }
            else
            {
