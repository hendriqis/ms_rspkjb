...
{
    public partial class MRReviewOfSystemNewInOPRSPKSB : DevExpress.XtraReports.UI.XtraReport
    {
        public MRReviewOfSystemNewInOPRSPKSB()
        {
            InitializeComponent();
        }
        ...
        public void InitializeReport(int VisitID)
        {
            ...
            if (param == "1")
            {
                ...
                if (entityRegistration.LinkedRegistrationID == 0 || entityRegistration.LinkedRegistrationID == null)
                {
                    ...
                    if (entityHd != null)
                    {
                        vReviewOfSystemRSSES entityDt = BusinessLayer.GetvReviewOfSystemRSSESList(string.Format("ID = {0} AND IsDeleted = 0", entityHd.ID)).FirstOrDefault();
                    }
                    else
                    {
                        string filterExpressionDt = string.Format("VisitID IN ({0}) AND MedicalResumeID IS NULL AND IsDeleted = 0 ORDER BY ID", VisitID);
                        vReviewOfSystemRSSES entityDt = BusinessLayer.GetvReviewOfSystemRSSESList(filterExpressionDt).FirstOrDefault();
                    }
                    ...
                    if (entityHd != null)
                    {
                        ...
                        sbNotes.AppendLine(string.Format("{0}", this.DataSource = lst));
                ...
                else
                {
                    ...
                    if (entityHd != null)
                    {
                        ...
                        if (entityHd != null)
                        {
                            ...
                            sbNotes.AppendLine(string.Format("{0}", this.DataSource = lst.Select(x => x.ReviewOfSystemDts)));
                            DetailReport.DataMember = "ReviewOfSystemDts";
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        if (entityHd2 != null)
                        {
                            vReviewOfSystemRSSES entityDt = BusinessLayer.GetvReviewOfSystemRSSESList(string.Format("ID = {0} AND IsDeleted = 0", entityHd2.ID)).FirstOrDefault();
                        }
                        else
                        {
                            string filterExpressionDt = string.Format("VisitID IN ({0}) AND MedicalResumeID IS NULL AND IsDeleted = 0 ORDER BY ID", VisitID);
                        ...
                        if (entityHd2 != null)
                        {
                            ...
                            List<vReviewOfSystemRSSES> lstDt = BusinessLayer.GetvReviewOfSystemRSSESList(string.Format("ID = {0} AND IsDeleted = 0", entityHd2.ID));
                            var lst = (from p in lstHd
                                       select new
                                       {
                                           ObservationDateInString = p.ObservationDateInString,
