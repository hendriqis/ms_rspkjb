...
{
    public partial class MRBodyDiagramRSPKSB : DevExpress.XtraReports.UI.XtraReport
    {
        public MRBodyDiagramRSPKSB()
        {
            InitializeComponent();
        }
        ...
        private void sbrBodyDiagramDt_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            if (GetCurrentColumnValue("ID") != null)
            {
                ...
                ((MRBodyDiagramDtRSPKSB)subreport.ReportSource).InitializeReport(lstDt.Where(p => p.ID == ID).ToList());
                //subreport.In
            }
        }
