...
{
    public partial class MedicalResume2RSBL : BaseRpt
    {
        public MedicalResume2RSBL()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            Registration oReg = BusinessLayer.GetRegistrationList(string.Format("RegistrationID = {0}", oCV.RegistrationID))[0];
            ...
            {
                lblKeluhan.Text = "";
                lblPastMedicalHistory.Text = "";
                lblPastMedicationHistory.Text = "";
            }
            else
            {
                lblKeluhan.Text = oComplaint.ChiefComplaintText;
                lblPastMedicalHistory.Text = oComplaint.PastMedicalHistory;
                lblPastMedicationHistory.Text = oComplaint.PastMedicationHistory;
            }
            #endregion
            ...
            List<vReviewOfSystemHd> oROSHd = BusinessLayer.GetvReviewOfSystemHdList(string.Format("VisitID = {0} AND GCParamedicMasterType = '{1}' AND IsDeleted = 0", oCV.VisitID, Constant.ParamedicType.Physician));
            if (oROSHd.Count != 0)
            {
            ...
            List<vVitalSignHd> oVTHd = BusinessLayer.GetvVitalSignHdList(string.Format("VisitID = {0} AND GCParamedicMasterType = '{1}' AND IsDeleted = 0", oCV.VisitID, Constant.ParamedicType.Physician));
            if (oVTHd.Count != 0)
            {
            ...
            #region Footer
            lblTanggalTTD.Text = string.Format("{0}, {1} Jam {2}", oHealthcare.City, oReg.RegistrationDate.ToString(Constant.FormatString.DATE_FORMAT), oReg.RegistrationTime);
            ...
            string contents = string.Format(@"{0}\r\n{1}",
                oPhysician.FullName, oPhysician.LicenseNo);
