...
{
    public partial class BResumeMedisRSAJ : BaseRpt
    {

        public BResumeMedisRSAJ()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            //info kanan
            if (entityCV.DischargeDate.ToString(Constant.FormatString.DATE_FORMAT) == "" || entityCV.DischargeDate.ToString(Constant.FormatString.DATE_FORMAT) == "01-Jan-1900")
            {
                cDischargeDate.Text = "";
            }
            else if (entityCV.DischargeDate.ToString(Constant.FormatString.DATE_FORMAT) != "")
            {
                String DischargeDate;
                DischargeDate = entityCV.DischargeDate.ToString(Constant.FormatString.DATE_FORMAT);
                ConsultVisit entityCV2 = BusinessLayer.GetConsultVisitList(string.Format("VisitID = {0}", param[0])).FirstOrDefault();
                if (entityCV2.DateOfDeath.ToString(Constant.FormatString.DATE_FORMAT) != "" && entityCV2.DateOfDeath.ToString(Constant.FormatString.DATE_FORMAT) != "01-Jan-1900")
                {
                    DischargeDate += " (Meninggal)";
                }
                else if (entityCV2.DateOfDeath.ToString(Constant.FormatString.DATE_FORMAT) == "")
                {
                    DischargeDate += "";
                }
                cDischargeDate.Text = DischargeDate;
            }

            cPatientPJ.Text = entityCV.ParamedicName;

            vRegistration entityR = BusinessLayer.GetvRegistrationList(string.Format("VisitID = {0}", param[0])).FirstOrDefault();
            ...
            vMedicalResume obj = BusinessLayer.GetvMedicalResumeList(string.Format("VisitID = {0} AND IsDeleted = 0", entityCV.VisitID)).FirstOrDefault();
            
            //if (lstEntityHdTestOrder.Count() > 0)
            //{
            //    subTestOrder.CanGrow = true;
            //    episodeSummaryTestOrderRptRSAJ.InitializeReport(entityCV.VisitID);
            //}
            //else
            //{
            ...
            if (obj != null)
            {
                if (!String.IsNullOrEmpty(obj.PlanningResumeText))
                {
                    lblPlanningSummaryText.Text = "Hasil Terlampir";
                }
                else
                {
            ...
            vConsultVisit5 lstEntity2 = BusinessLayer.GetvConsultVisit5List(String.Format("VisitID = {0} AND IsMainVisit = 1", entityCV.VisitID)).FirstOrDefault();
