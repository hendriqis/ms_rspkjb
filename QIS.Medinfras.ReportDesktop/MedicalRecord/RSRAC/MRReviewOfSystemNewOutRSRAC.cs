...
{
    public partial class MRReviewOfSystemNewOutRSRAC : DevExpress.XtraReports.UI.XtraReport
    {
        public MRReviewOfSystemNewOutRSRAC()
        {
            InitializeComponent();
        }
                ...
                if (entityHd != null)
                {
                    if (lstDt.Count > 0)
                    {
                        if (entityCV.PhysicianDischargedDate.ToString(Constant.FormatString.DATE_PICKER_FORMAT) == Constant.ConstantDate.DEFAULT_NULL)
                        {
                            ...
                            {
                                lblReviewOfSystem.Text = string.Format("{0} / {1}", entityCV.cfDischargeDate, entityCV.DischargeTime);
                            }
                            else
                            {
