...
{
    public partial class MRVitalSignNewMRInRSUKRIDA : DevExpress.XtraReports.UI.XtraReport
    {
        public MRVitalSignNewMRInRSUKRIDA()
        {
            InitializeComponent();
        }
        ...
        public void InitializeReport(int VisitID)
        {
            ...
            if (param == "1")
            {
                ...
                if (entityRegistration.LinkedRegistrationID == 0 || entityRegistration.LinkedRegistrationID == null)
                {
                    ...
                    if (entityHd != null)
                    {
                        vVitalSignDt entityDt = BusinessLayer.GetvVitalSignDtList(string.Format("ID = {0}", entityHd.ID)).FirstOrDefault();
                    }
                    else
                    {
                        string filterExpressionDt = string.Format("VisitID IN ({0}) AND MedicalResumeID IS NULL AND IsDeleted = 0 ORDER BY ID", VisitID);
                        vVitalSignDt entityDt = BusinessLayer.GetvVitalSignDtList(filterExpressionDt).FirstOrDefault();
                    }
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (entityHd2 != null)
                        {
                            vVitalSignDt entityDt = BusinessLayer.GetvVitalSignDtList(string.Format("ID = {0}", entityHd2.ID)).FirstOrDefault();
                        }
                        else
                        {
                            string filterExpressionDt = string.Format("VisitID IN ({0}) AND MedicalResumeID IS NULL AND IsDeleted = 0 ORDER BY ID", VisitID);
                        ...
                        if (entityHd2 != null)
                        {
                            ...
                            List<vVitalSignDt> lstDt = BusinessLayer.GetvVitalSignDtList(string.Format("ID = {0} AND IsDeleted = 0", entityHd2.ID));
                            var lst = (from p in lstHd
                                       select new
                                       {
                                           Remarks = p.Remarks,
