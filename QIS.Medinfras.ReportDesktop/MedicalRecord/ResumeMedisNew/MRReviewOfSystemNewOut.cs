...
{
    public partial class MRReviewOfSystemNewOut : DevExpress.XtraReports.UI.XtraReport
    {
        public MRReviewOfSystemNewOut()
        {
            InitializeComponent();
        }
        ...
        {
            string filterExpression = string.Format("VisitID IN ({0}) AND ID = {1} AND IsDeleted = 0", VisitID, MedicalResumeID);
            vMedicalResume entityMR = BusinessLayer.GetvMedicalResumeList(filterExpression).FirstOrDefault();
