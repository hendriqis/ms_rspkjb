...
{
    public partial class MRReviewOfSystemNewIn : DevExpress.XtraReports.UI.XtraReport
    {
        public MRReviewOfSystemNewIn()
        {
            InitializeComponent();
        }
        ...
        public void InitializeReport(int VisitID)
        {
            ...
            if (param == "1")
            {
                ...
                if (entityRegistration.LinkedRegistrationID == 0 || entityRegistration.LinkedRegistrationID == null)
                {
                    ...
                    if (entityHd != null)
                    {
                        vReviewOfSystemDt entityDt = BusinessLayer.GetvReviewOfSystemDtList(string.Format("ID = {0} ORDER BY TagProperty", entityHd.ID)).FirstOrDefault();
                    }
                    else
                    {
                        string filterExpressionDt = string.Format("VisitID IN ({0}) AND MedicalResumeID IS NULL AND IsDeleted = 0 ORDER BY TagProperty", VisitID);
                    ...
                    if (entityHd != null)
                    {
                        ...
                        List<vReviewOfSystemDt> lstDt = BusinessLayer.GetvReviewOfSystemDtList(string.Format("ID = {0} AND IsDeleted = 0 ORDER BY TagProperty", entityHd.ID));
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (entityHd2 != null)
                        {
                            vReviewOfSystemDt entityDt = BusinessLayer.GetvReviewOfSystemDtList(string.Format("ID = {0} ORDER BY TagProperty", entityHd2.ID)).FirstOrDefault();
                        }
                        else
                        {
                            string filterExpressionDt = string.Format("VisitID IN ({0}) AND MedicalResumeID IS NULL AND IsDeleted = 0 ORDER BY TagProperty", VisitID);
                        ...
                        if (entityHd2 != null)
                        {
                            ...
                            List<vReviewOfSystemDt> lstDt = BusinessLayer.GetvReviewOfSystemDtList(string.Format("ID = {0} AND IsDeleted = 0 ORDER BY TagProperty", entityHd2.ID));
                            var lst = (from p in lstHd
                                       select new
                                       {
                                           ObservationDateInString = p.ObservationDateInString,
