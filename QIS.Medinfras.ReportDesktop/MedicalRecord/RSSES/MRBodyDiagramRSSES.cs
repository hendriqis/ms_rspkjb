...
{
    public partial class MRBodyDiagramRSSES : DevExpress.XtraReports.UI.XtraReport
    {
        public MRBodyDiagramRSSES()
        {
            InitializeComponent();
        }
        ...
        private void sbrBodyDiagramDt_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            if (GetCurrentColumnValue("ID") != null)
            {
                ...
                ((MRBodyDiagramDtRSSES)subreport.ReportSource).InitializeReport(lstDt.Where(p => p.ID == ID).ToList());
                //subreport.In
            }
        }
