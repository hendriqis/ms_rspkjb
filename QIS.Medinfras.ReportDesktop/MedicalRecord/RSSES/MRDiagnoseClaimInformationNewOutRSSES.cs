...
{
    public partial class MRDiagnoseClaimInformationNewOutRSSES : DevExpress.XtraReports.UI.XtraReport
    {
        public MRDiagnoseClaimInformationNewOutRSSES()
        {
            InitializeComponent();
        }
        ...
        {
            string filterExpression = string.Format("VisitID IN ({0}) AND GCDiagnoseType IN ('{1}','{2}') AND ClaimDiagnosisID IS NOT NULL AND IsDeleted = 0 ORDER BY GCDiagnoseType ASC, ID DESC", VisitID, Constant.DiagnoseType.MAIN_DIAGNOSIS, Constant.DiagnoseType.COMPLICATION);
