...
{
    public partial class BResumeMedisRSSES : BaseRpt
    {
        public BResumeMedisRSSES()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            #region Perkembangan Dalam Perawatan
            if (entity != null)
            {
                List<vMedicalResume> lstMedicalResume9 = BusinessLayer.GetvMedicalResumeList(string.Format("VisitID = {0} AND IsDeleted = 0 AND IsRevised = 0", entityCV.VisitID));

                if (lstMedicalResume9.Count() > 0)
                {
                    subTreatmentRSSES.CanGrow = true;
                    mrTreatmentRSSES.InitializeReport(entityCV.VisitID, entity.ID);
                }
                else
                {
                    subTreatmentRSSES.Visible = false;
            ...
            {
                if (!string.IsNullOrEmpty(AppConfigManager.QISPhysicalDirectory))
                {
                ...
                {
                    ttdDokter.ImageUrl = string.Format("{0}/Signature/{1}.png", "C:/_medinfras/appData/rsses/Paramedic", entityCV.ParamedicCode);
                }
            }
            else
            {
