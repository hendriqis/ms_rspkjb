...
{
    public partial class BResumeMedisNewRSSEBK : BaseRpt
    {
        public BResumeMedisNewRSSEBK()
        {
            InitializeComponent();
        }
        ...
        public override void InitializeReport(string[] param)
        {
            ...
            vPatient entityPatient = BusinessLayer.GetvPatientList(string.Format("MRN = {0}", entityCV.MRN)).FirstOrDefault();
            ...
            {
                lblReportTitle.Text = string.Format("RINGKASAN PASIEN PULANG\n(DISCHARGE SUMMARY)");
            }
            else
            {
                lblReportTitle.Text = string.Format("RINGKASAN PASIEN PULANG RAWAT INAP\n(INPATIENT DISCHARGE SUMMARY)");
            }
            ...
            if (entity != null)
            {
                ...
                if (lstPatientTransfer.Count() > 0)
                {
                    ...
                    mrPatientTransferRSSEBK.InitializeReport(entityCV.RegistrationID);
                }
                else
                {
            ...
            if (entity != null)
            {
                ...
                if (lstHdReviewOfSystem.Count() > 0)
                {
                    ...
                    mrReviewOfSystemNewOutRSSEBK.InitializeReport(entityCV.VisitID, entity.ID);
                }
                else
                {
            ...
            if (entity != null)
            {
                ...
                if (lstHdVitalSign.Count() > 0)
                {
                    ...
                    mrVitalSignNewMROutRSSEBK.InitializeReport(entityCV.VisitID, entity.ID);
                }
                else
                {
            ...
            if (entity != null)
            {
                List<vPatientDiagnosis> lstPatientDiagnosis2 = BusinessLayer.GetvPatientDiagnosisList(string.Format("VisitID = {0} AND IsDeleted = 0 AND GCDiagnoseType IN ('{1}','{2}')", entityCV.VisitID, Constant.DiagnoseType.MAIN_DIAGNOSIS, Constant.DiagnoseType.COMPLICATION));

                if (lstPatientDiagnosis2.Count() > 0)
                {
                    subDiagnoseOut.CanGrow = true;
                    mrDiagnoseInformationNewOutRSSEBK.InitializeReport(entityCV.VisitID);
                }
                else
                {
            ...
            if (entity != null)
            {
                ...
                if (lstPatientReferral.Count() > 0)
                {
                    ...
                    mrReferralRSSEBK.InitializeReport(entityCV.VisitID, entity.ID);
                }
                else
                {
            ...
            if (entity != null)
            {
                ...
                if (lstMedicalResume2.Count() > 0)
                {
                    subResidualPrescription.CanGrow = true;
                    mRResidualPrescriptionRSSEBK.InitializeReport(entityCV.VisitID, entity.ID);
                }
                else
                {
                    subResidualPrescription.Visible = false;
                }
            }
            else
            {
            ...
            if (entity != null)
            {
                ...
                if (lstMedicalResume5.Count() > 0)
                {
                    subFollowupControl.CanGrow = true;
                    mrFollowUpControl.InitializeReport(entityCV.VisitID, entity.ID);
                }
                else
                {
                    subFollowupControl.Visible = false;
                }
            }
            else
            {
            ...
            #region Footer : Per Page
            cFooterPatient.Text = string.Format("{0} | {1} | {2}", entityCV.cfPatientNameSalutation, entityPatient.DateOfBirthInString, entityReg.RegistrationNo);
            #endregion
        }
    }
}
