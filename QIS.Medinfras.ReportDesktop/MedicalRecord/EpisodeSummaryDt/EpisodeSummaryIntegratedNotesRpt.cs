...
{
    public partial class EpisodeSummaryIntegratedNotesRpt : DevExpress.XtraReports.UI.XtraReport
    {
        public EpisodeSummaryIntegratedNotesRpt()
        {
            InitializeComponent();
        }
        ...
        public void InitializeReport(int VisitID)
        {
            ...
            ConsultVisit entitycv = BusinessLayer.GetConsultVisitList(string.Format("RegistrationID = {0}", cvLinkedID)).FirstOrDefault();

            string filterExpression = "";
            if (cvLinkedID == 0 || cvLinkedID == null)
            {
               filterExpression = string.Format(
                "VisitID IN ({0}) AND GCPatientNoteType IN ('{1}', '{2}', '{3}', '{4}', '{5}') ORDER BY NoteDate DESC, NoteTime DESC",
               VisitID, Constant.PatientVisitNotes.NURSING_NOTES, Constant.PatientVisitNotes.SOAP_SUMMARY_NOTES, Constant.PatientVisitNotes.FOLLOWUP_NOTES, Constant.PatientVisitNotes.PHARMACY_NOTES, Constant.PatientVisitNotes.DIAGNOSTIC_SUPPORT_NOTES);
            }
            else
            {
                filterExpression = string.Format(
                 "VisitID IN ({0},{1}) AND GCPatientNoteType IN ('{2}', '{3}', '{4}', '{5}', '{6}') ORDER BY NoteDate DESC, NoteTime DESC",
                VisitID, entitycv.VisitID, Constant.PatientVisitNotes.NURSING_NOTES, Constant.PatientVisitNotes.SOAP_SUMMARY_NOTES, Constant.PatientVisitNotes.FOLLOWUP_NOTES, Constant.PatientVisitNotes.PHARMACY_NOTES, Constant.PatientVisitNotes.DIAGNOSTIC_SUPPORT_NOTES);
            }
        ...
        private void SetDataForBinding(List<ViewIntegratedNotes> lstIntegratedNotes, List<vPatientVisitNote> lstEntity)
        {
            ...
            foreach (vPatientVisitNote entity in lstEntity)
            {
                if (lstDateTime.Where(t => t.Date == entity.NoteDate).Count() == 0)
                {
                    ...
                    IEnumerable<vPatientVisitNote> lstSameDateNotes = lstEntity.Where(t => t.cfNoteDate == entity.cfNoteDate);
                    ...
                    {									
                        ...
                        string intructionText = string.Format("I : {0}", entitySameDateNote.InstructionText);
