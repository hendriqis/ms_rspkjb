...
{
    public partial class EpisodeSummaryIntegratedNotesRptRSAJ : DevExpress.XtraReports.UI.XtraReport
    {
        public EpisodeSummaryIntegratedNotesRptRSAJ()
        {
            InitializeComponent();
        }
        ...
        public void InitializeReport(int VisitID)
        {
            ...
            int cvLinkedID = 0;
            if (entityLinkedRegistration.LinkedRegistrationID != null && entityLinkedRegistration.LinkedRegistrationID != 0)
            {
                string filterVisit = String.Format("RegistrationID = '{0}'", entityLinkedRegistration.LinkedRegistrationID);
                ConsultVisit visitLinked = BusinessLayer.GetConsultVisitList(filterVisit).FirstOrDefault();
                cvLinkedID = visitLinked.VisitID;
            }

            string filterExpression = string.Format(
                "VisitID IN ({0},{1}) AND GCPatientNoteType IN ('{2}', '{3}', '{4}', '{5}', '{6}','{7}') ORDER BY NoteDate ASC, NoteTime ASC",
               VisitID, cvLinkedID, Constant.PatientVisitNotes.NURSING_NOTES, Constant.PatientVisitNotes.SOAP_SUMMARY_NOTES, Constant.PatientVisitNotes.FOLLOWUP_NOTES, Constant.PatientVisitNotes.PHARMACY_NOTES, Constant.PatientVisitNotes.DIAGNOSTIC_SUPPORT_NOTES, Constant.PatientVisitNotes.EMERGENCY_INITIAL_ASSESSMENT);
            List<ViewIntegratedNotes> lstIntegratedNotes = new List<ViewIntegratedNotes>();
                ...
                if (lstDateTime.Where(t => t.Date == entity.NoteDate).Count() == 0)
                {
                    ...
                    foreach (vPatientVisitNote entitySameDateNote in lstSameDateNotes)
                    {
                        ...
                        if (!entitySameDateNote.IsDeleted)
                        {
                            ...
                            string intructionText = string.Format("I : \r\n {0}", entitySameDateNote.InstructionText);
                            ...
                            if (!string.IsNullOrEmpty(entitySameDateNote.InstructionText))
                            {

                                notes += string.Format("{0} \r\n {1}", entitySameDateNote.NoteText, intructionText + "\r\n");
                            }
                            else
                            {
                            ...
                            else if (entitySameDateNote.GCPatientNoteType == Constant.PatientVisitNotes.SOAP_SUMMARY_NOTES || entitySameDateNote.GCPatientNoteType == Constant.PatientVisitNotes.EMERGENCY_INITIAL_ASSESSMENT)
                            {
                                if (!string.IsNullOrEmpty(newEntity.PhysicianNote)) newEntity.PhysicianNote += "\r\n";
