...
{
    public partial class EpisodeSummaryBodyDiagramRptNew : DevExpress.XtraReports.UI.XtraReport
    {
        public EpisodeSummaryBodyDiagramRptNew()
        {
            InitializeComponent();
        }
        ...
        private void sbrBodyDiagramDt_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            if (GetCurrentColumnValue("ID") != null)
            {
                ...
                ((EpisodeSummaryBodyDiagramDtRptNew)subreport.ReportSource).InitializeReport(lstDt.Where(p => p.ID == ID).ToList());
                //subreport.In
            }
        }
